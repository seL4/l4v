...
lemma power_two_max_word_fold:
  "2 ^ LENGTH('a::len) = Suc (unat (-1 :: 'a word))"
...
lemma unat_sum_boundE:
  "x \<le> x + (y::'a::len word) \<Longrightarrow> unat x + unat y \<le> unat (-1::'a::len word)"
  by (metis unat_plus_simple word_less_eq_iff_unsigned word_n1_ge)

lemma unat_sum_bound_equiv:
  "(unat x + unat y \<le> unat (-1::'a::len word)) = (x \<le> x + (y::'a::len word))"
