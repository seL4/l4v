...
theorem plus2_x_property:
  "\<lbrace>\<lambda>s0 s. plus2_inv tids s \<and> threadv s tid = 0 \<and> s = s0 \<and> tid \<in> tids \<and> finite tids\<rbrace>,\<lbrace>plus2_rel tids {tid}\<rbrace>
   plus2_x tid
   \<lbrace>plus2_rel tids (- {tid})\<rbrace>,\<lbrace>\<lambda>_ _ s. plus2_inv tids s \<and> threadv s tid = 2\<rbrace>"
...
text \<open>Just for fun, generalise to arbitrarily many copies of the @{term plus2} program.\<close>

lemma plus2_x_n_parallel_induct:
  "\<lbrakk>n > 0; n \<le> N\<rbrakk> \<Longrightarrow>
   ...
   \<lbrace>plus2_rel {..< N} ( - {..< n})\<rbrace>,\<lbrace>\<lambda>_ _ s. plus2_inv {..< N} s \<and> (\<forall>i < n. threadv s i = 2)\<rbrace>"
  apply (induct n)
   apply simp
  apply (case_tac n)
...
lemma par_tr_fin_principle_parallel:
  "\<lbrakk>par_tr_fin_principle f; par_tr_fin_principle g\<rbrakk>
