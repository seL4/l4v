...
definition 
  foldME ::"('b \<Rightarrow> 'a \<Rightarrow> ('s,('e + 'b)) nondet_monad) \<Rightarrow> 'a list \<Rightarrow> 'b \<Rightarrow> ('s, ('e + 'b)) nondet_monad"
where "foldME m xs a \<equiv> foldr (\<lambda>p q. q >>=E swp m p) xs (returnOk a)"
