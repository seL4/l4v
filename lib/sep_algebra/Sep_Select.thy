...
ML {*
  ...
  let
    val thms = SepSelect_Rules.get ctxt
  in
  ...
  let
    val thms = SepSelectAsm_Rules.get ctxt
  in
    sep_select_tactic (dresolve_tac ctxt thms) ns ctxt
  end

  fun sep_select_asms_method ns ctxt = SIMPLE_METHOD' (sep_select_asms_tactic ns ctxt)
  fun sep_selects_method ns ctxt     = SIMPLE_METHOD' (sep_selects_tactic ns ctxt)
*}
...
ML {* 
  ...
  let
    val lens_tac = if asm then dresolve_tac ctxt lens else resolve_tac ctxt lens
  in
    sep_select_method lens_tac n ctxt
  end

  fun sep_select_generic_method asm thms ns = sep_select_generic_method_inner ((thms,ns),asm)
*}
