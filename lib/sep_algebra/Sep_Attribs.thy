...
local
  ...
  fun attribute_thm thm  thm' =  REPEAT_TRYOF_N @{thm sep_remove_pure_imp_sep_imp} 
                                 (thm OF [atomize_thm thm']) (nprems_of thm' - 1)

  
  
  fun attribute_thm' thm ctxt thm' = thm OF [REPEAT_TRYOF_N @{thm curry} (thm' |> atomize_thm o simp ctxt) (nprems_of thm' - 1)]
...
*)
 ...
 val sep_curry_inner = attribute_thm @{thm sep_curry_atomised}
 val sep_curry = Thm.rule_attribute (K sep_curry_inner)
