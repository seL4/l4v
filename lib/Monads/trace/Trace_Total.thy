...
lemma validNF_alt_def:
  "\<lbrace>P\<rbrace> f \<lbrace>Q\<rbrace>! = (\<forall>s. P s \<longrightarrow> ((\<forall>(r', s') \<in> mres (f s). Q r' (with_env_of s s')) \<and> \<not> failed (f s)))"
...
definition validNF_property ::
  "('a \<Rightarrow> ('c, 's) mpred) \<Rightarrow> ('c, 's) monad_state \<Rightarrow> ('c, 's, 'a) tmonad \<Rightarrow> bool"
  where
  "validNF_property Q s b \<equiv> \<not> failed (b s) \<and> (\<forall>(r', s') \<in> mres (b s). Q r' (with_env_of s s'))"
...
definition validE_NF_property ::
  "('a \<Rightarrow> ('c, 's) mpred) \<Rightarrow> ('e \<Rightarrow> ('c, 's) mpred) \<Rightarrow> ('c, 's) monad_state \<Rightarrow>
   ('c, 's, 'e + 'a) tmonad \<Rightarrow> bool"
  where
  "validE_NF_property Q E s b \<equiv>
   \<not> failed (b s) \<and>
   (\<forall>(r', s') \<in> mres (b s). case r' of Inl x \<Rightarrow> E x (with_env_of s s')
...
lemma validNF_cong:
  "\<lbrakk> \<And>s. P s = P' s; \<And>s. P s \<Longrightarrow> m s = m' s;
     \<And>r' s' s. \<lbrakk> P s; (r', s') \<in> mres (m s) \<rbrakk> \<Longrightarrow> Q r' (with_env_of s s') = Q' r' (with_env_of s s') \<rbrakk>
