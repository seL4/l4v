...
definition exs_valid ::
  ...
  "\<lbrace>P\<rbrace> f \<exists>\<lbrace>Q\<rbrace> \<equiv> \<forall>s. P s \<longrightarrow> (\<exists>(rv, s') \<in> mres (f s). Q rv (with_env_of s s'))"

text \<open>The above for the exception monad\<close>
definition ex_exs_validE ::
  "('c, 's) mpred \<Rightarrow> ('c, 's, 'e + 'a) tmonad \<Rightarrow> ('a \<Rightarrow> ('c, 's) mpred) \<Rightarrow>
   ('e \<Rightarrow> ('c, 's) mpred) \<Rightarrow> bool"
...
definition wp :: "('c, 's, 'a) tmonad \<Rightarrow> ('a \<Rightarrow> ('c, 's) mpred) \<Rightarrow> ('c, 's) mpred" where
  "wp f Q \<equiv> \<lambda>s. apsnd (with_env_of s) ` mres (f s) \<subseteq> {(rv,s). Q rv s}"

definition cwp :: "('c, 's, 'a) tmonad \<Rightarrow> ('a \<Rightarrow> ('c, 's) mpred) \<Rightarrow> ('c, 's) mpred" where
...
lemma use_exs_valid:
  "\<lbrakk> \<lbrace>P\<rbrace> f \<exists>\<lbrace>Q\<rbrace>; P s \<rbrakk> \<Longrightarrow> \<exists>(r, s') \<in> mres (f s). Q r (with_env_of s s')"
