...
lemma in_bindE_L:
  "((Inl r, s') \<in> mres ((doE x \<leftarrow> f; g x odE) s)) \<Longrightarrow>
   (\<exists>s'' x. (Inr x, s'') \<in> mres (f s) \<and> (Inl r, s') \<in> mres (g x (with_env_of s s'')))
            \<or> ((Inl r, s') \<in> mres (f s))"
...
lemma in_return:
  "(r, s') \<in> mres (return v s) = (r = v \<and> s' = mstate s)"
...
lemma in_whenE:
  "((v, s') \<in> mres (whenE P f s)) = ((P \<longrightarrow> (v, s') \<in> mres (f s)) \<and> (\<not>P \<longrightarrow> v = Inr () \<and> s' = mstate s))"
...
lemma inr_in_unlessE_throwError[termination_simp]:
  "(Inr (), s') \<in> mres (unlessE P (throwError E) s) = (P \<and> s' = mstate s)"
...
lemma in_assert:
  "(r, s') \<in> mres (assert P s) = (P \<and> s' = mstate s)"
  by (simp add: assert_def return_def fail_def mres_def)

lemma in_assertE:
  "(r, s') \<in> mres (assertE P s) = (P \<and> r = Inr () \<and> s' = mstate s)"
  by (simp add: assertE_def returnOk_def return_def fail_def mres_def)

lemma in_assert_opt:
  "(r, s') \<in> mres (assert_opt v s) = (v = Some r \<and> s' = mstate s)"
  by (auto simp: assert_opt_def in_fail in_return split: option.splits)

lemma in_get:
  "(r, s') \<in> mres (get s) = (r = s \<and> s' = mstate s)"
  by (simp add: get_def mres_def)

lemma in_gets:
  "(r, s') \<in> mres (gets f s) = (r = f s \<and> s' = mstate s)"
  by (simp add: simpler_gets_def mres_def)

lemma in_put:
  "(r, s') \<in> mres (put x s) = (s' = mstate x \<and> r = ())"
  by (simp add: put_def mres_def)

lemma in_when:
  "(v, s') \<in> mres (when P f s) = ((P \<longrightarrow> (v, s') \<in> mres (f s)) \<and> (\<not>P \<longrightarrow> v = () \<and> s' = mstate s))"
  by (simp add: when_def in_return)

lemma in_unless:
  "(v, s') \<in> mres (unless P f s) = ((\<not> P \<longrightarrow> (v, s') \<in> mres (f s)) \<and> (P \<longrightarrow> v = () \<and> s' = mstate s))"
  by (simp add: unless_def in_when)

lemma in_unlessE:
  "(v, s') \<in> mres (unlessE P f s) = ((\<not> P \<longrightarrow> (v, s') \<in> mres (f s)) \<and> (P \<longrightarrow> v = Inr () \<and> s' = mstate s))"
...
lemma in_modify:
  "(v, s') \<in> mres (modify f s) = (s' = mstate (f s) \<and> v = ())"
  by (auto simp add: modify_def bind_def get_def put_def mres_def)

lemma gets_the_in_monad:
  "((v, s') \<in> mres (gets_the f s)) = (s' = mstate s \<and> f s = Some v)"
...
lemma in_bindE:
  "(rv, s') \<in> mres ((f >>=E (\<lambda>rv'. g rv')) s) =
   ((\<exists>ex. rv = Inl ex \<and> (Inl ex, s') \<in> mres (f s)) \<or>
    (\<exists>rv' s''. (rv, s') \<in> mres (g rv' (with_env_of s s'')) \<and> (Inr rv', s'') \<in> mres (f s)))"
...
lemma bind_det_exec:
  "mres (a s) = {(r,s')} \<Longrightarrow> mres ((a >>= b) s) = mres (b r (with_env_of s s'))"
  by (simp add: in_bind set_eq_iff)

lemma in_bind_det_exec:
  "mres (a s) = {(r,s')} \<Longrightarrow> (s'' \<in> mres ((a >>= b) s)) = (s'' \<in> mres (b r (with_env_of s s')))"
...
lemma bind_execI:
  "\<lbrakk> (r'',s'') \<in> mres (f s); \<exists>x \<in> mres (g r'' (with_env_of s s'')). P x \<rbrakk> \<Longrightarrow> \<exists>x \<in> mres ((f >>= g) s). P x"
