...
(* ceil\_log : log 2 ceiling function for nat, which is used for SC size encoding *)

(* ceil\_log is meant to be defined in terms of Discrete.log. In the below, however, we are using
   discrete\_log, which is just a copy of Discrete.log. This is to avoid the breakage in rec\_del
   (in ASpec) caused by importing Complex\_Main via HOL-Library.Discrete. We are not quite sure
   exactly why this breakage occurs, which was not an issue when Complex\_Main was imported via
   Word\_Lib.

   Below, a few lemmas for the properties we need about Discrete.log are copied from
   HOL-Library.Discrete along with the Discrete.log definition as discrete\_log *)

(* Discrte.log and related lemmas, copied and renamed *)
fun discrete_log :: "nat \<Rightarrow> nat"
  where [simp del]: "discrete_log n = (if n < 2 then 0 else Suc (discrete_log (n div 2)))"
...
lemma ceil_log_le_bound:
  ...
  apply (prop_tac "\<And>x y. x < y \<Longrightarrow> x \<le> y - Suc 0")
   apply arith
  by (metis discrete_log_le_iff discrete_log_exp)
