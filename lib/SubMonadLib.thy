...
locale submonad_args =
  fixes fetch :: "('c, 's) monad_state \<Rightarrow> 'b"
  fixes replace :: "'b \<Rightarrow> ('c, 's) monad_state \<Rightarrow> ('c, 's) monad_state"
  fixes guard :: "('c, 's) mpred"

  assumes args:
   "\<forall>x s. guard s \<longrightarrow> fetch (const_env (replace x) s) = x"
   "\<forall>x y s. replace x (const_env (replace y) s) = replace x s"
   "\<forall>s. replace (fetch s) s = s"

  assumes replace_preserves_guard:
   "\<And>s x. guard (const_env (replace x) s) = guard s"

definition submonad_fn ::
  "(('c, 's) monad_state \<Rightarrow> 't) \<Rightarrow>
   ('t \<Rightarrow> ('c, 's) monad_state \<Rightarrow> ('c, 's) monad_state) \<Rightarrow>
   ('c, 's) mpred \<Rightarrow>
   (unit, 't, 'r) nondet_monad \<Rightarrow>
   ('c, 's, 'r) nondet_monad"
  where
  "submonad_fn fetch replace guard m \<equiv> do
     stateAssert guard [];
     substate \<leftarrow> gets fetch;
     (rv, substate') \<leftarrow> select_f (m (monad_state () substate));
     modify (replace substate');
     return rv
   od"

locale submonad = submonad_args +
  fixes fn :: "(unit, 't, 'r) nondet_monad \<Rightarrow> ('c, 's, 'r) nondet_monad"

  assumes fn_is_sm: "fn = submonad_fn fetch replace guard"

lemma (in submonad_args) argsD1:
  "\<And>x s. guard s \<Longrightarrow> fetch (const_env (replace x) s) = x"
  by (simp add: args)
...
lemma modify_stateAssert:
  "\<lbrakk> \<And>s x. g (const_env (r x) s) = g s \<rbrakk> \<Longrightarrow>
   (modify (r x) >>= (\<lambda>u. stateAssert g [])) = (stateAssert g [] >>= (\<lambda>u. modify (r x)))"
  by (simp add: ext stateAssert_def bind_def get_def assert_def fail_def
                return_def modify_def put_def const_env_def)
...
lemma modify_gets:
  "\<lbrakk> \<And>x s. g (const_env (r x) s) = g s; \<And>x s. g s \<longrightarrow> f (const_env (r x) s) = x \<rbrakk>
  ...
  by (simp add: ext stateAssert_def assert_def modify_def bind_def get_def
                put_def gets_def return_def fail_def const_env_def)
...
lemma (in submonad) guard_preserved:
  "\<And>s s'. \<lbrakk> (rv, s') \<in> fst (fn m s) \<rbrakk> \<Longrightarrow> guard (with_env_of s s')"
  unfolding fn_is_sm submonad_fn_def
  by (clarsimp simp: stateAssert_def gets_def get_def bind_def modify_def put_def return_def
                     select_f_def replace_preserves_guard[unfolded const_env_def] in_monad)

lemma fst_stateAssertD:
  "\<And>s s' v. (v, s') \<in> fst (stateAssert g [] s) \<Longrightarrow> s' = mstate s \<and> g s"
  by (clarsimp simp: stateAssert_def in_monad)

lemma(in submonad) guarded_gets:
  "\<And>s. guard s \<Longrightarrow> fn (gets f) s = gets (f \<circ> monad_state () \<circ> fetch) s"
...
lemma (in submonad) gets:
  "fn (gets f) = (stateAssert guard [] >>= (\<lambda>_. gets (f \<circ> monad_state () \<circ> fetch)))"
  unfolding fn_is_sm submonad_fn_gets
  by (rule refl)

lemma (in submonad) return:
  "fn (return x) = (stateAssert guard [] >>= (\<lambda>_. return x))"
  using args gets
  by (fastforce simp: gets_def bind_def get_def return_def)

lemma (in submonad) mapM_guard_preserved:
  "\<lbrakk> guard (with_env_of t s); \<exists>rv. (rv, s') \<in> fst (mapM (fn \<circ> m) xs (with_env_of t s))\<rbrakk> \<Longrightarrow>
   guard (with_env_of t s')"
...
lemma (in submonad) mapM_x_guard_preserved:
  "\<lbrakk> guard (with_env_of t s); \<exists>rv. (rv, s') \<in> fst (mapM_x (fn \<circ> m) xs (with_env_of t s))\<rbrakk> \<Longrightarrow>
   guard (with_env_of t s')"
proof (induct xs arbitrary: s)
  case Nil
  thus ?case
...
lemma (in submonad) fn_stateAssert:
  ...
  apply (fastforce simp: bind_def assert_def get_def return_def const_env_def)
  done
...
proof (induct l)
  ...
  thus ?case
    ...
    apply (rename_tac s rv st rv' st')
    apply (subgoal_tac "g (with_env_of s st')")
...
lemma corres_submonad:
  "\<lbrakk> submonad f r g fn; submonad f' r' g' fn';
     ...
     \<forall>s s' ss ss'. (s, s') \<in> sr \<longrightarrow> (ss, ss') \<in> ssr \<longrightarrow> (const_env (r (mstate ss)) s, const_env (r' (mstate ss')) s') \<in> sr;
  ...
  apply (rule corres_underlying_split[OF _ _ stateAssert_sp stateAssert_sp, where r'=dc])
   apply (fastforce simp: corres_underlying_def stateAssert_def get_def
                          assert_def return_def bind_def)
  apply (rule corres_underlying_split[where r'="\<lambda>x y. (monad_state () x,  monad_state () y) \<in> ssr",
                                      OF _ _ hoare_TrueI hoare_TrueI])
   apply clarsimp
  apply (rule corres_underlying_split[where
                r'="\<lambda>(x, x') (y, y'). rvr x y \<and> (monad_state () x', monad_state () y') \<in> ssr",
                OF _ _ hoare_TrueI hoare_TrueI])
...
lemma gets_modify_comm:
  "\<And> s. \<lbrakk> g (const_env f s) = g s \<rbrakk> \<Longrightarrow>
   (do x \<leftarrow> modify f; y \<leftarrow> gets g; m x y od) s =
   (do y \<leftarrow> gets g; x \<leftarrow> modify f; m x y od) s"
  by (simp add: modify_def gets_def get_def bind_def put_def return_def const_env_def)
...
lemma submonad_comm:
  ...
  assumes z: "\<And>s x x'. r x (const_env (r' x') s) = r' x' (const_env (r x) s)"
  assumes gp: "\<And>s x. g (const_env (r' x) s) = g s" and gp': "\<And>s x. g' (const_env (r x) s) = g' s"
...
proof -
  have P: "\<And>x s. g s \<Longrightarrow> f (const_env (r' x) s) = f s"
    by (smt (verit, ccfv_threshold) gp submonad_args_def x1 x2 z)
  have Q: "\<And>x s. g' s \<Longrightarrow> f' (const_env (r x) s) = f' s"
    by (smt (verit, ccfv_threshold) const_env_monad_state gp' submonad_args.args(1)
                                    submonad_args.args(3) surjective x2 z)
  ...
  show ?thesis
    ...
    apply (simp add: bind_def get_def put_def modify_def z[unfolded const_env_def])
    done
qed
...
lemma submonad_singleton:
  "submonad_fn fetch replace \<top> (\<lambda>s. ({(rv s, s' s)}, False))
     = (\<lambda>s. ({(rv (monad_state () (fetch s)),
              mstate (replace (s' (monad_state () (fetch s))) s))}, False))"
  apply (rule ext)
...
lemma gets_submonad:
  "\<lbrakk> submonad_args fetch replace \<top>; \<And>s. f s = f' (monad_state () (fetch s)); m = gets f' \<rbrakk>
...
lemma modify_submonad:
  "\<lbrakk> \<And>s. f s = replace (mstate (f' (monad_state () (fetch s)))) s; m = modify f' \<rbrakk>
     \<Longrightarrow> modify f = submonad_fn fetch replace \<top> m"
...
lemma is_stateAssert_gets:
  ...
  apply (intro conjI impI; fastforce) (* faster than fastforce alone *)
  done
