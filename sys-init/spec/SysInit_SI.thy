...
where
  "init_tcb spec orig_caps tcb_id \<equiv> do
    ...
    \<comment> \<open>Set FPU disabled flag if required by spec; leave unset (default) if not.\<close>
    when (config_HAVE_FPU \<and> cdl_tcb_isFPUDisabled cdl_tcb) $ do
      seL4_TCB_SetFlags sel4_tcb 0 seL4_TCBFlag_fpuDisabled;
      return ()
    od;

    \<comment> \<open>FIXME: should eventually be allowed for AArch32 + HYP\<close>
