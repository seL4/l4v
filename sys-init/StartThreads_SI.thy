...
lemma is_waiting_thread_at_real_object_at [simp]:
  ...
  by (metis is_waiting_thread_at_tcb_at object_at_real_object_at(1))
...
lemma start_threads_sep:
  ...
  apply (rule mapM_x_set_sep' [where
               ...
               I="si_caps_at t dup_caps spec {obj_id. real_cnode_or_tcb_at obj_id spec} \<and>*
                  si_objects" and
  ...
  apply (rule hoare_chain)
    apply (rule_tac x = obj_id
                and xs = "{obj_id. real_cnode_or_tcb_at obj_id spec}"
