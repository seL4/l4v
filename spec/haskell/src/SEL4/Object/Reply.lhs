...
>         scReply <- getReply scReplyPtr
>         setReply scReplyPtr (scReply { replyNext = Just (Next replyPtr) })
>     reply <- getReply replyPtr
>     setReply replyPtr (reply { replyPrev = scReplyOpt })
>     setSchedContext scPtr (sc { scReply = Just replyPtr })
>     reply <- getReply replyPtr
>     setReply replyPtr (reply { replyNext = Just (Head $ scPtr) })
