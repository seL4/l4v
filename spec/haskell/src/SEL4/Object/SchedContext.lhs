...
Note that the start value is of type Maybe (PPtr TCB) for convenience, since this function will be called with
the value tcbQueueHead queue for some queue.

> findInsertionPoint :: Ord b => b -> Maybe (PPtr TCB) -> (PPtr TCB -> KernelR b) -> (b -> b -> Bool) -> Kernel (Maybe (PPtr TCB))
> findInsertionPoint val start f r =
...
Performs an ordered insert of the item t into a linked list of TCBs, according to the values given by
the reader function f, and the ordering r, assuming that the linked list is originally sorted. The linked list
must have head and end given by the queue q, and must use the tcbSchedNext field of the TCB as the next pointer
in the linked list. We first check to see whether t should be prepended to the list, then we check to see
whether t should be appended to the list. Otherwise, t should be inserted into the middle of the list. To do so,
we call the function findInsertionPoint, which will start at the head of the list and will traverse the list
until it finds an element in the list where the comparison of values fails. The item t is then inserted immediately
before this element.
