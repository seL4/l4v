...
have MIN_BUDGET_le_MAX_PERIOD: "2 * kernelWCET_us \<le> ARM.MAX_PERIOD_US"
  by (simp add: kernelWCET_us_def ARM.MAX_PERIOD_US_def)

have ticks_per_timer_unit_non_zero: "ticks_per_timer_unit \<noteq> 0"
  by (simp add: ticks_per_timer_unit_def)

have MIN_BUDGET_bound: "2 * unat kernelWCET_us * unat ticks_per_timer_unit < unat ARM.max_time"
...
have getCurrentTime_buffer_bound:
  "5 * unat ARM.MAX_PERIOD_US * unat ticks_per_timer_unit < unat ARM.max_time"
  apply (fastforce simp: unat_max_word[symmetric] ticks_per_timer_unit_def ARM.MAX_PERIOD_US_def)
  done
...
have init_domain_time_pos: "0 < us_to_ticks (15 * ARM.\<mu>s_in_ms)"
  apply (clarsimp simp: us_to_ticks_def word_less_nat_alt)
  apply (subst unat_mult_lem' | subst unat_div
         | fastforce simp: ARM.\<mu>s_in_ms_def ticks_per_timer_unit_def timer_unit_def unat_minus_one_word)+
  done

have init_domain_time_bound: "15 * unat ARM.\<mu>s_in_ms * unat ticks_per_timer_unit < unat ARM.max_time"
  apply (subst unat_mult_lem'
         | fastforce simp: ARM.\<mu>s_in_ms_def ticks_per_timer_unit_def unat_minus_one_word)+
  done

have getCurrentTime_buffer_pos:
  "0 < 5 * us_to_ticks ARM.MAX_PERIOD_US"
  apply (fastforce simp: us_to_ticks_def word_less_nat_alt ARM.MAX_PERIOD_US_def
                         ticks_per_timer_unit_def timer_unit_def)
  done

end

consts' kernelWCET_us :: ARM.ticks

axiomatization where
  kernelWCET_us_pos2: "0 < 2 * kernelWCET_us"
and
  MIN_BUDGET_le_MAX_PERIOD: "2 * kernelWCET_us \<le> ARM.MAX_PERIOD_US"

consts' ticks_per_timer_unit :: ARM.ticks
consts' timer_unit :: ARM.ticks
...
and
  MIN_BUDGET_bound: "2 * unat kernelWCET_us * unat ticks_per_timer_unit < unat ARM.max_time"
and
...
and
  ...
  init_domain_time_pos: "0 < us_to_ticks (15 * ARM.\<mu>s_in_ms)"
and
  \<comment> \<open>the 15 is from the domain time of the initial state\<close>
  init_domain_time_bound: "15 * unat ARM.\<mu>s_in_ms * unat ticks_per_timer_unit < unat ARM.max_time"
and
  \<comment> \<open>the 5 is from time_buffer_const (defined below)\<close>
  getCurrentTime_buffer_pos: "0 < 5 * us_to_ticks ARM.MAX_PERIOD_US"

definition "MAX_PERIOD = us_to_ticks ARM.MAX_PERIOD_US"

axiomatization
  ticks_to_us :: "ARM.ticks \<Rightarrow> ARM.ticks"
