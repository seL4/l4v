...
\item[\meth{Wait}] is used by an application to block until the target 
object is ready.

\item[\meth{Reply}] is used to respond to a \meth{Call}, using the
capability generated by the \meth{Call} operation.

\item[\meth{ReplyWait}] is a combination of \meth{Reply} and
\meth{Wait}. It exists for efficiency reasons: the common case of
...
Multiple \meth{Notify} system calls without an intervening \meth{Wait}
