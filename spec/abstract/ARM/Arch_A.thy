...
where
  ...
  od)"
| "arch_invoke_irq_control (IssueSGISignal irq targets control_slot sgi_slot) =
     without_preemption
       (cap_insert (ArchObjectCap (SGISignalCap irq targets)) control_slot sgi_slot)"
...
definition perform_sgi_invocation :: "sgi_signal_invocation \<Rightarrow> (unit,'z::state_ext) s_monad" where
  "perform_sgi_invocation iv \<equiv> case iv of
     SGISignalGenerate irq targets \<Rightarrow> do_machine_op $ ipiSendTarget (ucast irq) (ucast targets)"
