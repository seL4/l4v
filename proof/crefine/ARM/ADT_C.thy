...
definition
  cready_queues_to_H
  :: "(tcb_C ptr \<rightharpoonup> tcb_C) \<Rightarrow> (tcb_queue_C[256]) \<Rightarrow> word8 \<times> word8 \<Rightarrow> word32 list"
  where
...
definition
  cbitmap_L1_to_H :: "32 word[1] \<Rightarrow> (8 word \<Rightarrow> 32 word)"
where
  "cbitmap_L1_to_H l1 \<equiv> \<lambda>d. if d \<le> maxDomain then l1.[unat d] else 0"

definition
  cbitmap_L2_to_H :: "32 word[8][1] \<Rightarrow> (8 word \<times> nat \<Rightarrow> 32 word)"
where
