...
proof (rule rl)
  ...
  from al p0 show wb: "bits < word_bits"
    by (clarsimp elim!: is_aligned_get_word_bits[where 'a=32, folded word_bits_def])
qed
(*FIX me: move *)
...
fun
  epQ :: "endpoint \<Rightarrow> word32 list"
  where
  "epQ IdleEP = []"
  | "epQ (RecvEP ts) = ts"
  | "epQ (SendEP ts) = ts"
...
proof (rule cmap_relation_cong)
  ...
  proof (subst intvl_aligned_top [where 'a=32, folded word_bits_def, OF _ al _ _ wb])
    fix x tcb
    ...
    with xin koat show "objBits tcb \<le> bits" using wb
      ...
      apply clarsimp
      apply (drule (1) word_sub_mono2 [where b = "(2 :: word32) ^ objBits tcb - 1"
        and d = "2 ^ bits - 1", simplified field_simps])
      apply (subst field_simps [symmetric], subst olen_add_eqv [symmetric])
      apply (simp add: field_simps)
      apply (subst field_simps [symmetric], subst olen_add_eqv [symmetric])
      apply (rule is_aligned_no_overflow' [OF al])
      apply (subgoal_tac "(2 :: word32) ^ objBits tcb \<noteq> 0")
  ...
  { assume asid_map:
    ...
    have clift:
      "\<And>p. clift (hrs_htd_update (typ_region_bytes ptr bits)
                                  (t_hrs_' (globals s'))) (p::pde_C ptr) =
           (if p \<in> pde_Ptr ` {ptr..+2 ^ bits} then None else cslift s' p)"
