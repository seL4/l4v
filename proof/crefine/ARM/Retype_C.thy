...
lemma ccorres_placeNewObject_tcb:
  ...
  apply (subgoal_tac "c_guard (tcb_Ptr (regionBase + 0x200))")
   ...
   apply (rule bexI [OF _ placeNewObject_eq])
      apply (clarsimp simp: split_def new_cap_addrs_def)
      apply (cut_tac \<sigma>=\<sigma> and x=x and ks="ksPSpace \<sigma>" and p="tcb_Ptr (regionBase + 0x200)"
...
lemma ccorres_typ_region_bytes_dummy:
  ...
  apply (simp add: cmap_array_typ_region_bytes_triv
               ...
               objBitsT_simps[simplified word_size_bits_def] word_bits_def pteBits_def pdeBits_def
