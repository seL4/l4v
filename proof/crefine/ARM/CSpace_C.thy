  ...
  apply (cinit simp del: return_bind)
   apply (rule ccorres_symb_exec_l)
      ...
      apply (rule_tac r'="dc" and xf'="xfdc" in ccorres_split_nothrow)
          ...
          apply (simp add: cinterrupt_relation_def Kernel_C_maxIRQ ucast_irq_array_guard)
          apply (clarsimp simp: word_le_nat_alt unat_irq_array_guard split: if_split)
         apply ceqv
...
where
  "cleanup_info_wf' cap \<equiv> case cap of
      IRQHandlerCap irq \<Rightarrow> (ucast irq :: machine_word) \<le> Kernel_Config.maxIRQ
...
lemma postCapDeletion_ccorres:
  ...
  apply (clarsimp simp: isCap_simps)
  apply (frule cap_get_tag_isCap_unfolded_H_cap(5))
  apply (clarsimp simp: cap_irq_handler_cap_lift ccap_relation_def cap_to_H_def irq_len_val
                        cleanup_info_wf'_def c_valid_cap_def cl_valid_cap_def ucast_ucast_mask)
  done
...
lemma Arch_sameRegionAs_spec:
  ...
  apply (cases capa; simp add: cap_get_tag_isCap_unfolded_H_cap isCap_simps)
         apply (find_goal \<open>match premises in "capa = SGISignalCap ?irq ?targets" \<Rightarrow> -\<close>)
         apply (rule conjI, solves \<open>clarsimp simp: cap_tag_defs\<close>, clarsimp?)+
         apply (frule cap_get_tag_isCap_unfolded_H_cap)
         apply (rule conjI; clarsimp simp: from_bool_eq_if' from_bool_0 split: if_splits)
          apply (clarsimp simp: cap_get_tag_SGISignalCap ucast_eq_mask)
         apply (solves \<open>simp add: cap_get_tag_isCap isCap_simps\<close>)
...
lemma sameRegionAs_spec:
  ...
  apply (case_tac capa, simp_all add: cap_get_tag_isCap_unfolded_H_cap isCap_simps)
          ...
          apply (case_tac capb,
           ...
           apply (simp add: cap_to_H_def)
           apply (clarsimp simp: up_ucast_inj_eq c_valid_cap_def irq_len_val
...
lemma same_if_cond_eq:
  "x \<noteq> y \<Longrightarrow> ((if P then x else y) = (if Q then x else y)) = (P = Q)"
  by (simp split: if_split)

(* FIXME SGI: move to Wellformed_C or similar *)
...
lemma from_bool_inj[simp]: (* FIXME SGI: move to Word_Lib (copies also in AARCH64) *)
  "(from_bool x = from_bool y) = (x = y)"
  by (auto simp: from_bool_def split: bool.splits)

lemma cap_get_tag_isCap_unfolded_H_cap_page: (* FIXME SGI: move up *)
...
lemma Arch_sameObjectAs_spec:
  ...
  supply cap_tag_neqs[simp] if_P_then_t_else_f_eq_t[simp]
  apply vcg
  apply (clarsimp simp: ARM_H.sameObjectAs_def cong: if_cong)
  apply (case_tac capa; simp add: cap_get_tag_isCap_unfolded_H_cap
                                  cap_get_tag_isCap_unfolded_H_cap_page isCap_defs)
      apply fastforce+
    apply (case_tac capb; simp add: cap_get_tag_isCap_unfolded_H_cap
                                    cap_get_tag_isCap_unfolded_H_cap_page isCap_simps)
         apply fastforce+
