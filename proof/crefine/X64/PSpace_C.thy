...
lemma storePDPTE_Basic_ccorres':
  "\<lbrakk> cpdpte_relation pdpte pdpte' \<rbrakk> \<Longrightarrow>
   ccorres dc xfdc \<top> {s. ptr_val (f s) = p} hs
     (storePDPTE p pdpte)
     (Guard C_Guard {s. s \<Turnstile>\<^sub>c f s}
        (Basic (\<lambda>s. globals_update( t_hrs_'_update
            (hrs_mem_update (heap_update (f s) pdpte'))) s)))"
  apply (simp add: storePDPTE_def)
  apply (rule setObject_ccorres_helper)
    apply (simp_all add: objBits_simps archObjSize_def bit_simps)
  apply (rule conseqPre, vcg)
  apply (rule subsetI, clarsimp simp: Collect_const_mem)
  apply (rule cmap_relationE1, erule rf_sr_cpdpte_relation,
         erule ko_at_projectKO_opt)
  apply (rule conjI, fastforce intro: typ_heap_simps)
  apply (clarsimp simp: rf_sr_def cstate_relation_def Let_def)
  apply (rule conjI)
   apply (clarsimp simp: cpspace_relation_def typ_heap_simps
                         update_pdpte_map_to_pdptes
...
lemma storePDPTE_Basic_ccorres:
  "\<lbrakk> cpdpte_relation pdpte pdpte' \<rbrakk> \<Longrightarrow>
   ccorres dc xfdc \<top> {s. f s = p} hs
     (storePDPTE p pdpte)
     (Guard C_Guard {s. s \<Turnstile>\<^sub>c pdpte_Ptr (f s)}
        (Basic (\<lambda>s. globals_update( t_hrs_'_update
            (hrs_mem_update (heap_update (pdpte_Ptr (f s)) pdpte'))) s)))"
  apply (rule ccorres_guard_imp2)
   apply (erule storePDPTE_Basic_ccorres')
  apply simp
  done

lemma storePML4E_Basic_ccorres':
  "\<lbrakk> cpml4e_relation pml4e pml4e' \<rbrakk> \<Longrightarrow>
   ccorres dc xfdc
     \<top>
     {s. ptr_val (f s) = p} hs
     (storePML4E p pml4e)
     (Guard C_Guard {s. s \<Turnstile>\<^sub>c f s}
        (Basic (\<lambda>s. globals_update( t_hrs_'_update
            (hrs_mem_update (heap_update (f s) pml4e'))) s)))"
  apply (simp add: storePML4E_def)
  apply (rule setObject_ccorres_helper)
    apply (simp_all add: objBits_simps archObjSize_def bit_simps)
  apply (rule conseqPre, vcg)
  apply (rule subsetI, clarsimp simp: Collect_const_mem)
  apply (rule cmap_relationE1, erule rf_sr_cpml4e_relation,
         erule ko_at_projectKO_opt)
  apply (rule conjI, fastforce intro: typ_heap_simps)
  apply (case_tac "f x", clarsimp)
  apply (clarsimp simp: rf_sr_def cstate_relation_def Let_def)
  apply (rule conjI)
   apply (clarsimp simp: cpspace_relation_def typ_heap_simps
                         update_pml4e_map_to_pml4es
                         update_pml4e_map_tos
                         carray_map_relation_upd_triv)
   apply (erule cmap_relation_updI,
          erule ko_at_projectKO_opt, simp+)
  apply (simp add: cready_queues_relation_def
...
lemma storePML4E_Basic_ccorres:
  "\<lbrakk> cpml4e_relation pml4e pml4e' \<rbrakk> \<Longrightarrow>
   ccorres dc xfdc \<top> {s. f s = p} hs
     (storePML4E p pml4e)
     (Guard C_Guard {s. s \<Turnstile>\<^sub>c pml4e_Ptr (f s)}
        (Basic (\<lambda>s. globals_update(t_hrs_'_update
            (hrs_mem_update (heap_update (pml4e_Ptr (f s)) pml4e'))) s)))"
  apply (rule ccorres_guard_imp2)
