        ...
        (\<lambda>a. hrs_mem_update (heap_update (Ptr &(p\<rightarrow>[''tcbTimeSlice_C'']) :: machine_word ptr) (5 :: machine_word))
              (hrs_mem_update
                (heap_update ((Ptr &((Ptr &((Ptr &(p\<rightarrow>[''tcbArch_C'']) :: arch_tcb_C ptr)\<rightarrow>[''tcbContext_C''])
                     :: user_context_C ptr)\<rightarrow>[''registers_C''])) :: (machine_word[23]) ptr)
                  (Arrays.update
                     (Arrays.update
                       (Arrays.update (h_val (hrs_mem a) ((Ptr &((Ptr &((Ptr &(p\<rightarrow>[''tcbArch_C'']) :: arch_tcb_C ptr)\<rightarrow>[''tcbContext_C''])
                           :: user_context_C ptr)\<rightarrow>[''registers_C''])) :: (machine_word[23]) ptr))
                               (unat Kernel_C.CS) (selCS3 :: machine_word))
                           (unat Kernel_C.SS) (selDS3 :: machine_word))
                       (unat Kernel_C.FLAGS) ((1 << 9) || bit 1)))
...
proof -
  ...
  (* FIXME x64:  values need checking *)
                  ...
                  \<lparr>tcbArch_C := tcbArch_C (from_bytes (replicate (size_of TYPE(tcb_C)) 0))
                    \<lparr>tcbContext_C := tcbContext_C (tcbArch_C (from_bytes (replicate (size_of TYPE(tcb_C)) 0)))
                     \<lparr>registers_C :=
                        Arrays.update
                           (Arrays.update
                              (Arrays.update (registers_C (tcbContext_C (tcbArch_C (from_bytes (replicate (size_of TYPE(tcb_C)) 0)))))
                                    (unat Kernel_C.CS) selCS3)
                              (unat Kernel_C.SS) selDS3)
                           (unat Kernel_C.FLAGS) ((1 << 9) || bit 1)\<rparr>\<rparr>,
  ...
  hence "cpspace_relation ?ks (underlying_memory (ksMachineState \<sigma>))  (t_hrs_' ?gs)"
    ...
    -- "tcb"
     ...
     apply (rule pks)
     apply (rule tcb_rel[simplified FLAGS_default_eq])
    -- "ep"
...
lemma ccorres_placeNewObject_notification:
  "ccorresG rf_sr \<Gamma> dc xfdc
   (pspace_aligned' and pspace_distinct' and pspace_no_overlap' regionBase 4
      ...
      and ret_zero regionBase 32
      and K (regionBase \<noteq> 0
      \<and> {regionBase..+32} \<inter> kernel_data_refs = {}
      \<and> range_cover regionBase 5 5 1))
   ({s. region_actually_is_zero_bytes regionBase 32 s})
    hs
...
lemma ccorres_placeNewObject_tcb:
  "ccorresG rf_sr \<Gamma> dc xfdc
   ...
   ({s. region_actually_is_zero_bytes regionBase 0x800 s})
    hs
   (placeNewObject regionBase (makeObject :: tcb) 0)
   (\<acute>tcb :== tcb_Ptr (regionBase + 0x400);;
        (global_htd_update (\<lambda>s. ptr_retyp (Ptr (ptr_val (tcb_' s) - 0x400) :: (cte_C[5]) ptr)
...
lemma placeNewObject_pde:
  "ccorresG rf_sr \<Gamma> dc xfdc
   ( valid_global_refs' and pspace_aligned' and pspace_distinct' and pspace_no_overlap' regionBase pdBits
    ...
    (global_htd_update (\<lambda>_. (ptr_retyp (Ptr regionBase :: (pde_C[512]) ptr))))"
  apply (rule ccorres_from_vcg_nofail)
  apply clarsimp
  apply (rule conseqPre)
  apply vcg
...
lemma placeNewObject_pdpte:
  "ccorresG rf_sr \<Gamma> dc xfdc
   ( valid_global_refs' and pspace_aligned' and pspace_distinct' and pspace_no_overlap' regionBase pdptBits
    ...
    (global_htd_update (\<lambda>_. (ptr_retyp (Ptr regionBase :: (pdpte_C[512]) ptr))))"
  apply (rule ccorres_from_vcg_nofail)
  apply clarsimp
  apply (rule conseqPre)
  apply vcg
  ...
  "createObject_hs_preconds regionBase newType userSize d \<equiv>
     (invs' and pspace_no_overlap' regionBase (getObjectSize newType userSize)
           ...
           and K(regionBase \<noteq> 0
                   ...
                   \<and> (newType = APIObjectType apiobject_type.Untyped \<longrightarrow> userSize \<le> 61)
...
proof -
  ...
  sorry (*
    ...
    apply (cinit' lift: t_' regionBase_' userSize_' deviceMemory_')
     ...
     apply (rule ccorres_rhs_assoc)+
     apply (clarsimp simp: hrs_htd_update ptBits_def objBits_simps archObjSize_def
        X64_H.createObject_def pageBits_def pdBits_def)
    ...
    apply (intro conjI, simp_all add: bit_simps)[1]
        apply fastforce
    ...
    apply (simp add: vcpu_bits_def bit_simps is_aligned_weaken)
    done *)
qed
  ...
  apply (cinit lift: t_' regionBase_' userSize_' deviceMemory_')
   ...
   apply (rule_tac
    ...
    apply (clarsimp simp: object_type_from_H_def
      X64_H.toAPIType_def Kernel_C_defs toAPIType_def
    ...
    apply (rule ccorres_guard_imp)
      apply (rule_tac apiType=apiType in ccorres_apiType_split)
          ...
          apply (clarsimp simp: Kernel_C_defs object_type_from_H_def
            toAPIType_def X64_H.toAPIType_def nAPIObjects_def
...
lemma createObject_valid_cap':
  "\<lbrace>\<lambda>s. pspace_no_overlap' ptr (APIType_capBits ty us) s \<and>
         ...
         (ty = APIObjectType apiobject_type.CapTableObject \<longrightarrow> 0 < us) \<and>
         (ty = APIObjectType apiobject_type.Untyped \<longrightarrow> 4 \<le> us \<and> us \<le> 61) \<and> ptr \<noteq> 0\<rbrace>
...
lemma APIType_capBits_min:
  ...
  by (simp add: APIType_capBits_def objBits_simps' bit_simps
...
lemma createNewCaps_valid_cap_hd:
    "\<lbrace>\<lambda>s. pspace_no_overlap' ptr sz s \<and>
        ...
        (ty = APIObjectType ArchTypes_H.apiobject_type.Untyped \<longrightarrow> 4\<le> us \<and> us \<le> 61) \<and>
       ptr \<noteq> 0 \<rbrace>
...
lemma createObject_untyped_region_is_zero_bytes:
  "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^bsub>/UNIV\<^esub> {s. let tp = (object_type_to_H (t_' s));
      ...
      in (\<not> to_bool (deviceMemory_' s)
          ...
          \<and> sz < 64 \<and> (tp = APIObjectType ArchTypes_H.apiobject_type.Untyped \<longrightarrow> sz \<ge> 4)}
      Call createObject_'proc
...
sorry (*
  ...
  apply (case_tac newType,simp_all add:object_type_from_H_def Kernel_C_defs
     ...
     by (clarsimp simp: ARMSmallPageBits_def ARMLargePageBits_def
                        ARMSectionBits_def ARMSuperSectionBits_def bit_simps)+ *)
