     ...
     apply (ctac (no_vcg) add: getActiveIRQ_ccorres)
      ...
      apply (rule ccorres_split_nothrow_novcg)
          apply (rule_tac P="\<lambda>_. rv \<noteq> None" and R=\<top> in ccorres_cond_both)
            apply (auto simp: irqInvalid_def Kernel_C.irqInvalid_def split: option.splits)[1]
          ...
          apply (clarsimp simp: handleSpuriousIRQ_def)
...
lemma ccorres_get_registers:
  "\<lbrakk> \<And>cptr msgInfo. ccorres dc xfdc
     ((\<lambda>s. P s \<and> Q s \<and>
           obj_at' (\<lambda>tcb. (user_regs o atcbContextGet o tcbArch) tcb X64.capRegister = cptr
                      \<and>   (user_regs o atcbContextGet o tcbArch) tcb X64.msgInfoRegister = msgInfo)
  ...
  apply (erule_tac x="(user_regs o atcbContextGet o tcbArch) ko X64.capRegister" in meta_allE)
  apply (erule_tac x="(user_regs o atcbContextGet o tcbArch) ko X64.msgInfoRegister" in meta_allE)
  ...
  apply (clarsimp simp: ctcb_relation_def ccontext_relation_def cregs_relation_def
                        X64.msgInfoRegister_def X64.capRegister_def
