...
lemma cte_wp_at_casesE' [consumes 1, case_names cte tcb]:
  ...
  \<And>cte. \<lbrakk> ksPSpace s p = Some (KOCTE cte); is_aligned p cte_level_bits; P cte; ps_clear p cte_level_bits s \<rbrakk> \<Longrightarrow> R;
  ...
  by (fastforce simp: cte_wp_at_cases' cte_level_bits_def)
...
proof -
  ...
  have trivia: "size_of TYPE(pte_C[512]) = 2 ^ ptBits"
               "size_of TYPE(pde_C[512]) = 2 ^ pdBits"
               "size_of TYPE(pdpte_C[512]) = 2 ^ pdptBits"
               "size_of TYPE(pml4e_C[512]) = 2 ^ pml4Bits"
  ...
  note cmap_array = cmap_array_typ_region_bytes[where 'a=pte, OF refl _ al _ trivia(1)]
     ...
     cmap_array_typ_region_bytes[where 'a=pdpte, OF refl _ al _ trivia(3)]
     cmap_array_typ_region_bytes[where 'a=pml4e, OF refl _ al _ trivia(4)]
