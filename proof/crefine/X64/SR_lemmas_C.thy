...
definition
  "pageSize cap \<equiv> case cap of ArchObjectCap (PageCap f R ty sz dev m) \<Rightarrow> sz"
...
lemma map_to_ctes_upd_cte':
  "\<lbrakk> ksPSpace s p = Some (KOCTE cte'); is_aligned p 5; ps_clear p 5 s \<rbrakk>
...
lemma fst_setCTE:
  ...
  and     rl: "\<And>s'. \<lbrakk> ((), s') \<in> fst (setCTE dest cte s);
           ...
           (map_to_pdptes (ksPSpace s) = map_to_pdptes (ksPSpace s'));
...
lemma rf_sr_cpdpte_relation:
  "(s, s') \<in> rf_sr \<Longrightarrow> cmap_relation (map_to_pdptes (ksPSpace s))
                         (cslift s') pdpte_Ptr cpdpte_relation"
  by (clarsimp simp: rf_sr_def cstate_relation_def
                     Let_def cpspace_relation_def)

lemma rf_sr_cpml4e_relation:
  "(s, s') \<in> rf_sr \<Longrightarrow> cmap_relation (map_to_pml4es (ksPSpace s))
                         (cslift s') pml4e_Ptr cpml4e_relation"
  by (clarsimp simp: rf_sr_def cstate_relation_def
                     Let_def cpspace_relation_def)
...
lemma ntfnQueue_tail_mask_4 [simp]:
  ...
  apply (clarsimp simp: mask_def word_bw_assocs sign_extend_def)
  apply word_bitwise *)
...
lemma cap_get_tag_isCap_unfolded_H_cap:
  ...
  and "ccap_relation (capability.ArchObjectCap (arch_capability.PDPointerTableCap v34 v35)) cap' \<Longrightarrow> (cap_get_tag cap' = scast cap_pdpt_cap)"
...
lemma cap_get_tag_isCap_ArchObject2:
  ...
  and   "(cap_get_tag cap' = scast cap_page_directory_cap)
           = (isArchObjectCap cap \<and> isPageDirectoryCap (capCap cap))"
  and   "(cap_get_tag cap' = scast cap_pdpt_cap)
           = (isArchObjectCap cap \<and> isPDPointerTableCap (capCap cap))"
  and   "(cap_get_tag cap' = scast cap_pml4_cap)
...
lemma page_map_l4_at_carray_map_relation:
  "\<lbrakk> page_map_l4_at' pml4 s; cpspace_pml4e_array_relation (ksPSpace s) hp \<rbrakk>
    \<Longrightarrow> clift hp (pml4_Ptr pml4) \<noteq> None"
  ...
  apply (drule_tac x="p' && mask pml4Bits >> 3" in spec)
  apply (clarsimp simp: shiftr_shiftl1)
  apply (drule mp)
   apply (simp add: shiftr_over_and_dist mask_def bit_simps
                    order_le_less_trans[OF word_and_le1])
  apply (clarsimp simp: typ_at_to_obj_at_arches objBits_simps archObjSize_def bit_simps
                        is_aligned_andI1 add.commute word_plus_and_or_coroll2 and_not_mask[symmetric]
                 dest!: obj_at_ko_at' ko_at_projectKO_opt)
  done

lemma page_map_l4_at_rf_sr:
   "\<lbrakk> page_map_l4_at' pml4 s; (s, s') \<in> rf_sr \<rbrakk>
    \<Longrightarrow> cslift s' (pml4_Ptr pml4) \<noteq> None"
  by (clarsimp simp: rf_sr_def cstate_relation_def Let_def
                     cpspace_relation_def page_map_l4_at_carray_map_relation)

lemma pd_pointer_table_at_carray_map_relation:
  "\<lbrakk> pd_pointer_table_at' pdpt s; cpspace_pdpte_array_relation (ksPSpace s) hp \<rbrakk>
    \<Longrightarrow> clift hp (pdpt_Ptr pdpt) \<noteq> None"
  apply (clarsimp simp: carray_map_relation_def h_t_valid_clift_Some_iff)
  apply (drule spec, erule iffD1)
  apply (clarsimp simp: pd_pointer_table_at'_def)
  apply (drule_tac x="p' && mask pdptBits >> 3" in spec)
  apply (clarsimp simp: shiftr_shiftl1)
  apply (drule mp)
   apply (simp add: shiftr_over_and_dist mask_def bit_simps
                    order_le_less_trans[OF word_and_le1])
...
lemma pd_pointer_table_at_rf_sr:
   "\<lbrakk> pd_pointer_table_at' pd s; (s, s') \<in> rf_sr \<rbrakk>
    \<Longrightarrow> cslift s' (Ptr pd :: (pdpte_C[512]) ptr) \<noteq> None"
  by (clarsimp simp: rf_sr_def cstate_relation_def Let_def
                     cpspace_relation_def pd_pointer_table_at_carray_map_relation)
...
lemma page_directory_at_rf_sr:
   "\<lbrakk> page_directory_at' pd s; (s, s') \<in> rf_sr \<rbrakk>
    \<Longrightarrow> cslift s' (Ptr pd :: (pde_C[512]) ptr) \<noteq> None"
  by (clarsimp simp: rf_sr_def cstate_relation_def Let_def
...
lemma page_table_at_carray_map_relation:
  ...
  apply (clarsimp simp: shiftr_shiftl1)
  apply (drule mp)
   apply (simp add: shiftr_over_and_dist bit_simps mask_def
                    order_le_less_trans[OF word_and_le1])
  apply (clarsimp simp: typ_at_to_obj_at_arches objBits_simps archObjSize_def
                        bit_simps and_not_mask[symmetric]
