...
lemma updateASIDMap_isolatable:
  "thread_actions_isolatable idx (updateASIDMap f)"
  apply (clarsimp simp: updateASIDMap_def findVSpaceForASIDAssert_def assert_def
                        checkPML4At_def checkPML4UniqueToASID_def checkPML4ASIDMapMembership_def
                        stateAssert_def2 bind_assoc catch_def)
  apply (intro gets_isolatable thread_actions_isolatable_bind[OF _ _ hoare_pre(1)]
               findVSpaceForASID_isolatable thread_actions_isolatable_fail
               thread_actions_isolatable_returns conjI impI allI
               thread_actions_isolatable_if modify_isolatable|
             simp add: projectKO_opt_asidpool page_map_l4_at_partial_overwrite
                split: sum.splits
           | wp)+
  done
