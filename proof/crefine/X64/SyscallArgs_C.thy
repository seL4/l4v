...
(* FIXME x64: relook after msgReg merge *)
...
lemma getMRs_tcbContext:
  ...
  getMRs thread buffer info
  \<lbrace>\<lambda>rv s. obj_at' (\<lambda>tcb. atcbContextGet (tcbArch tcb) (X64_H.msgRegisters ! n) = rv ! n) (ksCurThread s) s\<rbrace>"
...
lemma ccap_relation_frame_tags:
  "ccap_relation (ArchObjectCap (PageCap v0 v1 mt v2 dev  v3)) cap \<Longrightarrow>
...
lemma ccap_relation_page_is_device:
  "ccap_relation (capability.ArchObjectCap (arch_capability.PageCap v0a v1 mt v2 d v3)) c
...
lemma lookupIPCBuffer_ccorres[corres]:
  ...
  sorry (* FIXME x64: waiting on bit size changes
...
lemma getMRs_user_word:
  ...
  sorry (* FIXME x64: waiting for msgReg merge
...
lemma length_msgRegisters:
  "length X64_H.msgRegisters = unat (scast n_msgRegisters :: machine_word)"
  sorry (*by (simp add: msgRegisters_unfold n_msgRegisters_def) *)
...
lemma getSyscallArg_ccorres_foo:
  ...
  apply (cinit lift: i_' ipc_buffer_')
   ...
   apply (rule_tac R=\<top> and P="\<lambda>_. n < unat (scast n_msgRegisters :: machine_word)" in ccorres_cond_both)
    ...
    apply (rule ccorres_add_return2)
    apply (rule ccorres_symb_exec_l)
       apply (rule_tac P="\<lambda>s. n < unat (scast n_msgRegisters :: machine_word) \<and> obj_at' (\<lambda>tcb. atcbContextGet (tcbArch tcb) (X64_H.msgRegisters!n) = x!n) (ksCurThread s) s"
                   and P' = UNIV
