...
definition
  "replyFromKernel_success_empty thread \<equiv> do
     ...
     asUser thread $ setRegister X64_H.badgeRegister 0;
...
lemma syscallMessage_ccorres:
  ...
  apply (simp add: X64_H.syscallMessage_def syscallMessageC_def
                   X64.syscallMessage_def
...
lemma msgRegisters_offset_conv:
  "\<And>offset i. \<lbrakk> offset + i < length X64_H.msgRegisters \<rbrakk> \<Longrightarrow>
   ...
   register_from_H (X64_H.msgRegisters ! (offset + i))"
...
lemma copyMRs_register_loop_helper:
  fixes n
  defines regs: "regs \<equiv> take (unat n) X64_H.msgRegisters"
  shows
...
shows
  ...
  apply (cinit lift: n_' sendBuf_' recvBuf_' sender_' receiver_'
   ...
   apply (rule_tac P = "length (take (unat n) X64_H.msgRegisters) <
                        2 ^ word_bits"
                in ccorres_gen_asm)
   apply (rule ccorres_split_nothrow_novcg)
     ...
     apply (rule_tac P="unat n \<le> length X64_H.msgRegisters" in ccorres_cases)
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule_tac i="length X64_H.msgRegisters"
...
lemma exceptionMessage_length_aux :
  "\<And>n. n < length X64_H.exceptionMessage \<Longrightarrow> n < unat n_exceptionMessage"
  by (simp add: X64.exceptionMessage_def X64_H.exceptionMessage_def n_exceptionMessage_def)
...
lemma copyMRsFault_ccorres_exception:
  ...
  apply (cinit' lift: sender_' receiver_' receiveIPCBuffer_'
   ...
   apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
       ...
       apply (rule_tac F="K $ obj_at' (\<lambda>tcb. map ((user_regs o atcbContext o tcbArch) tcb) X64_H.exceptionMessage = msg) sender"
                     in ccorres_mapM_x_while)
           apply (clarsimp simp: n_msgRegisters_def)
           apply (rule ccorres_guard_imp2)
            apply (rule_tac t=sender and r="X64_H.exceptionMessage ! n"
...
proof -
  ...
  apply (cinit' lift: sender_' receiver_' receiveIPCBuffer_'
   ...
   apply (simp only: mapM_x_append[where xs="take (unat n_msgRegisters) (zip as bs)"
                                         ...
                                         for as bs, simplified] bind_assoc)
   apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
       apply (rule_tac F="K $ obj_at' (\<lambda>tcb. map ((user_regs o atcbContext o tcbArch) tcb) X64_H.syscallMessage = msg) sender"
                     in ccorres_mapM_x_while)
           apply (clarsimp simp: n_msgRegisters_def)
           apply (rule ccorres_guard_imp2)
            apply (rule_tac t=sender and r="X64_H.syscallMessage ! n"
                            in ccorres_add_getRegister)
            apply (ctac(no_vcg))
             apply (rule_tac P="\<lambda>s. rv = msg ! n" in ccorres_cross_over_guard)
             apply (simp add: setMR_def length_msgRegisters n_msgRegisters_def
...
next
  ...
  have is_the_ep_maskCapRights:
    ...
    apply (case_tac acap)
     apply (clarsimp simp:X64_H.maskCapRights_def isPageCap_def)+
    done
  ...
  "\<And>slot cap. \<lbrace>\<lambda>s. \<not> (is_the_ep cap)\<rbrace> deriveCap slot cap \<lbrace>\<lambda>rv s. \<not> (is_the_ep rv)\<rbrace>, -"
    ...
    apply (rename_tac acap)
    apply (case_tac acap)
     apply (simp_all add:X64_H.deriveCap_def Let_def isCap_simps is_the_ep_def)
...
lemma lookupIPCBuffer_not_Some_0:
  ...
  apply (simp add: lookupIPCBuffer_def X64_H.lookupIPCBuffer_def)
...
lemma length_exceptionMessage:
  "length X64_H.exceptionMessage = unat n_exceptionMessage"
  ...
  "ccorres dc xfdc
           ...
           (do t \<leftarrow> getSanitiseRegisterInfo r;
               zipWithM_x (\<lambda>rs rd. do v \<leftarrow> asUser s (getRegister rs);
                                        asUser r (setRegister rd (X64_H.sanitiseRegister t rd v))
                                        od)
               X64_H.msgRegisters (take len X64_H.exceptionMessage)
            od)
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule ccorres_zipWithM_x_while)
             ...
             apply (rule ccorres_symb_exec_r)
              ...
              apply clarsimp
              apply vcg
              apply (rule conjI, simp add: X64_H.exceptionMessage_def
                                           X64.exceptionMessage_def word_of_nat_less)
              apply (simp add: msgRegisters_ccorres n_msgRegisters_def length_msgRegisters
                               X64_H.exceptionMessage_def X64.exceptionMessage_def
             ...
             apply (rule conseqPre, vcg)
             apply (clarsimp simp: word_of_nat_less X64_H.exceptionMessage_def
                                   X64.exceptionMessage_def)
          ...
          apply wp
            apply simp+
         apply (clarsimp simp: X64_H.exceptionMessage_def
                               X64.exceptionMessage_def
                               word_bits_def)
         apply unat_arith
        apply ceqv
...
lemma copyMRsFaultReply_ccorres_syscall:
  ...
  shows "ccorres dc xfdc
           ...
           (do t \<leftarrow> getSanitiseRegisterInfo r;
               a \<leftarrow> zipWithM_x (\<lambda>rs rd. do v \<leftarrow> asUser s (getRegister rs);
                                           asUser r (setRegister rd (X64_H.sanitiseRegister t rd v))
                                        od)
               X64_H.msgRegisters (take len X64_H.syscallMessage);
  ...
  note empty_fail_cond[simp] tl_drop_1[simp]
  show ?thesis
    apply (unfold K_def, rule ccorres_gen_asm) using [[goals_limit=1]]
    apply (cinit' lift: sender_' receiver_'
                        id___anonymous_enum_'
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule ccorres_zipWithM_x_while)
             ...
             apply (rule ccorres_symb_exec_r)
               ...
               apply vcg
              apply vcg
              apply (rule conjI, simp add: X64_H.syscallMessage_def
                                           X64.syscallMessage_def word_of_nat_less
     ...
     apply (ctac(no_vcg))
      apply (rule ccorres_symb_exec_r)
        apply (ctac(no_vcg) add: cteDeleteOne_ccorres[where w="scast cap_reply_cap"])
         ...
         apply (wpsimp wp: sts_valid_objs' setThreadState_st_tcb)+
         ...
         apply (rule ccorres_split_nothrow_novcg [OF ccorres_call,
           ...
           apply (rule ccorres_split_nothrow_novcg)
             ...
             apply (rule_tac R=\<top> in ccorres_cond2)
              ...
              apply (wp sts_valid_objs' setThreadState_st_tcb | simp)+
             apply (ctac add: setThreadState_ccorres_simple)
             apply wp
            apply ((wp threadSet_sch_act hoare_weak_lift_imp
...
lemma tcb_queue'_head_end_NULL:
  ...
  apply -
  apply (erule tcb_queue_relationE')
  apply (simp add: tcb_queue_head_empty_iff split: if_splits)
...
lemma tcbEPAppend_spec:
  ...
  apply (clarsimp split: if_split_asm)
   ...
   apply (rule conjI[rotated],
          simp add: packed_heap_update_collapse_hrs fpu_state_preservation typ_heap_simps')
   apply (rule ext)
  ...
  apply (intro conjI)
   ...
   apply (clarsimp simp: tcb_null_ep_ptrs_def typ_heap_simps h_t_valid_clift_Some_iff
                  split: if_split)
  apply (drule h_t_valid_clift_Some_iff[THEN iffD1]; clarsimp simp: packed_heap_update_collapse_hrs)
              ...
              apply (rule_tac xf'=canGrantReply_'
               ...
               apply (ctac(no_vcg))
                ...
                apply (rule_tac val="from_bool (blockingIPCIsCall sendState)"
                 ...
                 apply ctac
                   apply (ctac add: possibleSwitchTo_ccorres)
                  apply (wpsimp wp: sts_st_tcb' sts_valid_objs')
    ...
    apply (rule ccorres_split_nothrow_novcg)
      ...
      apply (ctac (no_vcg))
       ...
       apply (ctac (no_vcg))
        apply (ctac add: possibleSwitchTo_ccorres)
       apply (simp)
       apply (wp weak_sch_act_wf_lift_linear tcb_in_cur_domain'_lift)[1]
      apply (wp sts_valid_objs' sts_runnable)
