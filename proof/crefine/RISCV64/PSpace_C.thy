...
lemma ccorres_pre_getObject_sc:
  ...
  shows   "ccorres r xf
             ...
             {s. \<forall> sc sc'. cslift s (Ptr p) = Some sc' \<and> csched_context_relation sc sc'
                           \<longrightarrow> s \<in> P' sc} hs
             (getSchedContext p >>= (\<lambda>rv :: sched_context. f rv)) c"
  apply (ccorres_exec_l_pre ccorres_exec_l_pre: get_sc_sp')
  apply (force intro: stronger_ccorres_guard_imp cc dest: obj_at_cslift_sc)
  done
