...
proof -
  have "newContext = ?UC"
    by (simp add: newContext_def initContext_def sstatusSPIE_def fun_upd_def)
...
(* FIXME RISCV move to commented out page_map_l4_at'_array_assertion in VSpace_C *)
lemma page_map_page_table_at'_array_assertion:
  assumes "(s,s') \<in> rf_sr"
  assumes "page_table_at' pt s"
  shows "array_assertion (pte_Ptr pt) (2^ptTranslationBits) (hrs_htd (t_hrs_' (globals s')))"
  using assms array_assertion_abs_pt[where n="\<lambda>_. 2^ptTranslationBits" and x="\<lambda>_. (1::nat)"]
...
lemma page_table_pte_atI':
  ...
  apply (clarsimp simp: page_table_at'_def pageBits_def ptBits_def pte_bits_def word_size_bits_def)
  apply (erule_tac x="ucast x" in allE)
  apply (fastforce simp add: ucast_ucast_len ptTranslationBits_def)
  done

lemma copyGlobalMappings_ccorres:
  "ccorres dc xfdc (page_table_at' pt) (UNIV \<inter> {s. newLvl1pt_' s = Ptr pt}) []
...
proof -
  ...
  apply (cinit lift: newLvl1pt_' simp: ptIndex_maxPTLevel_pptrBase ptTranslationBits_def)
   ...
   apply csymbr
   apply clarsimp
    apply (rule_tac F="\<lambda>n s. globalPT = riscvKSGlobalPT (ksArchState s) \<and> page_table_at' pt s"
                and i="0x100"
    ...
    apply (rule ccorres_guard_imp2)
     ...
     (* FIXME RISCV: it is not clear that P' is the right instantiation here *)
     apply (rule_tac P="\<lambda>s. ko_at' pte (riscvKSGlobalPT (ksArchState s) + 0xFF8) s
                             \<and> page_table_at' pt s"
                 and P'="\<lbrace>\<acute>i = 0x100 + of_nat n \<rbrace>"
              in setObject_ccorres_helper)
       apply (rule conseqPre, vcg, clarsimp)

          apply (prop_tac "(0x100::machine_word) + of_nat n \<noteq> 0") subgoal sorry
          apply clarsimp
          apply (frule (1) page_map_page_table_at'_array_assertion)

          (* array assertion on dest. pte access *)
          apply (prop_tac "unat ((0xFF::machine_word) + of_nat n) < 2^ptTranslationBits")
    subgoal sorry
          apply (strengthen array_assertion_shrink_right[where n="2^ptTranslationBits"], simp)
          (* array assertion on src. (global PT) pte access *)
          apply (frule rf_sr_riscvKSGlobalPT, clarsimp)
          apply (frule page_table_pte_atI'[where x="0x1FF"])
           apply (clarsimp simp: ptTranslationBits_def)
          apply (frule (1) page_map_page_table_at'_array_assertion; clarsimp simp: bit_simps)
          (* FIXME RISCV: I don't think we can show that accessing the 512th entry of the page
                          table is valid, because it isn't. The C parser clearly expects Suc(i)
                          to form a valid access within the loop, and it is not clear how that's
                          ever possible to satisfy. More investigation needed.
          *)
  sorry (* FIXME RISCV this is the old X64 proof remainder
       apply (strengthen array_assertion_shrink_right[where n'=511 and n=512, simplified])
       apply (frule (1) page_map_page_table_at'_array_assertion; clarsimp simp: bit_simps)
       apply (frule page_table_pte_atI'[where x="0x1FF"]; simp add: bit_simps c_guard_abs_pte)
       apply (rule conjI, clarsimp simp: rf_sr_def cstate_relation_def Let_def)
       apply (clarsimp simp: rf_sr_riscvKSGlobalPT)
...
lemma createObject_child:
  ...
  apply wp
   ...
   apply (case_tac "(capfn ptr)")
    ...
    apply (auto simp: RISCV64_H.capUntypedSize_def bit_simps
                      is_aligned_no_wrap' field_simps APIType_capBits_def bit_simps'
          ...
          apply (rule ccorres_guard_impR)
           ...
           apply (rule ccorres_split_nothrow,
            ...
            apply (frule_tac x = "of_nat n" in range_cover_bound3)
             ...
             apply (simp add: range_cover.unat_of_nat_n)
            apply (clarsimp simp: field_simps shiftl_t2n blah)
           apply (clarsimp simp: createObject_c_preconds_def field_simps from_bool_to_bool_iff
