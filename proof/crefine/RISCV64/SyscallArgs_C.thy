...
lemma refill_index_ccorres:
  "ccorres
     ...
     (gets_the (readRefillIndex scPtr idx)) (Call refill_index_'proc)"
  ...
  apply (rule conjI)
   ...
   apply (rule_tac n="length (scRefills sc)" in array_assertion_shrink_right)
    apply (fastforce intro: sc_at_array_assertion' simp: obj_at_simps in_omonad)
   ...
   apply (subst unat_of_nat64')
    ...
    apply (clarsimp simp: word_bits_def refillSizeBytes_def)
   apply fastforce
  apply (rename_tac s s')
  apply (normalise_obj_at', rename_tac sc)
  apply (clarsimp simp: readRefillIndex_def refillIndex_def readSchedContext_def
...
lemma refill_capacity_ccorres:
  ...
  apply (normalise_obj_at', rename_tac sc)
  apply (frule_tac s=s in rf_sr_refill_buffer_relation)
...
lemma getMRs_tcbContext:
  ...
  getMRs thread buffer info
  \<lbrace>\<lambda>rv s. obj_at' (\<lambda>tcb. user_regs (atcbContextGet (tcbArch tcb)) (RISCV64_H.msgRegisters ! n) = rv ! n) (ksCurThread s) s\<rbrace>"
...
lemma length_msgRegisters:
  "length RISCV64_H.msgRegisters = unat (scast n_msgRegisters :: machine_word)"
    ...
    apply (rule ccorres_symb_exec_l)
       apply (rule_tac P="\<lambda>s. n < unat (scast n_msgRegisters :: machine_word) \<and>
                              obj_at' (\<lambda>tcb. user_regs (atcbContextGet (tcbArch tcb))
                                               (RISCV64_H.msgRegisters!n) = x!n) (ksCurThread s) s"
                   and P' = UNIV
