...
lemma decodeInvocation_ccorres:
  ...
  apply (simp flip: cur_tcb'_def ct_in_state'_rewrite)
  apply (frule ct_in_state'_cur_tcb')
  apply (frule simple_sane_strg[rule_format])
  apply clarsimp
  apply (prop_tac "ct_in_state' (\<lambda>st'. st' \<noteq> thread_state.Inactive  \<and> st' \<noteq> IdleThreadState
                                       \<and> (\<forall>rptr. st' \<noteq> BlockedOnReply rptr)) s")
