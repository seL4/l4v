...
lemma isArchFrameCap_ArchObjectCap:
  "isArchFrameCap (ArchObjectCap acap)
       = isFrameCap acap"
  by (simp add: isArchFrameCap_def isFrameCap_def)
...
lemma flex_user_data_at_rf_sr_dom_s:
  "\<lbrakk> (\<forall>p<2 ^ (pageBitsForSize sz - pageBits).
         typ_at' UserDataT (x + (p << pageBits)) s); (s, s') \<in> rf_sr \<rbrakk>
    \<Longrightarrow> {x ..+ 2 ^ pageBitsForSize sz} \<times> {SIndexVal, SIndexTyp 0}
    \<subseteq> dom_s (hrs_htd (t_hrs_' (globals s')))"
  apply (subst (asm) shiftl_t2n, subst (asm) mult.commute)
...
(* FIXME AARCH64 not clear what the 9 is here and whether it needs to be reconsidered for AARCH64 *)
...
lemma clearMemory_PageCap_ccorres:
  "ccorres dc xfdc (invs' and valid_cap' (ArchObjectCap (FrameCap ptr undefined sz False None))
      ...
      ({s. bits_' s = of_nat (pageBitsForSize sz)}
  ...
  apply (cinit' lift: bits_' ptr___ptr_to_unsigned_long_')
   ...
   apply (subgoal_tac "\<forall>p<2 ^ (pageBitsForSize sz - pageBits).
                               x \<Turnstile>\<^sub>c (Ptr (ptr + (of_nat p << pageBits)) :: user_data_C ptr)")
    prefer 2
   ...
   apply (simp add: shiftl_t2n' pageBits_def)
   ...
   apply (erule cmap_relation_If_upd)
    ...
    apply (rule set_eqI, rule iffI)
     ...
     apply (rule shiftr_less_t2n)
     apply (simp add: pbfs_atleast_pageBits[simplified pageBits_def])
...
lemma heap_to_user_data_in_user_mem'[simp]:
  ...
  apply (subst map_option_byte_to_word_heap)
   ...
   apply (simp add: AARCH64.pageBits_def)
...
lemma coerce_memset_to_heap_update:
  ...
  apply (simp add: typ_info_word word_rsplit_0 word_rsplit_0_32 upt_conv_Cons)
  apply (simp add: typ_info_word typ_info_ptr word_rsplit_0 word_rsplit_0_32 word_bits_def
                   replicateHider_def)
  done
