...
lemma pageBitsForSize_spec:
  ...
  apply (clarsimp simp: pageBitsForSize_def framesize_to_H_def
                        ARMSmallPage_def ARMLargePage_def ARMHugePage_def
                        ARMSmallPageBits_def ARMLargePageBits_def ARMHugePageBits_def bit_simps)
  apply (drule word_less_cases, erule disjE; simp)+
  done
...
lemma ccorres_pre_gets_armKSASIDTable_ksArchState:
  ...
  shows   "ccorres r xf
                  ...
                  (gets (armKSASIDTable \<circ> ksArchState) >>= (\<lambda>rv. f rv)) c"
  apply (rule ccorres_guard_imp)
    apply (rule ccorres_symb_exec_l)
       defer
       apply wp[1]
...
lemma ccorres_pre_gets_riscvKSASIDTable_ksArchState':
  assumes cc: "\<And>rv. ccorres r xf (P and (\<lambda>s. rv = (armKSASIDTable \<circ> ksArchState) s)) (P' rv) hs (f rv) c"
