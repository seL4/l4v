...
lemma makeUserPagePTE_spec:
  "\<forall>s. \<Gamma> \<turnstile>
   ...
   \<lbrace> let uxn = uxn_from_vmattributes (vm_attributes_to_H \<^bsup>s\<^esup>attributes) ||
               from_bool (\<^bsup>s\<^esup>vm_rights \<noteq> scast Kernel_C.VMReadOnly \<and>
                          \<^bsup>s\<^esup>vm_rights \<noteq> scast Kernel_C.VMReadWrite);
  ...
  apply (clarsimp simp: from_bool_def true_def false_def vm_attributes_lift_def split: bool.splits)
  done
