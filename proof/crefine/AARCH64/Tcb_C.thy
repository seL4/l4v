...
lemma invokeTCB_ThreadControl_ccorres:
             ...
             apply (rule ccorres_split_nothrow_novcg)
               ...
               apply (rule ccorres_Cond_rhs_Seq)
                ...
                apply (clarsimp simp: ccap_relation_def cap_thread_cap_lift cap_to_H_def
                                      canonical_address_tcb_ptr[THEN canonical_address_and_maskD])
               (* \<not> P *)
               apply simp
               apply (rule ccorres_cond_false_seq, simp)
               apply (rule ccorres_cond_false_seq, simp)
  ...
  apply (subst is_aligned_neg_mask_eq)
   ...
   apply (rule aligned_add_aligned)
     apply (fastforce simp add: obj_at'_def objBits_simps')
...
lemma frame_gp_registers_convs:
  ...
  "length AARCH64_H.gpRegisters = unat n_gpRegisters"
  "n < length AARCH64_H.frameRegisters \<Longrightarrow>
     index frameRegistersC n = register_from_H (AARCH64_H.frameRegisters ! n)"
  "n < length AARCH64_H.gpRegisters \<Longrightarrow>
     index gpRegistersC n = register_from_H (AARCH64_H.gpRegisters ! n)"
  apply (simp_all add: AARCH64_H.gpRegisters_def AARCH64_H.frameRegisters_def
                       AARCH64.gpRegisters_def n_gpRegisters_def
                       AARCH64.frameRegisters_def n_frameRegisters_def
  ...
  apply (auto simp: less_Suc_eq fcp_beta C_register_defs)
  done
          ...
          apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
            ...
            apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
                ...
                apply (rule_tac F="\<lambda>n. sysargs_rel args buffer
                                     and sysargs_rel_n args buffer (n + length AARCH64_H.frameRegisters + 2)
                 ...
                 apply arith
                apply (simp add: AARCH64_H.gpRegisters_def size_gpRegisters word_bits_def
                                 AARCH64_H.frameRegisters_def size_frameRegisters)
               apply simp
               apply (rule ceqv_refl)
              apply (ctac(no_vcg) add: getRestartPC_ccorres)
               apply simp
...
lemma msg_registers_convs:
  "length AARCH64_H.msgRegisters = unat n_msgRegisters"
  "n < length AARCH64_H.msgRegisters \<Longrightarrow>
     index msgRegistersC n = register_from_H (AARCH64_H.msgRegisters ! n)"
  apply (simp_all add: msgRegisters_unfold
                       AARCH64.msgRegisters_def n_msgRegisters_def
          ...
          apply (rule ccorres_symb_exec_l)
             apply (rule ccorres_symb_exec_l[OF _ _ _ empty_fail_getThreadState])
               ...
               apply (rule ccorres_if_lhs)
                  ...
                  apply (ctac add: setRegister_ccorres)
                    ...
                    apply (rule ccorres_split_nothrow_novcg)
                        apply (rule_tac F="\<lambda>m s. obj_at' (\<lambda>tcb. map ((user_regs o atcbContextGet o tcbArch) tcb) (genericTake n
                                                     (AARCH64_H.frameRegisters @ AARCH64_H.gpRegisters))
                                                              = reply) target s"
                ...
                apply (simp add: Collect_const_mem AARCH64_H.badgeRegister_def
...
lemma checkValidIPCBuffer_ccorres:
  ...
  apply (simp add:checkValidIPCBuffer_def AARCH64_H.checkValidIPCBuffer_def)
  apply (cases "isArchFrameCap cp \<and> \<not> isDeviceCap cp")
   apply (simp only: isCap_simps isDeviceCap.simps, safe)[1]
   apply (cinit lift: vptr_' cap_')
    apply (simp add: AARCH64_H.checkValidIPCBuffer_def if_1_0_0 ipcBufferSizeBits_def
                del: Collect_const)
    apply csymbr
   ...
   apply (clarsimp simp: cap_get_tag_isCap isCap_simps
                         Collect_const_mem ccap_relation_page_is_device
                         word_sle_def)
  apply (cases "isArchFrameCap cp")
   apply (simp only: isCap_simps isDeviceCap.simps)[1]
   apply clarsimp
   apply (cinit lift: vptr_' cap_')
    apply (simp add: AARCH64_H.checkValidIPCBuffer_def del: Collect_const)
    apply csymbr
    apply (rule ccorres_cond_false_seq)
    apply simp
    apply csymbr
    ...
    apply (clarsimp simp: throwError_def return_def exception_defs
                          syscall_error_rel_def syscall_error_to_H_cases)
   apply (simp add: cap_get_tag_isCap isCap_simps ccap_relation_page_is_device)
  apply (cinit' lift: vptr_' cap_')
   apply csymbr
   apply (simp add: AARCH64_H.checkValidIPCBuffer_def cap_get_tag_isCap)
...
lemma invokeTCB_SetTLSBase_ccorres:
  ...
  apply (cinit lift: thread_' tls_base_')
   ...
   apply vcg
  apply (clarsimp simp: tlsBaseRegister_def AARCH64.tlsBaseRegister_def
                        invs_weak_sch_act_wf invs_queues C_register_defs
                 split: if_split)
  done
