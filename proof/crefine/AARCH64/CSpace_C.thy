...
lemma Arch_maskCapRights_ccorres [corres]:
  ...
  apply (cinit' lift: cap_' cap_rights_mask_')
   apply csymbr
   apply (unfold AARCH64_H.maskCapRights_def)
...
lemma cap_get_tag_isCap_unfolded_H_cap2: (* FIXME AARCH64: move; potentially replace original *)
  ...
  and "ccap_relation (capability.ArchObjectCap (arch_capability.SGISignalCap irq targets)) cap' \<Longrightarrow> (cap_get_tag cap' = scast cap_sgi_signal_cap)"
...
lemma canonical_address_mdbNext_CL:
  ...
  apply (simp add: mdb_node_lift_def canonical_address_def canonical_address_of_def
                canonical_bit_def)
  apply (subst le_max_word_ucast_id; simp)
  apply (rule word_and_le')
  apply (rule order.trans[OF mask_shiftl_le_mask])
  apply simp
  done
...
definition arch_cleanup_info_wf' :: "arch_capability \<Rightarrow> bool" where
...
lemma Arch_postCapDeletion_ccorres:
  "ccorres dc xfdc
     ...
     (AARCH64_H.postCapDeletion acap)
...
lemma pageBitsForSize_64[simplified,simp]:
  "pageBitsForSize sz < LENGTH(machine_word_len)"
  by (cases sz, auto simp: bit_simps)

lemma word_of_nat_pageBitsForSize_64[simp]:
  "word_of_nat (pageBitsForSize sz) < (0x40::machine_word)"
  by (cases sz, auto simp: bit_simps)
...
definition
  ...
  "get_capPtr_CL \<equiv> \<lambda>cap. Ptr (case cap of
    ...
    | Some (Cap_vspace_cap c) \<Rightarrow> cap_vspace_cap_CL.capVSBasePtr_CL c
...
where
  ...
  Some (Cap_untyped_cap c) \<Rightarrow> True
    ...
    | Some (Cap_vspace_cap c) \<Rightarrow> True
...
lemma ccap_relation_get_capIsPhysical:
  ...
  apply (case_tac arch_cap; clarsimp simp: cap_lifts cap_lift_asid_control_cap
                      dest!: cap_get_tag_isCap_unfolded_H_cap)
  (* FIXME AARCH64 this should be automatable, probably in a way related to other handling of
     PageTableCap vs cap_lift in this file *)
  apply (rename_tac pt_t maddr)
  apply (case_tac pt_t; clarsimp simp: cap_lifts cap_lift_asid_control_cap
                      dest!: cap_get_tag_isCap_unfolded_H_cap)
  done
...
lemma framesize_to_H_eq:
  ...
  by (fastforce simp: framesize_to_H_def ARMSmallPage_def ARMLargePage_def ARMHugePage_def
                      word_le_make_less
               split: if_split
...
lemma ccap_relation_FrameCap_Size:
  ...
  apply (cases s; clarsimp simp: framesize_to_H_def framesize_from_H_def vm_page_size_defs
...
lemma Arch_deriveCap_ccorres:
  ...
  apply (cinit lift: cap_')
   apply csymbr
   apply (unfold AARCH64_H.deriveCap_def Let_def)
