  ...
  apply (cinit')
   ...
   apply (simp add: liftE_bind bind_assoc)
    ...
    apply wpc
     ...
     apply vcg
    apply vcg
    apply (clarsimp simp: irqInvalid_def ucast_ucast_b is_up mask_def)
...
lemma handleSyscall_ccorres:
                ...
                apply (ctac (no_vcg) add: getActiveIRQ_ccorres)
                 ...
                 apply (rule ccorres_split_nothrow_novcg)
                     apply (rule_tac R=\<top> and xf=xfdc in ccorres_when)
                      apply (case_tac rv; clarsimp simp: irqInvalid_def mask_def)
                     apply (ctac (no_vcg) add: handleInterrupt_ccorres)
                    apply ceqv
             ...
             apply (rule ccorres_split_nothrow_case_sum)
               ...
               apply (ctac (no_vcg) add: getActiveIRQ_ccorres)
                ...
                apply (rule ccorres_split_nothrow_novcg)
                    apply (rule ccorres_Guard)?
                    apply (rule_tac R=\<top> and xf=xfdc in ccorres_when)
                      apply (case_tac rv; clarsimp simp: irqInvalid_def irqInvalid_def mask_def)
                    apply (ctac (no_vcg) add: handleInterrupt_ccorres)
                   apply ceqv
  ...
  apply (clarsimp simp: simple_sane_strg ct_in_state'_def st_tcb_at'_def obj_at'_def
                        isReply_def irqInvalid_def)
  apply (auto simp: syscall_from_H_def Kernel_C.SysSend_def mask_def
...
lemma handleVCPUFault_ccorres:
  ...
  apply (cinit' lift: hsr___unsigned_long_')
   ...
   apply (ctac (no_vcg) add: armv_handleVCPUFault_no_op_ccorres[simplified isFpuEnable_def,
    ...
    apply clarsimp
   apply wpsimp
  apply (clarsimp simp: ct_running_imp_simple' ucast_and_mask_drop)
...
lemma handleHypervisorEvent_ccorres:
  "ccorres dc xfdc
           ((all_invs' (HypervisorEvent e)) and sch_act_simple)
           UNIV []
...
lemma ccorres_get_registers:
  ...
  apply (erule_tac x="(user_regs o atcbContextGet o tcbArch) ko AARCH64.capRegister" in meta_allE)
  apply (erule_tac x="(user_regs o atcbContextGet o tcbArch) ko AARCH64.msgInfoRegister" in meta_allE)
  ...
  apply (clarsimp simp: ctcb_relation_def ccontext_relation_def cregs_relation_def
                        C_register_defs AARCH64.capRegister_def AARCH64_H.capRegister_def
                        AARCH64.msgInfoRegister_def AARCH64_H.msgInfoRegister_def
