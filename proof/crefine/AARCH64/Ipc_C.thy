...
definition
  "replyFromKernel_success_empty thread \<equiv> do
     ...
     asUser thread $ setRegister AARCH64_H.badgeRegister 0;
...
lemma empty_fail_getSanitiseRegisterInfo[wp, simp]:
  ...
  (* FIXME AARCH64 why do we need empty_fail_getObject here but not on other arches?! *)
...
lemma length_syscallMessage:
  "length AARCH64_H.syscallMessage = unat n_syscallMessage"
  apply (simp add: syscallMessage_def AARCH64.syscallMessage_def
...
lemma syscallMessage_ccorres:
  ...
  apply (simp add: AARCH64_H.syscallMessage_def syscallMessageC_def
                   AARCH64.syscallMessage_def
  ...
  apply (rule monadic_rewrite_bind_tail)
   ...
   apply (case_tac f, simp_all add: handleFaultReply_def zipWithM_x_mapM_x zip_take)
      ...
      apply (clarsimp simp: handleFaultReply_def zipWithM_x_mapM_x
                            zip_Cons AARCH64_H.exceptionMessage_def
                            AARCH64.exceptionMessage_def
      ...
      apply (rule monadic_rewrite_bind_tail [where Q="\<lambda>_. tcb_at' r"])
       ...
       apply (subgoal_tac "\<forall>n :: machine_word. n \<le> scast n_syscallMessage \<longrightarrow> [n .e. msgMaxLength]
        ...
        apply (clarsimp simp: bind_assoc asUser_bind_distrib asUser_getRegister_threadGet_comm
                              ...
                              word_less_nat_alt AARCH64_H.sanitiseRegister_def
...
lemma getMessageInfo_ccorres:
  ...
  apply (clarsimp simp: typ_heap_simps AARCH64_H.msgInfoRegister_def AARCH64.msgInfoRegister_def
                        C_register_defs
                  dest!: c_guard_clift)
  done
...
lemma replyFromKernel_success_empty_ccorres [corres]:
  ...
  apply (simp add: AARCH64_H.msgInfoRegister_def AARCH64.msgInfoRegister_def
                   ...
                   AARCH64_H.badgeRegister_def AARCH64.badgeRegister_def
                   message_info_to_H_def C_register_defs)
  done

lemma msgRegisters_offset_conv:
  "\<And>offset i. \<lbrakk> offset + i < length AARCH64_H.msgRegisters \<rbrakk> \<Longrightarrow>
   index msgRegistersC (unat ((of_nat offset :: machine_word) + of_nat i)) =
   register_from_H (AARCH64_H.msgRegisters ! (offset + i))"
...
lemma copyMRs_register_loop_helper:
  fixes n
  defines regs: "regs \<equiv> take (unat n) AARCH64_H.msgRegisters"
  shows
...
shows
  ...
  apply (cinit lift: n_' sendBuf_' recvBuf_' sender_' receiver_'
   ...
   apply (rule_tac P = "length (take (unat n) AARCH64_H.msgRegisters) <
                        2 ^ word_bits"
                in ccorres_gen_asm)
   apply (rule ccorres_split_nothrow_novcg)
     ...
     apply (rule_tac P="unat n \<le> length AARCH64_H.msgRegisters" in ccorres_cases)
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule_tac i="length AARCH64_H.msgRegisters"
     ...
     apply (clarsimp simp: guard_is_UNIV_def upto_enum_def
                           ...
                           linorder_not_le)
    apply simp
    apply (wpsimp wp: mapM_x_wp' hoare_vcg_all_lift hoare_vcg_const_imp_lift
...
lemma exceptionMessage_ccorres:
  "n < unat n_exceptionMessage
      \<Longrightarrow> register_from_H (AARCH64_H.exceptionMessage ! n)
             = index exceptionMessageC n"
  apply (simp add: exceptionMessageC_def AARCH64_H.exceptionMessage_def
                   AARCH64.exceptionMessage_def MessageID_Exception_def)
...
lemma exceptionMessage_length_aux :
  "\<And>n. n < length AARCH64_H.exceptionMessage \<Longrightarrow> n < unat n_exceptionMessage"
  by (simp add: AARCH64.exceptionMessage_def AARCH64_H.exceptionMessage_def n_exceptionMessage_def)
...
lemma copyMRsFault_ccorres_exception:
  "ccorres dc xfdc
           (valid_pspace'
             ...
             and K (length msg = 3) \<comment> \<open>length AARCH64.exceptionMessage\<close>
           ...
           (UNIV \<inter> \<lbrace>\<acute>sender = tcb_ptr_to_ctcb_ptr sender\<rbrace>
                 ...
                 \<inter> \<lbrace> \<acute>length___unsigned_long = 3 \<rbrace> \<comment> \<open>length AARCH64.exceptionMessage\<close>
  ...
  apply (cinit' lift: sender_' receiver_' receiveIPCBuffer_'
   ...
   apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
       ...
       apply (rule_tac F="K $ obj_at' (\<lambda>tcb. map ((user_regs o atcbContext o tcbArch) tcb) AARCH64_H.exceptionMessage = msg) sender"
                     in ccorres_mapM_x_while)
           apply (clarsimp simp: n_msgRegisters_def)
           apply (rule ccorres_guard_imp2)
            apply (rule_tac t=sender and r="AARCH64_H.exceptionMessage ! n"
...
lemma copyMRsFault_ccorres_syscall:
  "ccorres dc xfdc
           (valid_pspace'
             ...
             and K (length msg = 12) \<comment> \<open>length AARCH64.syscallMessage\<close>
             and K (recvBuffer \<noteq> Some 0)
             and K (sender \<noteq> receiver))
           (UNIV \<inter> \<lbrace>\<acute>sender = tcb_ptr_to_ctcb_ptr sender\<rbrace>
                 \<inter> \<lbrace>\<acute>receiver = tcb_ptr_to_ctcb_ptr receiver\<rbrace>
                 \<inter> \<lbrace>\<acute>receiveIPCBuffer = Ptr (option_to_0 recvBuffer)\<rbrace>
                 \<inter> \<lbrace>\<acute>length___unsigned_long = 12 \<rbrace> \<comment> \<open>length AARCH64.syscallMessage\<close>
...
proof -
  ...
  apply (cinit' lift: sender_' receiver_' receiveIPCBuffer_'
   ...
   apply (simp only: mapM_x_append[where xs="take (unat n_msgRegisters) (zip as bs)"
                                         ...
                                         for as bs, simplified] bind_assoc)
   apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
       apply (rule_tac F="K $ obj_at' (\<lambda>tcb. map ((user_regs o atcbContext o tcbArch) tcb) AARCH64_H.syscallMessage = msg) sender"
                     in ccorres_mapM_x_while)
           apply (clarsimp simp: n_msgRegisters_def)
           apply (rule ccorres_guard_imp2)
            apply (rule_tac t=sender and r="AARCH64_H.syscallMessage ! n"
                            in ccorres_add_getRegister)
            apply (ctac(no_vcg))
             apply (rule_tac P="\<lambda>s. rv = msg ! n" in ccorres_cross_over_guard)
             apply (simp add: setMR_def length_msgRegisters n_msgRegisters_def
     ...
     apply (rule ccorres_Cond_rhs)
      ...
      apply (rule_tac F="\<lambda>_. obj_at' (\<lambda>tcb. map ((user_regs o atcbContext o tcbArch) tcb) AARCH64_H.syscallMessage = msg)
          ...
          apply (rule ccorres_guard_imp2)
          apply (rule_tac t=sender and r="AARCH64_H.syscallMessage ! (n + unat n_msgRegisters)"
                                   in ccorres_add_getRegister)
          apply (ctac(no_vcg))
     ...
     apply (simp add: Collect_True Collect_False ccorres_cond_iffs
      ...
      apply (rule_tac val="vcpuHSR aft" in symb_exec_r_fault)
       ...
       apply (ctac(no_vcg) add: setMR_ccorres)
        ...
        apply (rule ccorres_return_C, simp+)[1]
       apply wp
      apply (clarsimp simp: option_to_ptr_def seL4_VCPUFault_HSR_def guard_is_UNIV_def ucast_and_mask)
...
(* FIXME AARCH64 style issues: copied from ARM_HYP *)
     ...
     apply (rule_tac val="case_option 0 (K 1) (vgicMaintenanceData aft)" in symb_exec_r_fault)
      ...
      (* buffer = None *)
        ...
        apply (clarsimp simp: Collect_const_mem seL4_VGICMaintenance_IDX_def ucast_and_mask)
      (* buffer = Some x2 *)
       apply ccorres_rewrite
       apply (rule_tac P="valid_pspace'
                  and (case buffer of Some x \<Rightarrow> valid_ipc_buffer_ptr' x | None \<Rightarrow> \<top>)" and P'=UNIV
       ...
       apply (rule_tac val="x2" in symb_exec_r_fault)
        ...
        apply (rule ccorres_guard_imp)
          apply (ctac(no_vcg) add: setMR_ccorres)
           apply (rule ccorres_return_C, simp+)[1]
          apply wp
         apply (simp add: msgMaxLength_def Collect_const_mem seL4_VGICMaintenance_IDX_def ucast_and_mask)+
       apply (simp add: guard_is_UNIV_def)
      apply ccorres_rewrite
      apply (rule ccorres_rhs_assoc)+
      apply (rule ccorres_move_c_guard_tcb)
...
proof -
  ...
  show ?thesis
    ...
    apply (clarsimp simp: msgMaxLength_unfold length_syscallMessage msgFromLookupFailure_def
                         AARCH64_H.exceptionMessage_def
                         AARCH64.exceptionMessage_def
...
definition makeArchFaultMessage2 :: "arch_fault \<Rightarrow> machine_word" where
   ...
   apply (ctac(no_vcg) add: setMRs_fault_ccorres)
    apply (rule_tac R="obj_at' (\<lambda>tcb. tcbFault tcb = ft) sender"
     ...
     apply (ctac (no_vcg, c_lines 2) add: setMessageInfo_ccorres)
       apply (ctac add: setRegister_ccorres)
      apply wp
     apply (simp add: badgeRegister_def AARCH64.badgeRegister_def AARCH64.capRegister_def
...
next
  ...
  have is_the_ep_maskCapRights:
    ...
    apply (case_tac acap)
     apply (clarsimp simp: AARCH64_H.maskCapRights_def isFrameCap_def)+
    done
  ...
  "\<And>slot cap. \<lbrace>\<lambda>s. \<not> (is_the_ep cap)\<rbrace> deriveCap slot cap \<lbrace>\<lambda>rv s. \<not> (is_the_ep rv)\<rbrace>, -"
    ...
    apply (rename_tac acap)
    apply (case_tac acap)
     apply (simp_all add: AARCH64_H.deriveCap_def Let_def isCap_simps is_the_ep_def)
  ...
  have mask_right_eq_null:
    ...
    apply (rename_tac acap)
    apply (case_tac acap)
     apply (simp add: AARCH64_H.maskCapRights_def isFrameCap_def)+
    done
         ...
         apply (ctac add: copyMRs_ccorres)
           apply (ctac add: transferCaps_ccorres)
             ...
             apply (ctac(c_lines 2, no_vcg) add: setMessageInfo_ccorres)
               apply ctac
              apply wp
             apply (clarsimp simp: AARCH64_H.badgeRegister_def AARCH64.badgeRegister_def
                                   C_register_defs)
            apply wp
...
lemma lookupIPCBuffer_not_Some_0:
  ...
  apply (simp add: lookupIPCBuffer_def AARCH64_H.lookupIPCBuffer_def)
...
lemma replyFromKernel_error_ccorres [corres]:
  ...
  apply (simp add: msgInfoRegister_def
                   ...
                   AARCH64_H.badgeRegister_def AARCH64.badgeRegister_def
                   Kernel_C.badgeRegister_def AARCH64.capRegister_def
...
lemma length_exceptionMessage:
  "length AARCH64_H.exceptionMessage = unat n_exceptionMessage"
  by (simp add: AARCH64_H.exceptionMessage_def AARCH64.exceptionMessage_def n_exceptionMessage_def)

lemma Arch_getSanitiseRegisterInfo_ccorres:
  "ccorres ((=) \<circ> from_bool) ret__unsigned_long_'
     (tcb_at' r and no_0_obj' and valid_objs')
     (\<lbrace> \<acute>thread = tcb_ptr_to_ctcb_ptr r\<rbrace>) hs
  ...
  "ccorres dc xfdc
           ...
           (do t \<leftarrow> getSanitiseRegisterInfo r;
               zipWithM_x (\<lambda>rs rd. do v \<leftarrow> asUser s (getRegister rs);
                                        asUser r (setRegister rd (AARCH64_H.sanitiseRegister t rd v))
                                        od)
               AARCH64_H.msgRegisters (take len AARCH64_H.exceptionMessage)
            od)
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule ccorres_zipWithM_x_while)
             ...
             apply (rule ccorres_symb_exec_r)
              ...
              apply clarsimp
              apply vcg
              apply (rule conjI, simp add: AARCH64_H.exceptionMessage_def
                                           AARCH64.exceptionMessage_def word_of_nat_less)
              apply (simp add: msgRegisters_ccorres n_msgRegisters_def length_msgRegisters
                               AARCH64_H.exceptionMessage_def AARCH64.exceptionMessage_def
             ...
             apply (rule conseqPre, vcg)
             apply (clarsimp simp: word_of_nat_less AARCH64_H.exceptionMessage_def
                                   AARCH64.exceptionMessage_def)
          ...
          apply wp
            apply simp+
         apply (clarsimp simp: AARCH64_H.exceptionMessage_def
                               AARCH64.exceptionMessage_def
                               word_bits_def)
         apply unat_arith
        apply ceqv
  ...
  shows "ccorres dc xfdc
           ...
           (do t \<leftarrow> getSanitiseRegisterInfo r;
               a \<leftarrow> zipWithM_x (\<lambda>rs rd. do v \<leftarrow> asUser s (getRegister rs);
                                           asUser r (setRegister rd (AARCH64_H.sanitiseRegister t rd v))
                                        od)
               AARCH64_H.msgRegisters (take len AARCH64_H.syscallMessage);
                            ...
                            | Some bufferPtr \<Rightarrow>
                                   zipWithM_x (\<lambda>i rd. do v \<leftarrow> loadWordUser (bufferPtr + i * 8);
                                      ...
                                      (drop (unat (scast n_msgRegisters :: machine_word))
                                            (take len AARCH64_H.syscallMessage))
                                 od)
           (Call copyMRsFaultReply_'proc)"
  proof -
     ...
     apply (rule ccorres_split_nothrow_novcg)
         apply (rule ccorres_zipWithM_x_while)
             ...
             apply (rule ccorres_symb_exec_r)
               ...
               apply vcg
              apply vcg
              apply (rule conjI, simp add: AARCH64_H.syscallMessage_def
                                           AARCH64.syscallMessage_def word_of_nat_less
...
lemma receiveIPC_block_ccorres_helper:
  "ccorres dc xfdc (tcb_at' thread and valid_objs' and
                    pspace_canonical' and pspace_aligned' and pspace_distinct' and
                    sch_act_not thread and ep_at' epptr and
                    (\<lambda>s. sch_act_wf (ksSchedulerAction s) s) and
                    K (epptr = epptr && ~~ mask 4) and
    ...
    \<comment> \<open>IdleNtfn case\<close>
     ...
     apply (rule_tac val=word and xf'=badge_' and R="ko_at' ntfn ntfnptr"
      ...
      apply (ctac(no_vcg))
       ...
       apply assumption
      apply wp
     apply (clarsimp simp: guard_is_UNIV_def AARCH64_H.badgeRegister_def
                           AARCH64.badgeRegister_def C_register_defs
                           AARCH64.capRegister_def)
...
lemma doNBRecvFailedTransfer_ccorres[corres]:
  ...
  apply (cinit lift: thread_')
   apply (ctac add: setRegister_ccorres)
  by (clarsimp simp: C_register_defs AARCH64_H.badgeRegister_def
        ...
        apply (intro conjI)
          ...
          apply (rule cpspace_relation_ntfn_update_ntfn, assumption+)
           apply (simp add: cnotification_relation_def Let_def NtfnState_Idle_def
                            tcb_queue_relation'_def make_canonical_def canonical_bit_def)
          apply simp
         apply (clarsimp simp: carch_state_relation_def packed_heap_update_collapse_hrs)
        apply (simp add: cmachine_state_relation_def)
       apply (simp add: h_t_valid_clift_Some_iff)
...
lemma receiveSignal_block_ccorres_helper:
  "ccorres dc xfdc (tcb_at' thread and sch_act_not thread and valid_objs' and ntfn_at' ntfnptr and
                    pspace_canonical' and pspace_aligned' and pspace_distinct' and
                    (\<lambda>s. sch_act_wf (ksSchedulerAction s) s) and
    ...
    apply (rule_tac xf'=ret__unsigned_longlong_'
     ...
     apply wpc
      ...
      apply (rule_tac val=badge
       ...
       apply wp
      apply (clarsimp simp: guard_is_UNIV_def)
      apply (clarsimp simp: AARCH64.badgeRegister_def AARCH64.capRegister_def C_register_defs)
     \<comment> \<open>WaitingNtfn case\<close>
     apply (rename_tac list)
     apply (rule ccorres_cond_true)
     apply csymbr
