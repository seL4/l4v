...
lemma getQueue_ccorres:
  "ccorres ctcb_queue_relation queue_'
     ...
     \<lbrace>\<acute>idx___unsigned_long = word_of_nat (cready_queues_index_to_C tdom prio)\<rbrace> hs
     (getQueue tdom prio) (\<acute>queue :== \<acute>ksReadyQueues.[unat \<acute>idx___unsigned_long])"
  apply (rule ccorres_from_vcg)
  apply (rule allI, rule conseqPre, vcg)
   ...
   ccorres dc xfdc
     ...
     (Basic (\<lambda>s. globals_update
                  (ksReadyQueues_'_update
                   (\<lambda>_. Arrays.update (ksReadyQueues_' (globals s))
                                      (unat (idx___unsigned_long_' s)) cqueue)) s))"
  apply (rule ccorres_from_vcg)
  apply (rule allI, rule conseqPre, vcg)
...
proof -
  ...
  show ?thesis
    ...
    apply (rule conjI)
     ...
     apply (force dest!: he_ptrs_head_iff_he_ptrs_end simp: tcbQueueEmpty_def obj_at'_def)
    apply (clarsimp simp: typ_heap_simps ctcb_relation_def)
    apply (rule conjI)
...
lemma tcb_queue_remove_ccorres:
  ...
  by (intro conjI impI;
      clarsimp simp: ctcb_queue_relation_def typ_heap_simps option_to_ctcb_ptr_def
                     emptyHeadEndPtrs_def valid_tcb'_def)
...
lemma tcbQueueRemove_tcb_at'_head:
  ...
  apply (frule (1) tcb_ko_at_valid_objs_valid_tcb')
  apply (fastforce simp: valid_tcb'_def tcbQueueEmpty_def obj_at'_def)
  done
...
lemma setThreadState_ccorres[corres]:
   ...
   (\<lambda>s. tcb_at' thread s \<and> valid_objs' s \<and> valid_tcb_state' st s
        \<and> (ksSchedulerAction s = SwitchToThread thread \<longrightarrow> runnable' st)
        \<and> sch_act_wf (ksSchedulerAction s) s \<and> pspace_aligned' s \<and> pspace_distinct' s)
