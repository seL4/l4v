...
lemma page_table_at_partial_overwrite:
  "\<forall>x. tcb_at' (idx x) s \<Longrightarrow>
   page_table_at' pt_t p (ksPSpace_update (partial_overwrite idx f) s)
      = page_table_at' pt_t p s"
  by (simp add: page_table_at'_def typ_at_to_obj_at_arches
                obj_at_partial_overwrite_id2)

lemma getASIDPoolEntry_isolatable:
  "thread_actions_isolatable idx (getASIDPoolEntry asid)"
  supply if_split[split del]
  apply (simp add: getASIDPoolEntry_def getPoolPtr_def liftE_bindE liftME_def bindE_assoc
  ...
  apply (intro thread_actions_isolatable_bind[OF _ _ hoare_weaken_pre]
    ...
    apply (simp add: projectKO_opt_asidpool page_table_at_partial_overwrite split: asidpool.split
           | wpsimp wp: getASID_wp)+
  done
...
lemma findVSpaceForASID_isolatable:
  ...
  apply (intro thread_actions_isolatable_bind[OF _ _ hoare_weaken_pre]
         ...
         | simp add: projectKO_opt_asidpool page_table_at_partial_overwrite
                     findVSpaceForASID_def liftE_bindE liftME_def bindE_assoc
                     case_option_If2 assertE_def liftE_def checkPTAt_def
                     stateAssert_def2 split: asidpool_entry.split
         | wpsimp wp: getASID_wp)+
  done
...
lemma getASIDPool_isolatable:
  "thread_actions_isolatable idx (getObject v :: asidpool kernel)"
  by (rule getObject_isolatable; simp add: projectKO_opt_asidpool)
...
lemma setVCPU_isolatable:
  ...
  apply (subst setObject_assert_modify;
         simp add: projectKOs objBits_simps archObjSize_def vcpuBits_def vcpuBits_def pageBits_def)+
  apply (clarsimp simp: select_f_asserts state_assert_def bind_assoc exec_get assert_def obj_at_partial_overwrite_id2 split: if_splits)
  ...
  apply (rename_tac p')
  apply (erule_tac x=p' in allE)
  apply (clarsimp simp: obj_at'_def get_tcb_state_regs_defs atcbContextSet_def atcbContextGet_def split: tcb_state_regs.splits)
...
lemma setASIDPool_isolatable:
  "thread_actions_isolatable idx (setObject p (v::asidpool))"
  apply (clarsimp simp: thread_actions_isolatable_def monadic_rewrite_def isolate_thread_actions_def)
  apply (clarsimp simp: exec_gets getSchedulerAction_def)
  apply (subst setObject_assert_modify;
         simp add: projectKOs objBits_simps archObjSize_def vcpuBits_def vcpuBits_def pageBits_def)+
...
lemma cap_case_isPageTableCap:
  "(case cap of ArchObjectCap (PageTableCap pm pt_t (Some asid)) \<Rightarrow> fn pm asid | _ => g)
...
lemma armContextSwitch_isolatable:
  ...
  apply (intro thread_actions_isolatable_bind[OF _ _ hoare_weaken_pre]
         ...
         | split option.split asidpool.split asidpool_entry.split
         | simp add: liftM_def comp_def findFreeVMID_def invalidateASID_def updateASIDPoolEntry_def
                     getPoolPtr_def invalidateVMIDEntry_def storeVMID_def
         | wp hoare_drop_imps)+
  done
...
lemma oblivious_setObject_ksPSpace_asidpool[simp]:
  "\<lbrakk> \<forall>s. ksPSpace (f s) = ksPSpace s;
     \<forall>s g. ksPSpace_update g (f s) = f (ksPSpace_update g s) \<rbrakk> \<Longrightarrow>
   oblivious f (setObject p (v :: asidpool))"
  apply (simp add: setObject_def split_def updateObject_default_def
                   projectKO_def2 alignCheck_assert magnitudeCheck_assert)
  apply (intro oblivious_bind, simp_all)
  done
...
lemma oblivious_updateASIDPoolEntry_schact:
  "oblivious (ksSchedulerAction_update f) (updateASIDPoolEntry f' asid)"
  by (safe intro!: oblivious_bind
      | simp add: findVSpaceForASID_def getPoolPtr_def oblivious_liftM updateASIDPoolEntry_def)+
...
lemma oblivious_setVMRoot_schact:
  ...
  by (safe intro!: oblivious_bind oblivious_bindE oblivious_catch oblivious_mapM_x
      ...
      | simp add: liftE_def liftME_def invalidateASID_def findVSpaceForASID_def
                  getASIDPoolEntry_def getPoolPtr_def oblivious_liftM checkPTAt_def
                  armContextSwitch_def getVMID_def loadVMID_def findFreeVMID_def
                  updateASIDPoolEntry_def
                  invalidateVMIDEntry_def storeVMID_def setGlobalUserVSpace_def
             split: if_split capability.split arch_capability.split option.split pt_type.split
                    asidpool.split asidpool_entry.split)+
...
lemma oblivious_switchToThread_schact:
  ...
  by (safe intro!: oblivious_bind
              | simp_all add: ready_qs_runnable_def idleThreadNotQueued_def
                              ...
                              oblivious_lazyFpuRestore_schact)+
...
lemma setObject_modify_assert:
  ...
  apply (clarsimp simp: project_inject)
  apply (simp add: objBits_def objBitsT_koTypeOf[symmetric]
                    koTypeOf_injectKO)
  done
...
lemma asUser_asUserFPU_get:
  ...
  apply (clarsimp simp: obj_at'_def atcbContextSet_def thread_fetch_def atcbContextGet_def thread_replace_def)
  done
...
lemma threadSet_isolatable:
  ...
  apply (subst setObject_assert_modify;
         simp add: objBits_simps tcbBlockSizeBits_def)+
  ...
  apply (clarsimp simp: partial_overwrite_def put_tcb_state_regs_def get_tcb_state_regs_def)
  apply (drule_tac x=xa in spec)
  apply (clarsimp simp: obj_at'_def put_tcb_state_regs_tcb_unchanged)
  done
...
lemma switchToThread_isolatable:
  ...
  apply (intro thread_actions_isolatable_bind[OF _ _ hoare_weaken_pre]
               ...
               vcpuSwitch_isolatable lazyFpuRestore_isolatable)
...
lemmas fastpath_isolatables
    = setEndpoint_isolatable getCTE_isolatable
      thread_actions_isolatable_assert cteInsert_isolatable
