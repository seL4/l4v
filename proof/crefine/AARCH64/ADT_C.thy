...
where
  "handleHypervisorEvent_C t = (case t
    of hyp_fault_type.ARMVCPUFault hsr \<Rightarrow> (CALL handleVCPUFault(ucast hsr)))"
  ...
  "carch_state_to_H cstate \<equiv>
   ARMKernelState
      (array_map_conv (\<lambda>x. if x=NULL then None else Some (ptr_val x))
                      (mask asid_high_bits) (armKSASIDTable_' cstate))
      armKSKernelVSpace_C
      (array_map_conv (\<lambda>x. if x = 0 then None else Some x)
                      (mask vmid_bits) (armKSHWASIDTable_' cstate))
...
lemma cpspace_asidpool_relation_unique:
  ...
  apply (case_tac "p \<in> dom (map_to_asidpools ah)")
   ...
   apply (case_tac "i \<le> mask asid_low_bits"; simp)
    apply (clarsimp simp: casid_map_relation_def split: option.splits asid_map_CL.splits)
