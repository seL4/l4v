...
proof -
  ...
  have msg_aux:
    "\<forall>p. elem p (zip [4..<120] (drop 4 msg)) \<longrightarrow>
         (\<lambda>(x1, y1). setMR receiver None x1 y1) p = (\<lambda>_ . return (length msgRegisters)) p"
    by (clarsimp simp: setMR_def less_than_4 n_msgRegisters_def length_msgRegisters)
