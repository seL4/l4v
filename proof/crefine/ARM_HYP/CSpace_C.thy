...
lemma Arch_sameRegionAs_spec:
  ...
  apply (cases capa; simp add: cap_get_tag_isCap_unfolded_H_cap isCap_simps)
        ...
        \<comment> \<open>capa is SGISignalCap\<close>
...
lemma Arch_As_spec:
  ...
  apply vcg
  apply (clarsimp simp: ARM_HYP_H.sameObjectAs_def cong: if_cong)
...
lemma sameObjectAs_spec:
  ...
  supply isArchCap_tag_simps[simp] cap_tag_neqs[simp]
  apply (vcg exspec=Arch_As_spec)
  apply (clarsimp simp: sameObjectAs_def)
  apply (case_tac capa; simp add: cap_get_tag_isCap_unfolded_H_cap isCap_simps; fastforce?)
  ...
  apply (case_tac capb; clarsimp simp: cap_get_tag_isCap_unfolded_H_cap isCap_simps)
             apply (find_goal \<open>frule_tac cap'=cap_b in cap_get_tag_isArchCap_unfolded_H_cap\<close>)
             apply fastforce+
  done
