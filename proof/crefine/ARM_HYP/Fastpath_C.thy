      ...
      apply (rule ccorres_symb_exec_r, rule_tac xf'="ret__unsigned_'" in ccorres_abstract,
        ...
        apply (ctac(no_vcg) add: armv_contextSwitch_HWASID_ccorres)
         ...
         apply (rule ccorres_split_nothrow_novcg_dc)
            ...
            apply (clarsimp simp: simpler_modify_def rf_sr_def cstate_relation_def
                                  Let_def carch_state_relation_def cmachine_state_relation_def)
           apply (ctac add: clearExMonitor_fp_ccorres)
          apply wp
         apply (simp add: guard_is_UNIV_def)
        apply (wpsimp wp: hoare_drop_imps dmo_contextSwitch_HWASID_atcbVCPUPtr_cases_helper hoare_vcg_all_lift
                          hoare_vcg_imp_lift)
    ...
    apply (intro conjI impI allI
           ; clarsimp simp: isCap_simps capAligned_def objBits_simps' ccap_relation_pd_helper
                     ...
                     dest!: ptr_val_tcb_ptr_mask2[unfolded objBits_def mask_def, simplified]
                            isValidVTableRootD
                     split: sum.splits)
         apply (clarsimp simp: ctcb_relation_def pd_bits_def pde_bits_def)+
    done
qed
