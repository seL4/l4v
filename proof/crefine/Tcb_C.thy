...
lemma decodeUnbindAEP_ccorres:
  ...
  apply (cinit' lift: cap_' simp: decodeUnbindAEP_def)
   ...
   apply (ctac add: setThreadState_ccorres)
    ...
    apply simp
    apply (wp sts_invs_minor' hoare_option_case_wp sts_bound_tcb_at' | wpc | simp)+
   apply (vcg exspec=setThreadState_modifies)
...
lemma decodeBindAEP_ccorres:
  ...
  apply (cinit lift: cap_' extraCaps_' simp: decodeBindAEP_def)
   ...
   apply ctac
     ...
     apply (rule ccorres_assert2)
     apply (rule ccorres_Cond_rhs_Seq)
      apply (rule_tac P="?Q' (capAEPPtr rva) (capAEPCanReceive rva) rva" in ccorres_inst)
      apply (rule ccorres_rhs_assoc)+
      apply csymbr
      apply csymbr
      ...
      apply (simp add: bool_case_If if_to_top_of_bindE if_to_top_of_bind bind_assoc
      ...
      apply (rule_tac xf'="ret__int_'" and val="from_bool (aepBoundTCB aep \<noteq> None \<or> 
       ...
       apply wpc
         ...
         apply csymbr
         apply (clarsimp simp: if_to_top_of_bind returnOk_bind to_bool_eq_0[symmetric] simp del: Collect_const split del: split_if)
         apply (rule ccorres_Cond_rhs_Seq)
