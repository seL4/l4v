...
lemma vs_lookup_aobj_at_lift:
  ...
  apply (simp add: vs_lookup_def vs_lookup1_def vs_refs_def)
  apply (simp add: ex_ko_at_def2)
  apply (subst arch_only_obj_pred.obj_at_aobj_at)
  subgoal arch_only for \<dots> ko
   apply unfold_locales
   by (cases ko; simp)
  apply (subst arch_only_obj_pred.obj_at_aobj_at[OF arch_only])
...
lemma vs_lookup_pages_aobj_at_lift:
  ...
  subgoal arch_only for \<dots> ko
   apply unfold_locales
   by (cases ko; simp add: vs_refs_pages_def)
  apply (subst arch_only_obj_pred.obj_at_aobj_at[OF arch_only])
  apply (rule hoare_lift_Pf[where f="arch_state", OF _ arch_state])
  apply (rule hoare_lift_Pf[where f="\<lambda>s rs p rs' p'. aobj_at (P' p rs rs' p') p s" for P'])
   apply (rule hoare_vcg_prop)
...
lemma valid_arch_objs_lift_weak:
  ...
  apply (simp add: ko_at_ako[symmetric])
  apply (rule obj_at)
  done
...
lemma set_object_pt_not_vs_lookup_pages:
  ...
  apply (clarsimp simp: aobj_at_def)
   ...
   apply (clarsimp simp: obj_at_def split:split_if_asm)
   apply (case_tac "pa=p")
    apply (clarsimp simp: vs_refs_pages_def vs_refs_pages_arch_def graph_of_def)
...
lemmas valid_table_caps_lift = valid_table_caps_lift'[OF _ _ empty_table.obj_at_lift]
...
lemmas valid_arch_caps_lift = valid_arch_caps_lift'[OF _ _ _ empty_table.obj_at_lift]
...
lemma valid_global_objs_lift':
  ...
  apply (rule hoare_pre)
   ...
   apply (wp obj ko[simplified ko_at_ako] emp hoare_vcg_const_Ball_lift hoare_ex_wp)
  apply clarsimp
  done
...
lemma arch_lifts:
  ...
  apply -
  subgoal 
  apply (simp add: valid_global_pd_mappings_def valid_pd_kernel_mappings_def obj_at_fun_lift)
  ...
  apply (safe; clarsimp simp: valid_pt_kernel_mappings_def obj_at_fun_lift)
  ...
  apply (rule valid_global_objs_lift)
    ...
    apply (rule hoare_vcg_conj_lift)
     apply (wp aobj_at valid_arch_obj_atyp empty_table.obj_at_lift | simp add: ko_at_ako[symmetric])+
  done
  subgoal
...
lemma valid_ao_at_lift:
  ...
  by (wp hoare_vcg_ex_lift y[simplified ko_at_ako] valid_arch_obj_typ z)
...
lemma valid_global_pd_mappings_pres:
  ...
  apply atomize
  apply (clarsimp simp: valid_global_pd_mappings_def aobj_at_def obj_at_def)
  apply (clarsimp simp: valid_pd_kernel_mappings_def valid_pd_kernel_mappings_arch_def
                 split: Structures_A.kernel_object.split_asm arch_kernel_obj.split_asm)
  apply (drule_tac x=x in spec)
  apply (clarsimp simp: valid_pde_kernel_mappings_def valid_pd_kernel_mappings_arch_def obj_at_def
                        valid_pt_kernel_mappings_def valid_pt_kernel_mappings_arch_def[abs_def]pde_ref_def
                 split: Arch_Structs_A.ARM.pde.split_asm)
  apply (simp split: Structures_A.kernel_object.split_asm
                     arch_kernel_obj.split_asm)
  apply (drule spec, drule spec, drule(1) mp)
  apply (drule mp)
   apply (clarsimp simp: valid_global_objs_def obj_at_def empty_table_def empty_table_arch_def)
   apply (drule_tac x=x in spec)
...
lemma valid_table_caps_ptD:
  ...
  apply (erule allE)+
  apply (erule (1) impE)  
  apply (clarsimp simp add: is_pt_cap_def cap_asid_def cap_asid_arch_def)
  apply (erule impE, rule refl)
  apply (clarsimp simp: obj_at_def aobj_at_def empty_table_def empty_table_arch_def)
  done
