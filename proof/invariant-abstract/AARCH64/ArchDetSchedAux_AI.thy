...
lemma set_asid_pool_bound_sc_obj_tcb_at[wp]:
  ...
  apply (rename_tac ko, case_tac ko;
         clarsimp simp: obj_at_kh_kheap_simps vs_all_heap_simps
                 split: if_splits)
  done
...
lemma perform_asid_control_invocation_bound_sc_obj_tcb_at[wp]:
  ...
  apply (rule bound_sc_obj_tcb_at_nonz_cap_lift;
...
crunch perform_asid_control_invocation
  ...
  and misc[wp]: "\<lambda>s. P (scheduler_action s) (ready_queues s) (cur_domain s) (release_queue s)"
...
lemma perform_asid_control_invocation_obj_at_live:
  ...
  "\<lbrace>\<lambda>s. N (obj_at P p s)
   ...
   perform_asid_control_invocation aci
   \<lbrace>\<lambda>_ s. N (obj_at P p s)\<rbrace>"
  ...
  apply (rule_tac S="region_ptr && ~~mask pageBits = region_ptr \<and> is_aligned region_ptr pageBits
         ...
         fastforce simp: valid_aci_def cte_wp_at_caps_of_state valid_cap_simps
                         cap_aligned_def pageBits_def word_size_bits_def
  ...
  apply (wpsimp wp: cap_insert_cspace_agnostic_obj_at[OF csp]
                    ...
                    retype_region_obj_at_live[where sz=pageBits, OF live]
  ...
  apply (frule (2) descendants_of_empty_untyped_range[where p=p]; simp)
  done
...
lemma perform_asid_control_invocation_pred_tcb_at_live:
  ...
  "\<lbrace>\<lambda>s. N (pred_tcb_at proj P p s)
        ...
        \<and> scheduler_action s = resume_cur_thread\<rbrace>
   perform_asid_control_invocation aci
   \<lbrace>\<lambda>_ s. N (pred_tcb_at proj P p s)\<rbrace>"
  unfolding pred_tcb_at_def
  using live
  by (auto intro!: perform_asid_control_invocation_obj_at_live
             simp: cspace_agnostic_pred_def tcb_to_itcb_def)
...
lemma perform_asid_control_invocation_sc_at_pred_n_live:
  ...
  "\<lbrace>\<lambda>s. Q (sc_at_pred_n N proj P p s)
        ...
        \<and> scheduler_action s = resume_cur_thread\<rbrace>
   perform_asid_control_invocation aci
   \<lbrace>\<lambda>_ s. Q (sc_at_pred_n N proj P p s)\<rbrace>"
  unfolding sc_at_pred_n_def
  using live
  by (auto intro!: perform_asid_control_invocation_obj_at_live
             simp: cspace_agnostic_pred_def live_def)
...
lemma perform_asid_control_invocation_ep_at_pred_live:
  ...
  "\<lbrace>\<lambda>s. Q (ep_at_pred P p s)
        ...
        \<and> scheduler_action s = resume_cur_thread\<rbrace>
   perform_asid_control_invocation aci
   \<lbrace>\<lambda>_ s. Q (ep_at_pred P p s)\<rbrace>"
  unfolding ep_at_pred_def2
  using live
  by (auto intro!: perform_asid_control_invocation_obj_at_live
             simp: cspace_agnostic_pred_def live_def)
...
lemma perform_asid_control_invocation_ntfn_at_pred_live:
  ...
  "\<lbrace>\<lambda>s. Q (ntfn_at_pred P p s)
        ...
        \<and> scheduler_action s = resume_cur_thread\<rbrace>
   perform_asid_control_invocation aci
   \<lbrace>\<lambda>_ s. Q (ntfn_at_pred P p s)\<rbrace>"
  unfolding ntfn_at_pred_def2
  using live
  by (auto intro!: perform_asid_control_invocation_obj_at_live
             simp: cspace_agnostic_pred_def live_def)
