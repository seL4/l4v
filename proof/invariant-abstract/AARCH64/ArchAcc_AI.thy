...
lemma set_vcpu_eps_of[wp]:
  "set_vcpu ptr vcpu \<lbrace>\<lambda>s. P (eps_of s)\<rbrace>"
  by (set_object_easy_cases def: set_vcpu_def)

lemma set_vcpu_ntfns_of[wp]:
  "set_vcpu ptr vcpu \<lbrace>\<lambda>s. P (ntfns_of s)\<rbrace>"
  by (set_object_easy_cases def: set_vcpu_def)

lemma set_vcpu_tcbs_of[wp]:
  "set_vcpu ptr vcpu \<lbrace>\<lambda>s. P (tcbs_of s)\<rbrace>"
  by (set_object_easy_cases def: set_vcpu_def)
...
lemma set_asid_pool_valid_idle [wp]:
  "set_asid_pool p ap \<lbrace>valid_idle\<rbrace>"
...
crunch set_asid_pool
  ...
  (wp: replies_with_sc_lift replies_blocked_lift cur_sc_tcb_lift fault_tcbs_valid_states_lift
   ignore: set_asid_pool)
...
crunch set_asid_pool
  ...
  and valid_replies[wp]: valid_replies
  (wp: valid_cur_fpu_lift valid_replies_lift crunch_wps)
...
crunch store_pte
  ...
  (wp: set_pt_zombies set_pt_ifunsafe set_object_wp valid_irq_node_typ
       set_pt_valid_global valid_irq_handlers_lift set_pt_cur vmid_inv_ap_lift cur_vcpu_typ_lift)
...
lemma set_pt_sch_act[wp]:
  ...
  including unfold_objects
  by (wpsimp simp: set_pt_def wp: set_object_wp)
...
crunch set_pt
  ...
  and cur_sc_tcb[wp]: cur_sc_tcb
  and valid_replies[wp]: valid_replies
  (wp: replies_with_sc_lift replies_blocked_lift cur_sc_tcb_lift ignore: set_pt)
...
crunch store_pte
  ...
  and replies_blocked[wp]: "\<lambda>s. P (replies_blocked s)"
  and valid_replies[wp]: valid_replies
  (wp: crunch_wps fault_tcbs_valid_states_lift)
