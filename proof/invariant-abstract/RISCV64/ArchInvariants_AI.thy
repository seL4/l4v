...
definition pt_lookup_target ::
  ...
  "pt_lookup_target bot_level pt_root vref \<equiv> do {
     (level, slot) \<leftarrow> pt_lookup_slot_from_level max_pt_level bot_level pt_root vref \<circ> ptes_of;
...
definition valid_global_tables :: "'z::state_ext state \<Rightarrow> bool" where
  "valid_global_tables \<equiv> \<lambda>s.
    \<forall>bot_level vref level pt_ptr.
       ...
       pt_lookup_target bot_level (riscv_global_pt (arch_state s)) vref s = Some (level, pt_ptr) \<longrightarrow>
...
definition valid_uses_2 :: "(obj_ref \<Rightarrow> riscvvspace_region_use) \<Rightarrow> bool" where
  "valid_uses_2 uses \<equiv>
     \<forall>p. (\<not>canonical_address p \<longrightarrow> uses p = RISCVVSpaceInvalidRegion)
          ...
          \<comment> \<open>We allow all unreserved canonical virtual addresses as the user window:\<close>
          \<and> (p < user_vtop \<longrightarrow> canonical_address p \<longrightarrow> uses p = RISCVVSpaceUserRegion)
...
lemma valid_uses_kernel_window:
  ...
  by (erule_tac x=p in allE) (auto simp: user_vtop_def pptrUserTop_def pptr_base_def)
...
lemma valid_global_vspace_mappings_kwD:
  "\<lbrakk> valid_global_vspace_mappings s; vref \<in> kernel_window s \<rbrakk>
   \<Longrightarrow> translate_address (riscv_global_pt (arch_state s)) vref s = Some (addrFromPPtr vref)"
...
lemma user_vtop_ge0[intro!,simp]:
  "0 < user_vtop"
  by (simp add: user_vtop_def pptrUserTop_def pptrBase_def canonical_bit_def)

lemma user_region0[intro!,simp]:
  "valid_uses s \<Longrightarrow> 0 \<in> user_region s"
  by (simp add: valid_uses_def user_region_def)
...
lemma user_vtop[simp]:
  "user_vtop = pptr_base"
  by (simp add: user_vtop_def pptrUserTop_def pptr_base_def)
...
lemma valid_uses_user_region_eq:
  "valid_uses s \<Longrightarrow> (p \<in> user_region s) = (p < pptr_base \<and> canonical_address p)"
  unfolding valid_uses_def user_region_def
  by (erule_tac x=p in allE) (auto simp: not_le)
...
lemma canonical_vref_for_levelI:
  ...
  apply (simp add: canonical_address_def canonical_address_of_def vref_for_level_def pptr_base_mask bit_simps)
  apply word_bitwise
...
lemma vref_for_level_user_region:
  ...
  by (clarsimp simp: valid_uses_user_region_eq vref_for_level_pptr_baseI canonical_vref_for_levelI)
...
lemma user_region_slots:
  ...
  apply word_bitwise
  by (clarsimp simp: word_bits_def canonical_bit_def word_size)
...
lemma equal_mappings_translate_address:
  "\<lbrakk> vspace_for_asid asid s = Some root_pt; vref \<in> kernel_window s;
     ...
     valid_vspace_objs s; valid_asid_table s; valid_uses s;
     pspace_aligned s \<rbrakk>
  \<Longrightarrow> translate_address root_pt vref s = translate_address (riscv_global_pt (arch_state s)) vref s"
  by (blast dest: equal_mappings_pt_slot_offset translate_address_equal_top_slot)
...
lemma pt_lookup_target_update[iff]:
  "pt_lookup_target bot_level pt_root vref (f s) = pt_lookup_target bot_level pt_root vref s"
  by (simp add: pt_lookup_target_def obind_def oassert_def oreturn_def split: option.splits)

lemma translate_address_update[iff]:
  "translate_address pt_root vref (f s) = translate_address pt_root vref s"
  by (simp add: translate_address_def obind_def oassert_def oreturn_def split: option.splits)
