...
crunch activate_thread,switch_to_thread, handle_hypervisor_fault,
       ...
       activate_thread, switch_to_thread, check_domain_time
  for valid_vspace_objs'[wp]: "valid_vspace_objs'"
  ...
  apply (cases e, simp_all add: call_kernel_def preemption_path_def)
       apply (rule bind_wp_fwd)
        ...
        apply (rule_tac Q="\<lambda>_. valid_vspace_objs'" in handleE_wp[rotated])
         apply (rule_tac Q'="\<lambda>_. invs and ct_running and valid_vspace_objs' and
           (\<lambda>s. scheduler_action s = resume_cur_thread) and
           ct_schedulable" in bindE_wp_fwd)
          apply (rule liftE_wp)
         ...
         apply (rule_tac Q'="\<lambda>rv. invs and (\<lambda>s. rv \<longrightarrow> ct_running s) and
           ...
           (\<lambda>s. rv \<longrightarrow> ct_schedulable s)" in bindE_wp_fwd)
          apply (rule liftE_wp)
       ...
       apply wpsimp
    (***)
      apply (rule_tac Q'="\<lambda>_. valid_vspace_objs'" in bind_wp_fwd)
       apply (rule validE_valid)
       apply (rule liftE_wp)
       apply (rule_tac Q'="\<lambda>_. invs and ct_running and
           valid_vspace_objs' and
        ...
        apply (clarsimp simp: pred_tcb_at_def obj_at_def schedulable_def opt_pred_def opt_map_def
                              tcbs_of_kh_def
                       split: option.splits)
       apply (rule_tac Q'="\<lambda>rv. invs and (\<lambda>s. rv \<longrightarrow> ct_running s) and valid_vspace_objs'"
                       in bind_wp_fwd)
        apply (wpsimp wp: check_budget_restart_true)
       apply (wpsimp+)[2]
    (***)
   ...
   apply (rule_tac Q'="\<lambda>_. valid_vspace_objs'" in bind_wp_fwd)
    ...
    apply (rule_tac Q'="\<lambda>_. invs and ct_running and
     ...
     apply (clarsimp simp: pred_tcb_at_def obj_at_def schedulable_def opt_pred_def opt_map_def
                    ...
                    split: option.splits)
    apply (rule_tac Q'="\<lambda>rv. invs and (\<lambda>s. rv \<longrightarrow> ct_running s) and
           valid_vspace_objs'" in bind_wp_fwd)
     apply (wpsimp wp: check_budget_restart_true)
    apply (wpsimp+)[2]
    (***)
  apply wpsimp
