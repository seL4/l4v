...
crunch set_extra_badge, do_machine_op
...
locale Ipc_AI_2 = Ipc_AI state_ext_t
  ...
  assumes make_arch_fault_msg_inv[wp]:
    "\<And>ft t. make_arch_fault_msg ft t \<lbrace>P :: 'state_ext state \<Rightarrow> bool\<rbrace>"
...
crunch set_extra_badge
 for pred_tcb_at[wp]: "\<lambda>s. Q (pred_tcb_at proj P p s)"
crunch set_extra_badge
...
crunch do_machine_op, set_extra_badge
...
crunch refill_unblock_check
 for valid_replies_pred[wp]: "\<lambda>s.  valid_replies_pred P s"
...
lemma update_waiting_notification_sym_refs_helper:
  "\<lbrakk>sym_refs (state_refs_of s); valid_objs s;  kheap s ntfnptr = Some (Notification ntfn);
   ...
   \<Longrightarrow> sym_refs
         (\<lambda>b. if b = a
              then tcb_non_st_state_refs_of s a
              else if b = ntfnptr
                   then ntfn_q_refs_of
                        (case list of
                             [] \<Rightarrow> IdleNtfn
...
lemma update_waiting_invs:
  "\<lbrace>\<lambda>s. invs s \<and> (\<exists>ntfn. ko_at (Notification ntfn) ntfnptr s \<and> ntfn_obj ntfn = WaitingNtfn q)\<rbrace>
  ...
  apply (rule conjI)
   ...
   apply (clarsimp elim!: fault_tcbs_valid_states_not_fault_tcb_states
                          pred_tcb_weakenE
                    simp: pred_neg_def)
  apply (prop_tac "head \<noteq> ntfnptr")
...
crunch blocked_cancel_ipc, reply_remove_tcb, cancel_signal
...
locale Ipc_AI_3 = Ipc_AI_2 state_ext_t
  for state_ext_t :: "'state_ext::state_ext itself" +
...
lemma complete_signal_invs:
  ...
  apply safe
   ...
   apply (fastforce intro!: ex_cap_to_after_update dest!: get_tcb_SomeD
                      simp: obj_at_def tcb_cap_cases_def is_tcb)+
  done
...
crunch set_message_info, set_mrs
...
crunch complete_signal, do_nbrecv_failed_transfer, reply_push, receive_signal
  for cap_to[wp]: "ex_nonz_cap_to p"
