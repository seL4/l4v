...
declare arch_tcb_sanitise_condition_invs[Syscall_AI_assms]
crunch pred_tcb_at[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "pred_tcb_at proj P t"
crunch invs[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "invs"
crunch cap_to[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "ex_nonz_cap_to c"
crunch it[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "\<lambda>s. P (idle_thread s)"
crunch caps[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "\<lambda>s. P (caps_of_state s)"
crunch cur_thread[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "\<lambda>s. P (cur_thread s)"
crunch valid_objs[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "valid_objs"
crunch cte_wp_at[wp,Syscall_AI_assms]: handle_arch_fault_reply, arch_tcb_sanitise_condition "\<lambda>s. P (cte_wp_at P' p s)"

crunch typ_at[wp, Syscall_AI_assms]: invoke_irq_control, arch_tcb_sanitise_condition "\<lambda>s. P (typ_at T p s)"
