...
lemma cancel_all_signals_st_tcb_at_helper:
  ...
  shows "\<lbrace>V q\<rbrace>
          mapM_x (\<lambda>t. do set_thread_state t Restart;
                         sc_ptr <- get_tcb_obj_ref tcb_sched_context t;
...
lemma suspend_invs:
  ...
  apply (rule hoare_seq_ext[OF sched_context_cancel_yield_to_invs])
...
lemma suspend_unlive:
  ...
  apply (simp flip: bind_assoc)
  apply (rule hoare_seq_ext[OF sched_context_cancel_yield_to_not_live0])
  apply (simp add: bind_assoc)
...
lemma cancel_all_signals_invs_helper:
  "\<lbrace>all_invs_but_sym_refs
     ...
     mapM_x (\<lambda>t. do y \<leftarrow> set_thread_state t Structures_A.thread_state.Restart;
                    sc_ptr <- get_tcb_obj_ref tcb_sched_context t;
                    y \<leftarrow> if_sporadic_cur_sc_assert_refill_unblock_check sc_ptr;
...
lemma restart_thread_if_no_fault_obj_at_impossible':
  "\<lbrakk>\<And>tcb. \<not> P' (TCB tcb); \<And>sc n. \<not> P' (SchedContext sc n)\<rbrakk> \<Longrightarrow> restart_thread_if_no_fault t \<lbrace>\<lambda>s. P (obj_at P' p s)\<rbrace>"
