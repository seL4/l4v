...
abbreviation sc_refill_ready_times_of :: "'z state \<Rightarrow> obj_ref \<rightharpoonup> time" where
  "sc_refill_ready_times_of s \<equiv> sc_refills_of s |> hd_opt ||> r_time"

abbreviation tcb_refill_ready_times_of :: "'z state \<Rightarrow> obj_ref \<rightharpoonup> time" where
