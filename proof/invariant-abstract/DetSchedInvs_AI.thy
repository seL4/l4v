...
lemma budget_ready_set_object_no_change_tcb:
  "\<lbrakk>\<And>x. tcb_sched_context (f x) = tcb_sched_context x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_ready t and ko_at (TCB tcb) tptr\<rbrace>
    set_object tptr (TCB (f tcb)) \<lbrace>\<lambda>rv. budget_ready t\<rbrace>"
  apply (wpsimp simp: set_object_def pred_tcb_at_def obj_at_def)
  by (fastforce simp: obj_at_def is_refill_ready_def)

lemma budget_sufficient_set_object_no_change_tcb:
  "\<lbrakk>\<And>x. tcb_sched_context (f x) = tcb_sched_context x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_sufficient t and ko_at (TCB tcb) tptr\<rbrace>
    set_object tptr (TCB (f tcb)) \<lbrace>\<lambda>rv. budget_sufficient t\<rbrace>"
  apply (wpsimp simp: set_object_def pred_tcb_at_def obj_at_def)
  by (fastforce simp: obj_at_def is_refill_sufficient_def)

lemma budget_ready_set_object_no_change_sc:
  "\<lbrakk>\<And>x. sc_refills (f x) = sc_refills x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_ready t and ko_at (SchedContext sc n) tptr\<rbrace>
      set_object tptr (SchedContext (f sc) n) \<lbrace>\<lambda>rv. budget_ready t\<rbrace>"
  apply (wpsimp simp: pred_tcb_at_def set_object_def obj_at_def)
  by (fastforce simp: obj_at_def is_refill_ready_def)

lemma budget_sufficient_set_object_no_change_sc:
  "\<lbrakk>\<And>x. sc_refills (f x) = sc_refills x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_sufficient t and ko_at (SchedContext sc n) tptr\<rbrace>
      set_object tptr (SchedContext (f sc) n) \<lbrace>\<lambda>rv. budget_sufficient t\<rbrace>"
  apply (wpsimp simp: pred_tcb_at_def set_object_def obj_at_def)
  by (fastforce simp: obj_at_def is_refill_sufficient_def)
...
lemma budget_ready_thread_set_no_change:
  "\<lbrakk>\<And>x. tcb_sched_context (f x) = tcb_sched_context x\<rbrakk>
         \<Longrightarrow> \<lbrace>budget_ready t\<rbrace> thread_set f tptr \<lbrace>\<lambda>rv. budget_ready t\<rbrace>"
   apply (clarsimp simp: thread_set_def)
    apply (rule hoare_seq_ext[OF _ assert_get_tcb_ko'])
...
lemma budget_sufficient_thread_set_no_change:
  "\<lbrakk>\<And>x. tcb_sched_context (f x) = tcb_sched_context x\<rbrakk>
         \<Longrightarrow> \<lbrace>budget_sufficient t\<rbrace> thread_set f tptr \<lbrace>\<lambda>rv. budget_sufficient t\<rbrace>"
   apply (clarsimp simp: thread_set_def)
    apply (rule hoare_seq_ext[OF _ assert_get_tcb_ko'])
...
lemma budget_sufficient_update_sched_context_no_change:
  "\<lbrakk>\<And>x. sc_refills (f x) = sc_refills x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_sufficient t\<rbrace> update_sched_context scp f \<lbrace>\<lambda>rv. budget_sufficient t\<rbrace>"
  apply (clarsimp simp:  update_sched_context_def)
  apply (rule hoare_seq_ext[OF _ get_object_sp])
  apply (wpsimp simp: pred_tcb_at_def obj_at_def set_object_def wp: get_object_wp)
  by (fastforce simp: is_refill_sufficient_def obj_at_def)

lemma budget_ready_update_sched_context_no_change:
  "\<lbrakk>\<And>x. sc_refills (f x) = sc_refills x\<rbrakk>
     \<Longrightarrow> \<lbrace>budget_ready t\<rbrace> update_sched_context scp f \<lbrace>\<lambda>rv. budget_ready t\<rbrace>"
  apply (clarsimp simp:  update_sched_context_def)
  apply (rule hoare_seq_ext[OF _ get_object_sp])
  apply (wpsimp simp: pred_tcb_at_def obj_at_def set_object_def wp: get_object_wp)
  by (fastforce simp: is_refill_ready_def obj_at_def)
  ...
  "valid_release_q_2 queue kh =
    (\<forall>t \<in> set queue. is_etcb_at' t (etcbs_of' kh) \<and>
            (*  (case etcbs_of' kh t of None \<Rightarrow> False | Some x \<Rightarrow> True) \<and> *) (* redundant *)
              st_tcb_at_kh runnable t kh \<and> active_sc_tcb_at_kh t kh \<and>
              distinct queue)"
...
definition valid_blocked_2 where
   "valid_blocked_2 queues rlq kh sa ct \<equiv>
    (\<forall>t st. not_queued_2 queues t \<longrightarrow> not_in_release_q_2 rlq t \<longrightarrow> st_tcb_at_kh ((=) st) t kh \<longrightarrow>
            t \<noteq> ct \<longrightarrow> sa \<noteq> switch_thread t \<longrightarrow> (\<not> (active st \<and> active_sc_tcb_at_kh t kh)))"
...
lemma valid_release_q_lift:
  ...
  apply (rule hoare_lift_Pf[where f="\<lambda>s. etcbs_of s", OF _ b])
  apply (wpsimp wp: hoare_vcg_ball_lift hoare_vcg_all_lift hoare_vcg_conj_lift a a')
  done
