...
lemma is_derived_is_same_vspace_table_type:
  "is_derived m p cap cap' \<Longrightarrow>
  ...
  apply (clarsimp simp: cap_master_cap_def is_vspace_table_cap_defs
                 split: cap.splits arch_cap.splits)+
  done
   ...
   apply (wp set_cap_valid_arch_caps get_cap_wp set_untyped_cap_as_full_valid_arch_caps
      ...
      apply (wp hoare_vcg_all_lift hoare_vcg_imp_lift set_untyped_cap_as_full_cte_wp_at_neg
       ...
       apply (wp set_untyped_cap_as_full_obj_at_impossible,
             (wp hoare_vcg_all_lift hoare_vcg_imp_lift set_untyped_cap_as_full_cte_wp_at_neg
       ...
       apply (wp set_untyped_cap_as_full_cte_wp_at hoare_vcg_all_lift hoare_vcg_imp_lift
                  set_untyped_cap_as_full_cte_wp_at_neg hoare_vcg_ex_lift | clarsimp)+
     apply (wp get_cap_wp)+
  apply (intro conjI allI impI disj_subst; clarsimp)
           defer
           ...
           apply (drule table_cap_ref_eq_rewrite; solves \<open>clarsimp\<close>)
          apply (clarsimp simp:empty_table_caps_of valid_arch_caps_def)
          apply (frule(1) valid_table_capsD; clarsimp simp: cte_wp_at_caps_of_state)
            apply (clarsimp simp: is_derived_is_same_vspace_table_type)
           apply (frule is_derived_cap_asid_issues)
...
lemma set_untyped_cap_as_full_ioports:
  "\<lbrace>valid_ioports and (\<lambda>s. cte_wp_at ((=) src_cap) src s)\<rbrace>
     set_untyped_cap_as_full src_cap cap src
...
lemma update_cap_data_validI [CSpace_AI_assms]:
  ...
  apply (rename_tac arch_cap)
  apply (case_tac arch_cap;
         clarsimp simp: is_cap_defs arch_update_cap_data_def Let_def valid_cap_def cap_aligned_def)
  done
...
lemma no_cap_to_obj_with_diff_ref_triv:
  "\<lbrakk> valid_objs s; valid_cap cap s;
     \<not> is_pt_cap cap; \<not> is_pd_cap cap; \<not> is_pdpt_cap cap; \<not> is_pml4_cap cap;
     table_cap_ref cap = None \<rbrakk>
...
lemma setup_reply_master_arch_caps[wp, CSpace_AI_assms]:
  ...
  apply (clarsimp simp: cte_wp_at_caps_of_state is_vspace_table_cap_defs vs_cap_ref_def)
