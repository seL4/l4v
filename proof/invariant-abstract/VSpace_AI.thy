...
definition 
  "valid_pdi pdi \<equiv> case pdi of
    ArchInvocation_A.PageDirectoryFlush typ start end pstart pd asid \<Rightarrow>
      pd_at_asid asid pd and K (asid \<le> mask asid_bits \<and> asid \<noteq> 0)
  | ArchInvocation_A.PageDirectoryNothing \<Rightarrow> \<top>"
...
lemma set_cap_valid_slots[wp]:
  "\<lbrace>valid_slots x2\<rbrace> set_cap cap (a, b) 
          \<lbrace>\<lambda>rv s. valid_slots x2 s \<rbrace>"  apply (case_tac x2)
...
lemma set_cap_valid_pte [wp]:
  "\<lbrace>valid_pte pte\<rbrace> set_cap cap p \<lbrace>\<lambda>rv. valid_pte pte\<rbrace>"
  by (wp valid_pte_lift set_cap_typ_at)
...
lemma set_cap_valid_pde [wp]:
  "\<lbrace>valid_pde pde\<rbrace> set_cap cap p \<lbrace>\<lambda>rv. valid_pde pde\<rbrace>"
  by (wp valid_pde_lift set_cap_typ_at)


locale vs_lookup_map_some_pdes =
...
lemma set_pd_valid_vs_lookup_map:
  ...
  apply (erule (1) vs_lookup_pagesE_alt)
   ...
   apply (frule vs_lookup_pages_apI)
     apply (simp split: split_if_asm)+
   apply (thin_tac "\<forall>r. (r \<unrhd> p) s \<longrightarrow> ?Q r")+
...
lemma set_pd_valid_arch_caps:
  ...
  apply (erule_tac P="is_pd_cap cap" in disjE)
   ...
   apply (clarsimp simp add: is_pt_cap_def valid_cap_def obj_at_def a_type_def)
  ...
  apply (rule conjI)
   ...
   apply (erule disjE)
    ...
    apply (drule valid_vs_lookupD[OF vs_lookup_pages_step[OF vs_lookup_pages_step]])
     ...
     apply simp
    apply clarsimp
   apply (erule_tac x=r in allE, drule (1) mp, drule_tac P="(%x. \<forall>q' pt . ?Q x q' pt y ?s)" in bspec)
   apply simp
   apply clarsimp
  apply (rule conjI)
   apply clarsimp
...
lemma vs_lookup_vs_lookup_pagesI':
  ...
  apply (rule_tac y="([VSRef (ucast c) (Some APageDirectory), VSRef (ucast b) (Some AASIDPool),
           VSRef (ucast a) None], (Platform.ptrFromPAddr addr))" in rtrancl_trans)
   apply (rule rtrancl_into_rtrancl[OF rtrancl.intros(1)])
   apply (clarsimp simp: vs_lookup1_def obj_at_def vs_refs_def graph_of_def)
   apply (rule_tac x="(c,(Platform.ptrFromPAddr addr))" in image_eqI)
    apply simp 
...
lemma store_pde_map_invs:
  ...
  apply (rule conjI)
   ...
   apply (rule vs_refs_add_one)
    apply (simp add: pde_ref_def)
   apply (simp add: kernel_vsrefs_kernel_mapping_slots)
  apply (rule conjI)
   apply (clarsimp simp: empty_pde_at_def)
   apply (clarsimp simp: obj_at_def)
  ...
  apply (rule conjI)
   apply clarsimp
   apply (case_tac pde, simp_all add: pde_ref_def)[1]
  apply (rule conjI)
  ...
  apply (rule conjI)
   ...
   apply (thin_tac " \<forall>p. ?Q p \<longrightarrow> ?P p")+
   apply (simp add: is_pd_cap_def vs_cap_ref_def
             split: cap.split_asm arch_cap.split_asm option.split_asm)
  apply (rule conjI)
   apply clarsimp
  apply (rule conjI)
...
lemma arch_update_cap_invs_map:
  ...
  apply (thin_tac "cap_range ?a = cap_range ?b")
  apply (rule conjI)
   apply (rule ext)
...
lemma mapM_x_swp_store_empty_table':
  ...
  apply (thin_tac "\<lbrace>?P\<rbrace> ?f \<lbrace>?Q\<rbrace>")
...
lemma store_pte_no_lookup_pages:
  ...
  apply (thin_tac "(?ref \<unrhd> ?p) (kheap_update ?f s)")
...
lemma perform_page_directory_invocation_invs[wp]:
  ...
  apply (cases pdi)
   ...
   apply (wp dmo_invs set_vm_root_for_flush_invs
    ...
    apply (clarsimp simp: valid_def)
    apply (thin_tac "?p \<in> fst (set_vm_root_for_flush ?a ?b s)")
    apply safe[1]
     apply (drule_tac Q="\<lambda>_ m'. underlying_memory m' p = underlying_memory m p"
            in use_valid)
       apply ((clarsimp | wp)+)[3]
...
lemma flush_page_invs:
  ...
  apply (wp dmo_invs hoare_vcg_all_lift load_hw_asid_wp
     ...
     apply (rule hoare_pre_imp[of _ \<top>], assumption)
     apply (clarsimp simp: valid_def)
     apply (thin_tac "?x : fst (set_vm_root_for_flush ?a ?b ?c)")
     apply safe
      apply (drule_tac Q="\<lambda>_ m'. underlying_memory m' p = underlying_memory m p"
             in use_valid)
        apply ((clarsimp | wp)+)[3]
...
lemma store_pte_unmap_page:
  ...
  apply (clarsimp simp: vs_lookup1_def obj_at_def split: split_if_asm)
      ...
      apply (thin_tac "(VSRef ?a (Some AASIDPool), ?b) \<in> ?c")
      apply (clarsimp simp: graph_of_def
                     split: Structures_A.kernel_object.split_asm 
                            ARM_Structs_A.arch_kernel_obj.splits 
                            split_if_asm)
      apply (erule_tac P="a = ?c" in swap)
...
lemma vs_lookup_pages_pteD:
  ...
  apply clarsimp
  apply (frule_tac vs="[?z]" in vs_lookup_pages_2ConsD)
  apply clarsimp
...
lemma unmap_page_unmapped:
  ...
  apply (case_tac sz, simp_all)
     ...
     apply (clarsimp simp: pde_ref_pages_def 
       ...
       apply (drule_tac p="Platform.ptrFromPAddr ?x" in vs_lookup_vs_lookup_pagesI')
          apply ((simp add: obj_at_def a_type_def)+)[3]
       apply (frule_tac p="Platform.ptrFromPAddr ?a" in valid_arch_objsD)
      ...
      apply (rule_tac Q="\<lambda>_ s. invs s \<and>
                               cte_wp_at (\<lambda>c. is_pg_cap c \<and>
                                 (\<forall>ref. vs_cap_ref c = Some ref \<longrightarrow>
                                        \<not> (ref \<unrhd> obj_ref_of c) s)) prod s"
                   in hoare_strengthen_post)
       prefer 2
