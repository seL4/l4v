...
lemma no_overflow_ordered_disjoint_implies_refills_sum_unat_no_overflow:
  "\<lbrakk>refills \<noteq> [];
   ...
   \<Longrightarrow> refills_unat_sum refills \<le> Suc (unat (max_word :: time))"
...
lemma unat_sum_list_at_most_unat_max_word:
  "\<lbrakk>refills \<noteq> [];
    ...
    ordered_disjoint refills;
    MIN_BUDGET \<le> refills_sum refills\<rbrakk>
   \<Longrightarrow> refills_unat_sum refills \<le> unat (max_word :: time)"
  supply map_map[simp del]
...
lemma refill_unblock_check_active_sc_at[wp]:
  ...
  apply (wpsimp wp: update_sched_context_wp get_refills_wp whileLoop_wp'
              simp: is_round_robin_def merge_refills_def refill_pop_head_def set_refills_def
                    active_sc_at_def refill_head_overlapping_loop_def)
  apply fastforce
  done
...
lemma refill_unblock_check_not_active_sc_at[wp]:
  ...
  apply (wpsimp wp: update_sched_context_wp get_refills_wp whileLoop_wp'
              simp: is_round_robin_def merge_refills_def refill_pop_head_def set_refills_def
                    active_sc_at_def pred_neg_def refill_head_overlapping_loop_def)
  apply fastforce
  done
...
lemma refill_budget_check_bound_sc:
  ...
  apply (wpsimp simp: refill_budget_check_def is_round_robin_def merge_refills_def
                      set_refills_def non_overlapping_merge_refills_def refill_pop_head_def
                      head_insufficient_loop_def refill_head_overlapping_loop_def
                  wp: set_refills_bound_sc whileLoop_wp' get_refills_wp update_sched_context_wp
