...
(* FIXME: move *)
lemmas budget_sufficient_defs = bound_sc_tcb_at_kh_def obj_at_kh_def pred_tcb_at_def obj_at_def
                                refill_sufficient_kh_def is_refill_sufficient_def

lemmas budget_ready_defs = pred_tcb_at_def obj_at_def bound_sc_tcb_at_kh_def obj_at_kh_def
...
lemma rt_assumptions:
  ...
  "\<lbrakk>kheap s rp = Some (Reply reply);
   (\<exists>thread. reply_tcb reply = Some thread \<and> (st_tcb_at ((=) (BlockedOnReply rp)) thread s))\<rbrakk>
  ...
  "st_tcb_at (\<lambda>st. \<exists>rp. st = (BlockedOnReply rp) \<longrightarrow>
...
lemmas cur_time_assumption = rt_assumptions(6)
(*
...
locale DetSchedSchedule_AI =
  ...
  assumes handle_arch_fault_reply_simple_sched_action[wp]:
    "\<And>t' f t x y. \<lbrace>simple_sched_action\<rbrace> handle_arch_fault_reply f t x y \<lbrace>\<lambda>_. simple_sched_action::det_state \<Rightarrow> _\<rbrace>"
  assumes handle_arch_fault_reply_valid_ep_q[wp]:
    "\<And>t' f t x y. \<lbrace>valid_ep_q\<rbrace> handle_arch_fault_reply f t x y \<lbrace>\<lambda>_. valid_ep_q::det_state \<Rightarrow> _\<rbrace>"
...
lemma set_simple_ko_valid_blocked_except_set[wp]:
  "\<lbrace>valid_blocked_except_set S\<rbrace> set_simple_ko f ptr ep \<lbrace>\<lambda>rv. valid_blocked_except_set S\<rbrace>"
...
lemma ball_mapM_scheme:  (* maybe move? is this generic enough? *)
  ...
  shows "distinct xs \<Longrightarrow> \<lbrace> (\<lambda>s. \<forall>t\<in>set xs. Q t s) and P\<rbrace> mapM f xs \<lbrace>\<lambda>_. P\<rbrace>"
  apply (induct xs)
...
lemma ball_mapM_x_scheme:
  assumes x: "\<And>t t'. \<lbrace> \<lambda>s. Q t' s \<and> t' \<noteq> t \<rbrace> f t \<lbrace> \<lambda>_. Q t' \<rbrace>"
  and y:  "\<And>t. \<lbrace> Q t and P \<rbrace> f t \<lbrace>\<lambda>_. P \<rbrace>"
  shows "distinct xs \<Longrightarrow> \<lbrace> (\<lambda>s. \<forall>t\<in>set xs. Q t s) and P\<rbrace> mapM_x f xs \<lbrace>\<lambda>_. P\<rbrace>"
...
lemma sched_context_donate_valid_sched:
  ...
  by (wpsimp wp: ssc_bound_tcb_at_cases hoare_drop_imp tcb_dequeue_not_queued_gen
...
lemma tcb_release_enqueue_valid_sched:
  ...
  apply (subst valid_sched_release_queue_cong[where queue'="thread#qs"])
  ...
  apply clarsimp
  apply (frule_tac a=aa and b=b and b'=ba and ys=r in distinct_zip_snd_unique)
  by (simp+)
...
lemma valid_sched_action_release_queue_cong:
  "set queue = set queue' \<Longrightarrow> distinct queue \<Longrightarrow> distinct queue'
   \<Longrightarrow> valid_sched_action_2 (scheduler_action s) (kheap s) (cur_thread s) (cur_domain s) queue
       = valid_sched_action_2 (scheduler_action s) (kheap s) (cur_thread s) (cur_domain s) queue'"
  by (clarsimp simp: valid_sched_def valid_release_q_def valid_sched_action_def
                     weak_valid_sched_action_def valid_blocked_def not_in_release_q_def)
...
lemma tcb_release_enqueue_valid_sched_except_blocked:
  ...
  apply (rule hoare_seq_ext[OF _ gets_sp])
  apply wpsimp
   apply (rule_tac Q="\<lambda>rv s. valid_sched_except_blocked_2 (ready_queues s) (scheduler_action s) (cur_domain s)
            (cur_time s) (kheap s) (cur_thread s) (idle_thread s) (thread # qs)
           \<and> length qs = length rv" in hoare_strengthen_post)
    apply (wpsimp wp: mapM_wp_inv_length cong: valid_sched_release_queue_cong')
   defer
  ...
  apply (intro conjI; simp?)
   apply (subst valid_release_q_release_queue_cong[where queue'="thread # qs"];
   ...
   apply (frule_tac a=aa and b=b and b'=ba and ys=r in distinct_zip_snd_unique)
     apply (simp+)
  apply (subst valid_sched_action_release_queue_cong[where queue'="thread # qs"];
           (solves \<open>simp\<close>)?)
   apply (subst tcb_release_enqueue_set_ident; simp)
  apply (subst distinct_shuffle_left)
  apply clarsimp
...
lemma tcb_release_enqueue_valid_release_q:
  ...
  apply (subgoal_tac "distinct (tcb_ptr # qs)")
   prefer 2
   apply (clarsimp simp: valid_release_q_def)
  apply (subst valid_release_q_release_queue_cong[where queue'="tcb_ptr # qs"];
         (solves \<open>simp\<close>)?)
   apply (subst tcb_release_enqueue_set_ident; simp)
  apply (subst distinct_shuffle_left)
  apply clarsimp
...
and active_sc_tcb_at target and not_in_release_q target and
     ...
     budget_sufficient target) s ))\<rbrace>
         mapM_x (\<lambda>t. do possible_switch_to t;
                     ...
                     od) queue \<lbrace>\<lambda>_. valid_sched::det_state \<Rightarrow> _\<rbrace>"
  apply (induct queue; clarsimp simp: mapM_append_single mapM_x_Nil mapM_x_Cons bind_assoc)
  by (wpsimp wp: possible_switch_to_weak_valid_sched hoare_vcg_ball_lift)
...
lemma strenthen_post_invs_valid_objs:
...
lemma refill_unblock_check_valid_release_q:
  "\<lbrace>valid_release_q\<rbrace> refill_unblock_check sc_ptr \<lbrace>\<lambda>rv. valid_release_q\<rbrace>"
  apply (clarsimp simp: valid_release_q_def)
  apply (wpsimp simp: Ball_def
...
lemma refill_unblock_check_valid_blocked_except_set:
  "\<lbrace>valid_blocked_except_set S\<rbrace>
   refill_unblock_check sc_ptr
...
lemma valid_objs_SendEP_distinct:
  "valid_objs s
   \<Longrightarrow> (kheap s epptr = Some (Endpoint (SendEP eplist)))
   \<Longrightarrow> distinct eplist"
...
and ct_in_cur_domain[wp]: "ct_in_cur_domain::det_state \<Rightarrow> _"
and valid_idle_etcb[wp]: "valid_idle_etcb::det_state \<Rightarrow> _"
  (wp: crunch_wps)
...
crunch scheduler_act[wp]: set_extra_badge, cap_insert "\<lambda>s :: det_state. P (scheduler_action s)"
  (wp: crunch_wps)
...
(* FIXME: probably need to remove valid_idle_etcb_lift from wp set *)
crunches do_ipc_transfer
...
(* fixme: perhaps replace the weaker version *)
lemma possible_switch_to_valid_blocked_stronger:
...
lemma send_ipc_valid_sched_subset_for_handle_timeout:
  ...
  apply (case_tac list; simp)
  apply (rename_tac dest tail)
  apply (wpsimp wp: possible_switch_to_valid_ready_qs possible_switch_to_valid_blocked_stronger)
      ...
      apply (wpsimp wp_del: valid_idle_etcb_lift wp: hoare_vcg_disj_lift)
     apply (wpsimp wp: )
    apply (wpsimp wp: gts_wp)
   apply clarsimp
...
(* FIXME: add pred_tcb_at_tcb_at to elim! set? *)
...
lemma refill_unblock_check_valid_ep_q[wp]:
  "\<lbrace>valid_ep_q\<rbrace> refill_unblock_check param_a \<lbrace>\<lambda>_. valid_ep_q\<rbrace>"
  by (wpsimp wp: valid_ep_q_lift hoare_vcg_disj_lift refill_unblock_check_active_sc_tcb_at)
...
crunch release_queue[wp]: reply_remove "\<lambda>s::det_state. not_in_release_q a s"
...
lemma do_reply_transfer_valid_sched:
  "\<lbrace>valid_sched and valid_ep_q and valid_reply_scs and
                invs and
  ...
  apply (wpsimp wp: possible_switch_to_valid_sched4)
    ...
    (* key transition point *)
      ...
      apply (wpsimp wp: )
          ...
          apply clarsimp
          apply (rule_tac Q="\<lambda>rv. (valid_sched)
                and (valid_ep_q and invs and  simple_sched_action and
    ...
    apply (rule_tac Q="\<lambda>rv. (valid_sched) and valid_ep_q and invs and simple_sched_action and
                           ...
                           not_queued a s \<and> (ct_active s \<or> ct_idle s) \<and>
                           not_in_release_q a s \<and>
...
lemma refill_unblock_check_has_budget:
  "\<lbrace>has_budget tptr and bound_sc_tcb_at ((=) (Some sc_ptr)) tptr\<rbrace>
   refill_unblock_check sc_ptr
...
lemma send_signal_WaitingNtfn_helper:
  "ntfn_obj ntfn = WaitingNtfn wnlist \<Longrightarrow>
   \<lbrace>ko_at (Notification ntfn) ntfnptr and
    ...
    valid_ntfn_q and valid_sched and invs\<rbrace>
...
lemma send_signal_valid_sched:
  ...
  apply (case_tac "ntfn_obj ntfn"; clarsimp)
    ...
    apply (case_tac st; clarsimp simp: receive_blocked_def)
           ...
           apply (clarsimp simp: pred_conj_def pred_tcb_at_eq_commute)
           apply (clarsimp simp: not_cur_thread_def ct_in_state_def pred_tcb_at_def obj_at_def)
          prefer 8
           apply (rename_tac queue)
...
lemma refill_unblock_check_valid_sched[wp]:
  "\<lbrace>valid_sched\<rbrace> refill_unblock_check a \<lbrace>\<lambda>_. valid_sched::det_state \<Rightarrow> _\<rbrace>"
  unfolding valid_sched_def
...
lemma budget_ready_cur_thread_lift:
  assumes A: "\<And>t. f \<lbrace>\<lambda>s. t \<noteq> (cur_thread s)\<rbrace>"
  assumes B: "\<And>t. f \<lbrace>\<lambda>s. budget_ready t s\<rbrace>"
  shows "f \<lbrace>\<lambda>s. budget_ready (cur_thread s) s\<rbrace>"
...
lemma sched_context_yield_to_valid_sched:
  "\<lbrace>valid_sched and simple_sched_action and (\<lambda>s. sc_tcb_sc_at (\<lambda>sctcb. \<exists>t. sctcb = Some t \<and> t \<noteq> cur_thread s) sc_ptr s) and
   ct_active and ct_schedulable\<rbrace>
  ...
  apply (wpsimp wp_del: tcb_sched_action_enqueue_valid_sched)
      ...
      apply (wpsimp wp: active_sc_tcb_at_cur_thread_lift budget_ready_cur_thread_lift budget_sufficient_cur_thread_lift)
     apply (wpsimp simp: get_sc_obj_ref_def)+
    apply (rule_tac Q="\<lambda>_ s. valid_sched s \<and> simple_sched_action s \<and>
     ...
     apply (clarsimp simp: sc_at_pred_n_def obj_at_def)
    apply (wpsimp wp: ct_schedulable_lift simp: get_sc_obj_ref_def)+
  apply (clarsimp simp: sc_at_pred_n_def obj_at_def)
  done

lemma invoke_sched_context_valid_sched:
  "\<lbrace>valid_sched and valid_sched_context_inv iv and invs and simple_sched_action and
...
lemma set_refills_valid_sched_action[wp]:
  "set_refills sc_ptr refills \<lbrace>valid_sched_action\<rbrace>"
  by (wpsimp wp: valid_sched_action_lift)

lemma set_refills_weak_valid_sched_action[wp]:
  "set_refills sc_ptr refills \<lbrace>weak_valid_sched_action\<rbrace>"
  by (wpsimp wp: weak_valid_sched_action_lift)
...
lemma set_refills_valid_ep_q:
  ...
  apply (drule_tac x=xb in spec, clarsimp)
  apply (drule_tac x=xb in spec, clarsimp)
...
lemma refill_update_valid_ready_qs:
  "\<lbrace>valid_ready_qs and K (MIN_BUDGET \<le> budget) and K (0 < mrefills)\<rbrace>
   refill_update sc_ptr period budget mrefills
   \<lbrace>\<lambda>rv. valid_ready_qs\<rbrace>"
  supply if_split [split del]
  unfolding refill_update_def maybe_add_empty_tail_def refill_add_tail_def
...
lemma refill_update_valid_release_q:
  "\<lbrace>valid_release_q and K (0 < mrefills)\<rbrace>
   refill_update sc_ptr period budget mrefills
   \<lbrace>\<lambda>rv. valid_release_q\<rbrace>"
  unfolding refill_update_def maybe_add_empty_tail_def refill_add_tail_def
  apply (wpsimp wp: is_round_robin_wp update_sched_context_wp set_refills_valid_release_q)
  apply (fastforce simp: valid_release_q_def active_sc_tcb_at_defs st_tcb_at_kh_def not_in_release_q_def
                  split: if_splits)
  done
...
lemma refill_new_valid_blocked_except_set:
  ...
  apply safe
  apply (clarsimp simp: active_sc_tcb_at_kh_def bound_sc_tcb_at_kh_def obj_at_kh_def
  test_sc_refill_max_kh_def st_tcb_at_kh_def
  active_sc_tcb_at_def pred_tcb_at_def obj_at_def test_sc_refill_max_def split: option.splits if_splits |
...
lemma commit_time_valid_sched:
  "\<lbrace>valid_sched and
    simple_sched_action\<rbrace>
   commit_time
   \<lbrace>\<lambda>_. valid_sched :: det_state \<Rightarrow> _\<rbrace>"
   unfolding valid_sched_def
   apply (wpsimp wp: commit_time_valid_release_q
...
lemma end_timeslice_valid_sched_subset:
  "\<lbrace>valid_release_q and valid_ready_qs and
    ...
    ct_not_in_release_q and ct_not_queued and schact_is_rct and invs and ct_active
  ...
  apply (subgoal_tac "bound_sc_tcb_at (\<lambda>x. x = Some (cur_sc s)) (cur_thread s) s")
   ...
   apply (intro conjI; intro impI allI; (clarsimp  dest!: get_tcb_SomeD))
...
(* FIXME: replace various other lemmas with this strong one *)
lemma update_sched_context_cur_sc_tcb_strong:
...
lemma check_budget_valid_sched_fallout:
    "ko_at (SchedContext sc n) ptr s \<Longrightarrow>
   (refill_list_sum (sc_refills sc) < (2 ^ LENGTH(64)))"
  "ko_at (SchedContext sc n) ptr s \<Longrightarrow>
   sc_period sc = 0 \<Longrightarrow>
   1 < length (sc_refills sc)"
 "\<lbrace>ct_not_in_release_q\<rbrace>
  ...
  "\<lbrace>schact_is_rct\<rbrace>
   handle_event e
   -, \<lbrace>\<lambda>rv. schact_is_rct\<rbrace>"
  "\<lbrace>ct_schedulable\<rbrace>
   handle_event e
   -, \<lbrace>\<lambda>rv. ct_schedulable\<rbrace>"
  sorry (* fallout from check_budget_valid_sched *)

lemmas bounded_refill_lists = check_budget_valid_sched_fallout(1-2)
lemmas handle_event_on_exception = check_budget_valid_sched_fallout(3-6)
...
lemma charge_budget_valid_sched:
  "\<lbrace>valid_sched and invs and ct_not_in_release_q and ct_not_queued and schact_is_rct and valid_ep_q
  ...
  apply (wpsimp wp: reschedule_required_valid_sched
      ...
      apply (rule_tac Q="\<lambda>ya s.
               (if (st_tcb_at runnable (cur_thread s) s)
      ...
      apply (wpsimp wp: hoare_vcg_if_lift_strong)
       ...
       apply (wpsimp wp: sc_consumed_update_valid_queues ct_not_in_release_q_lift
                         ...
                         update_sched_context_valid_blocked' update_sched_context_cur_sc_tcb_strong
     ...
     apply (wpsimp simp: Let_def)
       apply (wpsimp wp: hoare_vcg_if_lift_strong)
        ...
        apply (wpsimp wp: set_refills_valid_release_q set_refills_valid_ready_qs
                          set_refills_valid_ep_q
                          ct_not_in_release_q_lift ct_not_queued_lift
                          active_sc_tcb_at_cur_thread_lift
                          valid_blocked_lift_to_valid_blocked_except_set_empty
                          [OF set_refills_valid_blocked_except_set])
       apply (wpsimp simp: valid_sched_def
  ...
  apply (subgoal_tac "bound_sc_tcb_at (\<lambda>x. x = (Some (cur_sc s))) (cur_thread s) s")
   apply (subgoal_tac "scheduler_action s = resume_cur_thread \<and>
    ...
    apply (subgoal_tac "budget_sufficient tcb_ptr s \<and> budget_ready tcb_ptr s")
     ...
     apply (rule_tac b="refill_list_sum (sc_refills sc)" in dual_order.strict_trans2)
      apply (rule_tac n=n and ptr = "cur_sc s" and s=s in bounded_refill_lists(1))
      apply (clarsimp simp: obj_at_def)
     ...
     apply (rule_tac n=n and ptr = "cur_sc s" and s=s in bounded_refill_lists(2))
      apply (clarsimp simp: obj_at_def)
     apply (clarsimp simp: obj_at_def)
    apply (clarsimp simp: valid_ready_qs_def in_ready_q_def)
   ...
   apply ((subst sym_refs_bound_sc_tcb_iff_sc_tcb_sc_at[symmetric, OF refl refl]); clarsimp)
  apply (clarsimp simp: invs_def cur_sc_tcb_def schact_is_rct_def valid_state_def valid_pspace_def)
  apply (subst sym_refs_bound_sc_tcb_iff_sc_tcb_sc_at[OF refl refl], clarsimp)
  apply (clarsimp simp: sc_at_pred_n_def obj_at_def)
  done

(* FIXME: word_random = word_plus_mono_right2 in word lemmas*)

lemma check_budget_valid_sched:
  "\<lbrace>valid_sched and invs and ct_not_in_release_q and ct_not_queued
    and schact_is_rct and ct_schedulable\<rbrace>
   check_budget
   \<lbrace>\<lambda>_. valid_sched::det_state \<Rightarrow> _\<rbrace>"
...
lemma update_sc_badge_valid_sched:
  "\<lbrace>valid_sched and (\<lambda>s. \<exists>n. obj_at (\<lambda>x. x = SchedContext sc n) sc_ptr s)\<rbrace>
   ...
   by (wpsimp wp: valid_sched_lift_pre_conj set_sc_badge_active_sc_tcb_at set_sc_badge_budget_ready
                  set_sc_badge_budget_sufficient)
...
(* Fixme this should be crunchable *)
...
(* FIXME: this is strong and should replace other lemmas *)
...
(* fixme: ready_or_relaeased should be strucutre to make this trivial *)
...
lemma invoke_sched_control_configure_valid_sched:
  "\<lbrace>valid_sched and valid_sched_control_inv iv and schact_is_rct and ready_or_released and invs
    and ct_not_in_release_q and ct_schedulable and ct_not_queued\<rbrace>
  ...
  apply (wpsimp wp: reschedule_preserves_valid_sched)
         ...
         apply (rule_tac Q="\<lambda>yc s. sc_tcb_sc_at (\<lambda>a. a = Some tcb_ptr) sc_ptr s \<and>
                                   sc_ptr \<noteq> idle_sc_ptr \<and> invs s \<and>
       ...
       apply (rule_tac Q="\<lambda>yb. invs and sc_tcb_sc_at (\<lambda>a. a = Some tcb_ptr) sc_ptr and
        ...
        apply (intro conjI)
         ...
         apply (subgoal_tac "tcb_ptr = cur_thread s")
          apply (clarsimp simp: valid_blocked_except_set_cur_thread dest!: schact_is_rct_simple)
         apply (clarsimp simp: invs_def cur_sc_tcb_def sc_at_pred_n_def obj_at_def schact_is_rct_def)
        apply (clarsimp dest!: schact_is_rct_simple)
       apply (wpsimp wp: hoare_vcg_if_lift_strong hoare_vcg_all_lift hoare_vcg_imp_lift'
...
lemma perform_invocation_valid_sched:
  "\<lbrace>invs and valid_ntfn_q and valid_invocation i and ct_active and scheduler_act_sane and valid_sched
        and ready_or_released and valid_reply_scs
...
lemma handle_invocation_valid_sched:
  "\<lbrace>invs and valid_sched and valid_ntfn_q and ready_or_released and ct_active and valid_ep_q
    and ct_not_queued and ct_not_in_release_q and valid_reply_scs and
...
lemma check_budget_restart_valid_sched:
  "\<lbrace>valid_sched and invs and ct_not_in_release_q and ct_not_queued and schact_is_rct and ct_schedulable\<rbrace>
...
lemma handle_yield_valid_sched[wp]:
  "\<lbrace>valid_sched and invs and ct_not_in_release_q and ct_not_queued and
    cur_sc_tcb and schact_is_rct and ct_schedulable\<rbrace>
   handle_yield
   \<lbrace>\<lambda>rv. valid_sched::det_state \<Rightarrow> _\<rbrace>"
  unfolding handle_yield_def
  by (wpsimp simp: set_sc_obj_ref_def wp: charge_budget_valid_sched; erule valid_sched_implies_valid_ipc_qs)
...
lemma call_kernel_valid_sched_helper:
  "\<lbrace>\<lambda>s. (valid_sched and invs and
        ...
        invs s \<and> ct_not_in_release_q s \<and> ct_not_queued s \<and>
         schact_is_rct s \<and> ct_schedulable s\<rbrace>
   check_budget
  ...
  apply (wp schedule_valid_sched activate_thread_valid_sched handle_interrupt_valid_sched | simp)+
    ...
    apply (rule hoare_strengthen_post
      [where Q="\<lambda>irq s. irq \<notin> Some ` non_kernel_IRQs \<and> valid_sched s \<and> invs s
                        ...
                        \<and> schact_is_rct s \<and> ct_schedulable s"])
     apply (wpsimp wp: getActiveIRQ_neq_non_kernel)
     apply (auto simp: schact_is_rct_def)[2]
   apply (rule_tac Q="\<lambda>rv s. valid_sched s \<and> invs s" and
      E="\<lambda>rv s. (ct_not_in_release_q s \<and> ct_not_queued s \<and> schact_is_rct s \<and> ct_schedulable s) \<and> valid_sched s \<and> invs s" in hoare_post_impErr)
