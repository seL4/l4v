...
lemma set_thread_state_runnable_valid_sched_except_blocked:
  ...
  by (auto simp: released_ipc_queues_defs vs_all_heap_simps ipc_queued_thread_state_def
...
lemma set_thread_state_Inactive_not_queued_valid_sched_except_blocked:
  ...
  by (auto simp: valid_ready_qs_def valid_release_q_def released_ipc_queues_defs
...
lemma released_ipc_queuesE:
  assumes "released_ipc_queues_2 curtime tcb_sts tcb_scps sc_refill_cfgs"
  assumes "\<And>t. pred_map ipc_queued_thread_state tcb_sts' t
                ...
                \<longrightarrow> pred_map_eq None tcb_scps' t \<or> released_sc_tcb_at_pred curtime' tcb_scps' sc_refill_cfgs' t"
  shows "released_ipc_queues_2 curtime' tcb_sts' tcb_scps' sc_refill_cfgs'"
...
lemma set_thread_state_released_ipc_queues:
  "\<lbrace>released_ipc_queues
    and (\<lambda>s. ipc_queued_thread_state ts \<longrightarrow>
             released_if_bound_sc_thread_2 ref (cur_time s) (tcb_scps_of s) (sc_refill_cfgs_of s) )\<rbrace>
   set_thread_state ref ts
...
lemma set_tcb_sched_context_None_released_ipc_queues:
  "\<lbrace>released_ipc_queues\<rbrace>
   ...
   \<lbrace>\<lambda>_. released_ipc_queues\<rbrace>"
  apply (wpsimp wp: valid_sched_wp simp: released_ipc_queues_defs vs_all_heap_simps not_queued_def)
  by (auto)
...
lemma set_tcb_sched_context_valid_sched_Some:
  "\<lbrace>\<lambda>s. valid_sched s
        \<and> (scheduler_action s = switch_thread ref \<or> in_ready_q ref s \<or> ipc_queued_thread ref s
           \<longrightarrow> is_sc_active scp s \<and> is_refill_sufficient 0 scp s \<and> is_refill_ready scp s)
        \<and> (in_release_q ref s \<longrightarrow> is_sc_active scp s \<and> sc_ready_times_of s scp = tcb_ready_times_of s ref)
        \<and> (not_queued ref s \<and> not_in_release_q ref s \<and> is_sc_active scp s \<and> st_tcb_at runnable ref s
  ...
  by (intro conjI impI; clarsimp simp: vs_all_heap_simps fun_upd_def released_ipc_queues_defs)
...
lemma set_sc_is_sc_active_indep:
  "\<forall>sc. sc_active (f (\<lambda>_. tptr) sc) \<longleftrightarrow> sc_active sc
   \<Longrightarrow> set_sc_obj_ref f ref tptr \<lbrace>\<lambda>s. P (is_sc_active scp s)\<rbrace>"
  apply (wpsimp simp: set_sc_obj_ref_def update_sched_context_def set_object_def
                wp: get_object_wp)
  by (clarsimp simp: is_sc_active_def obj_at_def)

lemma set_sc_tcb_is_sc_active[wp]:
  "set_sc_obj_ref sc_tcb_update ref tptr \<lbrace>\<lambda>s. P (is_sc_active scp s)\<rbrace>"
...
lemmas set_sc_obj_ref_active_sc_tcb_at =
  bound_sc_obj_tcb_at_set_sc_obj_ref_idem[where P=active_scrc, simplified]

lemmas set_sc_obj_ref_budget_sufficient =
  bound_sc_obj_tcb_at_set_sc_obj_ref_idem[where P="refill_sufficient_sc 0", simplified]

lemmas set_sc_obj_ref_budget_ready =
  bound_sc_obj_tcb_at_cur_time_set_sc_obj_ref_idem[where P=refill_ready_sc, simplified]
...
lemma set_refills_released_ipc_queues:
  "\<lbrace>\<lambda>s. released_ipc_queues s \<and>
    (\<forall>tcb_ptr. pred_map_eq (Some sc_ptr) (tcb_scps_of s) tcb_ptr \<longrightarrow>
               ipc_queued_thread tcb_ptr s \<longrightarrow>
               refill_sufficient 0 (hd refills) \<and> refill_ready (cur_time s) (hd refills))\<rbrace>
   set_refills sc_ptr refills
   \<lbrace>\<lambda>rv. released_ipc_queues\<rbrace>"
  apply (wpsimp wp: valid_sched_wp)
  by (fastforce simp: released_ipc_queues_defs vs_all_heap_simps heap_upd_def)
...
lemma set_refills_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and
    (\<lambda>s. is_active_sc sc_ptr s \<longrightarrow> pred_map (\<lambda>sc. cfg_valid_refills (sc\<lparr>scrc_refills := refills\<rparr>)) (sc_refill_cfgs_of s) sc_ptr)\<rbrace>
   set_refills sc_ptr refills
...
lemma set_refills_valid_sched:
  "\<lbrace>\<lambda>s. valid_sched s
        ...
        \<and> (is_active_sc scp s \<longrightarrow> pred_map (\<lambda>sc. cfg_valid_refills (sc\<lparr>scrc_refills := refills\<rparr>)) (sc_refill_cfgs_of s) scp)\<rbrace>
   set_refills scp refills
   \<lbrace>\<lambda>_. valid_sched\<rbrace>"
...
lemma set_refills_valid_sched_not_in:
  "\<lbrace>valid_sched and sc_not_in_release_q sc_ptr and
    ...
    and (\<lambda>s. is_active_sc sc_ptr s \<longrightarrow> pred_map (\<lambda>sc. cfg_valid_refills (sc\<lparr>scrc_refills := refills\<rparr>)) (sc_refill_cfgs_of s) sc_ptr)\<rbrace>
   set_refills sc_ptr refills
   \<lbrace>\<lambda>_. valid_sched\<rbrace>"
  unfolding valid_sched_def
...
(* FIXME: this naming is inconsistent *)
...
abbreviation cur_sc_blocked_is_offset :: "time \<Rightarrow> 'z::state_ext state \<Rightarrow> bool" where
  "cur_sc_blocked_is_offset consumed s \<equiv>
      (\<forall>t. bound_sc_tcb_at ((=) (Some (cur_sc s))) t s
...
lemma activate_thread_released_ipc_queues[wp]:
  ...
  apply (wpsimp simp: activate_thread_def wp: valid_sched_wp gts_wp')
  by (clarsimp simp: released_ipc_queues_defs vs_all_heap_simps ipc_queued_thread_state_def)
...
lemma switch_to_idle_thread_valid_sched:
  "\<lbrace>valid_sched_action and valid_idle and valid_ready_qs and valid_release_q
    and valid_blocked and ct_in_q and valid_idle_etcb and released_ipc_queues and valid_machine_time
    and active_sc_valid_refills\<rbrace>
     switch_to_idle_thread
   \<lbrace>\<lambda>_. valid_sched :: 'state_ext state \<Rightarrow> _\<rbrace>"
  by (wpsimp simp: valid_sched_def)
...
(* FIXME: The proof of this lemma should be scrapped or improved when valid_ipc_q invariants
  ...
  apply (subgoal_tac "(\<exists>ptr. ep_at_pred (\<lambda>ep. t \<in> set (ep_queue ep)) ptr s) \<and> t \<noteq> sc_ptr")
   apply (intro conjI)
     apply (clarsimp simp: active_sc_tcb_at_kh_if_split is_sc_active_kh_if_split)
...
lemma possible_switch_to_weak_valid_sched_action[wp]:
  "\<lbrace>weak_valid_sched_action and st_tcb_at runnable target
    and (bound_sc_tcb_at (\<lambda>sc. sc = None) target or released_sc_tcb_at target)\<rbrace>
   possible_switch_to target
   \<lbrace>\<lambda>_. weak_valid_sched_action\<rbrace>"
...
lemma possible_switch_to_valid_sched_action[wp]:
  "\<lbrace>valid_sched_action and st_tcb_at runnable target and released_sc_tcb_at target\<rbrace>
  ...
  "\<lbrace>valid_sched_except_blocked and valid_blocked_except t
    ...
    and (\<lambda>s. runnable st \<longrightarrow> active_sc_tcb_at t s
           \<longrightarrow> (in_ready_q t s \<or> in_release_q t s \<or> t = cur_thread s
               \<or> scheduler_action s = switch_thread t))
    and (\<lambda>s. ipc_queued_thread_state st \<longrightarrow> released_if_bound_sc_thread t s)\<rbrace>
   set_thread_state t st
   \<lbrace>\<lambda>rv. valid_sched\<rbrace>"
  by (wpsimp simp: valid_sched_def
...
lemma restart_thread_if_no_fault_valid_sched:
  "\<lbrace>\<lambda>s. valid_sched s
        ...
        \<and> released_if_bound_sc_tcb_at t s \<and> t \<noteq> idle_thread s\<rbrace>
...
lemma cancel_all_ipc_loop_body_valid_sched:
  ...
  by (simp add: valid_sched_def released_ipc_queues_defs)
...
lemma set_thread_state_possible_switch_to_valid_sched:
  ...
  by (auto simp: valid_sched_def released_ipc_queues_defs vs_all_heap_simps)
...
lemma set_sc_refill_max_valid_sched_unbound_sc:
  ...
  apply (intro conjI)
    ...
    apply (fastforce elim!: valid_blockedE' simp: heap_upd_def vs_all_heap_simps active_sc_def split: if_splits)
   apply (fastforce simp: released_ipc_queues_defs heap_upd_def vs_all_heap_simps)
  by (clarsimp simp: active_sc_valid_refills_def heap_upd_def vs_all_heap_simps active_sc_def)
...
lemma sched_context_donate_valid_blocked:
  "\<lbrace>\<lambda>s. valid_blocked_except_set (insert t S) s
         \<and> (pred_map runnable (tcb_sts_of s) t \<and> is_active_sc scp s
             \<longrightarrow> t \<in> S \<or> t = cur_thread s)\<rbrace>
...
\<comment> \<open>When sched_context_donate is called from reply_remove, we are not always able to ensure that
    ...
    released_ipc_queued_thread in that context.\<close>
...
lemma sched_context_donate_released_sc_released_ipc_queues:
  "\<lbrace>\<lambda>s. released_ipc_queues s \<and> (ipc_queued_thread t s \<longrightarrow> released_sc_at scp s)\<rbrace>
   sched_context_donate scp t
   \<lbrace>\<lambda>rv. released_ipc_queues\<rbrace>"
  apply (wpsimp wp: set_tcb_sched_context_valid_sched_pred get_sc_obj_ref_wp
              simp: sched_context_donate_def)
...
lemma sched_context_donate_valid_sched:
  "\<lbrace>\<lambda>s. valid_sched s \<and> pred_map_eq None (tcb_scps_of s) t
         ...
         \<and> (ipc_queued_thread t s \<longrightarrow> released_sc_at scp s)\<rbrace>
   sched_context_donate scp t
   \<lbrace>\<lambda>rv. valid_sched\<rbrace>"
...
lemma released_ipc_queues_set_scp_None:
  assumes "released_ipc_queues_2 ctime tcb_sts tcb_scps sc_refill_cfgs"
  shows "released_ipc_queues_2 ctime tcb_sts (tcb_scps(ref \<mapsto> None)) sc_refill_cfgs"
...
lemma set_sc_replies_update_is_sc_active[wp]:
  "set_sc_obj_ref sc_replies_update scp replies \<lbrace>\<lambda>s. P (is_sc_active t s)\<rbrace>"
  apply (clarsimp simp: set_sc_obj_ref_def update_sched_context_def)
  apply (wpsimp simp: set_object_def wp: get_object_wp)
  by (auto simp: obj_at_def is_sc_active_def)

lemma update_sc_replies_update_is_sc_active[wp]:
  "update_sched_context scp (sc_replies_update f) \<lbrace>\<lambda>s. P (is_sc_active t s)\<rbrace>"
...
lemma released_ipc_queues_except_strengthen:
  "released_ipc_queues_2 ctime sts scps refill_cfgs
   \<Longrightarrow> released_ipc_queues_2 ctime (sts(tp \<mapsto> Inactive)) scps refill_cfgs"
  apply (erule released_ipc_queuesE)
...
lemma reply_remove_valid_sched:
  ...
  apply wp
     ...
     apply (wpsimp wp: sched_context_donate_valid_sched_action sched_context_donate_valid_blocked
       ...
       apply wpsimp+
  apply (clarsimp simp: valid_sched_def elim!: released_ipc_queues_except_strengthen)
  done
...
lemma sched_context_resume_valid_sched:
  ...
  apply (rule_tac V="\<not> released_sc_tcb_at t s" in revcut_rl
...
lemma sched_context_resume_valid_sched_except_blocked: (* we could use invs *)
  ...
  apply (wpsimp wp: postpone_valid_sched_except_blocked is_schedulable_wp' thread_get_wp'
              simp: sched_context_resume_def)
  apply (clarsimp simp: valid_sched_def sc_at_pred_n_def get_tcb_ko_at obj_at_def)
  apply (rule_tac V="\<not> released_sc_tcb_at tp s" in revcut_rl
         , clarsimp simp: vs_all_heap_simps)
  by (auto elim!: weak_valid_sched_action_contrap[OF valid_sched_action_weak_valid_sched_action]
                  valid_ready_qs_contrap)
...
lemma set_priority_valid_sched:
  "\<lbrace>valid_sched and ct_released and ct_active\<rbrace>
  ...
  apply (intro conjI; intro impI)
   ...
   apply (intro conjI; intro impI)
     apply (clarsimp simp: valid_ready_qs_def valid_blocked_thread_def tcb_at_kh_simps released_sc_tcb_at_def)
...
lemma sched_context_resume_cond_released_if_bound_sc_thread:
...
(*   apply wpsimp
  ...
  apply safe
     apply (clarsimp simp: released_if_bound_sc_thread_equiv2 st_tcb_at_def obj_at_def
                    dest!: is_schedulable_opt_Some get_tcb_SomeD)
    apply (clarsimp simp: released_if_bound_sc_thread_equiv2 st_tcb_at_def obj_at_def pred_tcb_at_def active_sc_tcb_at_def
                          is_sc_active_def
                   dest!: is_schedulable_opt_Some get_tcb_SomeD)
   apply (simp only: released_if_bound_sc_thread_equiv2 pred_tcb_at_eq_commute)
   ...
   apply (clarsimp simp: is_refill_ready_def pred_tcb_at_def obj_at_def
                  dest!: get_tcb_SomeD)
  apply (clarsimp simp: released_if_bound_sc_thread_equiv2 sc_at_pred_n_def obj_at_def
                 dest!: is_schedulable_opt_Some get_tcb_SomeD)
...
lemma set_tcb_sched_context_released_ipc_queues:
  "\<lbrace>\<lambda>s. released_ipc_queues s
        \<and> (\<forall>sc_ptr. sc = Some sc_ptr \<longrightarrow> ipc_queued_thread ref s
                    \<longrightarrow> released_sc_at sc_ptr s)\<rbrace>
   set_tcb_obj_ref tcb_sched_context_update ref sc
   \<lbrace>\<lambda>_. released_ipc_queues\<rbrace>"
  apply (wpsimp wp: valid_sched_wp simp: released_ipc_queues_defs vs_all_heap_simps)
  by (rule ccontr) auto

lemma sched_context_bind_tcb_valid_sched:
  "\<lbrace>valid_sched and simple_sched_action
    and bound_sc_tcb_at ((=) None) tcbptr
    and not_cur_thread tcbptr
    and (\<lambda>s. ipc_queued_thread tcbptr s \<longrightarrow> released_sc_at scptr s)\<rbrace>
  ...
  apply (wpsimp wp: is_schedulable_wp reschedule_valid_sched_const)
     apply (rule_tac Q="\<lambda>r s. valid_sched_except_blocked s
      ...
      apply (intro allI conjI;
             intro impI;
             clarsimp simp: valid_sched_def is_schedulable_bool_def2)
...
lemma maybe_sched_context_bind_tcb_valid_sched:
  "\<lbrace>valid_sched and simple_sched_action and bound_sc_tcb_at ((=) None) tcbptr and
    not_cur_thread tcbptr and (\<lambda>s. ipc_queued_thread tcbptr s \<longrightarrow> released_sc_at scptr s)\<rbrace>
...
lemma install_tcb_cap_ct_active:
  "\<lbrace>invs and tcb_at target and ct_active and released_ipc_queues\<rbrace>
   install_tcb_cap target slot 3 slot_opt
...
lemma tcs_valid_sched:
  ...
  apply (wp maybeM_wp_drop_None)
   ...
   apply ((wpsimp wp: hoare_vcg_disj_lift hoare_vcg_all_lift install_tcb_cap_ct_active
                      install_tcb_cap_bound_sc_tcb_at static_imp_wp hoare_vcg_imp_lift
...
lemma refill_budget_check_valid_refills[wp]:
   "\<lbrace>(\<lambda>s. \<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n) \<longrightarrow>
    unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat usage
...
lemma power_two_max_word_fold:
  "2 ^ LENGTH(64) = Suc (unat (max_word :: 64 word))"
...
method overflow_hammer = ((assumption
...
(* The first new precondition is needed to ensure that the new window
   does not include max_word. *)
(* The second new precondition is needed to ensure that the second refill in the last case
   has at least MIN_BUDGET. It should be enough to show (sc_budget sc) - MINBUDGET + usage does
   not overflow but it is simpler to do it this way. *)
(* The second precondition is also necessary because MAX_SC_PERIOD bounds MINBUDGET
   and if usage + MINBUDGET overflows then the comparison in the first if statement doesn't
   make any sense. *)
lemma refill_budget_check_round_robin_valid_refills[wp]:
   "\<lbrace>(\<lambda>s. \<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n) \<longrightarrow>
             unat (cur_time s) + unat (MAX_SC_PERIOD) \<le> unat (max_word::time)
          \<and> unat usage + unat (MAX_SC_PERIOD) \<le> unat (max_word::time)))
     and valid_refills p\<rbrace>
  ...
  apply (rule hoare_if)
   ...
   apply (clarsimp simp: vs_all_heap_simps valid_refills_def obj_at_def MIN_REFILLS_def split: if_splits)
   apply (prop_tac "unat (r_amount h) + unat (r_amount t) \<le> unat (max_word :: time)")
...
(*
  ...
  apply (wpsimp simp: set_refills_def update_sched_context_def set_object_def get_object_def
                      valid_refills_def vs_all_heap_simps obj_at_def)
  apply (rename_tac sc' s n')
...
lemma commit_time_valid_refills[wp]:
  "\<lbrace>(\<lambda>s. \<forall>sc n. kheap s (cur_sc s) = Some (SchedContext sc n)
          \<and> unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat (consumed_time s)
                     \<le> unat (max_word :: time)
          \<and> unat (cur_time s) + unat (MAX_SC_PERIOD) \<le> unat (max_word :: time)
          \<and> unat (consumed_time s) + unat (MAX_SC_PERIOD) \<le> unat (max_word :: time)) and
    (\<lambda>s. valid_refills ptr s)\<rbrace>
   commit_time
   \<lbrace>\<lambda>_ s. valid_refills ptr s\<rbrace>"
  ...
  apply wpsimp
  apply (case_tac "sc_active sc"; simp)
   apply (case_tac "0 < consumed"; simp)
    ...
    apply (intro conjI impI)
     ...
     apply (wpsimp wp: refill_budget_check_round_robin_valid_refills
                       hoare_drop_imp)
    \<comment> \<open>\<not>is_round_robin\<close>
    apply (wpsimp simp: set_object_def sc_valid_refills_def window_def
                 wp: get_object_wp update_sc_consumed_valid_refills hoare_drop_imps
                        ...
                        refill_budget_check_valid_refills
...
lemma thread_set_valid_refills[wp]:
  ...
  apply (clarsimp simp: obj_at_def vs_all_heap_simps dest!:get_tcb_SomeD)
  done
...
lemma charge_budget_valid_refills:
  "\<lbrace>(\<lambda>s. \<forall>sc n. kheap s (cur_sc s) = Some (SchedContext sc n)
          \<longrightarrow> unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat consumed
                     \<le> unat (max_word :: time)
          \<and> unat (cur_time s) + unat (MAX_SC_PERIOD) \<le> unat (max_word::time))
     and (\<lambda>_. unat consumed + unat MAX_SC_PERIOD \<le> unat (max_word::time))
  ...
  apply (rule hoare_seq_ext[OF _ get_sched_context_sp])
  apply (rule hoare_seq_ext[OF _ get_sched_context_sp])
  apply (rule_tac B="\<lambda>_. valid_refills scptr" in hoare_seq_ext[rotated], wpsimp)
  apply (rule_tac B="\<lambda>_. valid_refills scptr" in hoare_seq_ext[rotated], wpsimp)
  ...
  apply (rule_tac B="\<lambda>_ s. is_active_sc csc s
                           \<and> (\<forall>t. ipc_queued_thread t s
                                   \<longrightarrow> pred_map_eq (Some csc) (tcb_scps_of s) t
                                       \<or> released_if_bound_sc_tcb_at t s)"
           in hoare_seq_ext)
   apply wpsimp
...
lemma refill_budget_check_round_robinsdfsf_valid_refills[wp]:
   "\<lbrace>(\<lambda>s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp)\<rbrace>
    refill_budget_check_round_robin usage
    \<lbrace>\<lambda>_ s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>"
  supply if_split [split del]
  unfolding refill_budget_check_round_robin_def
  by (wpsimp wp: valid_sched_wp hoare_drop_imp)

lemma refill_budget_check_round_robinsdfsf_14valid_refills[wp]:
   "\<lbrace>(\<lambda>s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp)\<rbrace>
    refill_budget_check usage
    \<lbrace>\<lambda>_ s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>"
  supply if_split [split del]
...
lemma commit_time_valid_re41fills[wp]:
  "\<lbrace>(\<lambda>s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp)\<rbrace>
   commit_time
   \<lbrace>\<lambda>_ s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>"
...
thm refill_budget_check_round_robin_valid_refills
(* See refill_budget_check_round_robin_valid_refills *)
lemma refill_budget_check_round_robin_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and (\<lambda>s.
             unat (cur_time s) + unat (MAX_SC_PERIOD) \<le> unat (max_word::time)
          \<and> unat consumed + unat (MAX_SC_PERIOD) \<le> unat (max_word::time))\<rbrace>
...
lemma refill_budget_check_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and (\<lambda>s. \<forall>sc n. kheap s (cur_sc s) = Some (SchedContext sc n) \<longrightarrow>
             unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat consumed \<le> unat (max_word::time))\<rbrace>
...
lemma active_sc_valid_refillsE:
  "pred_map cfg_active (sc_refill_cfgs_of s) scp \<Longrightarrow> active_sc_valid_refills s \<Longrightarrow> valid_refills scp s"
...
lemma commit_time_active_sc_valid_refills:
  ...
  apply (subgoal_tac "sc_not_in_release_q (cur_sc s) s", simp)
   ...
   apply (rule_tac y="unat (cur_time s) + 2 * unat MAX_SC_PERIOD + unat kernelWCET_ticks" in order_trans[rotated], simp)
   apply (rule_tac y="unat (cur_time s) + unat kernelWCET_ticks + 2 * unat MAX_SC_PERIOD" in order_trans[rotated], simp)
   apply (rule_tac y="unat (r_time (refill_hd scb)) + unat (consumed_time s) + 2 * unat (sc_period scb)" in order_trans, simp)
   apply (rule add_le_mono)
    apply (overflow_hammer)
   apply simp
   apply (clarsimp simp: word_le_nat_alt[symmetric])
   apply (subgoal_tac "valid_refills (cur_sc s) s")
    apply (subgoal_tac "sc_valid_refills scb")
     apply (clarsimp simp: sc_valid_refills_def)
    apply (clarsimp simp: valid_refills_def2 vs_all_heap_simps)
   apply (erule active_sc_valid_refillsE[rotated])
   apply (clarsimp simp: vs_all_heap_simps active_sc_def obj_at_def)
  apply (clarsimp)
  done
...
lemma ruc_not_pred_map_sc_active[wp]:
  "refill_unblock_check sc_ptr \<lbrace>\<lambda>s. \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>"
  unfolding refill_unblock_check_def
  by (wpsimp wp: valid_sched_wp hoare_drop_imp)
...
lemma valid_refills_unat_sum_list_helper_helper:
  "ordered_disjoint (list :: refill list) \<Longrightarrow>
   ...
   (unat (r_time (hd list)) + unat (sum_list (map r_amount (list))) \<le> unat (max_word :: time))"
...
lemma valid_refills_unat_sum_list_helper:
  "ordered_disjoint (list :: refill list) \<Longrightarrow>
   no_overflow list \<Longrightarrow> list \<noteq> [] \<Longrightarrow>
   unat (r_amount (hd list)) + unat (sum_list (map r_amount (tl list))) \<le> unat (max_word :: time)"
  unfolding refills_sum_def
  apply (induction list; simp)
  apply (case_tac list; simp)
   apply (clarsimp simp: ordered_disjoint_def no_overflow_def)
...
lemma ruc_cur_sc_offset_ready_2:
  "\<lbrace>(\<lambda>s'. cur_sc_offset_ready_2 a b c (kheap s')) and (\<lambda>s. x \<noteq> c)\<rbrace>
   refill_unblock_check x
   \<lbrace>\<lambda>_ s. cur_sc_offset_ready_2 a b c (kheap s)\<rbrace>"
  unfolding refill_unblock_check_def
  apply (wpsimp wp: set_refills_wp hoare_drop_imp hoare_vcg_if_lift2 get_refills_wp)
  by (clarsimp simp: cur_sc_offset_ready_2_def)
...
lemma ruc_cur_sc_offset_ready_indep[wp]:
  "\<lbrace>(\<lambda>s. a \<noteq> cur_sc s)
   ...
   \<lbrace>\<lambda>_ s. (sc_not_in_release_q (cur_sc s) s \<longrightarrow>
                 cur_sc_offset_ready (consumed_time s) s)\<rbrace>"
  by (wpsimp wp: hoare_vcg_imp_lift' ruc_cur_sc_offset_ready_2 | wps)+
...
lemma schedule_valid_sched_helper:
  "\<lbrace> valid_sched and ct_active and ready_or_release
     and (\<lambda>s. active_sc_tcb_at (cur_thread s) s)
...
lemma awaken_ct_not_queued:
  "\<lbrace>\<lambda>s. ct_not_queued s \<and> scheduler_act_sane s \<and>
    (\<exists>scp. bound_sc_tcb_at (\<lambda>x. x = (Some scp)) (cur_thread s) s \<and> sc_refills_sc_at (\<lambda>x. x\<noteq>[]) scp s) \<and>
    (in_release_q (cur_thread s) s \<longrightarrow> \<not> budget_ready (cur_thread s) s)\<rbrace>
   awaken
   \<lbrace>\<lambda>_. ct_not_queued\<rbrace>"
  apply (clarsimp simp: awaken_def)
  apply (rule hoare_seq_ext[OF _ gets_sp])
...
*)
lemma schedule_valid_sched:
  "\<lbrace> ct_active and ready_or_release and
...
(*   apply (rule conjI, clarsimp simp: in_queue_2_def)
  ...
  apply (drule (1) invs_non_empty_refills[unfolded obj_at_def, simplified, rotated])+
  apply simp
  done *)
...
lemma invoke_tcb_valid_sched:
  "\<lbrace>invs
    and valid_sched and ct_active and ct_released
    and simple_sched_action
    and (\<lambda>s. bound_sc_tcb_at bound (cur_thread s) s)
...
lemma maybe_donate_sc_valid_ready_qs:
  "\<lbrace> valid_machine_time and active_sc_valid_refills and valid_ready_qs and scheduler_act_not tcb_ptr and not_queued tcb_ptr \<rbrace>
     maybe_donate_sc tcb_ptr ntfnptr
   \<lbrace> \<lambda>_. valid_ready_qs \<rbrace>"
...
lemma cancel_badged_sends_filterM_valid_sched:
  ...
  apply (rule hoare_gen_asm, rule ball_filterM_scheme)
   ...
   apply (clarsimp simp: valid_sched_def released_ipc_queues_defs)
  apply simp
  done
...
lemma sched_context_donate_active_sc_tcb_at_donate_helper:
  ...
  \<lbrace>\<lambda>s. bound_sc_tcb_at ((=) None) tcb_ptr s \<and>
            is_sc_active sc_ptr s \<and> (\<exists>n. ko_at (SchedContext sc n) sc_ptr s)\<rbrace>
     do y <- tcb_sched_action tcb_sched_dequeue a;
        y <- tcb_release_remove a;
        y <- set_tcb_obj_ref tcb_sched_context_update a None;
        test_reschedule a
     od
  \<lbrace>\<lambda>_.  bound_sc_tcb_at ((=) None) tcb_ptr and is_sc_active sc_ptr\<rbrace>"
...
lemma sched_context_donate_active_sc_tcb_at_donate:
  "\<lbrace> bound_sc_tcb_at ((=) None) tcb_ptr and is_sc_active sc_ptr\<rbrace>
  ...
  apply (rule hoare_seq_ext[OF _ get_sched_context_sp])
  apply (rule_tac B="\<lambda>_. bound_sc_tcb_at ((=) None) tcb_ptr and is_sc_active sc_ptr" in hoare_seq_ext)
   ...
   apply (clarsimp simp: is_sc_active_kh_simp tcb_at_kh_simps obj_at_def vs_all_heap_simps)
...
lemma sched_context_donate_released_sc_tcb_at:
  "\<lbrace> tcb_at tcb_ptr
     and is_sc_active sc_ptr
     and is_refill_ready sc_ptr
     and is_refill_sufficient 0 sc_ptr\<rbrace>
      sched_context_donate sc_ptr tcb_ptr
   \<lbrace>\<lambda>_. released_sc_tcb_at tcb_ptr\<rbrace>"
  apply (clarsimp simp: sched_context_donate_def get_sc_obj_ref_def assert_opt_def)
  apply (rule hoare_seq_ext[OF _ get_sched_context_sp])
  apply (rule_tac B="\<lambda>_. tcb_at tcb_ptr and is_sc_active sc_ptr and is_refill_ready sc_ptr
     and is_refill_sufficient 0 sc_ptr" in hoare_seq_ext)
   apply (wpsimp simp: set_tcb_obj_ref_def set_object_def
                       update_sched_context_def set_sc_obj_ref_def
                   wp: get_object_wp hoare_drop_imp)
   apply (clarsimp simp: is_sc_active_kh_simp tcb_at_kh_simps obj_at_def vs_all_heap_simps is_tcb)
...
lemma reply_push_released_ipc_queues[wp]:
  "\<lbrace>released_ipc_queues and released_if_bound_sc_thread caller\<rbrace>
   reply_push caller callee reply_ptr can_donate
   \<lbrace>\<lambda>rv. released_ipc_queues\<rbrace>"
  supply if_split [split del]
  ...
  apply (rule hoare_seq_ext[OF _ gsc_sp])
  apply (rule hoare_seq_ext[OF _ gsc_sp])
  apply (wpsimp wp: sched_context_donate_released_sc_released_ipc_queues get_simple_ko_wp)
          apply (rule_tac Q="\<lambda>_. released_ipc_queues and released_if_bound_sc_thread caller
                                 and bound_sc_tcb_at ((=) (sc_caller)) caller"
                 in hoare_strengthen_post[rotated])
           apply (clarsimp simp: obj_at_kh_kheap_simps vs_all_heap_simps split: if_splits)
  by (wpsimp wp: set_thread_state_released_ipc_queues hoare_drop_imp)+
...
lemma send_ipc_valid_sched_helper0:
  "\<lbrace>valid_sched
    and released_if_bound_sc_thread dest
...
lemma send_ipc_valid_sched_helper_no_reply:
  "\<lbrace>valid_sched and ipc_queued_thread dest and st_tcb_at active tptr
    ...
    and released_if_bound_sc_thread dest
    and released_if_bound_sc_thread tptr
  ...
  apply (wpsimp wp: sched_context_donate_valid_sched sched_context_donate_bound_sc_tcb_at_None
                    sched_context_donate_released_sc_tcb_at hoare_vcg_disj_lift
                    get_tcb_obj_ref_wp)
  apply (clarsimp simp: tcb_at_kh_simps obj_at_def vs_all_heap_simps is_sc_active_kh_simp is_tcb)
  by (case_tac "tcb_state y"; simp)

lemma reply_push_active_sc_tcb_at_helper:
  "\<lbrace> (\<lambda>s. donate \<longrightarrow> (is_sc_active sc_ptr and is_refill_ready sc_ptr and is_refill_sufficient 0 sc_ptr) s)
...
lemma reply_push_released_if_bound_sc_thread_no_donation:
  "\<lbrace>released_if_bound_sc_thread callee\<rbrace>
     reply_push caller callee' reply_ptr False
   \<lbrace>\<lambda>rv. released_if_bound_sc_thread callee\<rbrace>"
  supply if_weak_cong[cong del]
  apply (simp add: reply_push_def)
  apply (rule hoare_seq_ext[OF _ gsc_sp])
  apply (rule hoare_seq_ext[OF _ gsc_sp])
...
lemma sched_context_donate_released_if_bound:
  "\<lbrace>\<lambda>s. bound_sc_tcb_at ((=) None) callee s \<and> is_sc_active y s \<and> is_refill_ready y s \<and> is_refill_sufficient 0 y s\<rbrace>
   sched_context_donate y callee
   \<lbrace>\<lambda>r. released_if_bound_sc_thread callee\<rbrace>"
...
lemma reply_push_released_if_bound:
  "\<lbrace>released_if_bound_sc_thread callee and released_if_bound_sc_thread caller\<rbrace>
   reply_push caller callee reply_ptr can_donate
   \<lbrace>\<lambda>r. released_if_bound_sc_thread callee\<rbrace>"
  supply if_split[split del]
  apply (simp add: reply_push_def)
  ...
  apply wpsimp
    ...
    apply (rule_tac Q= "\<lambda>_. bound_sc_tcb_at ((=) sc_callee) callee and bound_sc_tcb_at ((=) sc_caller) caller and released_if_bound_sc_thread callee and released_if_bound_sc_thread caller" in hoare_strengthen_post[rotated])
     apply (clarsimp simp: obj_at_def pred_tcb_at_eq_commute split: if_split)
     apply (intro conjI; clarsimp simp: tcb_at_kh_simps vs_all_heap_simps is_sc_active_kh_simp)
    apply wpsimp+
  done
...
lemma send_ipc_valid_sched_helper_some_reply:
  ...
  apply (case_tac "call \<or> (\<exists>y. fault = Some y)"; simp split del: if_split)
   apply wpsimp
     apply (wpsimp wp: reply_push_valid_sched reply_push_released_if_bound)
    apply wpsimp
    apply (wpsimp wp: set_thread_state_valid_sched )
   apply (clarsimp simp: valid_sched_def pred_map_eq_def)
  apply (wpsimp wp: sched_context_donate_valid_sched hoare_vcg_disj_lift
                    sched_context_donate_bound_sc_tcb_at_None sched_context_donate_released_sc_tcb_at
                    get_tcb_obj_ref_wp)
  by (clarsimp simp: tcb_at_kh_simps obj_at_def vs_all_heap_simps is_sc_active_kh_simp is_tcb)
...
lemma send_ipc_valid_sched:
  "\<lbrace>valid_sched and scheduler_act_not thread and not_queued thread
    ...
    and released_if_bound_sc_thread thread
  ...
  apply (intro conjI allI impI)
   apply (erule (1) released_ipc_queuesE1[OF valid_sched_released_ipc_queues])
...
lemma send_fault_ipc_valid_sched[wp]:
  "\<lbrace>valid_sched and st_tcb_at active tptr and scheduler_act_not tptr
     and not_queued tptr and ct_in_state activatable and invs and (\<lambda>_. valid_fault fault)
...
lemma reply_push_valid_sched_no_donation:
  "\<lbrace> valid_sched_except_blocked and valid_blocked_except thread and not_in_release_q thread and
     scheduler_act_not thread and not_queued thread and released_if_bound_sc_thread thread\<rbrace>
...
lemma send_ipc_valid_release_q_for_handle_timeout:
  ...
  apply (rename_tac dest tail)
  apply (wpsimp wp: set_thread_state_valid_release_q)
           apply (wpsimp wp: hoare_drop_imp hoare_vcg_all_lift set_thread_state_valid_release_q)+
          apply (clarsimp cong: conj_cong)
...
lemma send_ipc_valid_blocked_for_handle_timeout:
  ...
  apply wpsimp
          ...
          apply (rule_tac Q="\<lambda>_. valid_blocked_except_set S" in hoare_strengthen_post[rotated])
           apply (clarsimp simp: obj_at_def is_tcb pred_tcb_at_def released_sc_tcb_at_def vs_all_heap_simps)
          apply wpsimp
         ...
         apply (wpsimp simp: thread_get_def)
        apply (wpsimp wp: get_tcb_obj_ref_wp)
       apply (rule_tac Q="\<lambda>r. valid_blocked_except_set S" in hoare_strengthen_post[rotated])
        apply clarsimp
...
lemma send_ipc_valid_ready_qs_for_handle_timeout:
  "\<lbrace>valid_ready_qs and weak_valid_sched_action and released_ipc_queues and not_queued thread\<rbrace>
   send_ipc True False badge True False thread epptr
  ...
  apply wpsimp
            ...
            apply wpsimp
           apply (wpsimp wp: get_tcb_obj_ref_wp)
          apply (rule_tac Q="\<lambda>_. valid_ready_qs and released_if_bound_sc_thread dest"
                 in hoare_strengthen_post[rotated])
           apply (clarsimp simp: obj_at_def is_tcb pred_tcb_at_def released_sc_tcb_at_def vs_all_heap_simps)
          apply wpsimp
            apply (wpsimp wp: reply_push_typ_at tcb_at_typ_at reply_push_released_if_bound_sc_thread_no_donation)
       ...
       apply (rule_tac Q="\<lambda>r. valid_ready_qs and not_queued thread and not_queued dest and
                              scheduler_act_not dest and released_if_bound_sc_thread dest"
              in hoare_strengthen_post[rotated])
        apply clarsimp
    ...
    apply (wpsimp wp: gts_wp)
   apply clarsimp
   apply (rule_tac Q="\<lambda>r. valid_ready_qs and weak_valid_sched_action and released_ipc_queues and not_queued thread"
          in hoare_strengthen_post[rotated])
    apply (clarsimp simp: obj_at_def is_tcb pred_tcb_at_def)
    apply (intro conjI)
    ...
    apply (subgoal_tac "ipc_queued_thread dest s")
     apply (clarsimp simp: released_ipc_queues_defs)
    apply (clarsimp simp: vs_all_heap_simps)
   apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift' hoare_vcg_disj_lift)
  apply (clarsimp simp: obj_at_def)
  done

lemma send_ipc_released_ipc_queues_for_handle_timeout:
  "\<lbrace>released_ipc_queues and released_if_bound_sc_thread thread\<rbrace>
   send_ipc True False badge True False thread epptr
  ...
  apply (wpsimp wp: set_thread_state_released_ipc_queues)
           apply (wpsimp wp: get_tcb_obj_ref_wp)
          apply (rule_tac Q="\<lambda>_. released_ipc_queues"
                 in hoare_strengthen_post[rotated])
           apply (clarsimp simp: obj_at_def is_tcb pred_tcb_at_def released_sc_tcb_at_def vs_all_heap_simps)
          apply wpsimp
           apply (wpsimp wp: set_thread_state_released_ipc_queues)
         ...
         apply (wpsimp simp: thread_get_def)
        apply (wpsimp wp: get_tcb_obj_ref_wp)
       apply (rule_tac Q="\<lambda>r. released_ipc_queues and released_if_bound_sc_thread dest
                              and released_if_bound_sc_thread thread"
              in hoare_strengthen_post[rotated])
        apply clarsimp
       apply (wpsimp wp: reply_unlink_tcb_released_ipc_queues)
      apply (wpsimp wp: )
     apply (wpsimp wp: )
    apply (wpsimp wp: gts_wp)
   apply clarsimp
   apply (rule_tac Q="\<lambda>r. released_ipc_queues and released_if_bound_sc_thread thread"
          in hoare_strengthen_post[rotated])
    apply (clarsimp simp: )
    apply (subgoal_tac "released_if_bound_sc_thread dest s")
     apply (clarsimp simp: )
     apply (clarsimp simp: released_ipc_queues_defs pred_map_simps)
    apply (erule released_ipc_queuesE1)
    apply (clarsimp simp: tcb_at_kh_simps pred_map_def)
...
lemma send_ipc_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills\<rbrace>
   send_ipc e y badge h g thread epptr
   \<lbrace>\<lambda>rv. active_sc_valid_refills ::'state_ext state \<Rightarrow> _\<rbrace>"
  supply if_weak_cong[cong del]
  apply (simp add: send_ipc_def)
  apply (rule hoare_seq_ext [OF _ get_simple_ko_sp])
  apply (case_tac ep, simp_all)
  ...
  apply (case_tac list; simp)
  apply (rename_tac dest tail)
  apply (wpsimp wp: set_thread_state_released_ipc_queues hoare_drop_imp hoare_vcg_all_lift)+
  done
...
lemmas send_ipc_valid_sched_subset_for_handle_timeout = send_ipc_valid_ready_qs_for_handle_timeout
                                                        ...
                                                        send_ipc_released_ipc_queues_for_handle_timeout
                                                        send_ipc_active_sc_valid_refills
...
lemma send_ipc_valid_sched_for_handle_timeout:
  ...
  apply (wpsimp wp: send_ipc_valid_sched_helper0)
            apply (wpsimp wp: reply_push_valid_sched_no_donation reply_push_released_if_bound_sc_thread_no_donation)
           apply (wpsimp wp: set_thread_state_valid_sched)
           apply (wpsimp wp: hoare_vcg_disj_lift)
          apply (wpsimp wp: thread_get_wp)+
       apply (rule_tac Q="\<lambda>r. (valid_sched_except_blocked and
                               ...
                               released_if_bound_sc_thread thread and
                               released_if_bound_sc_thread dest and
...
lemma do_reply_transfer_valid_sched:
  "\<lbrace>valid_sched and valid_reply_scs and
                invs and
                simple_sched_action and ct_in_state activatable and tcb_at sender\<rbrace>
  ...
  apply (wpsimp wp: possible_switch_to_valid_sched_strong)
          ...
          apply (rule_tac Q="\<lambda>r. invs
                                 ...
                                 and not_in_release_q a and ct_in_state activatable
    ...
    (* key transition point *)
      apply (rule_tac Q="\<lambda>r. valid_sched_except_blocked and
                 (\<lambda>s. if (st_tcb_at runnable a s \<and> bound_sc_tcb_at bound a s) then
                         (invs s \<and> not_cur_thread a s \<and>
                 active_sc_tcb_at a s \<and>
                 scheduler_act_not a s \<and>
                 not_queued a s \<and>
                 not_in_release_q a s \<and> ct_in_state activatable s \<and>
      ...
      apply (wpsimp wp: )
          ...
          apply clarsimp
          apply (rule_tac Q="\<lambda>rv. (valid_sched)
                and (invs and simple_sched_action and
    ...
    apply (rule_tac Q="\<lambda>rv. (valid_sched)  and invs and simple_sched_action and
                           ...
                           not_queued a s \<and> ct_in_state activatable s \<and>
                           not_in_release_q a s \<and>
    ...
    apply (wpsimp wp: reply_remove_valid_sched reply_remove_invs
...
lemma handle_fault_valid_sched:
  "\<lbrace>valid_sched and st_tcb_at active thread and not_queued thread and not_in_release_q thread
      ...
      and released_sc_tcb_at thread\<rbrace>
  ...
  by (fastforce simp: released_sc_tcb_at_def tcb_at_kh_simps vs_all_heap_simps pred_neg_def)
...
lemma maybe_donate_sc_cond_released_sc_tcb_at:
  "\<lbrace>released_if_bound_sc_thread tcbptr and st_tcb_at runnable tcbptr\<rbrace>
...
lemma send_signal_WaitingNtfn_helper:
  ...
  apply (intro conjI)
   ...
   apply (subgoal_tac "ipc_queued_thread (hd wnlist) s", clarsimp simp: released_ipc_queues_defs )
   apply (clarsimp simp: tcb_at_kh_simps pred_map_def )
...
lemma reply_unlink_tcb_weak_budget_conditions[wp]:
  ...
  apply (clarsimp simp: is_sc_active_def obj_at_def reply_at_pred_def
                 split: option.splits)
...
lemma blocked_cancel_ipc_BOR_released_sc_tcb_at:
  "\<lbrace>(\<lambda>s. released_sc_tcb_at tcbptr s) and valid_objs\<rbrace>
   blocked_cancel_ipc (BlockedOnReceive ep r) tcbptr r
   \<lbrace>\<lambda>rv s::('state_ext state). (released_sc_tcb_at tcbptr s)\<rbrace>"
  unfolding blocked_cancel_ipc_def
  apply (wpsimp simp: get_thread_state_def
                  wp: thread_get_wp get_simple_ko_wp set_simple_ko_wp
                      get_ep_queue_wp get_blocking_object_wp)
...
lemma cancel_ipc_BOR_other:
  "\<lbrace>(st_tcb_at ((=) (BlockedOnReceive tptr reply)) tcbptr) and invs and
        (\<lambda>s. released_if_bound_sc_thread tcbptr s)\<rbrace>
      cancel_ipc tcbptr
   \<lbrace>\<lambda>rv s::('state_ext state). released_if_bound_sc_thread tcbptr s\<rbrace>"
  unfolding cancel_ipc_def
  apply (rule hoare_seq_ext [OF _ gts_sp])
  apply (case_tac state; clarsimp)
         prefer 4
         apply (wpsimp wp: thread_set_valid_objs
           ...
           apply (wpsimp wp: blocked_cancel_ipc_BOR_released_sc_tcb_at)
          apply (rule hoare_vcg_disj_lift
...
lemma maybe_donate_sc_released_ipc_queues:
  ...
  by (wpsimp wp: set_tcb_sched_context_valid_sched_pred get_sc_obj_ref_wp
                 sched_context_donate_released_sc_released_ipc_queues get_sk_obj_ref_wp
                 get_tcb_obj_ref_wp)
  ...
  "ntfn_obj ntfn = IdleNtfn
   ...
   \<Longrightarrow> \<lbrace>st_tcb_at ((=) (BlockedOnReceive ep r_opt)) tcbptr and
        ...
        and current_time_bounded 1\<rbrace>
         ...
         od
       \<lbrace>\<lambda>_. valid_sched :: 'state_ext state \<Rightarrow> _\<rbrace>"
  apply (rule_tac B="\<lambda>_. released_if_bound_sc_thread tcbptr
...
and current_time_bounded 1"
   ...
   apply (intro conjI)
      apply (rule released_ipc_queuesE1)
       apply (simp add: valid_sched_def)
  ...
  apply (wpsimp wp: possible_switch_to_valid_sched_strong)
    ...
    apply (rule_tac Q="\<lambda>r s. tcbptr \<noteq> idle_thread s \<and> st_tcb_at runnable tcbptr s
                             ...
                             \<and> (active_sc_tcb_at tcbptr s \<longrightarrow> not_in_release_q tcbptr s
                                \<longrightarrow> released_if_bound_sc_thread tcbptr s)"
                 in hoare_strengthen_post[rotated])
...
crunches receive_ipc_preamble
  for released_if_bound[wp]: "released_if_bound_sc_thread t"
...
abbreviation (input) receive_ipc_valid_sched_preconds ::
  ...
  "receive_ipc_valid_sched_preconds t ep_ptr reply reply_opt ep ex_invs \<equiv>
    \<lambda>s. valid_sched s
         ...
         \<and> released_if_bound_sc_thread t s
          ...
          clarsimp simp: pred_tcb_at_eq_commute st_tcb_at_def obj_at_def
                 intro!: valid_sched_valid_blocked
                  ...
                  elim!: valid_ready_qs_not_queued_not_runnable[OF valid_sched_valid_ready_qs]
                         ...
                         released_ipc_queuesE1[OF valid_sched_released_ipc_queues])
...
lemma receive_ipc_valid_sched:
  "\<lbrace>\<lambda>s. valid_sched s
        ...
        \<and> not_in_release_q thread s
        \<and> scheduler_act_not thread s
        \<and> released_if_bound_sc_thread thread s
        \<and> invs s\<rbrace>
...
lemma receive_signal_valid_sched:
  "\<lbrace>valid_sched and scheduler_act_not thread and not_queued thread and not_in_release_q thread
                ...
                and released_if_bound_sc_thread thread\<rbrace>
...
lemma handle_recv_valid_sched:
  "\<lbrace>valid_sched and invs and ct_active and ct_not_in_release_q
      and ct_not_queued and scheduler_act_sane and ct_released\<rbrace>
...
lemma handle_recv_valid_sched':
 ...
 and ct_not_in_release_q  and ct_released\<rbrace>
...
lemma sched_context_yield_to_valid_sched_helper2:
  "\<lbrace>sc_tcb_sc_at ((=) (Some tcb_ptr)) sc_ptr and
   ...
   do in_release_q <- gets (in_release_queue tcb_ptr);
      ...
      then do y <- refill_unblock_check sc_ptr;
              ...
              curtime <- gets cur_time;
              y <- assert (sc_refill_ready curtime sc \<and> sc_refill_sufficient 0 sc);
  ...
  apply (case_tac schedulable; clarsimp simp: bind_assoc)
           ...
           apply (wpsimp wp_del: tcb_sched_enqueue_valid_sched reschedule_valid_sched_const
            ...
            apply (wpsimp simp: thread_get_def
                 ...
                 | strengthen valid_sched_valid_ready_qs valid_sched_weak_valid_sched_action
                              valid_sched_valid_blocked valid_sched_released_ipc_queues)+
    apply (wpsimp wp: hoare_drop_imp hoare_vcg_all_lift
   ...
   apply (intro conjI)
     ...
     apply (clarsimp simp: sc_at_pred_n_def obj_at_def)
    apply (clarsimp simp: vs_all_heap_simps is_sc_active_kh_simp)
   apply (clarsimp simp: vs_all_heap_simps is_sc_active_kh_simp)
  apply wpsimp
  done
...
lemma refill_new_not_active_sc:
  "\<lbrace>\<lambda>s. if (scp = sc_ptr) then mrefills = 0 else \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>
   refill_new sc_ptr mrefills budget period
   \<lbrace>\<lambda>rv s.  \<not> pred_map cfg_active (sc_refill_cfgs_of s) scp\<rbrace>"
  unfolding refill_new_def
  apply (wpsimp wp: update_sched_context_wp)
  by (clarsimp simp: vs_all_heap_simps active_sc_def split: if_splits)

lemma refill_new_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and ((\<lambda>s. unat (cur_time s) + unat period \<le> unat (max_word :: time))
  ...
  by (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps pred_map_eq_def split: if_splits)
...
lemma refill_update_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and
    ((\<lambda>s. unat (cur_time s) + 2 * unat period \<le> unat (max_word :: time))
     and K (MIN_REFILLS \<le> max_refills \<and> budget \<le> period \<and> MIN_SC_BUDGET \<le> budget \<and> period \<le> MAX_SC_PERIOD))
     and (\<lambda>s. \<forall>x. if (x=p) then ((\<exists>sc n. obj_at (\<lambda>ko. ko = SchedContext sc n
                \<and> unat (r_time (refill_hd sc)) + 2 * unat period \<le> unat (max_word :: time)) x s \<and> sc_valid_refills sc))
          else (pred_map cfg_active (sc_refill_cfgs_of s) x \<longrightarrow>
                 (\<exists>sc n. obj_at
                      (\<lambda>ko. ko = SchedContext sc n \<and> unat (r_time (refill_hd sc)) + 2 * unat period \<le> unat (max_word :: time))
                      x s)
)
)
       \<rbrace>
  ...
  apply (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps pred_map_eq_def MIN_REFILLS_def split: if_splits)
...
lemma refill_new_schedulable_ipc_queues:
  ...
  apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift' hoare_vcg_disj_lift
              simp: Ball_def pred_tcb_at_eq_commute[symmetric])
   apply (wpsimp wp: refill_new_released_sc_tcb_at)
...
lemma end_timeslice_valid_release_q:
  ...
  apply wpsimp
                         ...
                         apply (wpsimp wp: tcb_fault_update_sym_refs
                                     simp: thread_get_def)+
  ...
  apply (subgoal_tac "bound_sc_tcb_at (\<lambda>x. x = Some (cur_sc s)) (cur_thread s) s")
   apply (clarsimp dest!: get_tcb_SomeD
                    simp: obj_at_kh_kheap_simps vs_all_heap_simps pred_map_simps
                          ct_in_state_def runnable_eq_active)
...
lemma end_timeslice_valid_ready_qs:
  "\<lbrace>valid_ready_qs and weak_valid_sched_action and released_ipc_queues and
  ...
  apply wpsimp
                          apply (wpsimp wp: send_ipc_valid_sched_subset_for_handle_timeout)
                         apply (wpsimp wp: tcb_sched_append_valid_blocked_except_set_const tcb_fault_update_sym_refs
                                     simp: thread_get_def)+
...
lemma end_timeslice_valid_blocked[wp]:
  ...
  by (wpsimp wp: send_ipc_valid_sched_subset_for_handle_timeout
                 ...
                 get_tcb_queue_wp
                 postpone_valid_blocked_except_set gts_wp
           simp: thread_get_def)
...
lemma end_timeslice_released_ipc_queues:
  ...
  by (wpsimp wp: send_ipc_valid_sched_subset_for_handle_timeout
                 tcb_sched_append_valid_blocked_except_set_const
           simp: thread_get_def released_sc_tcb_at_def)

lemma end_timeslice_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills\<rbrace>
   end_timeslice canTimeout
...
lemmas end_timeslice_valid_sched_subset = end_timeslice_valid_release_q
                                          ...
                                          end_timeslice_released_ipc_queues
                                          end_timeslice_active_sc_valid_refills
...
lemma restart_scheduler_act_sane[wp]:
  "\<lbrace>scheduler_act_sane and (\<lambda>s. x = cur_thread s \<longrightarrow> ct_in_state activatable s)\<rbrace>
   restart x
   \<lbrace>\<lambda>rv. scheduler_act_sane:: det_state \<Rightarrow> _\<rbrace>"
  supply if_split [split del]
  unfolding restart_def
...
lemma blocked_cancel_ipc_ct_in_state:
  "\<lbrace>ct_in_state P and (\<lambda>s. x \<noteq> cur_thread s \<or> P Inactive)\<rbrace>
   blocked_cancel_ipc a x c
   \<lbrace>\<lambda>rv. ct_in_state P:: det_state \<Rightarrow> _\<rbrace>"
  supply if_split [split del]
  unfolding blocked_cancel_ipc_def
  apply (wpsimp wp: set_thread_state_ct_st hoare_vcg_if_lift2 hoare_vcg_imp_lift get_simple_ko_wp
                    reply_unlink_tcb_ct_in_state get_ep_queue_wp get_blocking_object_wp
...
lemma suspend_ct_in_state:
  ...
  apply (rule_tac Q="\<lambda>_ s. (if (x = cur_thread s)
                            then (P Inactive)
                            else (ct_in_state P s))"
         in hoare_strengthen_post[rotated])
  apply (clarsimp simp: split: if_splits)
  apply (wpsimp wp: hoare_vcg_if_lift2 hoare_vcg_imp_lift set_thread_state_ct_st
                    set_tcb_obj_ref_ct_in_state get_tcb_obj_ref_wp cancel_ipc_ct_in_state)
  apply (clarsimp simp: split: if_splits)
  done
...
lemma invoke_tcb_scheduler_act_sane:
...
lemma invoke_tcb_scheduler_act_saneE_E[wp]:
  "\<lbrace>scheduler_act_sane\<rbrace>
   invoke_tcb x
   -, \<lbrace>\<lambda>rv. scheduler_act_sane:: det_state \<Rightarrow> _\<rbrace>"
 supply if_split [split del]
  apply (case_tac x; simp)
    ...
    apply (wpsimp wp: hoare_vcg_if_lift2 mapM_x_wp_inv hoare_vcg_imp_lift' suspend_ct_in_state)
         prefer 7
         apply (rename_tac t ntfn)
         apply (case_tac ntfn; simp, wpsimp)
        apply (wpsimp wp: hoare_vcg_if_lift2 hoare_drop_imp)+
  done
...
lemma refill_budget_check_round_robin_released_ipc_queues:
  "\<lbrace>released_ipc_queues
    ...
    and (\<lambda>s. \<forall>t. bound_sc_tcb_at ((=) (Some (cur_sc s))) t s
                 \<longrightarrow> ipc_queued_thread t s
  ...
  apply (intro conjI impI allI
    ...
    apply (clarsimp simp: refill_sufficient_defs split: if_splits)
     apply (subgoal_tac "MIN_BUDGET \<le> r_amount (refill_hd sc)")
      apply (simp add: word_le_nat_alt unat_sub)
      apply (subgoal_tac "unat (2 * MIN_BUDGET) = 2 * unat MIN_BUDGET")
       apply linarith
      apply (simp add: MIN_BUDGET_def)
      using unat_MIN_BUDGET_MIN_SC_BUDGET[simplified MIN_BUDGET_def MIN_SC_BUDGET_def]
      apply fastforce
     apply (clarsimp simp: vs_all_heap_simps is_sc_active_def obj_at_def)
    apply (clarsimp simp: vs_all_heap_simps is_sc_active_def obj_at_def tcb_at_kh_simps)
   apply (clarsimp simp: refill_sufficient_defs)
   apply (drule_tac x=tcb_ptr in spec)+
   apply (clarsimp simp: vs_all_heap_simps is_sc_active_def obj_at_def tcb_at_kh_simps split: if_splits)
  apply (clarsimp simp: refill_sufficient_defs)
  apply (drule_tac x=tcb_ptr in spec)
  apply (clarsimp simp: vs_all_heap_simps is_sc_active_def obj_at_def)
  apply (subgoal_tac "valid_refills (cur_sc s) s")
   apply (simp add: valid_refills_def vs_all_heap_simps)
  using MIN_BUDGET_le_MIN_SC_BUDGET apply force
  apply (erule active_implies_valid_refills[rotated],
         clarsimp simp:  active_sc_def vs_all_heap_simps)
  done
...
lemma refill_budget_check_released_ipc_queues:
  ...
  apply (intro conjI; intro allI impI)
   ...
   apply (rule FalseE)
   apply (subgoal_tac "sc_refill_ready (cur_time s) sc")
    apply (erule_tac x=tcb_ptr in allE, clarsimp simp: tcb_at_kh_simps pred_map_eq_def)
    apply (clarsimp simp: vs_all_heap_simps refill_sufficient_defs)
  ...
  apply (intro conjI)
    (* refill_sufficient *)
   apply (clarsimp simp: vs_all_heap_simps refill_sufficient_defs Let_def split: if_splits)
   apply auto[1]
      apply (metis MIN_BUDGET_le_MIN_SC_BUDGET MIN_SC_BUDGET_def le_minus' mult_2)
     apply (metis word_le_imp_diff_le word_le_less_eq word_le_not_less)
    apply (metis word_le_imp_diff_le word_le_less_eq word_le_not_less)
   apply (metis word_le_imp_diff_le word_le_less_eq word_le_not_less)
    (* refill_ready *)
  apply (subgoal_tac "valid_refills (cur_sc s) s")
   apply (subgoal_tac "sc_refill_max sc \<ge> MIN_REFILLS \<and> MIN_SC_BUDGET \<le> sc_budget sc")
    apply (clarsimp simp: vs_all_heap_simps refill_ready_def cur_sc_offset_ready_def Let_def tcb_at_kh_simps MIN_REFILLS_def split: if_splits)
    apply (erule_tac x=tcb_ptr in allE, clarsimp simp: tcb_at_kh_simps pred_map_eq_def)
    apply (subgoal_tac "r_time (refill_hd sc) + usage \<le> cur_time s + kernelWCET_ticks")
     apply auto[1]
       apply (case_tac "sc_refills sc"; simp)
      apply (clarsimp simp: MIN_SC_BUDGET_def)
      apply (subgoal_tac "sc_budget sc = r_amount (refill_hd sc)", simp)
      apply (subgoal_tac "sc_budget sc = refills_sum (sc_refills sc)", simp)
       apply (case_tac "sc_refills sc"; simp)
      apply (clarsimp simp: valid_refills_def obj_at_def vs_all_heap_simps sc_valid_refills_def)
     apply (case_tac "sc_refills sc"; simp)
    apply (simp add: word_le_nat_alt)
    ...
    apply (rule unat_add_lem')
    apply (subst no_olen_add_nat[symmetric], erule cur_time_no_overflow)
   apply (erule_tac x=tcb_ptr in allE, clarsimp simp: tcb_at_kh_simps pred_map_eq_def)
   apply (clarsimp simp: valid_refills_def obj_at_def vs_all_heap_simps)
  apply (clarsimp simp: sc_valid_refills_def)
  apply (subgoal_tac "active_sc_tcb_at tcb_ptr s")
   apply (drule (1) active_sc_tcb_at_bound_valid_refills, clarsimp simp: tcb_at_kh_simps pred_map_eq_normalise)
  apply (clarsimp simp: released_ipc_queues_defs tcb_at_kh_simps released_sc_tcb_at_def)
...
lemma charge_budget_valid_blocked[wp]:
  ...
  by (wpsimp wp: reschedule_required_valid_blocked end_timeslice_valid_sched_subset
                 assert_inv gts_wp is_schedulable_wp')

lemma charge_budget_valid_release_q:
  "\<lbrace>valid_release_q and invs and ct_not_in_release_q and cur_sc_chargeable \<rbrace>
   charge_budget consumed canTimeout
   \<lbrace>\<lambda>_. valid_release_q :: 'state_ext state \<Rightarrow> _\<rbrace>"
  supply if_split [split del]
  ...
  apply wpsimp
       ...
       apply (rule_tac Q="\<lambda>ya s. valid_release_q s \<and>
                      cur_sc_chargeable s \<and>
                      invs s" in hoare_strengthen_post[rotated])
...
(* The remaining subgoal seems to want the current thread to be released.
   it's not clear if this is a reasonable assumption. *)
lemma charge_budget_released_ipc_queues:
  ...
  apply (wpsimp)
       ...
       apply (rule_tac Q="\<lambda>ya s.
                (if (pred_map runnable (tcb_sts_of s) (cur_thread s) \<and> ct_not_in_release_q s \<and> active_sc_tcb_at (cur_thread s) s)
                 then released_ipc_queues s
                 else released_ipc_queues s)" in hoare_strengthen_post[rotated])
        apply (clarsimp simp: tcb_at_kh_simps ct_in_state_def runnable_eq_active
                       split: if_splits)
  subgoal sorry (* charge_budget subgoal: charge_budget can block the thread *)
       apply (wpsimp wp: hoare_vcg_if_lift_strong sc_consumed_add_invs)
      apply (wpsimp wp: hoare_vcg_if_lift_strong)
          apply (rule_tac hoare_strengthen_post[rotated], erule ct_active_sc_helper)
          apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift')
...
lemma charge_budget_valid_ready_qs:
  "\<lbrace>valid_ready_qs
    ...
    and valid_machine_time
    and active_sc_valid_refills
    and weak_valid_sched_action and invs
  ...
  apply wpsimp
             ...
             apply (wpsimp wp: assert_inv)
            apply (wpsimp wp: gts_wp is_schedulable_wp')+
       apply (rule_tac Q="\<lambda>ya s. ct_not_queued s \<and>
                                 cur_sc_chargeable s \<and>
                                 valid_ready_qs s \<and>
                                 released_ipc_queues s \<and>
                                 weak_valid_sched_action s" in hoare_strengthen_post[rotated])
        apply (clarsimp simp: tcb_at_kh_simps ct_in_state_def runnable_eq_active
                       split: if_splits)
       apply wpsimp
      apply (wpsimp wp: refill_budget_check_round_robin_valid_ready_qs_not_queued
                        ...
                        refill_budget_check_round_robin_weak_valid_sched_action
...
lemma charge_budget_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and (\<lambda>s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time) \<and>
         unat consumed + unat MAX_SC_PERIOD \<le> unat (max_word :: time))
and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
               unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat consumed
               \<le> unat (max_word :: time)))\<rbrace>
   charge_budget consumed canTimeout
   \<lbrace>\<lambda>_. active_sc_valid_refills :: 'state_ext state \<Rightarrow> _\<rbrace>"
  supply if_split [split del]
  apply (clarsimp simp: charge_budget_def)
  by (wpsimp wp: end_timeslice_valid_sched_subset hoare_drop_imp
                 refill_budget_check_round_robin_active_sc_valid_refills
                 refill_budget_check_active_sc_valid_refills)
...
lemma charge_budget_valid_sched:
  "\<lbrace>valid_sched and invs and ct_not_in_release_q and ct_not_queued and scheduler_act_sane
    and cur_sc_chargeable and cur_sc_blocked_is_offset consumed and
   (\<lambda>s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time) \<and>
        unat consumed + unat MAX_SC_PERIOD \<le> unat (max_word :: time))
    and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
               unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat consumed
               \<le> unat (max_word :: time)))\<rbrace>
...
lemma check_budget_valid_ready_qs:
  "\<lbrace>valid_ready_qs and released_ipc_queues and valid_machine_time and weak_valid_sched_action
    and invs
    and ct_not_queued
    and active_sc_valid_refills
...
lemma check_budget_active_sc_valid_refills:
  "\<lbrace>active_sc_valid_refills and (\<lambda>s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time) \<and>
         unat (consumed_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time))
and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
               unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat (consumed_time s)
               \<le> unat (max_word :: time)))\<rbrace>
   check_budget
   \<lbrace>\<lambda>_. active_sc_valid_refills::'state_ext state \<Rightarrow> _\<rbrace>"
  unfolding check_budget_def
  by (wpsimp wp: get_sched_context_wp charge_budget_active_sc_valid_refills
                 hoare_vcg_if_lift2 hoare_drop_imp hoare_vcg_all_lift)
...
and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
   ...
   \<lbrace>\<lambda>_. valid_sched::'state_ext state \<Rightarrow> _\<rbrace>"
  apply (clarsimp simp: valid_sched_def)
  apply (wpsimp wp: check_budget_valid_ready_qs check_budget_active_sc_valid_refills
                    check_budget_valid_release_q
                    check_budget_valid_sched_action
                    check_budget_released_ipc_queues)
  apply (clarsimp simp: valid_sched_action_def consumed_time_bounded_def current_time_bounded_def)
  done
...
lemma update_sched_context_valid_sched_no_change:
  "\<forall>sc. sc_active (f sc) = sc_active sc \<Longrightarrow>
  ...
  apply (wpsimp wp: valid_sched_lift active_sc_tcb_at_update_sched_context_no_change
              ...
              simp: vs_all_heap_simps obj_at_kh_kheap_simps update_sched_context_def active_sc_def sc_valid_refills_def)
  done
...
lemma update_sched_context_valid_refills_indep:
  ...
  apply (wpsimp wp: update_sched_context_wp)
  apply (clarsimp simp: valid_refills_def vs_all_heap_simps obj_at_def)
  done
...
lemma pst_vs_for_invoke_sched_control_configure:
  "\<lbrace>valid_sched_except_blocked and valid_blocked_except target
    ...
    and (\<lambda>s. not_in_release_q target s \<longrightarrow> released_if_bound_sc_thread target s)\<rbrace>
    possible_switch_to target
   \<lbrace>\<lambda>rv. valid_sched :: 'state_ext state \<Rightarrow> _\<rbrace>"
  apply (wp possible_switch_to_valid_sched_strong)
  apply (clarsimp simp: valid_sched_def )
...
lemma tcb_release_remove_ready_or_release:
...
lemma invoke_sched_control_configure_valid_sched:
  "\<lbrace>valid_sched and valid_sched_control_inv iv and schact_is_rct and ready_or_release and invs
    and ct_not_in_release_q and ct_released and ct_not_queued and (\<lambda>s. valid_refills (cur_sc s) s)
    and current_time_bounded 1\<rbrace>
  ...
  apply (case_tac "sc_tcb sc = None")
   ...
   apply (intro conjI)
          using MIN_BUDGET_le_MIN_SC_BUDGET apply simp
         apply (simp add: MIN_REFILLS_def active_sc_def)
        apply (clarsimp simp: vs_all_heap_simps obj_at_def)
        apply (rename_tac tcb_ptr tcb)
  ...
  apply (erule order_trans[rotated])
  apply (rule order_trans[OF add_left_mono], overflow_hammer, clarsimp)
  ...
  apply (rule_tac B="\<lambda>rv s. iscc_valid_sched_predicate sc_ptr tcb_ptr mrefills budget s
                            \<and> budget \<le> period \<and> period \<le> MAX_SC_PERIOD \<and> current_time_bounded 1 s
                            \<and> ex_nonz_cap_to sc_ptr s
                            \<and> sc_refill_max_sc_at (\<lambda>rm. rm = sc_refill_max sc) sc_ptr s
                            \<and> sc_tcb_sc_at (\<lambda>to. to = sc_tcb sc) sc_ptr s
   ...
   apply (wpsimp wp: tcb_release_remove_valid_sched_except_blocked tcb_release_remove_valid_blocked
                     tcb_release_remove_ready_or_release tcb_release_remove_not_in_release_q
                     hoare_vcg_conj_lift)
  ...
  apply (rule_tac B="\<lambda>rv s. iscc_valid_sched_predicate sc_ptr tcb_ptr mrefills budget s \<and> budget \<le> period \<and> period \<le> MAX_SC_PERIOD \<and> current_time_bounded 1 s
   ...
   apply (intro conjI impI)
     ...
     apply (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps)
  subgoal sorry (* cur_sc_in_release_q_imp_zero_consumed? *)
  subgoal sorry (* cur_sc_offset_ready (consumed_time s) s YES. *)
  subgoal sorry (* cur_time boudned 2 *)
  subgoal sorry (* cur_time boudned 2 *)
  subgoal sorry (* cur_time boudned 2 *)

  apply (rule_tac B="\<lambda>rv s. valid_sched_except_blocked s \<and> budget \<le> period \<and> period \<le> MAX_SC_PERIOD \<and> current_time_bounded 1 s
                            \<and> valid_blocked_except tcb_ptr s
                            \<and> invs s
   ...
   apply (clarsimp split: if_split)
   apply (intro conjI impI)
     apply (simp add: sc_at_pred_n_def obj_at_def pred_tcb_at_def)
     apply (frule invs_valid_global_refs)
     apply (frule invs_sym_refs)
     apply (frule invs_valid_objs)
     apply (frule invs_iflive)
     using sc_tcb_not_idle_thread apply blast
    apply (clarsimp simp: vs_all_heap_simps pred_tcb_at_def obj_at_def)
   apply (rule valid_blocked_except_set_not_runnable; blast?)
   apply (clarsimp simp: pred_map_eq_def pred_map_def obj_at_kh_kheap_simps)
  ...
  apply (intro conjI impI)
   ...
   apply (intro conjI impI)
    ...
    apply (prop_tac "\<forall>t. pred_map_eq (Some sc_ptr) (tcb_scps_of s) t \<longrightarrow> t = tcb_ptr")
     apply (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps sc_at_pred_n_def obj_at_def)
     apply (rename_tac tcb') \<comment> \<open>this is the TCB associated with the green variable t\<close>
     apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; blast?)
     apply (frule_tac tp=t and tcb=tcb' in sym_ref_tcb_sc; blast?)
     apply simp
    apply (intro conjI impI allI)
           apply (simp add: pred_tcb_at_def obj_at_def pred_map_eq_def pred_map_def
                            vs_all_heap_simps)
           apply metis
          apply fast
         apply (erule schact_is_rct_simple)
        apply (fastforce dest: ipc_queued_thread_state_not_runnable
                         simp: obj_at_def pred_tcb_at_def sc_at_pred_n_def vs_all_heap_simps)
   subgoal sorry  (* cur_time boudned 2 *)
apply (clarsimp simp: obj_at_def split: if_split)
  apply (intro conjI; intro impI; simp?)
    apply (clarsimp simp: sc_at_pred_n_def obj_at_def)
  apply (intro conjI)
   subgoal sorry (* I HAVE NO RESOLUTION FOR THIS *)
   apply (prop_tac "valid_refills sc_ptr s")
  apply (erule active_sc_valid_refillsE[rotated])
  apply (clarsimp simp: vs_all_heap_simps active_sc_def)
  apply (clarsimp simp: vs_all_heap_simps)
    apply (clarsimp simp: vs_all_heap_simps)
   subgoal sorry (* I HAVE NO RESOLUTION FOR THIS *)
       apply (simp add: pred_tcb_at_def obj_at_def pred_map_eq_def pred_map_def vs_all_heap_simps)
       apply metis
      apply (clarsimp simp: obj_at_def pred_map_eq_def vs_all_heap_simps sc_at_pred_n_def)
      apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; simp?)
      apply fastforce
     apply (clarsimp simp: vs_all_heap_simps obj_at_def sc_at_pred_n_def)
     apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; simp?)
     using active_sc_def apply metis
    apply (simp add: MIN_REFILLS_def active_sc_def)
   ...
   apply (intro conjI impI)
          using MIN_BUDGET_le_MIN_SC_BUDGET apply simp
         apply (simp add: MIN_REFILLS_def active_sc_def)
        apply (simp add: pred_map_eq_def pred_map_def, intro allI impI)
        apply (subgoal_tac "t=tcb_ptr")
         apply force
        apply (clarsimp simp: vs_all_heap_simps  obj_at_def sc_at_pred_n_def)
        apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
        apply (frule_tac tp=t and tcb=b in sym_ref_tcb_sc; blast?)
        apply fastforce
       apply (erule schact_is_rct_simple)
      apply (clarsimp simp: MIN_REFILLS_def)
  subgoal sorry (* easy *)
     apply (clarsimp simp: obj_at_def sc_at_pred_n_def)
     apply (clarsimp simp: vs_all_heap_simps released_ipc_queues_defs pred_tcb_at_def obj_at_def
                           sc_at_pred_n_def)
     apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
     apply (frule_tac tp=tcb_ptra and tcb=tcba in sym_ref_tcb_sc; simp?)
     apply simp
    apply (clarsimp simp: pred_map_eq_def pred_map_def vs_all_heap_simps pred_tcb_at_def
                          obj_at_def sc_at_pred_n_def)
    apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
    apply simp
   apply (clarsimp simp: pred_map_eq_def pred_map_def vs_all_heap_simps pred_tcb_at_def obj_at_def
                         sc_at_pred_n_def
                  split: if_splits)
   apply (intro conjI impI; clarsimp?)
    apply (simp add: MIN_REFILLS_def)
   apply (simp add: vs_all_heap_simps active_sc_def)
   apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
   apply fastforce
  ...
  apply (clarsimp simp: valid_sched_def)
   ...
   apply (intro conjI impI)
       ...
       apply (erule schact_is_rct_simple)
      apply (clarsimp simp: MIN_REFILLS_def)
  subgoal sorry (* easy: cur_time bounded *)
     apply (clarsimp simp: obj_at_def sc_at_pred_n_def)
     apply (clarsimp simp: vs_all_heap_simps released_ipc_queues_defs pred_tcb_at_def obj_at_def
                           sc_at_pred_n_def)
     apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
     apply (frule_tac tp=tcb_ptra and tcb=tcb in sym_ref_tcb_sc; simp?)
     apply simp
    apply (clarsimp simp: pred_map_eq_def pred_map_def vs_all_heap_simps pred_tcb_at_def
                          obj_at_def sc_at_pred_n_def)
    apply (frule_tac scp=sc_ptr and sc=sca in sym_ref_sc_tcb; fastforce?)
...
lemma perform_invocation_valid_sched:
  ...
  apply (cases i; simp)
            ...
            apply (clarsimp simp: ct_in_state_def released_sc_tcb_at_def tcb_at_kh_simps vs_all_heap_simps)
...
lemma handle_invocation_valid_sched:
  ...
  apply (clarsimp simp: valid_sched_def ct_not_in_q_def valid_ready_qs_def not_queued_def
                    ...
                    in_release_queue_def released_sc_tcb_at_def current_time_bounded_def)
...
lemma handle_interrupt_valid_sched:
  "\<lbrace>valid_sched
    ...
    and (\<lambda>s. irq \<in> non_kernel_IRQs \<longrightarrow> scheduler_act_sane s \<and> ct_not_queued s)
    and current_time_bounded 1 \<rbrace>
...
lemma send_ipc_cannot_donate_active_sc_tcb_at[wp]:
  "send_ipc block call badge can_grant False thread epptr
   \<lbrace>\<lambda>(s:: 'state_ext state). active_sc_tcb_at t s\<rbrace>"
  unfolding send_ipc_def
...
lemma invs_strengthen_cur_sc_tcb_are_bound:
  ...
  by (fastforce simp: cur_sc_tcb_are_bound_def tcb_at_kh_simps[symmetric]
...
lemma handle_yield_valid_sched:
  "\<lbrace>valid_sched
    and invs
    and ct_not_in_release_q and ct_not_queued and ct_not_blocked
    and (\<lambda>s. valid_refills (cur_sc s) s)
    and scheduler_act_sane
    and cur_sc_chargeable
    and (\<lambda>s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time))
    and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
               unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) + unat (r_amount (refill_hd sc))
               \<le> unat (max_word :: time)))
    and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
               unat (r_amount (refill_hd sc)) + unat MAX_SC_PERIOD \<le> unat (max_word :: time)))\<rbrace>
   handle_yield
   \<lbrace>\<lambda>rv. valid_sched::'state_ext state \<Rightarrow> _\<rbrace>"
  unfolding handle_yield_def
  apply (wpsimp simp: set_sc_obj_ref_def wp: charge_budget_valid_sched get_refills_wp)
  apply (intro conjI; clarsimp simp: obj_at_def)
  apply (rule FalseE)
  apply (clarsimp simp: cur_sc_chargeable_def)
  apply (drule_tac x=t in spec)+
  apply (clarsimp simp: tcb_at_kh_simps pred_map_eq_def ct_in_state_def)
  apply (fastforce simp: pred_map_simps)
  done
...
lemma update_time_stamp_ct_released[wp]:
 "\<lbrace>ct_released and valid_machine_time :: 'state_ext state \<Rightarrow> _\<rbrace>
  ...
  apply (rule_tac Q="\<lambda>_ s. \<exists>t. t = cur_thread s \<and> active_sc_tcb_at t s \<and> budget_sufficient t s \<and> budget_ready t s"
         in hoare_strengthen_post)
  by (wpsimp wp: hoare_vcg_ex_lift simp: vs_all_heap_simps)+
...
crunches update_time_stamp
  for valid_refills_cur_sc[wp]: "\<lambda>s. valid_refills (cur_sc s) s"
...
lemma charge_budget_scheduler_act_sane[wp]:
  "\<lbrace>scheduler_act_sane::det_state \<Rightarrow> _\<rbrace>
...
lemma send_ipc_scheduler_act_sane[wp]:
  "\<lbrace>scheduler_act_sane and ct_not_blocked\<rbrace>
   send_ipc d f x22 x23 e thread x21
   \<lbrace>\<lambda>_. scheduler_act_sane ::det_state \<Rightarrow> _\<rbrace>"
...
lemma send_signal_scheduler_act_sane:
  "\<lbrace>scheduler_act_sane and ct_not_blocked and invs\<rbrace>
   send_signal x31 x32
   \<lbrace>\<lambda>_. scheduler_act_sane ::det_state \<Rightarrow> _\<rbrace>"
  unfolding send_signal_def
  apply (wpsimp wp: possible_switch_to_scheduler_act_sane' is_schedulable_wp)+
  ...
  apply (prop_tac "tcb_at (hd x) s")
   apply (clarsimp)
  apply (prop_tac "(hd x, NTFNSignal) \<in> state_refs_of s x31")
...
lemma handle_fault_reply_valid_sched_pred_strong[wp]:
  "handle_fault_reply x2 x d mrs
   \<lbrace>(valid_sched_pred_strong P) ::det_state \<Rightarrow> _\<rbrace>"
  by (case_tac x2; wpsimp)

lemma do_reply_transfer_scheduler_act_sane[wp]:
  "\<lbrace>scheduler_act_sane and ct_not_blocked\<rbrace>
   do_reply_transfer sender x4
   \<lbrace>\<lambda>_. scheduler_act_sane ::det_state \<Rightarrow> _\<rbrace>"
...
lemma invoke_domain_valid_sched_misc[wp]:
  "invoke_domain thread domain
   \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s)
          (idle_thread s) (release_queue s) (last_machine_time_of s) (tcb_sts_of s)
          (tcb_scps_of s) (sc_refill_cfgs_of s)\<rbrace>"
...
lemma perform_invocation_scheduler_act_sane[wp]:
  "\<lbrace>simple_sched_action and ct_in_state activatable and invs\<rbrace>
   perform_invocation False False True x2a
   \<lbrace>\<lambda>_. scheduler_act_sane :: det_state \<Rightarrow> _\<rbrace>"
  apply (case_tac x2a; simp)
  by (wpsimp wp: hoare_drop_imp invoke_cnode_scheduler_act_sane send_signal_scheduler_act_sane
                 invoke_tcb_scheduler_act_sane)+
...
lemma sched_context_donate_bound_not_cur_thread:
  "\<lbrace>\<lambda>s. x2 \<noteq> cur_thread s\<rbrace>
   sched_context_donate xa x2
   \<lbrace>\<lambda>rv s. \<forall>x. sc_tcb_sc_at ((=) (Some x)) xa s \<longrightarrow> x \<noteq> cur_thread s\<rbrace>"
  unfolding sched_context_donate_def
  apply (rule_tac hoare_seq_ext[OF _ gsct_sp])
  apply (rule_tac B = "\<lambda>_ s. x2 \<noteq> cur_thread s" in hoare_seq_ext)
  apply (wpsimp wp: set_tcb_obj_ref_wp)
  apply (wpsimp wp: set_sc_obj_ref_wp)
  apply (clarsimp simp: obj_at_def sc_at_pred_n_def)
  by wpsimp
...
lemma maybe_donate_sc_ct_not_in_release_q[wp]:
  "\<lbrace>ct_not_in_release_q and (\<lambda>s. x2 \<noteq> cur_thread s):: det_state \<Rightarrow> _\<rbrace>
   maybe_donate_sc x2 x31
   \<lbrace>\<lambda>_. ct_not_in_release_q :: det_state \<Rightarrow> _\<rbrace>"
  unfolding maybe_donate_sc_def
  apply (wpsimp wp: )
...
lemma update_waiting_ntfn_ct_not_in_release_q[wp]:
  "\<lbrace>ct_not_in_release_q and (\<lambda>s. a \<noteq> [] \<longrightarrow> hd a \<noteq> cur_thread s):: det_state \<Rightarrow> _\<rbrace>
   update_waiting_ntfn x31 a s d x32
   \<lbrace>\<lambda>_. ct_not_in_release_q :: det_state \<Rightarrow> _\<rbrace>"
  unfolding update_waiting_ntfn_def
  by (wpsimp wp: )
...
lemma send_signal_ct_not_in_release_q[wp]:
  ...
  apply (subgoal_tac "(hd x, NTFNSignal) \<in> state_refs_of s x31")
  apply (drule sym_refsD; clarsimp)
  apply (drule TCBSignal_in_state_refs_of)
...
lemma do_reply_transfer_ct_not_in_release_q[wp]:
  ...
  apply (wpsimp wp: postpone_ct_not_in_release_q)
              apply (rule_tac Q="\<lambda>_ s. ct_not_in_release_q s \<and>
                              (\<forall>t. sc_tcb_sc_at ((=) (Some t)) sc_ptr s \<longrightarrow> t \<noteq> cur_thread s)" in hoare_strengthen_post[rotated])
               apply (clarsimp )
              apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift')
             apply (wpsimp wp: assert_inv)
            apply (wpsimp wp: get_tcb_obj_ref_wp gts_wp)+
            apply (rule_tac Q="\<lambda>_ s. ct_not_in_release_q s \<and>
                            (\<forall>scp. bound_sc_tcb_at ((=) (Some scp)) x s \<longrightarrow> (\<forall>t. sc_tcb_sc_at ((=) (Some t)) scp s \<longrightarrow> t \<noteq> cur_thread s))" in hoare_strengthen_post[rotated])
             apply (clarsimp simp: obj_at_def pred_tcb_at_def sc_at_pred_n_def split: if_split, fastforce)
            apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift')
           apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift')
          apply (rule_tac Q="\<lambda>_ s. ct_not_in_release_q s \<and>
                          (\<forall>scp. bound_sc_tcb_at ((=) (Some scp)) x s \<longrightarrow> (\<forall>t. sc_tcb_sc_at ((=) (Some t)) scp s \<longrightarrow> t \<noteq> cur_thread s))" in hoare_strengthen_post[rotated])
           apply (clarsimp simp: obj_at_def pred_tcb_at_def sc_at_pred_n_def split: if_split, fastforce)
          apply (wpsimp)
               apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift' thread_set_no_change_tcb_sched_context_converse
                           simp: tcb_at_kh_simps)
              apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift' thread_set_no_change_tcb_sched_context_converse
                          simp: tcb_at_kh_simps)
             apply (rule_tac Q="\<lambda>_ s. ct_not_in_release_q s \<and>
                             (invs s \<and> x \<noteq> cur_thread s)" in hoare_strengthen_post[rotated])
              apply clarsimp
              apply (subgoal_tac "x = cur_thread s", simp)
              apply (erule bound_sc_tcb_at_eq_inj'[rotated 2], clarsimp)
              apply (clarsimp simp: tcb_at_kh_simps pred_map_eq_normalise)
             apply (wpsimp simp: thread_get_def wp: hoare_drop_imp)+
        apply (rule_tac Q="\<lambda>_ s. ct_not_in_release_q s \<and>
                        (invs s \<and> x \<noteq> cur_thread s)" in hoare_strengthen_post[rotated])
         apply clarsimp
         apply (subgoal_tac "x = cur_thread s", simp)
         apply (erule bound_sc_tcb_at_eq_inj'[rotated]; clarsimp)
        apply (wpsimp simp: thread_get_def wp: reply_remove_invs get_simple_ko_wp gts_wp)+
  apply (intro conjI)
   apply (fastforce)
  apply (clarsimp simp: ct_in_state_def tcb_at_kh_simps vs_all_heap_simps pred_map_eq_normalise)
  done

lemma handle_invocation_scheduler_act_sane[wp]:
  "\<lbrace>schact_is_rct and invs and ct_active:: det_state \<Rightarrow> _\<rbrace>
  ...
  apply (intro conjI)
   ...
   apply (clarsimp simp: live_def ct_in_state_def pred_tcb_at_def obj_at_def, fastforce)
  apply (clarsimp simp: ct_in_state_def intro!: fault_tcbs_valid_states_active)
  done

lemma reply_push_ct_in_state_const:
  "\<forall>y. P (BlockedOnReply y)
  \<Longrightarrow> reply_push thread x21 y can_donate
      \<lbrace>ct_in_state P :: det_state \<Rightarrow> _\<rbrace>"
  unfolding reply_push_def
  by (wpsimp wp: set_thread_state_ct_in_state hoare_vcg_if_lift2 hoare_drop_imp hoare_vcg_all_lift
                    get_simple_ko_wp)

lemma reply_unlink_tcb_ct_in_state_const:
  "\<forall>y. P (Inactive)
  \<Longrightarrow> reply_unlink_tcb x21 xb
      \<lbrace>ct_in_state P :: det_state \<Rightarrow> _\<rbrace>"
  unfolding reply_unlink_tcb_def
  by (wpsimp wp: set_thread_state_ct_in_state hoare_vcg_if_lift2 hoare_drop_imp hoare_vcg_all_lift
                 get_simple_ko_wp)

lemma send_ipc_ct_in_state_const:
  "\<lbrakk>P Inactive; P Running; \<forall>y. P (BlockedOnReply y); \<forall>x z. P (BlockedOnSend x z)\<rbrakk>
   \<Longrightarrow> send_ipc block call badge can_grant can_donate thread epptr
       \<lbrace>ct_in_state P :: det_state \<Rightarrow> _\<rbrace>"
  unfolding send_ipc_def
  by (wpsimp wp: set_thread_state_ct_in_state hoare_drop_imp
                 reply_unlink_tcb_ct_in_state_const reply_push_ct_in_state_const
                 hoare_vcg_all_lift)

lemma handle_timeout_ct_not_BlockedOnNtfn[wp]:
  "handle_timeout ct tt \<lbrace>ct_not_BlockedOnNtfn :: det_state \<Rightarrow> _\<rbrace>"
  unfolding handle_timeout_def
  by (wpsimp wp: send_ipc_ct_in_state_const thread_set_ct_in_state simp: send_fault_ipc_def)

lemma end_timeslice_ct_not_BlockedOnNtfn[wp]:
  "end_timeslice t \<lbrace>ct_not_BlockedOnNtfn :: det_state \<Rightarrow> _\<rbrace>"
  unfolding end_timeslice_def
  by (wpsimp wp: hoare_drop_imp)

lemma charge_budget_ct_not_BlockedOnNtfn[wp]:
  "charge_budget consumeda t \<lbrace>ct_not_BlockedOnNtfn :: det_state \<Rightarrow> _\<rbrace>"
  unfolding charge_budget_def
  by (wpsimp wp: hoare_drop_imp)

lemma check_budget_ct_not_BlockedOnNtfn[wp]:
  "check_budget \<lbrace>ct_not_BlockedOnNtfn :: det_state \<Rightarrow> _\<rbrace>"
  unfolding check_budget_def
  by (wpsimp wp: hoare_drop_imp)
...
lemma ct_not_blocked_cur_sc_blocked_is_offset_trivial:
  ...
  apply (clarsimp simp: cur_sc_tcb_only_sym_bound_def tcb_at_kh_simps ct_in_state_def)
...
lemma update_time_stamp_cur_sc_blocked_is_offset_helper:
  ...
  apply (wpsimp wp: handle_invocation_valid_sched check_budget_restart_valid_sched |
...
(* These need to be resolved before we can say that we have solved handle_event_valid_sched.
   I believe that the first two statements are provable, but perhaps not the third. In either
   case, we need to prove these lemmas, or delete them and solve handle_event_valid_sched
   another way. *)
lemma handle_invocation_ct_conditions_temp[wp]:
  "handle_invocation False False True True reply_cptr
   \<lbrace>ct_not_in_release_q :: det_state \<Rightarrow> _\<rbrace>"
  "handle_invocation False False True True reply_cptr
   \<lbrace>ct_not_queued :: det_state \<Rightarrow> _\<rbrace>"
  "handle_invocation False False True True reply_cptr
   \<lbrace>ct_released :: det_state \<Rightarrow> _\<rbrace>"
  unfolding handle_invocation_def syscall_def
  apply (wpsimp wp: hoare_drop_imp)
...
lemma refill_bounded_helper2:
  "valid_refills (cur_sc s) s \<Longrightarrow>
   cur_sc_offset_ready (consumed_time s) s \<Longrightarrow>
   unat (cur_time s) + unat kernelWCET_ticks + 2 * unat MAX_SC_PERIOD \<le> unat (max_word :: time) \<Longrightarrow>
kheap s (cur_sc s) = Some (SchedContext sc n) \<Longrightarrow>
                  unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) +
                  unat (consumed_time s)
                  \<le> unat (max_word :: time)"
  unfolding cur_sc_offset_ready_def
  apply clarsimp
  apply (subgoal_tac "unat (sc_period sc) \<le> unat MAX_SC_PERIOD")
  apply (erule_tac order_trans[rotated])
  apply (rule_tac y="unat (r_time (refill_hd sc)) + unat (consumed_time s) + 2 * unat (sc_period sc)" in order_trans, simp)
  apply (rule_tac y="unat (r_time (refill_hd sc)) + unat (consumed_time s) + 2 * unat (sc_period sc)" in order_trans, simp)
  apply (erule add_mono, simp)
  apply (clarsimp simp: valid_refills_def vs_all_heap_simps, overflow_hammer)
  done

lemma refill_bounded_helper:
  "valid_refills (cur_sc s) s
   \<Longrightarrow> 2*unat MAX_SC_PERIOD \<le> unat (max_word :: time)
   \<Longrightarrow> (\<forall>sc. (\<exists>n. kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
                   unat (r_amount (refill_hd sc)) + unat MAX_SC_PERIOD \<le> unat (max_word :: time))"
  apply clarsimp
  apply (erule order_trans[rotated])
...
definition is_next_time
where "is_next_time a s \<equiv> \<exists>b. (a, b) \<in> fst (do_machine_op getCurrentTime s)"

definition pred_next_time
where "pred_next_time P s \<equiv> \<forall>a. is_next_time a s \<longrightarrow> P a"

lemma to_handle_final_time_obligationsAA:
  "\<lbrace>\<lambda>s. pred_next_time (\<lambda>nt. unat nt + unat MAX_SC_PERIOD \<le> unat (max_word :: time)) s\<rbrace>
    update_time_stamp
   \<lbrace>\<lambda>_ s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time)\<rbrace>"
  unfolding update_time_stamp_def
...
lemma asadadad[simp]:
  "pred_next_time (\<lambda>x. True) s = True"
  by (clarsimp simp: pred_next_time_def)

lemma update_time_stamp_cur_time[wp]:
  "\<lbrace>\<lambda>s. pred_next_time (\<lambda>x. Q x) s\<rbrace>
    update_time_stamp
...
lemma strengthen_consumed_time_bound:
  "unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time)
   ...
   \<Longrightarrow> unat (consumed_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time)"
  by linarith

lemma refill_bounded_helper3:
  "valid_refills (cur_sc s) s
   \<Longrightarrow> cur_sc_offset_ready 0 s
   \<Longrightarrow> current_time_bounded 3 s
   \<Longrightarrow> (\<forall>sc. (\<exists>n. kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
                   unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) +
                   unat (r_amount (refill_hd sc))
                   \<le> unat (max_word :: time)) "
  apply (clarsimp simp: current_time_bounded_def)
  apply (erule order_trans[rotated])
  apply (rule_tac y="(unat (cur_time s) + unat kernelWCET_ticks) + 2 * unat MAX_SC_PERIOD + unat MAX_SC_PERIOD" in order_trans)
   apply (rule add_le_mono)
    apply (rule add_le_mono)
     apply (clarsimp simp: cur_sc_offset_ready_def)
    apply (clarsimp simp: valid_refills_def vs_all_heap_simps, overflow_hammer)
...
lemma check_budget_restart_valid_sched_weaker:
  ...
  apply (erule order_trans[rotated])
  apply (rule_tac y="unat (r_time (refill_hd sc)) + unat (consumed_time s) + 2 * unat (sc_period sc)" in order_trans)
   apply clarsimp
  apply (rule add_le_mono)
   apply (clarsimp simp: cur_sc_offset_ready_def vs_all_heap_simps)
  apply (clarsimp simp: valid_refills_def vs_all_heap_simps, overflow_hammer)
  done
...
(* FIXME: move *)
abbreviation next_time_bounded
where "next_time_bounded k s \<equiv> pred_next_time (current_time_bounded_2 k) s"
...
lemma next_time_bounded_current_time_bounded:
  ...
  apply (clarsimp simp: min_def; intro allI impI conjI)
   ...
   apply (subst word_of_nat_minus)
    apply (rule_tac y="unat (max_word :: time)" in order_trans)
     apply (clarsimp simp: word_le_nat_alt[symmetric])
    apply (clarsimp simp: max_word_def)
...
lemma next_time_bounded_weaken1:
  "next_time_bounded 1 s \<Longrightarrow> pred_next_time (\<lambda>x. unat x + unat MAX_SC_PERIOD \<le> unat (max_word :: time)) s"
  by (erule pred_next_time_strengthen, clarsimp simp: current_time_bounded_def)

lemma current_time_bounded_weaken1:
  "current_time_bounded 2 s \<Longrightarrow> 2 * unat MAX_SC_PERIOD \<le> unat (max_word :: time)"
...
lemma sdfsfsdfsfsfsf:
  ...
  apply (rule bound_sc_tcb_at_eq_inj'; fastforce simp: sc_at_pred_n_eq_commute)
  apply (clarsimp simp: ct_in_state_def vs_all_heap_simps tcb_at_kh_simps)
  done
  ...
  (* SyscallEvent *)
       subgoal for syscall
         apply (case_tac syscall, simp_all add: handle_send_def handle_call_def liftE_bindE)
            ...
            apply (wpsimp wp: handle_yield_valid_sched check_budget_restart_valid_sched_weaker
                   | strengthen refill_bounded_helper refill_bounded_helper3)+
    ...
    subgoal
      apply wpsimp
         ...
         apply (wpsimp wp: check_budget_valid_sched_weaker hoare_vcg_all_lift)
       apply(rule_tac Q="\<lambda>_. valid_sched and invs and ct_not_in_release_q
         ...
         apply (intro conjI impI)
              apply fastforce
             apply (fastforce intro: invs_cur_sc_chargeableE)
            apply (frule_tac cons="consumed_time s" in sdfsfsdfsfsfsf; simp?)
            apply (fastforce elim: ct_in_state_weaken)
           apply (fastforce elim: ct_in_state_weaken)
...
lemma call_kernel_valid_sched_check_budget_helper:
  "\<lbrace>valid_sched
    ...
    and (\<lambda>s. cur_sc_blocked_is_offset (consumed_time s) s)
    and (\<lambda>s. valid_refills (cur_sc s) s)
    and (\<lambda>s. unat (cur_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time) \<and>
             unat (consumed_time s) + unat MAX_SC_PERIOD \<le> unat (max_word :: time))
    and (\<lambda>s. (\<forall>sc n. (kheap s (cur_sc s) = Some (SchedContext sc n)) \<longrightarrow>
                    unat (r_time (refill_hd sc)) + 2 * unat (sc_period sc) +
                    unat (consumed_time s)
