...
lemma refill_split_check_valid_sched_misc[wp]:
  "refill_split_check usage \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_time s) (cur_domain s)
                                   (cur_thread s) (idle_thread s)
                                   (ready_queues s) (release_queue s) (scheduler_action s)
                                   (last_machine_time_of s) (tcbs_of s)\<rbrace>"
...
lemma cancel_all_signals_loop_valid_sched:
  ...
  by (wpsimp wp: set_thread_state_pred_map_tcb_sts_of)
...
lemma strong_refill_ready_tcb_helper:
  " bound_sc_tcb_at bound target s \<Longrightarrow>
    valid_refills target b s \<Longrightarrow>
...
lemma refill_budget_check_valid_sched_action_act_not:
  sorry (* wait for new_refill_logic *)
...
lemma refill_split_check_schedulable_ipc_queues:
  "\<lbrace>schedulable_ipc_queues\<rbrace>
   refill_split_check usage
   \<lbrace>\<lambda>_. schedulable_ipc_queues\<rbrace>"
  unfolding refill_split_check_def
  sorry (* wait for new_refill_logic (this function deleted) *)
...
lemma send_ipc_valid_sched_subset_for_handle_timeout:
  "\<lbrace>valid_release_q and valid_ready_qs and weak_valid_sched_action and valid_blocked and valid_idle_etcb
    and schedulable_ipc_queues and (\<lambda>s. sym_refs (state_refs_of s))
    and not_in_release_q thread and not_queued thread and scheduler_act_not thread
    and tcb_at thread\<rbrace>
   send_ipc True False badge True False thread epptr
   \<lbrace>\<lambda>rv. (valid_release_q and
               valid_ready_qs and
               weak_valid_sched_action and valid_blocked and valid_idle_etcb) ::'state_ext state \<Rightarrow> _\<rbrace>"
  supply if_weak_cong[cong del]
  apply (simp add: send_ipc_def)
  apply (rule hoare_seq_ext [OF _ get_simple_ko_sp])
  apply (case_tac ep, simp_all)
    apply (wpsimp wp: set_thread_state_valid_release_q
                      set_thread_state_valid_ready_qs set_thread_state_weak_valid_sched_action
                      set_thread_state_valid_blocked)
   apply (wpsimp wp: set_thread_state_valid_release_q
                     set_thread_state_valid_ready_qs set_thread_state_weak_valid_sched_action
                     set_thread_state_valid_blocked)
  ...
  apply wpsimp
              ...
              apply (wpsimp wp: sts_st_tcb_at' hoare_vcg_disj_lift set_thread_state_valid_release_q
                                set_thread_state_valid_ready_qs
                                set_thread_state_weak_valid_sched_action set_thread_state_valid_blocked_const)
             apply wpsimp
          ...
          apply (rule_tac Q="\<lambda>_. valid_release_q and valid_ready_qs and weak_valid_sched_action
                             ...
                             and schedulable_if_bound_sc_thread dest" in hoare_strengthen_post[rotated])
         apply (clarsimp simp: obj_at_def is_tcb pred_tcb_at_def schedulable_sc_tcb_at_def vs_all_heap_simps)
          apply wpsimp
            apply (wpsimp wp: reply_push_weak_valid_sched_action_no_donation reply_push_valid_blocked_no_donation'
                              reply_push_typ_at tcb_at_typ_at reply_push_schedulable_if_bound_sc_thread_no_donation valid_idle_etcb_lift)
           apply (wpsimp wp: set_thread_state_valid_release_q
                             set_thread_state_valid_ready_qs set_thread_state_weak_valid_sched_action
                             set_thread_state_valid_blocked hoare_vcg_disj_lift)
          apply wpsimp
         apply (wpsimp wp: thread_get_wp)
        apply (wpsimp wp: get_tcb_obj_ref_wp)
       apply (rule_tac Q="\<lambda>r. valid_release_q and valid_ready_qs and weak_valid_sched_action and valid_blocked and valid_idle_etcb
                              ...
                              scheduler_act_not dest  and tcb_at dest and tcb_at thread and schedulable_if_bound_sc_thread dest"
              in hoare_strengthen_post[rotated])
        apply clarsimp
   ...
   apply (rule_tac Q="\<lambda>r. valid_release_q and valid_ready_qs and weak_valid_sched_action
                          ...
                          and tcb_at thread and not_queued thread and schedulable_if_bound_sc_thread dest and
  ...
  apply (clarsimp simp: obj_at_def)
  apply (subgoal_tac "ipc_queued_thread dest s")
  apply (clarsimp simp: schedulable_ipc_queues_defs tcb_at_kh_simps schedulable_sc_tcb_at_def pred_map_eq_def, fastforce)
...
lemma reply_remove_ct_active[wp]:
  "\<lbrace>(\<lambda>s. ct_active s \<or> ct_idle s)
    and (\<lambda>s. \<forall>t. reply_tcb_reply_at ((=) (Some t)) r s \<longrightarrow> cur_thread s \<noteq> t)\<rbrace>
     reply_remove t r
   \<lbrace>\<lambda>_ s::('state_ext state). (ct_active s \<or> ct_idle s)\<rbrace>"
  unfolding reply_remove_def
  apply (wpsimp wp: hoare_vcg_disj_lift)
...
lemma send_signal_WaitingNtfn_helper:
  ...
  "ntfn_obj ntfn = WaitingNtfn wnlist \<Longrightarrow>
   \<lbrace>ko_at (Notification ntfn) ntfnptr and
    ...
    valid_sched and invs and valid_machine_time\<rbrace>
  ...
  apply (wpsimp wp: possible_switch_to_valid_sched_strong)
        ...
        (active_sc_tcb_at tcbptr s \<longrightarrow> not_in_release_q tcbptr s \<longrightarrow> schedulable_if_bound_sc_thread tcbptr s)" in hoare_strengthen_post[rotated])
         ...
         apply (intro conjI; intro allI impI)
          apply (intro conjI; intro allI impI)
           apply (clarsimp simp: not_in_release_q_def schedulable_if_bound_sc_thread_def active_sc_tcb_at_defs)
        ...
        apply (wpsimp wp: maybe_donate_sc_valid_ready_qs
                          ...
                          maybe_donate_sc_valid_blocked_except_set maybe_donate_sc_cond_schedulable_if_bound_sc_thread)
       apply wpsimp
  ...
  apply (intro conjI)
   ...
   apply (clarsimp simp: pred_tcb_at_eq_commute st_tcb_at_def obj_at_def)
  apply (subst schedulable_if_bound_sc_thread_update_ntfn;
...
(* sym_refs *)
  ...
  sorry (* send_signal_WaitingNtfn_helper *)
...
lemma blocked_cancel_ipc_BOR_valid_sched':
  ...
  apply (wpsimp simp: get_thread_state_def get_blocking_object_def thread_get_def
        ...
        apply (rule_tac Q="\<lambda>r s. valid_sched s \<and> scheduler_act_not tcbptr s \<and>
         ...
         apply (clarsimp simp: valid_sched_def)
(*
        apply (wpsimp simp: get_blocking_object_def wp: get_simple_ko_wp reply_unlink_tcb_valid_sched)+
  ...
  sorry (* blocked_cancel_ipc_BOR_valid_sched' *)
...
lemma send_signal_BOR_helper:
  ...
  "ntfn_obj ntfn = IdleNtfn
   ...
   \<Longrightarrow> \<lbrace>st_tcb_at ((=) (BlockedOnReceive ep r_opt)) tcbptr and ko_at (Notification ntfn) ntfnptr and
        valid_sched and invs  and valid_machine_time and (\<lambda>s. tcbptr \<noteq> idle_thread s)\<rbrace>
  ...
  apply (wpsimp wp: possible_switch_to_valid_sched_strong)
    ...
    apply (rule_tac Q="\<lambda>xc s. (valid_sched_except_blocked s \<and> valid_blocked_except_set {tcbptr} s)
                       ...
                       \<and> st_tcb_at runnable tcbptr s \<and> valid_machine_time s \<and> schedulable_if_bound_sc_thread tcbptr s \<and> invs s"
                    in hoare_strengthen_post[rotated])
  ...
  by (auto dest: valid_sched_valid_blocked elim!: pred_tcb_weakenE
          ...
          intro!: valid_sched_not_runnable_scheduler_act_not
                  valid_sched_not_runnable_not_in_release_q
                  valid_eq_q_Blocked_on_Receive_schedulable_if_bound_sc_thread
                  valid_sched_not_runnable_not_queued
                  if_live_then_nonz_cap_invs) *)
  sorry (* send_signal_BOR_helper *)
...
lemma send_signal_valid_sched:
  "\<lbrace> valid_sched and invs and valid_machine_time\<rbrace>
...
lemma reschedule_valid_sched:
  "\<lbrace>valid_ready_qs and valid_release_q and ct_not_in_q and
    valid_sched_action and valid_blocked and valid_idle_etcb and schedulable_ipc_queues and valid_machine_time\<rbrace>
     reschedule_required
   \<lbrace>\<lambda>rv. valid_sched \<rbrace>"
  ...
  apply (wpsimp wp: tcb_sched_enqueue_valid_sched reschedule_valid_sched_const)
   ...
   apply (rule_tac Q="K (xa = the (Some True)) and
         (valid_ready_qs and valid_release_q and ct_not_in_q and
          ...
          valid_blocked and schedulable_ipc_queues and
    ...
    apply (wpsimp simp: thread_get_def wp: set_tcb_queue_wp get_tcb_queue_wp)
    apply (clarsimp simp: valid_sched_def)
    apply (rule conjI)
     apply (clarsimp simp: valid_ready_qs_def valid_sched_action_2_def tcb_sched_enqueue_def
                           weak_valid_sched_action_2_def vs_all_heap_simps
...
lemma invoke_domain_valid_sched:
  ...
  in hoare_strengthen_post[rotated])
   ...
   apply (wpsimp wp: valid_blocked_lift thread_set_domain_not_idle_valid_idle_etcb thread_set_domain_valid_ready_qs_not_q
                     thread_set_domain_act_not_valid_sched_action thread_set_domain_ct_in_cur_domain simp: valid_sched_def)
  ...
  sorry (* invoke_domain_valid_sched *)
...
lemma sched_context_resume_ready_and_sufficient:
  ...
  apply (case_tac sched; clarsimp)
   ...
   apply (case_tac "runnable ts \<and> 0 < sc_refill_max sc \<and>
    ...
    apply clarsimp *) subgoal sorry (* sched_context_resume_ready_and_sufficient *)
   apply wpsimp(*
   apply (clarsimp simp: is_schedulable_bool_def
                  split: option.splits dest!: get_tcb_SomeD) *) subgoal sorry (* sched_context_resume_ready_and_sufficient *)
  apply wpsimp
  apply (clarsimp simp: pred_tcb_at_def obj_at_def)
  apply (drule invs_sym_refs)(*
  apply (drule (1) ARM.sym_ref_tcb_sc[where tp=tcbptr])
  ...
  subgoal sorry (* sched_context_resume_ready_and_sufficient *)
  done
...
(*              apply (wpsimp wp: tcb_sched_enqueue_valid_sched)
  ...
  sorry (* sched_context_yield_to_valid_sched_helper2 *)

(* end *)

lemma sched_context_resume_valid_sched_misc[wp]:
  "sched_context_resume scp \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s)
                                   (idle_thread s) (scheduler_action s) (last_machine_time_of s) (kheap s)\<rbrace>"
  unfolding sched_context_resume_def
  by (wpsimp wp: hoare_drop_imp)
...
lemma refill_add_tail_valid_sched_misc[wp]:
  "refill_add_tail sc_ptr rfl \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s) (idle_thread s)
          (ready_queues s) (release_queue s) (scheduler_action s) (last_machine_time_of s)
          (etcbs_of s) (tcb_sts_of s) (tcb_scps_of s)\<rbrace>"
  unfolding refill_add_tail_def by wpsimp

lemma maybe_add_empty_tail_valid_sched_misc[wp]:
  "maybe_add_empty_tail sc_ptr \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s) (idle_thread s)
          (ready_queues s) (release_queue s) (scheduler_action s) (last_machine_time_of s)
          (etcbs_of s) (tcb_sts_of s) (tcb_scps_of s)\<rbrace>"
  unfolding maybe_add_empty_tail_def by wpsimp
...
crunches set_refills, refill_update, refill_new
  ...
  apply (intro iffI)
   apply (subgoal_tac "t \<noteq> sc_ptr")
    apply (clarsimp simp: vs_all_heap_simps obj_at_def)
    apply (rule_tac x=ref' in exI, clarsimp)
    apply (clarsimp simp: vs_all_heap_simps obj_at_def refills_ready_def sc_at_kh_simps)
    apply (case_tac "sc_refills sc"; simp)
   apply (clarsimp simp: vs_all_heap_simps obj_at_def refills_ready_def)
  apply (subgoal_tac "t \<noteq> sc_ptr")
   apply (clarsimp simp: vs_all_heap_simps obj_at_def split: if_splits)
   apply (rule_tac x=ref' in exI, clarsimp)
   apply (clarsimp simp: vs_all_heap_simps obj_at_def refills_ready_def sc_at_kh_simps)
   apply (case_tac "sc_refills sc"; simp)
  apply (clarsimp simp: vs_all_heap_simps obj_at_def refills_ready_def)
  done
  "\<lbrace>\<lambda>s. P (budget_sufficient t s) \<and> sc_refill_cfg_sc_at (\<lambda>l. length (scrc_refills l) \<ge> 1) sc_ptr s\<rbrace>
   refill_add_tail sc_ptr rfl
  apply (intro iffI)
   apply (subgoal_tac "t \<noteq> sc_ptr")
    apply (clarsimp simp: vs_all_heap_simps obj_at_def)
    apply (rule_tac x=ref' in exI, clarsimp)
    apply (clarsimp simp: vs_all_heap_simps obj_at_def sufficient_refills_def refills_capacity_def sc_at_kh_simps)
    apply (case_tac "sc_refills sc"; simp)
   apply (clarsimp simp: vs_all_heap_simps obj_at_def refills_ready_def)
  apply (subgoal_tac "t \<noteq> sc_ptr")
   apply (clarsimp simp: vs_all_heap_simps obj_at_def split: if_splits)
...
lemma sc_ready_times_cong:
  ...
  apply (clarsimp simp: vs_all_heap_simps)
  apply (rule sc_ready_times_cong)
  apply (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps)
  apply (clarsimp simp: sc_ready_times_2_def vs_all_heap_simps map_project_def sc_ready_time_def
                        sc_refill_cfg_sc_at_def obj_at_def intro!: ext)
  apply (case_tac "sc_refills sc"; simp)
  done
...
lemma update_sched_context_active_sc_tcb_at:
  "\<lbrace>\<lambda>s. active_sc_tcb_at t s
        \<and> (\<forall>sc. pred_map_eq (Some sc_ptr) (tcb_scps_of s) t
                \<longrightarrow> refill_max_pos (sc_refill_max (f sc)))\<rbrace>
   update_sched_context sc_ptr f
   \<lbrace>\<lambda>x s. active_sc_tcb_at t s\<rbrace>"
   apply (wpsimp wp: update_sched_context_wp)
   apply (clarsimp simp: vs_all_heap_simps obj_at_kh_kheap_simps split: if_splits, fastforce)
   done
...
lemma refill_update_valid_release_q:
  "\<lbrace>valid_release_q and (\<lambda>s.\<forall>t\<in>set (release_queue s). bound_sc_tcb_at (\<lambda>p. p = Some sc_ptr) t s \<longrightarrow> 0 < mrefills)\<rbrace>
   refill_update sc_ptr period budget mrefills
   \<lbrace>\<lambda>rv. valid_release_q\<rbrace>"
  unfolding refill_update_def valid_release_q_def maybe_add_empty_tail_def
  apply (wpsimp wp: hoare_vcg_all_lift hoare_vcg_imp_lift' refill_add_tail_active_sc_tcb_at
...
(*   apply (clarsimp simp: valid_ready_qs_def active_sc_tcb_at_defs st_tcb_at_kh_def not_queued_def
  ...
  sorry (* update_sched_context_valid_ready_qs_helper2 *)
...
lemma refill_new_valid_ready_qs:
  ...
  sorry (* refill_new_valid_ready_qs *)
...
lemma refill_new_valid_sched_action:
  ...
  by (clarsimp simp: valid_sched_action_def is_activatable_def weak_valid_sched_action_def simple_sched_action_def
    ...
    | safe)+
  sorry (* refill_new_valid_sched_action *)
...
lemma end_timeslice_valid_sched_subset:
  "\<lbrace>valid_release_q and valid_ready_qs and
    weak_valid_sched_action and valid_blocked and valid_idle_etcb  and
...
(*           apply (wpsimp wp: postpone_valid_release_q postpone_valid_ready_qs
  ...
  sorry (* end_timeslice_valid_sched_subset *)
...
lemma refill_full_inv[wp]:
  "\<lbrace>P\<rbrace> refill_full scp \<lbrace>\<lambda>r s. P s\<rbrace>"
  unfolding refill_full_def
  by wpsimp

lemma refill_budget_check_valid_sched_misc[wp]:
  "refill_budget_check con cap \<lbrace>\<lambda>s. P (consumed_time s) (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s) (idle_thread s)
          (ready_queues s) (release_queue s) (scheduler_action s) (last_machine_time_of s)
          (etcbs_of s) (tcb_sts_of s) (tcb_scps_of s)\<rbrace>"
  unfolding refill_budget_check_def
  by (wpsimp wp: refill_full_inv)

  sorry (* refill_budget_check_weak_valid_sched_action_act_not *)
  sorry (* refill_budget_check_valid_sched_action_act_not *)
lemma refill_budget_check_schedulable_ipc_queues:
  ...
  apply (wpsimp wp: refill_budget_check_valid_ready_qs_not_queued refill_budget_check_valid_release_q_not_in_release_q
                     refill_budget_check_valid_sched_action_act_not refill_budget_check_schedulable_ipc_queues
  done
...
(*   by (wpsimp wp: active_sc_tcb_at_update_sched_context_no_change
  ...
  sorry (* refill_budget_check_active_sc_tcb_at *)
...
lemma sc_with_tcb_propE1:
  ...
  by clarsimp
  sorry (* update_min_budget_merge_sufficient *)

(*
lemma update_sched_context_weak_valid_sched_action':
  "\<lbrace>weak_valid_sched_action
    and K (\<forall>sc. 0 < sc_refill_max sc \<longrightarrow> 0 < sc_refill_max (f sc))
    and K (\<forall>sc. sc_refills (f sc) = sc_refills (sc))\<rbrace>
      update_sched_context ref f
   \<lbrace>\<lambda>_. weak_valid_sched_action\<rbrace>"
  apply (simp add: update_sched_context_def)
  apply (wpsimp simp: set_object_def wp: get_object_wp)
  apply (clarsimp simp: weak_valid_sched_action_def vs_all_heap_simps obj_at_kh_kheap_simps
                        sc_active_def)
  apply (case_tac "ref'=ref", simp, blast)
  done
...
lemma update_sched_context_valid_blocked':
  ...
  apply (simp add: update_sched_context_def)
  apply (wpsimp simp: set_object_def wp: get_object_wp)
  apply (fastforce simp: vs_all_heap_simps obj_at_def refill_max_pos_def valid_blocked_defs
                  split: if_split_asm option.splits)
  done

lemma valid_ready_qs_schedulable_if_bound_sc_thread:
  "valid_ready_qs s \<Longrightarrow> in_ready_q t s \<Longrightarrow> schedulable_if_bound_sc_thread t s"
  by (clarsimp simp: valid_ready_qs_def in_ready_q_def obj_at_def split: option.splits)
...
lemma tcb_sched_dequeue_valid_blocked_except_set:
  ...
  unfolding tcb_sched_action_def
  apply (wpsimp wp: set_tcb_queue_wp get_tcb_queue_wp thread_get_wp)(*
  ...
  sorry (* tcb_sched_dequeue_valid_blocked_except_set *)
...
crunches check_budget
  for valid_sched_misc[wp]:
    "\<lambda>s::'state_ext state. P (cur_sc s) (cur_time s) (cur_domain s) (cur_thread s) (idle_thread s)
                             (last_machine_time_of s) (etcbs_of s)"
  (wp: crunch_wps simp: crunch_simps)
...
abbreviation
  ready_or_released
where
  "ready_or_released s \<equiv> ready_or_released2 (ready_queues s) (release_queue s)"

lemmas ready_or_released_def = ready_or_released2_def
...
lemma handle_invocation_valid_sched:
  "\<lbrace>invs and valid_sched and ready_or_released and ct_active and valid_machine_time
...
(*   by (wpsimp wp: set_thread_state_not_queued_valid_release_q gts_wp get_simple_ko_wp) *)
  sorry (* reply_unlink_tcb_valid_release_q *)
...
lemma tcb_ready_time_sc_replies_update[simp]:
  "\<lbrakk>active_sc_tcb_at x s; kheap s ref = Some (SchedContext sc n)\<rbrakk> \<Longrightarrow>
     tcb_ready_time x
        ...
        = tcb_ready_time x s"
  by (fastforce simp: vs_all_heap_simps tcb_ready_times_defs opt_map_def map_project_def
                      map_join_def sc_refill_cfgs_of_scs_def scs_of_kh_def
               split: option.split)

lemma reply_remove_tcb_valid_release_q[wp]:
  "\<lbrace> valid_release_q and not_in_release_q tp\<rbrace>
   reply_remove_tcb tp rp \<lbrace> \<lambda>_. valid_release_q:: 'state_ext state \<Rightarrow> _ \<rbrace>"
  apply (clarsimp simp: reply_remove_tcb_def)
(*   by (wpsimp wp: get_simple_ko_wp hoare_drop_imp update_sched_context_valid_release_q) *)
  sorry (* reply_remove_tcb_valid_release_q *)
...
lemma handle_interrupt_valid_machine_time[wp]:
  "handle_interrupt irq \<lbrace>valid_machine_time:: 'state_ext state \<Rightarrow> _\<rbrace>"
  unfolding handle_interrupt_def
...
lemma do_reply_transfer_valid_machine_time[wp]:
  "do_reply_transfer a b \<lbrace>\<lambda>s::'state_ext state. P (cur_time s) (last_machine_time_of s)\<rbrace>"
...
crunches invoke_irq_control, invoke_cnode, invoke_domain, invoke_tcb, send_signal, handle_fault
  for valid_machine_time[wp]: "\<lambda>s::'state_ext state. P (cur_time s) (last_machine_time_of s)"
  (wp: crunch_wps ignore: do_machine_op)

lemma perform_invocation_valid_machine_time[wp]:
  "perform_invocation a b c iv \<lbrace>valid_machine_time:: 'state_ext state \<Rightarrow> _\<rbrace>"
  by (case_tac iv; wpsimp)

lemma handle_invocation_valid_machine_time[wp]:
  "handle_invocation a b c d e \<lbrace>valid_machine_time:: 'state_ext state \<Rightarrow> _\<rbrace>"
  unfolding handle_invocation_def syscall_def(*
  by (wpsimp wp: hoare_drop_imp simp: lookup_cap_and_slot_def) *)
  sorry (* handle_invocation_valid_machine_time *)
 "\<lbrace>valid_machine_time and is_refill_ready scp :: 'state_ext state \<Rightarrow> _\<rbrace>
  ...
  sorry (* update_time_stamp_is_refill_ready *)
lemma handle_event_valid_machine_time[wp]:
  "handle_event e \<lbrace>valid_machine_time:: 'state_ext state \<Rightarrow> _\<rbrace>"
  apply (case_tac e; simp)
  subgoal for sc
