...
lemma delete_cap_simple_format:
  "\<lbrace><ptr \<mapsto>c cap \<and>* R>  and K (\<not> ep_related_cap cap)\<rbrace>
   delete_cap_simple ptr
   \<lbrace>\<lambda>_. <ptr \<mapsto>c NullCap \<and>* R>\<rbrace>"
  by (clarsimp simp: pred_conj_def delete_cap_simple_wp)

lemma sep_map_i_cdl_irq:
  "<irq \<mapsto>irq irq_obj \<and>* R> s \<Longrightarrow> cdl_irq_node s irq = irq_obj"
  apply (clarsimp simp: sep_map_c_def sep_map_irq_def sep_conj_def sep_state_projection_def
                        plus_sep_state_def sep_state_add_def map_add_def)
  apply (drule fun_cong[where x=irq])
  apply (clarsimp simp: sep_disj_sep_state_def sep_state_disj_def split: option.splits)
...
lemma invoke_irq_handler_clear_handler_wp:
  ...
  unfolding invoke_irq_handler_def
  apply clarsimp
  apply (wp sep_wp: delete_cap_simple_format[where cap=cap])
  apply clarsimp
...
lemma invoke_irq_handler_set_handler_wp:
  "\<lbrace><irq \<mapsto>irq obj \<and>* (obj, 0) \<mapsto>c cap' \<and>* R> and K (\<not> ep_related_cap cap' \<and> \<not> is_untyped_cap cap)\<rbrace>
  ...
  apply (wp sep_wp: insert_cap_child_wp insert_cap_sibling_wp
                    delete_cap_simple_format[where cap=cap'])
  apply clarsimp
  apply (frule sep_map_i_cdl_irq, clarsimp simp: get_irq_slot_def)
  apply sep_solve
  done
...
lemma invoke_irq_control_issue_handler_wp:
  ...
  by (clarsimp simp: invoke_irq_control_def | wp sep_wp: insert_cap_child_wp | sep_solve)+
...
lemma decode_invocation_irq_ack_rv':
  "\<lbrace>\<lambda>s. P (AckIrq (the $ cdl_cap_irq cap)) s \<rbrace>
   ...
   \<lbrace>P\<rbrace>, -"
  by (wpsimp simp: decode_irq_handler_invocation_def)
...
lemma decode_invocation_irq_endpoint_rv':
  "\<lbrace>\<lambda>s. P (SetIrqHandler (the $ cdl_cap_irq cap) endpoint_cap endpoint_ptr) s \<and>
        caps = [(endpoint_cap, endpoint_ptr)] @ xs \<and> is_ntfn_cap endpoint_cap\<rbrace>
   decode_irq_handler_invocation cap cap_ref caps IrqHandlerSetEndpointIntent
   \<lbrace>P\<rbrace>, -"
  apply (rule validE_R_gen_asm_conj)
  apply (wpsimp simp: decode_irq_handler_invocation_def get_index_def wp: throw_on_none_rv)
  done
...
lemma decode_irq_control_issue_irq_rv:
  "\<lbrace>\<lambda>s. P (IssueIrqHandler irq target_ref (cap_object root_cap, offset index r)) s \<and>
   ...
   decode_irq_control_invocation target target_ref caps
                                 (IrqControlIssueIrqHandlerIntent irq index depth)
   \<lbrace>P\<rbrace>, -"
...
lemma invoke_irq_control_issue_sgi_wp:
  "\<lbrace> <dest_slot \<mapsto>c cap \<and>* R> \<rbrace>
  invoke_irq_control (ArchIssueIrqHandler (ARMIssueSGISignal irq target control_slot dest_slot))
  \<lbrace>\<lambda>_. <dest_slot \<mapsto>c SGISignalCap irq target \<and>* R> \<rbrace>"
...
lemma reset_cap_cdl_cap_irq:
  "reset_cap_asid c = IrqHandlerCap irq \<Longrightarrow> the (cdl_cap_irq c) = irq"
  by (clarsimp simp: reset_cap_asid_def cdl_cap_irq_def the_def split: cdl_cap.splits)

lemma not_memory_reset:
  "\<not> is_memory_cap (reset_cap_asid cap) \<Longrightarrow> reset_cap_asid cap = cap "
  by (clarsimp simp: is_memory_cap_def reset_cap_asid_def split:cdl_cap.splits)

lemma not_ep_related_reset_cap_asid:
  "reset_cap_asid c = IrqControlCap \<Longrightarrow> \<not>ep_related_cap c"
  by (clarsimp simp: reset_cap_asid_def ep_related_cap_def split: cdl_cap.splits)

lemma reset_cap_asid_control:
  "reset_cap_asid c = reset_cap_asid cap \<Longrightarrow> is_irqcontrol_cap cap \<Longrightarrow> is_irqcontrol_cap c"
  by (clarsimp simp: reset_cap_asid_def is_irqcontrol_cap_def split: cdl_cap.splits)

(* Note: the cap from the TCB and the cap in the CNode, both pointing to the root CNode, should be
   different, but this breaks the proof.*)
lemma seL4_IRQHandler_IRQControl_Get_helper:
  ...
  shows
    "\<lbrace>\<guillemotleft>root_tcb_id \<mapsto>f root_tcb  \<and>* (root_tcb_id, tcb_pending_op_slot) \<mapsto>c RunningCap \<and>*
       irq \<mapsto>irq obj \<and>* cap_object root_cap \<mapsto>f CNode (empty_cnode root_size) \<and>*
     ...
     \<lbrace>\<lambda>fail. \<guillemotleft>root_tcb_id \<mapsto>f root_tcb \<and>*
              ...
              irq \<mapsto>irq obj \<and>* control_ptr \<mapsto>c c_cap \<and>* target_ptr \<mapsto>c irq_cap \<and>*
  ...
  apply (wp do_kernel_op_pull_back)
   apply (rule call_kernel_with_intent_allow_error_helper[where
                 ...
                 intent_op = "IrqControlIntent
                                (IrqControlIssueIrqHandlerIntent irq node_index node_depth)" and
                 tcb = t and
         ...
         apply (rule_tac P = "iv = InvokeIrqControl
                                     (IssueIrqHandler irq control_ptr (cap_object root_cap,
       ...
       apply (simp add: decode_invocation_def, wp)
        apply (rule_tac P = "irq_control_intent=IrqControlIssueIrqHandlerIntent irq node_index node_depth"
                        in hoare_gen_asmE, simp)
        apply (wp decode_irq_control_issue_irq_rv[where root_cap = root_cap and
...
lemma seL4_IssueSGISignal_helper:
  assumes unify: "irq_cap = SGISignalCap (ucast irq) (ucast target) \<and>
                  target_index = offset node_index root_size \<and>
                  root_index = offset croot root_size \<and>
                  control_index = offset control_cap root_size \<and>
                  target_ptr = (cap_object root_cap, target_index) \<and>
  ...
  shows
    "\<lbrace>\<guillemotleft>root_tcb_id \<mapsto>f root_tcb  \<and>* (root_tcb_id, tcb_pending_op_slot) \<mapsto>c RunningCap \<and>*
      ...
      K (\<not> ep_related_cap c_cap \<and> one_lvl_lookup root_cap word_bits root_size \<and>
         ...
         unat node_depth \<le> word_bits \<and> 0 < unat node_depth \<and>
         is_irqcontrol_cap c_cap \<and> is_cnode_cap root_cap \<and> is_cnode_cap root_cap)\<rbrace>
     seL4_IssueSGISignal control_cap irq target croot node_index node_depth
     \<lbrace>\<lambda>fail. \<guillemotleft>root_tcb_id \<mapsto>f root_tcb \<and>*
              (root_tcb_id, tcb_pending_op_slot) \<mapsto>c RunningCap \<and>*
              \<comment> \<open>Root CNode\<close>
              cap_object root_cap \<mapsto>f CNode (empty_cnode root_size) \<and>*
  ...
  apply (wp do_kernel_op_pull_back)
   apply (rule call_kernel_with_intent_allow_error_helper[where
                 ...
                 intent_op = "IrqControlIntent
                                (ArchIrqControlIssueIrqHandlerIntent
                                   (ARMIssueSGISignalIntent irq target node_index node_depth))" and
                 tcb = t and
                 intent_cptr = control_cap and
                 intent_extra = "[croot]", simplified])
               apply clarsimp
         ...
         apply (rule_tac P = "iv = InvokeIrqControl
                                     (ArchIssueIrqHandler
                                        (ARMIssueSGISignal (ucast irq) (ucast target) control_ptr
                                                           (cap_object root_cap,
                                                            offset node_index root_size)))"
                         in hoare_gen_asmEx)
         apply (clarsimp simp: unify)
...
lemma seL4_IssueSGISignal_Get:
  ...
  \<lbrace>\<lambda>fail.
    ...
    (cnode_id, control_index) \<mapsto>c IrqControlCap \<and>*
    (cnode_id, root_index) \<mapsto>c cnode_cap \<and>*
    (cnode_id, target_index) \<mapsto>c SGISignalCap (ucast irq) (ucast target) \<and>* R\<guillemotright>\<rbrace>"
  apply (rule hoare_gen_asm, elim conjE)
...
lemma seL4_IRQHandler_SetEndpoint_wp_helper:
  assumes unify: "irq_cap = IrqHandlerCap irq \<and>
                  ...
                  endpoint_ptr = (cnode_id, irq_handler_slot) \<and>
                  irq_ptr = (cnode_id, offset irq_handler_cap root_size) \<and>
                  root_tcb  = Tcb t \<and>
                  cnode_id = cap_object cnode_cap"
  shows
    "\<lbrace>\<guillemotleft>root_tcb_id \<mapsto>f root_tcb  \<and>* (root_tcb_id, tcb_pending_op_slot) \<mapsto>c RunningCap \<and>*
       irq \<mapsto>irq obj \<and>* (obj, 0) \<mapsto>c cap' \<and>* cnode_id \<mapsto>f CNode (empty_cnode root_size) \<and>*
       (root_tcb_id, tcb_cspace_slot) \<mapsto>c cnode_cap  \<and>* endpoint_ptr \<mapsto>c endpoint_cap \<and>*
       irq_ptr \<mapsto>c irq_cap \<and>* R\<guillemotright> and
      K (\<not>ep_related_cap cap' \<and> \<not>ep_related_cap irq_cap \<and> \<not>is_untyped_cap endpoint_cap \<and>
         \<not>is_memory_cap endpoint_cap \<and> one_lvl_lookup cnode_cap word_bits root_size \<and>
     ...
     \<lbrace>\<lambda>fail. \<guillemotleft>root_tcb_id \<mapsto>f root_tcb \<and>*
              ...
              irq \<mapsto>irq obj \<and>* (obj, 0) \<mapsto>c endpoint_cap \<and>* irq_ptr \<mapsto>c irq_cap \<and>* R\<guillemotright>\<rbrace>"
  apply (simp add: seL4_IRQHandler_SetEndpoint_def sep_state_projection2_def)
  apply (wp do_kernel_op_pull_back)
   apply (rule call_kernel_with_intent_allow_error_helper[where
                 check = True and
                 Perror = \<top> and
                 intent_op = "IrqHandlerIntent IrqHandlerSetEndpointIntent" and
                 tcb = t and
                 intent_cptr = irq_handler_cap and
                 intent_extra = "[endpoint_cptr]", simplified])
               apply clarsimp
              apply (wp set_cap_wp[sep_wand_wp] mark_tcb_intent_error_sep_inv)+
         apply (rule_tac P = "iv = InvokeIrqHandler (SetIrqHandler (the $ cdl_cap_irq irq_cap)
                                                                   endpoint_cap endpoint_ptr)"
                         in hoare_gen_asmEx)
         apply clarsimp
         apply (wp sep_wp: invoke_irq_handler_set_handler_wp set_cap_wp)+
       apply (rule_tac P = "c=irq_cap" in hoare_gen_asmEx)
       apply (simp add: unify decode_invocation_def)
       apply wp
        apply (rule_tac P = "rv = IrqHandlerSetEndpointIntent" in hoare_gen_asmE, simp)
        apply (wp decode_invocation_irq_endpoint_rv'[where endpoint_cap=endpoint_cap and
                                                           endpoint_ptr = endpoint_ptr and
                                                           xs = "[]"])
       apply (wpsimp simp: throw_opt_def get_irq_handler_intent_def)
    ...
    apply (wp hoare_vcg_ball_lift hoare_vcg_imp_lift hoare_vcg_ex_lift hoare_vcg_all_lift
  ...
  apply (intro impI conjI)
    ...
    apply (intro impI conjI allI)
         ...
         apply fastforce
        apply clarsimp
       apply (fastforce simp: not_memory_reset)
...
lemma seL4_IRQHandler_SetEndpoint_wp:
  "\<lbrace>\<guillemotleft>root_tcb_id \<mapsto>f root_tcb \<and>*
     ...
     (cnode_id, irq_handler_slot) \<mapsto>c IrqHandlerCap irq \<and>* R\<guillemotright> and
     K (is_tcb root_tcb \<and>
        cnode_id = cap_object cnode_cap \<and>
  ...
  apply (wp seL4_IRQHandler_SetEndpoint_wp_helper[where irq_handler_slot=endpoint_slot and
                                                        cap'=old_cap and t="obj_tcb root_tcb"])
  apply clarsimp
  apply (rule pred_conjI, sep_solve)
  apply (case_tac endpoint_cap; simp add: is_memory_cap_def cap_type_def)
  apply (case_tac old_cap; simp add: ep_related_cap_def cap_type_def)
  done
