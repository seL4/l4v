...
proof (simp, induct ids' arbitrary: ids)
  case Nil
  show ?case
...
lemma create_objects_wp:
  "\<lbrace> K (obj_refs = map (\<lambda>x. {x}) (map pick obj_refs) \<and> distinct (map pick obj_refs) ) and
     <(\<And>* ptr\<in>pick ` set obj_refs. ptr \<mapsto>o Untyped) \<and>* P> \<rbrace>
   create_objects obj_refs (Some obj)
   \<lbrace>\<lambda>r s. <(\<And>* ptr\<in>pick ` set obj_refs. ptr \<mapsto>o obj) \<and>* P > s \<rbrace>"
  ...
  apply (rule hoare_pre)
   ...
   apply simp
  apply (clarsimp simp flip: set_map)
  ...
  apply (clarsimp simp: zip_map1 zip_map2 set_zip_same Fun.comp_def split_def)
  apply (induct obj_refs; simp)
  apply (fastforce simp:sep_any_def)
  done

lemma nonempty_pick_in: (* FIXME: duplicate in drefine/Lemmas_D.thy *)
...
lemma update_range_simple:
  "\<lbrakk> idx \<noteq> 0; ptr \<le> ptr + of_nat idx \<rbrakk> \<Longrightarrow>
...
lemma reset_cap_asid_untyped_cap_eqD:
  "reset_cap_asid c = UntypedCap dev a b \<Longrightarrow> c = UntypedCap dev a b"
  using reset_cap_asid_simps2
  by blast
...
lemma dummy_detype_if_untyped:
  ...
  apply (clarsimp simp: plus_sep_state_def sep_state_add_def asid_reset_def sep_disj_sep_state_def
                        intent_reset_def update_slots_def object_wipe_slots_def sep_state_disj_def
  ...
  apply (case_tac obj)
    apply (clarsimp simp: plus_sep_state_def sep_state_add_def asid_reset_def sep_disj_sep_state_def
                          intent_reset_def update_slots_def object_wipe_slots_def
                          object_to_sep_state_def object_project_def object_clean_def
                          sep_state_disj_def)+
  done
...
proof (induct xs)
  case Nil
  show ?case
    by (wpsimp simp: mapME_x_def sequenceE_x_def)
next
  case Cons
  then
  show ?case
...
lemma reset_untyped_cap_wp_trivial':
  "\<lbrace><uref \<mapsto>c UntypedCap dev obj_range free_range \<and>* P> and (\<lambda>_. obj_range = free_range)\<rbrace>
   reset_untyped_cap uref
   \<lbrace>\<lambda>_. <uref \<mapsto>c UntypedCap dev obj_range free_range \<and>* P>\<rbrace>, -"
  by (wp reset_untyped_cap_wp_trivial)

lemma reset_untyped_cap_wp_trivial'':
  "\<lbrace><uref \<mapsto>c UntypedCap dev obj_range free_range \<and>* P> and (\<lambda>_. obj_range = free_range) and R ()\<rbrace>
   reset_untyped_cap uref
   \<lbrace>R\<rbrace>, -"
  unfolding validE_R_def
  apply (wpsimp wp: hoare_whenE_not mapME_x_wp simp: reset_untyped_cap_def)
  apply (sep_frule (direct) opt_cap_sep_imp, clarsimp dest!: reset_cap_asid_untyped_cap_eqD)
  done
...
lemma distinct_pick:
  ...
  by (induct xs; clarsimp)
     (fastforce simp: Ball_def)

lemma distinct_singleton_union_eq:
  "\<lbrakk>distinct xs; \<forall>S\<in>set xs. \<exists>v. {v} = S\<rbrakk> \<Longrightarrow> \<Union> (set xs) = pick ` (set xs)"
  by (induct xs; clarsimp)

lemma map_fst_zip_helper: (* FIXME: move to Lib *)
...
(* FIXME: move; consider moving next 3 to Lib *)
lemma fst_case_prod[simp]:
  "fst (case v of (x, y) \<Rightarrow> (f x, g y)) = f (fst v) "
  by (simp add: case_prod_unfold)

lemma snd_case_prod[simp]:
  "snd (case v of (x, y) \<Rightarrow> (f x, g y)) = g (snd v)"
  by (simp add: case_prod_unfold)

lemma case_prod_of_case_prod[simp]:
  "(case case v of (x, y) \<Rightarrow> (f x, g y) of (x', y') \<Rightarrow> f' x' y') = f' (f (fst v)) (g (snd v))"
  by (simp add: case_prod_unfold)

(* These terms below occur a lot in retype proofs. Abbreviate to make the goals more readable. *)
abbreviation
  "aligned_min_of r1 r2 nt ts \<equiv> alignUp (min_of_range r1 r2) (obj_bits_cdl nt ts)"

abbreviation
  "cond_min_of P r1 r2 nt ts \<equiv> if P then aligned_min_of r1 r2 nt ts else min_of_range r1 r2"
...
lemma invoke_untyped_wp:
  "\<lbrace>K (default_object nt ts minBound = Some obj \<and> nt \<noteq> UntypedType
       ...
       \<and> distinct created_range \<and> base_ptr = Min obj_range \<and> sz = (obj_bits_cdl nt ts)
       \<and> new_idx = unat (aligned_ptr + (of_nat n << sz) - base_ptr)
       \<and> (\<not> has_kids \<longrightarrow> aligned_ptr = min_of_range free_range obj_range)
       \<and> (has_kids \<longrightarrow> aligned_ptr = alignUp (min_of_range free_range obj_range) sz)
       \<and> (\<not> has_kids \<longrightarrow> free_range = obj_range)) and
    (\<lambda>s. cdl_current_domain s = minBound) and
    <\<And>* map (\<lambda>dest_slot. dest_slot \<mapsto>c NullCap) slots
   ...
   \<lbrace>\<lambda>_.
     <\<And>* map (\<lambda>(dest_slot, obj_refs). dest_slot \<mapsto>c (default_cap nt obj_refs ts dev))
              (zip slots created_range)
  ...
  apply (rule conjI, intro impI, clarsimp)
   ...
   apply (prop_tac "\<And>* map (\<lambda>x. fst x \<mapsto>c NullCap)
                            (zip slots (generate_range
                                          (aligned_min_of free_range obj_range nt ts)
                                          nt
                                          (obj_bits_cdl nt ts)
                                          (length slots)))
    ...
    apply (subst map_fst_zip_helper, simp)
   apply (clarsimp)
   apply (sep_cancel)+
   apply (clarsimp simp: generate_range_def zip_map2 map_fst_zip_helper comp_def)
  ...
  apply (rule sep_map_sep_foldI)+
  apply (clarsimp simp: generate_range_def)
  apply (subst map_fst_zip_helper, simp, sep_cancel+)
  apply (clarsimp simp: generate_range_def zip_map2 map_fst_zip_helper comp_def)
  done
...
lemma decode_untyped_invocation_rvu:
  "nt \<noteq> UntypedType \<Longrightarrow>
  ...
  apply (simp add: decode_invocation_def get_untyped_intent_def throw_opt_def
                   get_index_def throw_on_none_def decode_untyped_invocation_def mapME_x_singleton)
  apply (wpsimp wp: lookup_slot_for_cnode_op_rvu')
  done

lemma set_parent_has_children[wp]:
  "\<lbrace>\<top>\<rbrace> set_parent cref uref \<lbrace>\<lambda>r. has_children uref\<rbrace>"
  unfolding set_parent_def
  apply wpsimp
  apply (case_tac cref)
  apply (auto simp: is_cdt_parent_def has_children_def)
  done
...
lemma create_cap_has_children[wp]:
  ...
  by (wpsimp simp: simp: create_cap_def split_def)
...
lemma invoke_untyped_one_has_children:
  "uinv = (Retype uref nt ts [slot] has_kids (Suc 0)) \<Longrightarrow>
   \<lbrace>K (nt \<noteq> UntypedType)\<rbrace> invoke_untyped uinv \<lbrace>\<lambda>r. has_children uref\<rbrace>, -"
  apply (simp add: invoke_untyped_def)
  apply wp
       apply (rule_tac P = "zip [slot] (generate_range
                                          (cond_min_of has_kids
                                                       (available_range untyped_cap)
                                                       (untyped_cap_range untyped_cap)
                                                       nt ts)
                                          nt
                                          (obj_bits_cdl nt ts)
                                          (Suc 0))
                            \<noteq> []" in hoare_gen_asm)
...
lemma invoke_untyped_exception:
  "uinv = (Retype uref nt ts [dest_slot] has_kids (Suc 0)) \<Longrightarrow>
   \<lbrace>K (\<not> has_kids \<longrightarrow> obj_range = free_range) and
    <dest_slot \<mapsto>c NullCap \<and>* uref \<mapsto>c UntypedCap dev obj_range free_range \<and>* sep_true>\<rbrace>
   invoke_untyped uinv
...
lemma invoke_untyped_one_wp:
  ...
  defines "aligned_ptr b \<equiv> cond_min_of b free_range obj_range nt ts"
  defines "base_ptr \<equiv> Min obj_range"
  shows
    "uinv = (Retype uref nt ts [dest_slot] has_kids (Suc 0)) \<Longrightarrow>
     \<lbrace>K (default_object nt ts minBound = Some obj \<and> nt \<noteq> UntypedType
         \<and> aligned_ptr has_kids \<in> tot_free_range \<and> finite tot_free_range
         \<and> (\<not> has_kids \<longrightarrow> obj_range = free_range)) and
      (\<lambda>s. cdl_current_domain s = minBound) and
      <dest_slot \<mapsto>c NullCap
     ...
     \<lbrace>\<lambda>_ s.
        <dest_slot \<mapsto>c (default_cap nt {aligned_ptr has_kids} ts dev)
         ...
         \<and>* uref \<mapsto>c UntypedCap dev obj_range
                                (update_range obj_range base_ptr
                                              (unat (aligned_ptr has_kids +
                                                     (1 << obj_bits_cdl nt ts) - base_ptr)))
         \<and>* P> s\<rbrace>, -"
  apply simp
  apply (wp invoke_untyped_wp[where free_range = free_range and
                                    obj_range = obj_range and
                                    aligned_ptr = "aligned_ptr has_kids" and
                                    sz = "obj_bits_cdl nt ts" and
                                    obj = obj and
                                    base_ptr = base_ptr,
  ...
  "\<lbrakk> cap_object root_cnode_cap = root_cnode;
   ...
   \<lbrace> K (nt \<noteq> UntypedType) and
     \<guillemotleft>root_tcb_id \<mapsto>f (Tcb tcb)
      ...
      \<and>* P\<guillemotright> and
    (\<lambda>s. if has_children (root_cnode,ucptr_slot) (kernel_state s)
         then aligned_ptr = aligned_min_of free_range obj_range nt (unat ts)
         else obj_range = free_range \<and>
              aligned_ptr = min_of_range free_range obj_range)\<rbrace>
   ...
   apply (rule_tac ?Pd2.0 = "\<lambda>s'. has_children (root_cnode, ucptr_slot) s' =
        ...
        apply (rule hoare_vcg_conj_elimE[where P=P and P'=P for P, simplified, rotated])
         ...
         apply (rule hoare_vcg_conj_liftE_R)
          apply (rule invoke_untyped_one_wp[
                        ...
                        simplified sep_wp_simp, THEN sep_lift_generic_R, simplified])
          apply clarsimp
          apply (rule_tac s = "has_children (cap_object root_cnode_cap, offset ucptr root_size) s"
                          in trans,
                 solves simp, rule refl)
         apply (wp invoke_untyped_one_has_children[where
                     nt = nt and
                     ts = "unat ts" and
                     slot = "(cap_object root_cnode_cap, offset ncptr root_size)"])
         apply clarsimp
         apply (rule_tac s = "has_children (cap_object root_cnode_cap, offset ucptr root_size) s"
                         in trans,
                solves simp, rule refl)
        apply (rule hoare_pre)
         apply (rule_tac has_kids = "has_children (cap_object root_cnode_cap,
                                                   offset ucptr root_size) s"
                         in invoke_untyped_exception[where
                              dest_slot = "(cap_object root_cnode_cap, offset ncptr root_size)" and
                              uref = "(cap_object root_cnode_cap, offset ucptr root_size)" and
                              nt = nt and
                              ts = "unat ts" and
      ...
      apply (rule_tac P = "nt \<noteq> UntypedType \<and> c = UntypedCap dev obj_range free_range \<and>
                           cs = [(root_cnode_cap,(root_cnode,offset croot root_size))]"
                      in hoare_gen_asmEx)
      apply (simp only:)
     ...
     apply (rule wp_no_exception_seq)
      apply wp
     apply (wp lookup_cap_and_slot_rvu[where r = root_size and cap' = "root_cnode_cap"])
    apply (wp lookup_cap_and_slot_rvu[where r = root_size and
                                            cap = root_cnode_cap and
                                            cap' = "UntypedCap dev obj_range free_range"])
...
lemma invoke_untyped_cdt_inc[wp]:
  ...
  apply (case_tac uinv, simp)
  apply (wpsimp wp: mapM_x_wp[OF _ subset_refl] set_parent_other unless_wp unlessE_wp
                simp: create_cap_def)
...
lemma object_at_cdl_cdt[simp]:
  "object_at P ptr (s\<lparr>cdl_cdt := a\<rparr>) = object_at P ptr s"
...
lemma lookup_cap_rvu':
  ...
  apply (wpsimp wp: lookup_slot_rvu[where cnode_cap=cnode_cap] get_cap_rv)
  apply safe
...
crunch  handle_pending_interrupts, lookup_cap
  for cdl_current_thread[wp]: "\<lambda>s. P (cdl_current_thread s)"
  (wp: hoare_drop_imps)

lemma update_thread_no_pending:
  "\<lbrace>no_pending and
    K (\<forall>x. (case cdl_tcb_caps x tcb_pending_op_slot of
              Some cap \<Rightarrow> \<not> is_pending_cap cap
            | _ \<Rightarrow> True)\<longrightarrow>
           (case cdl_tcb_caps (t x) tcb_pending_op_slot of
              Some cap \<Rightarrow> \<not> is_pending_cap cap
            | _ \<Rightarrow> True))\<rbrace>
   update_thread thread_ptr t
...
lemma update_thread_tcb_at:
  "\<lbrace>K (\<forall>x. P (t x))\<rbrace>
   update_thread thread_ptr t
   \<lbrace>\<lambda>_. tcb_at' P thread_ptr\<rbrace>"
...
lemma detype_one_wp:
  "<obj_id \<mapsto>o - \<and>* R> s \<Longrightarrow> <obj_id \<mapsto>o Untyped \<and>* R> (detype {obj_id} s)"
  apply (clarsimp simp: detype_def sep_any_exist)
  apply (clarsimp simp: sep_state_projection_def sep_map_o_conj sep_any_def)
  apply (rule conjI)
   apply (rule ext)
   apply (clarsimp simp: object_project_def restrict_map_def object_to_sep_state_def)
...
lemma detype_set_sep_helper:
  "\<lbrakk> finite S; <(\<And>* x\<in> S. x \<mapsto>o -) \<and>* R> s \<rbrakk> \<Longrightarrow> <(\<And>* x \<in> S. x \<mapsto>o Untyped) \<and>* R> (detype S s)"
...
crunch insert_cap_child, corrupt_ipc_buffer,
  ...
  for cdl_parent[wp]: "\<lambda>s. cdl_cdt s slot = Some parent"
   (wp: crunch_wps set_parent_other simp: crunch_simps corrupt_intents_def)
...
lemma transfer_caps_loop_cdl_parent:
  ...
  apply (wpsimp wp: crunch_wps simp: crunch_simps | assumption)+
  done
...
lemma set_object_no_pending:
  "\<lbrace>no_pending and K (\<forall>cap. object_slots x tcb_pending_op_slot = Some cap \<longrightarrow> \<not> is_pending_cap cap)\<rbrace>
   set_object a x
   \<lbrace>\<lambda>_. no_pending\<rbrace>"
...
lemma set_cap_no_pending[wp]:
  "snd slot = tcb_pending_op_slot \<longrightarrow> \<not> is_pending_cap cap \<Longrightarrow> set_cap slot cap \<lbrace>no_pending\<rbrace>"
  apply (simp add: set_cap_def)
  apply (cases slot, simp)
  apply (rename_tac oid' idx)
  ...
  apply clarsimp
  apply (rename_tac obj)
  apply (intro conjI impI allI; clarsimp simp: opt_cap_def slots_of_def)
  apply (case_tac obj; fastforce simp: object_slots_def update_slots_def)
  done
...
lemma default_object_no_pending_cap:
  "\<lbrakk> default_object b c d = Some obj; object_slots obj tcb_pending_op_slot = Some cap\<rbrakk>
...
lemma reset_untyped_cap_no_pending[wp]:
  ...
  apply wp
      apply (rule_tac P = "snd cref = tcb_pending_op_slot \<longrightarrow> \<not> is_pending_cap cap"
                      in hoare_gen_asmEx)
...
lemma is_pending_cap_reset_cap_asid:
  "reset_cap_asid c = reset_cap_asid c' \<Longrightarrow> is_pending_cap c = is_pending_cap c'"
  by (case_tac c'; clarsimp simp: is_pending_cap_def dest!:reset_cap_asid_simps2)

lemmas is_pending_cap_simps[simp] = is_pending_cap_def[split_simps cdl_cap.split]

lemma map_nonempty_split: "xs \<noteq> [] \<Longrightarrow> map f xs = f (hd xs)# map f (tl xs)"
  by (induct xs; clarsimp)

lemma zip_generate_range_single:
  "\<lbrakk>type \<noteq> UntypedType; n \<noteq> 0\<rbrakk> \<Longrightarrow> zip [x] (generate_range start type sz n) = [(x, {start})]"
  apply (simp add: generate_range_def comp_def)
  apply (case_tac "[0..<n] = []"; simp add: map_nonempty_split)
  done

lemma create_cap_touch_wp:
  "create_cap new_type sz parent_slot dev xs \<lbrace>< (fst xs) \<mapsto>c - \<and>* R >\<rbrace>"
...
lemma sep_map_list_conjD:
  "\<lbrakk>(\<And>* map P xs) s; \<And>v s. \<lbrakk>v \<in> set xs; P v s\<rbrakk> \<Longrightarrow> P' v s\<rbrakk>
...
lemma retype_region_touch_wp:
  "\<lbrace>\<lambda>s. obj_refs = map ((\<lambda>x. {x}) \<circ> pick) obj_refs \<and>
        distinct (map pick obj_refs) \<and>
        target_type \<noteq> UntypedType \<and>
        <(\<And>* ptr\<in>set (map pick obj_refs). ptr \<mapsto>o -) \<and>* P> s\<rbrace>
   retype_region target_bits target_type obj_refs
   \<lbrace>\<lambda>_. <(\<And>* ptr\<in>set (map pick obj_refs). ptr \<mapsto>o -) \<and>* P>\<rbrace>"
  apply (clarsimp simp: retype_region_def)
  apply (wpsimp wp: create_objects_touch_wp[simplified, THEN sep_lift_generic, simplified])
  apply (rule conjI)
   apply (cases target_type; clarsimp simp: default_object_def)
  apply sep_solve
  done

lemma update_available_range_touch_wp:
  "\<lbrace><cap_ref \<mapsto>c - \<and>* P>\<rbrace>
   update_available_range ptr idx cap_ref cap
   \<lbrace>\<lambda>_. <cap_ref \<mapsto>c - \<and>* P>\<rbrace>"
...
lemma invoke_untyped_touch_wp:
  "invocation = Retype uref nt (unat ts) [dest_slot] has_kids (Suc 0) \<Longrightarrow>
   \<lbrace>\<lambda>s. (\<not>has_kids \<longrightarrow> obj_range = free_range) \<and> nt \<noteq> UntypedType \<and>
        <dest_slot \<mapsto>c NullCap \<and>*
         uref \<mapsto>c UntypedCap dev obj_range free_range \<and>*
         (cond_min_of has_kids free_range obj_range nt (unat ts)) \<mapsto>o - \<and>* R> s \<rbrace>
   invoke_untyped invocation
   \<lbrace>\<lambda>_. <uref \<mapsto>c - \<and>* dest_slot \<mapsto>c - \<and>*
         (cond_min_of has_kids free_range obj_range nt (unat ts)) \<mapsto>o - \<and>* R>\<rbrace>"
  apply (rule hoare_name_pre_state)
  apply (clarsimp simp:  invoke_untyped_def)
  ...
  "invocation = Retype uref nt (unat ts) [dest_slot] has_kids (Suc 0) \<Longrightarrow>
   \<lbrace>\<lambda>s. (\<not>has_kids \<longrightarrow> obj_range = free_range) \<and> nt \<noteq> UntypedType \<and>
        <dest_slot \<mapsto>c NullCap \<and>*
         ...
         (cond_min_of has_kids free_range obj_range nt (unat ts)) \<mapsto>o - \<and>*  R> s \<rbrace>
   invoke_untyped invocation
   \<lbrace>\<lambda>_. <uref \<mapsto>c - \<and>*
         dest_slot \<mapsto>c - \<and>*
         (cond_min_of has_kids free_range obj_range nt (unat ts)) \<mapsto>o - \<and>* R>\<rbrace>, -"
  apply (simp only:)
...
abbreviation (input) retype_with_kids :: "cdl_invocation \<Rightarrow> bool" where
  "retype_with_kids uinv \<equiv>
     case uinv of (InvokeUntyped (Retype uref nt ts dest has_kids n)) \<Rightarrow> has_kids"
...
lemma seL4_Untyped_Retype_inc_no_preempt:
  "\<lbrakk> cap_object root_cnode_cap = root_cnode;
     ...
     one_lvl_lookup root_cnode_cap 32 root_size;
     guard_equal root_cnode_cap ucptr 32;
     guard_equal root_cnode_cap croot 32 \<rbrakk> \<Longrightarrow>
   \<lbrace> K (nt \<noteq> UntypedType \<and> default_object  nt (unat ts) minBound = Some obj) and
     \<guillemotleft>root_tcb_id \<mapsto>f (Tcb tcb)
      \<and>* (root_tcb_id, tcb_pending_op_slot) \<mapsto>c RunningCap
      \<and>* (cap_object root_cnode_cap \<mapsto>f CNode (empty_cnode root_size))
      \<and>* (root_cnode, ucptr_slot) \<mapsto>c UntypedCap dev obj_range free_range
     ...
     (\<lambda>s. if has_children (root_cnode,ucptr_slot) (kernel_state s)
          then aligned_ptr = aligned_min_of free_range obj_range nt (unat ts)
          else obj_range = free_range \<and> aligned_ptr = min_of_range free_range obj_range) and
     (\<lambda>s. cdl_cdt (kernel_state s) child = Some parent) \<rbrace>
   seL4_Untyped_Retype ucptr nt ts croot node_index 0 ncptr_slot_nat 1
   \<lbrace>\<lambda>_ s. cdl_cdt (kernel_state s) child = Some parent\<rbrace>"
  apply (simp add: seL4_Untyped_Retype_def sep_state_projection2_def)
  apply (rule hoare_name_pre_state)
  apply (rule hoare_pre)
   apply (rule do_kernel_op_pull_back)
   apply (rule_tac ?Pd2.0="\<lambda>s'. has_children (root_cnode, ucptr_slot) s' =
                                has_children (root_cnode, ucptr_slot) (kernel_state s)
                                \<and> Q s'" for Q
                   in call_kernel_with_intent_allow_error_helper[where
                        check= False and tcb = tcb and Q = Q and Perror = Q for Q,simplified])
               apply fastforce
              apply wp+
         apply (rule_tac P = "\<exists>has_kids. iv = InvokeUntyped (Retype (root_cnode, ucptr_slot)
                                                                    nt
                                                                    (unat ts)
                                                                    [(root_cnode, ncptr_slot)]
                                                                    has_kids 1)"
         ...
         apply (rule hoare_vcg_conj_elimE[where P = P and P' = P for P, simplified, rotated])
          ...
          apply (rule hoare_strengthen_postE_R[OF hoare_vcg_conj_liftE_R])
           ...
           apply (rule invoke_untyped_touch_wp_R [where
                         ...
                         and dest_slot = "(cap_object root_cnode_cap, offset ncptr root_size)"
                         and free_range = free_range
                         and nt = nt
                         and ts = ts
                         and obj_range = obj_range,
                         simplified sep_wp_simp, THEN sep_lift_generic_R])
           apply clarsimp
           apply (rule_tac s="retype_with_kids (InvokeUntyped (Retype _ _ _ _ has_kids _))" in trans,
                  simp, rule refl)
          apply clarsimp
          apply (rule conjI)
          ...
          apply fastforce
         apply (rule hoare_pre)
          apply (rule invoke_untyped_exception[where free_range = free_range and
                                                     obj_range = obj_range],
                 simp)
         apply clarsimp
         apply sep_solve
     ...
     apply (rule hoare_weaken_preE)
      apply (rule lookup_cap_and_slot_rvu[where r = root_size and
                                                cap' = "UntypedCap dev obj_range free_range"])
     apply force
    apply (wp hoare_vcg_ball_lift update_thread_intent_update
              hoare_vcg_imp_lift hoare_vcg_ex_lift hoare_vcg_all_lift
              is_cnode_cap_guard_equal update_thread_intent_update)
   apply (rename_tac s')
   apply clarsimp
   apply (case_tac "has_children (cap_object root_cnode_cap, offset ucptr root_size) s'", clarsimp)
