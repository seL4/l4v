...
sublocale detype_locale'_gen by (unfold_locales; fact AARCH64.arch_deletionIsSafe)
...
lemma createObject_cte_wp_at'[Detype_R_assms]:
  ...
  apply (wpsimp wp: createObjects_orig_cte_wp_at'[where sz = "(Types_H.getObjectSize ty us)"]
                    threadSet_cte_wp_at'
         | clarsimp simp: AARCH64_H.createObject_def placeNewDataObject_def
...
lemma createObject_gsUntypedZeroRanges_commute[Detype_R_assms]:
  ...
  apply (simp add: global.createObject_def AARCH64_H.createObject_def
...
lemma createObject_pspace_aligned_distinct':
  ...
  apply (rule hoare_pre)
   apply (wp placeNewObject_pspace_aligned' unless_wp
          ...
          | simp add: AARCH64_H.createObject_def Retype_H.createObject_def gen_objBits_simps
