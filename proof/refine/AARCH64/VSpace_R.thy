...
(* FIXME AARCH64: checkPTAt currently used in both contexts (Normal and VSRoot) -- if we later need
                  more precision here (= VSRootPT_T), we'll need to split checkPTAt into separate
                  functions *)
...
crunches findFreeVMID, loadVMID
  for no_0_obj'[wp]: no_0_obj'
...
crunches vcpuDisable, vcpuEnable, vcpuSave, vcpuRestore, deleteASID
  for no_0_obj'[wp]: no_0_obj'
