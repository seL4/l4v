...
definition smc_invocation_map :: "smc_invocation \<Rightarrow> smcinvocation" where
  "smc_invocation_map smci \<equiv> case smci of AARCH64_A.SMCCall args \<Rightarrow> SMCCall args"
...
where
  "archinv_relation ai ai' \<equiv> case ai of
   ...
   | arch_invocation.InvokeSMCCall smci \<Rightarrow>
       ai' = InvokeSMCCall (smc_invocation_map smci)"
...
where
  "valid_arch_inv' ai \<equiv> case ai of
   ...
   | InvokeSMCCall _ \<Rightarrow> \<top>"
...
lemma ARMMMU_improve_cases:
  "(if isFrameCap cap then Q
    ...
    else if isSMCCap cap then W
    else if isSGISignalCap cap then X
    else undefined)
    =
   (if isFrameCap cap then Q
    else if isPageTableCap cap \<and> capPTType cap = NormalPT_T then R
...
lemmas arch_decodeInvocation_inv = ArchRetypeDecls_H_AARCH64_H_decodeInvocation_inv
...
lemma decodeARMFrameInvocation_corres:
  ...
  supply corres_throwError_ser[corres del]
...
declare corres_gets_gicvcpu_numlistregs[corres]
...
lemma decodeSMCInvocation_corres:
  "corres (ser \<oplus> archinv_relation) invs invs'
          (decode_smc_invocation (mi_label mi) args (arch_cap.SMCCap smc_badge))
          (decodeSMCInvocation (mi_label mi) args (SMCCap smc_badge))"
  unfolding decodeSMCInvocation_def decode_smc_invocation_def
...
lemma arch_decodeInvocation_corres[Arch_R_assms]:
  ...
  \<comment> \<open>SMC\<close>
  apply (simp add: isCap_simps)
  apply (corres corres: decodeSMCInvocation_corres)
  done
...
lemma arch_performInvocation_corres[Arch_R_assms]:
  ...
  apply (cases ai)
   ...
   \<comment> \<open>InvokeSMCCall\<close>
  apply (simp add: perform_smc_invocation_def performSMCInvocation_def)
  apply (rename_tac smc_inv, case_tac smc_inv)
  apply (simp add: smc_invocation_map_def)
  apply (corres corres: corres_machine_op | corres_cases_both)+
  done
...
lemma arch_decodeInvocation_wf[wp]:
  ...
  apply (cases arch_cap)
   ...
   \<comment> \<open>SMCCap\<close>
   apply (clarsimp simp: AARCH64_H.decodeInvocation_def decodeSMCInvocation_def)
...
lemma assoc_invs':
  ...
  apply (wpsimp wp: sch_act_wf_lift tcb_in_cur_domain'_lift valid_queues_lift
                    ...
                    setVCPU_valid_arch' valid_dom_schedule'_lift
...
lemma performSMCInvocation_invs[wp]:
  "performSMCInvocation args \<lbrace>invs'\<rbrace>"
