...
lemma deriveCap_derived:
  ...
  apply (rename_tac arch_capability)
  apply (case_tac arch_capability;
         simp add: AARCH64_H.deriveCap_def Let_def isCap_simps
              split: if_split,
         safe)
...
lemma arch_deriveCap_untyped_derived[wp]:
  ...
  apply (wpsimp simp: AARCH64_H.deriveCap_def Let_def untyped_derived_eq_ArchObjectCap
           split_del: if_split
  ...
  assumes updated_relations:
    "\<And>s s'. \<lbrakk> P s; P' s'; (s, s') \<in> state_relation \<rbrakk>
              \<Longrightarrow> cdt_relation ((\<noteq>) None \<circ> new_caps s) (new_mdb s) (new_ctes s')
                  ...
                  \<and> new_pt_types s = gsPTTypes (new_as' s')"
...
proof -
  ...
  show ?thesis
    ...
    apply (rule corres_underlying_decomposition[OF pspace_corres])
     ...
     apply (wpsimp wp: hoare_vcg_conj_lift mdb_wp rvk_wp list_wp updates abs_irq_together simp: pt_types_of_heap_eq)+
...
lemma ex_nonz_tcb_cte_caps':
  ...
  apply (subgoal_tac "s \<turnstile>' cteCap cte")
   ...
   apply (clarsimp simp: valid_cap'_def obj_at'_def dom_def AARCH64.typ_at_to_obj_at_arches
...
lemma setupReplyMaster_invs'[wp]:
  ...
  apply (simp add: invs'_def valid_state'_def)
   apply (wpsimp wp: setupReplyMaster_valid_pspace' sch_act_wf_lift tcb_in_cur_domain'_lift
                     ...
                     valid_bitmaps_lift valid_irq_node_lift valid_dom_schedule'_lift
          | rule refl)+
...
lemma arch_update_setCTE_mdb[CSpace_R_2_assms]:
  ...
  apply (rule conjI)
   ...
   apply (clarsimp split: if_split_asm)
     apply (clarsimp simp: gen_isCap_simps valid_arch_badges_def)
    prefer 3
...
lemma setUntypedCapAsFull_safe_parent_for'[CSpace_R_2_assms]:
  ...
  apply (intro conjI impI)
   ...
   apply (subgoal_tac "mdb_inv_preserve (ctes_of s)
    ...
    apply (clarsimp simp: gen_isCap_simps modify_map_def cte_wp_at_ctes_of simp del: fun_upd_apply)
...
lemma maskedAsFull_revokable_safe_parent[CSpace_R_2_assms]:
  ...
  apply (clarsimp simp: Retype_H.isCapRevocable_def AARCH64_H.isCapRevocable_def maskedAsFull_def
...
lemma dest_no_parent_n:
  ...
  apply (clarsimp simp: is_simple_cap'_def Retype_H.isCapRevocable_def AARCH64_H.isCapRevocable_def
                 split: capability.splits arch_capability.splits;
         clarsimp simp: isCap_simps)
   apply (clarsimp simp: sameRegionAs_def2 safe_parent_for'_def safe_parent_for_arch'_def isCap_simps)
