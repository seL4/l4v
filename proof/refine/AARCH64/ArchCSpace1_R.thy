...
lemma sameRegionAs_SGISignalCap[simp]:
  ...
  by (auto simp add: sameRegionAs_def AARCH64_H.sameRegionAs_def isCap_simps split: if_splits)
...
lemma same_arch_region_as_relation:
  ...
  by (cases c; cases c')
     (auto simp: AARCH64_H.sameRegionAs_def sameRegionAs_def Let_def isCap_simps mask_def
...
lemma updateCapData_Master:
  ...
  apply (cases cap, simp_all add: updateCapData_def isCap_simps Let_def
                           split: if_split_asm)
  apply (rename_tac arch_capability)
  apply (case_tac arch_capability, simp_all add: AARCH64_H.updateCapData_def)
  done
...
lemma capASID_mask [simp]:
  ...
  apply (rename_tac arch_capability)
  apply (case_tac arch_capability,
         simp_all add: maskCapRights_def AARCH64_H.maskCapRights_def isCap_simps Let_def)
  done
...
arch_requalify_facts revokable_plus_orderD (*FIXME: arch-split*)
...
lemma valid_badges_def2:
  ...
  apply (case_tac cap; clarsimp simp: gen_isCap_simps)
      by (fastforce simp: AARCH64.sameRegionAs_def3 AARCH64.isCap_simps)+ (* FIXME arch-split *)
...
lemma (in mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps isMDBParentOf_CTE is_derived'_def
   ...
   apply (erule notE, rule direct_parent)
     apply (simp add: mdb_next_unfold)
    apply simp
   apply (simp add: parentOf_def src isMDBParentOf_CTE AARCH64.isCap_simps (* FIXME arch-split *)
  ...
  apply (erule AARCH64.sameRegionAsE, simp_all) (* FIXME arch-split *)
    ...
    apply (clarsimp simp add: gen_isCap_simps capMasterCap_def
                    simp del: not_ex
                       split: capability.splits)
   apply (clarsimp simp: AARCH64.isCap_simps)+ (* FIXME arch-split *)
  done
...
lemma maskedAsFull_revokable:
  ...
  apply (case_tac c'; simp add: maskedAsFull_def is_derived'_def isCap_simps badge_derived'_def
...
lemma sameRegionAs_SGISignalCap2[simp]:
  ...
  by (auto simp: sameRegionAs_def AARCH64_H.sameRegionAs_def isCap_simps
...
lemma valid_badges_preserve_oneway:
  ...
  apply (erule impE)
   ...
   apply (clarsimp simp: isCap_simps)
   apply (drule_tac x="ArchObjectCap (SGISignalCap irqa target)" in fun_cong)
   apply (solves simp)
  apply (drule(1) misc)+
  apply clarsimp
  done
