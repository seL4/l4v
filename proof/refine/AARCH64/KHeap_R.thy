...
lemma setObject_other_corres:
  ...
  apply (rule conjI)
   ...
   apply (erule disjE)
    apply (insert t, clarsimp simp: is_other_obj_relation_type_CapTable a_type_def)
   apply (erule disjE)
   ...
   apply (insert t, clarsimp simp: is_other_obj_relation_type_PageTable a_type_def)
  ...
  (* FIXME AARCH64: slow due to multiple (too many?) splits *)
  apply (case_tac ob;
         simp add: a_type_def other_obj_relation_def etcb_relation_def
