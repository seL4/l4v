...
crunch state_refs_of'[wp]: emptySlot "\<lambda>s. P (state_refs_of' s)"
  (wp: crunch_wps)
crunch state_hyp_refs_of'[wp]: setInterruptState "\<lambda>s. P (state_hyp_refs_of' s)"
  (simp: state_hyp_refs_of'_pspaceI)
crunch state_hyp_refs_of'[wp]: emptySlot "\<lambda>s. P (state_hyp_refs_of' s)"
  (wp: crunch_wps)
...
lemma invalidateASIDEntry_invs'[wp]: (* FIXME AARCH64  *)
...
crunches setVMRoot, deleteASIDPool, invalidateTLBByASID, invalidateASIDEntry, vcpuFinalise
  for ctes_of[wp]: "\<lambda>s. P (ctes_of s)"
...
lemma arch_finaliseCap_removeable[wp]:
  ...
  apply (simp add: AARCH64_H.finaliseCap_def removeable'_def)
  apply (wpsimp wp: cteCaps_of_ctes_of_lift hoare_vcg_disj_lift hoare_vcg_ex_lift)
  sorry (* FIXME AARCH64 ASID pools etc
  done *)
...
lemmas setQueue_cteCaps_of[wp] = cteCaps_of_ctes_of_lift [OF setQueue_ctes_of]
lemmas threadSet_cteCaps_of = cteCaps_of_ctes_of_lift [OF threadSet_ctes_of]

crunches archThreadSet, vcpuUpdate, dissociateVCPUTCB
...
lemma [wp]: (* FIXME AARCH64 *)
  "\<lbrace>\<lambda>s. P (irq_node' s)\<rbrace> invalidateTLBByASID param_a \<lbrace>\<lambda>_ s. P (irq_node' s)\<rbrace>"
  sorry
...
lemma [wp]: (* FIXME AARCH64 *)
  "\<lbrace>\<lambda>s. P (irq_node' s)\<rbrace> loadVMID param_a \<lbrace>\<lambda>_ s. P (irq_node' s)\<rbrace>"
  sorry
...
lemma cancelAllSignals_valid_queues[wp]:
  ...
  apply (clarsimp simp: valid_ep'_def)
  apply (drule (1) ko_at_valid_objs')
  apply (auto simp: valid_obj'_def valid_ntfn'_def valid_tcb'_def
             split: endpoint.splits
              elim: valid_objs_valid_tcbE)
  done
...
crunch nosch[wp]: dissociateVCPUTCB, unmapPageTable "\<lambda>s. P (ksSchedulerAction s)"
...
lemma arch_finaliseCap_corres:
  ...
  apply (cases cap,
         simp_all add: arch_finalise_cap_def AARCH64_H.finaliseCap_def
    ...
    apply (clarsimp simp: valid_cap'_def)
   apply auto[1]
   sorry (* FIXME AARCH64 the cases are different now, indentation is misleading, etc
...
lemma set_thread_all_corres:
  ...
  apply (simp flip: trans_state_update)
  ...
  apply (clarsimp simp: obj_at'_def)
  apply (insert e is_t)
  by (clarsimp simp: other_obj_relation_def etcb_relation_def is_other_obj_relation_type
...
lemma tcb_update_all_corres':
  ...
  apply (rule_tac F="tcb_relation tcb tcb' \<and> etcb_relation etcbu tcbu'" in corres_req)
   ...
   apply (force simp: other_obj_relation_def ekheap_relation_def e)
  apply (erule conjE)
  apply (rule corres_guard_imp)
    apply (rule corres_rel_imp)
...
lemma thread_gets_the_all_corres:
  ...
  apply (rule corres_no_failI)
   apply wp
  apply (clarsimp simp: gets_def get_def return_def bind_def get_tcb_def thread_gets_the_all_def
                        threadGet_def ethread_get_def gets_the_def assert_opt_def get_etcb_def
                        is_etcb_at_def tcb_at_def liftM_def
