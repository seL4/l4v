  ...
  show "valid_badges ?C"
    ...
    apply (elim disjE)
     ...
     apply (rule conjI, clarsimp)
      ...
      apply (rule conjI; clarsimp simp: isCap_simps;
             fastforce simp: isCap_simps sameRegionAs_def cong: if_cong)
...
lemma arch_deriveCap_untyped_derived[wp]:
  ...
  apply (wpsimp simp: AARCH64_H.deriveCap_def Let_def untyped_derived_eq_ArchObjectCap split_del: if_split
...
lemma ex_nonz_tcb_cte_caps':
  ...
  apply (subgoal_tac "s \<turnstile>' cteCap cte")
   ...
   apply (clarsimp simp: valid_cap'_def obj_at'_def dom_def typ_at_to_obj_at_arches
...
lemma maskedAsFull_revokable_safe_parent:
  ...
  apply (clarsimp simp: isCapRevocable_def AARCH64_H.isCapRevocable_def maskedAsFull_def
                  split: if_splits capability.splits)
