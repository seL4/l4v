...
arch_requalify_facts (H)
  nullPointer_def (* NULL pointer is 0 on all architectures *)

<<<<<<< HEAD:proof/refine/RISCV64/CSpace_I.thy
...
||||||| 0d43d8dee:proof/refine/RISCV64/CSpace_I.thy
...
locale CSpace_I =
  ...
  assumes isArchFrameCap_non_arch:
    "\<And>cap. \<not>is_ArchObjectCap cap \<Longrightarrow> isArchFrameCap cap = False"
  assumes acapClass_arch_capMasterCap[simp]:
    "\<And>acap. acapClass (arch_capMasterCap acap) = acapClass acap"
  assumes capUntypedPtr_arch_capMasterCap[simp]:
    "\<And>acap. Arch.capUntypedPtr (arch_capMasterCap acap) = Arch.capUntypedPtr acap"
  assumes acapBits_arch_capMasterCap[simp]:
    "\<And>acap. acapBits (arch_capMasterCap acap) = acapBits acap"
...
>>>>>>> verification/master:proof/refine/CSpace_I.thy
...
lemma getCTE_inv[wp]:
  "\<lbrace>P\<rbrace> getCTE addr \<lbrace>\<lambda>rv. P\<rbrace>"
...
lemma getSlotCap_inv [wp]: "\<lbrace>P\<rbrace> getSlotCap addr \<lbrace>\<lambda>rv. P\<rbrace>"
=======
lemma getSlotCap_inv[wp]:
  "\<lbrace>P\<rbrace> getSlotCap addr \<lbrace>\<lambda>rv. P\<rbrace>"
>>>>>>> verification/master:proof/refine/CSpace_I.thy
  by (simp add: getSlotCap_def, wp)

lemma cap_case_CNodeCap:
  "(case cap of CNodeCap _ _ _ _ \<Rightarrow> P
           | _ \<Rightarrow> Q)
   = (if isCNodeCap cap then P else Q)"
  by (cases cap, simp_all add: gen_isCap_simps)
...
lemma capRange_of_untyped [simp]:
  ...
  by (simp add: capRange_def gen_isCap_simps capUntypedSize_def)
...
(* this lemma is not safe as part of generic simpset, as it clogs
   `capMasterCap (ArchObjectCap x) = capMasterCap y` in proofs *)
lemma capMasterCap_ArchObjectCap:
  "capMasterCap (capability.ArchObjectCap acap) = capability.ArchObjectCap (arch_capMasterCap acap)"
  by (simp add: capMasterCap_def)

lemmas gen_capMasterCap_simps[simp] =
  capMasterCap_def[split_simps capability.split,
                   simplified capMasterCap_ArchObjectCap, simplified]
...
lemma gen_isCap_Master:
  ...
  by (auto simp: gen_isCap_simps capMasterCap_def split: capability.split)

lemma gen_capMasterCap_eqDs1:
...
lemma sameObjectAs_sym:
  ...
  by (auto simp add: sameObjectAs_def2 eq_commute conj_comms)
=======
lemmas gen_capMasterCap_eqDs[dest!] = gen_capMasterCap_eqDs1 gen_capMasterCap_eqDs1[OF sym]
>>>>>>> verification/master:proof/refine/CSpace_I.thy
...
lemma capUntypedSize_simps[simp]:
  ...
  by (auto simp add: capUntypedSize_def gen_isCap_simps gen_objBits_simps
...
=======
           | clarsimp simp: gen_objBits_simps cteSizeBits_cte_level_bits
>>>>>>> verification/master:proof/refine/CSpace_I.thy
...
locale CSpace_I_2 = CSpace_I +
  assumes sameRegionAs_Null[simp]:
    "\<And>c. sameRegionAs c NullCap = False"
    "\<And>c. sameRegionAs NullCap c = False"
  assumes sameRegionAs_capRange_Int:
    "\<And>cap cap'.
  ...
  assumes sameRegionAs_classes:
    "\<And>cap cap'. sameRegionAs cap cap' \<Longrightarrow> capClass cap = capClass cap'"
  assumes sameObjectAs_sym:
    "\<And>c c'. sameObjectAs c c' = sameObjectAs c' c"
  assumes capBits_Master:
...
lemma isZombie_capClass:
  "isZombie cap \<Longrightarrow> capClass cap = PhysicalClass"
  by (clarsimp simp: gen_isCap_simps)
...
lemma distinct_zombies_seperateE:
  ...
  apply (intro impI allI conjI)
   ...
   apply (subgoal_tac "\<not> isUntypedCap (cteCap z) \<and> \<not> isArchFrameCap (cteCap z)")
    apply fastforce
   apply (clarsimp simp: gen_isCap_simps isArchFrameCap_non_arch)
  apply clarsimp
...
lemma setCTE_valid_objs'[wp]:
  ...
  apply (rule setObject_valid_objs')
   apply (clarsimp simp: prod_eq_iff lookupAround2_char1 updateObject_cte gen_objBits_simps)
...
lemma getCTE_ctes_wp:
  ...
  by (rule hoare_strengthen_post, rule getCTE_sp)
