...
| "arch_irq_control_inv_relation (ARM_A.IssueSGISignal irq targets ptr ptr') x =
     (x = ARM_HYP_H.IssueSGISignal irq (ucast targets) (cte_map ptr) (cte_map ptr'))"
...
| "arch_irq_control_inv_valid' (ARM_HYP_H.IssueSGISignal irq targets src_slot sgi_slot) =
...
(* 16 is hard-coded in Haskell and bitfield generator as width of "targets" field in SGISignal cap *)
lemma sgi_target_mask_bits_ucast_targets:
  "(targets :: machine_word) \<le> mask gicSGITargetMaskBits \<Longrightarrow>
   (ucast targets :: 16 word) = ucast (ucast targets :: sgi_target_mask)"
  by (simp add: gicSGITargetMaskBits_sgi_target_len) word_eqI_solve
...
lemma arch_decodeIRQControlInvocation_corres:
  ...
  apply (rule conjI, clarsimp)
   ...
   apply (cases caps, simp)
    apply (auto split: arch_invocation_label.splits list.splits invocation_label.splits
                 simp: length_Suc_conv list_all2_Cons1 liftE_bindE split_def)[2]
  ...
  apply (rule conjI, clarsimp)
   ...
   apply (corres corres: lookupSlotForCNodeOp_corres ensureEmptySlot_corres corres_returnOkTT
                 term_simp: numSGIs_def sgi_target_mask_bits_ucast_targets not_le)
    apply fastforce
   apply fastforce
...
lemma invoke_irq_handler_invs'[wp]:
  ...
  apply (cases i, simp_all add: Interrupt_H.invokeIRQHandler_def invokeIRQHandler_def)
    apply (rule conjI; wpsimp wp: dmo_maskInterrupt' simp: theIRQ_def)
...
lemma setIRQTrigger_corres:
  ...
  apply (rule corres_guard_imp)
    apply (rule corres_rel_imp)
     apply (wpsimp wp: corres_underlying_trivial)+
  done
...
lemma SGISignalCap_valid[simp, intro!]:
  "valid_cap' (ArchObjectCap (SGISignalCap irq targets)) s"
