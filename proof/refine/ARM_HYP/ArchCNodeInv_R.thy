...
crunch writeVCPUHardwareReg, readVCPUHardwareReg, check_export_arch_timer,
       get_cntv_off_64, get_cntv_cval_64
...
lemma sameRegion_cap'_src[simp]:
  ...
  apply (case_tac "isReplyCap src_cap"; clarsimp)
   apply (clarsimp simp: capMasterCap_def arch_capMasterCap_def
          ...
          fastforce simp: sameRegionAs_def ARM_HYP_H.sameRegionAs_def isCap_simps split: if_split_asm)+
  done
...
lemmas arch_assms =
  ARM_HYP.sameRegionAs_eq_child
  ARM_HYP.sameRegionAs_eq_parent
  ARM_HYP.sameRegion_ep
  ARM_HYP.sameRegion_ntfn
