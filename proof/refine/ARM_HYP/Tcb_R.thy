    ...
    apply (rule corres_split[OF tcbSchedDequeue_corres])
      apply (rule corres_split[OF ethread_set_corres], simp_all)[1]
        ...
        apply (rule corres_split[OF isRunnable_corres])
          ...
          apply wp
         apply clarsimp
         apply (wp hoare_weak_lift_imp hoare_vcg_if_lift hoare_wp_combs gts_wp)
        apply clarsimp
        apply (wp hoare_vcg_if_lift hoare_weak_lift_imp hoare_wp_combs isRunnable_wp)
      ...
      apply (wp hoare_drop_imps)
       apply ((wp hoare_vcg_imp_lift' hoare_vcg_if_lift hoare_vcg_all_lift
                  ...
                  threadSet_valid_queues'_no_state threadSet_valid_objs_tcbPriority_update
     ...
     apply ((wp hoare_vcg_imp_lift' hoare_vcg_if_lift hoare_vcg_all_lift
                isRunnable_wp threadSet_pred_tcb_no_state threadSet_valid_queues_no_state
                threadSet_cur threadSet_valid_queues'_no_state threadSet_valid_objs_tcbPriority_update
                threadSet_weak_sch_act_wf threadSet_ct_in_state'[simplified ct_in_state'_def]
             | simp add: etcb_relation_def)+)[1]
    apply clarsimp
    apply ((wp hoare_vcg_imp_lift' hoare_vcg_if_lift hoare_vcg_all_lift hoare_vcg_disj_lift
               tcbSchedDequeue_not_in_queue
               tcbSchedDequeue_valid_queues
               tcbSchedDequeue_ct_in_state'[simplified ct_in_state'_def] | simp add: etcb_relation_def)+)[1]
   apply clarsimp
...
lemma setP_invs':
  ...
  apply (simp add: setPriority_def)
  apply (wp rescheduleRequired_all_invs_but_ct_not_inQ valid_irq_node_lift
        | simp add: if_apply_def2)+
    apply (rule hoare_strengthen_post, rule threadSet_invs_trivial, simp+)
   ...
   apply (rule_tac Q="\<lambda>_. invs' and obj_at' (Not \<circ> tcbQueued) t
           ...
           in hoare_post_imp)
   apply (clarsimp dest: obj_at_ko_at' simp: obj_at'_def inQ_def)
...
proof -
  ...
  have V: "getThreadVSpaceRoot a = return (cte_map (a, tcb_cnode_index 1))"
    apply (clarsimp simp add: getThreadVSpaceRoot objBits_defs)
            ...
            apply (rule corres_split_norE[OF T [OF y V], simplified])
             ...
             apply (rule_tac Q'="\<lambda>_. ?T2_pre" in hoare_post_imp_R[simplified validE_R_def, rotated])
              ...
              apply (clarsimp simp: emptyable_def is_nondevice_page_cap_simps is_cap_simps
                                    is_cnode_or_valid_arch_def obj_ref_none_no_asid cap_asid_def arch_cap_fun_lift_def
...
lemma bindNotification_invs':
  ...
  apply (subst delta_sym_refs, assumption)
    subgoal by (fastforce simp: ntfn_q_refs_of'_def obj_at'_def projectKOs
