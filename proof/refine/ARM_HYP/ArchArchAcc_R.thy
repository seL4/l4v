...
(* automatic consequences of valid_arch_state_def *)
...
lemma storePDE_state_refs_of[wp]:
  ...
  by (wpsimp wp: headM_inv hoare_drop_imps setObject_state_refs_of_eq
             simp: storePDE_def updateObject_default_def in_monad)
...
lemma getObject_PDE_corres [@lift_corres_args, corres]:
  ...
  apply (clarsimp simp: typ_at'_def ko_wp_at'_def)
  apply (frule_tac s'=ba in in_magnitude_check, simp add: objBits_simps vspace_bits_defs)
   apply assumption
  apply (simp add: in_magnitude_check objBits_simps vspace_bits_defs)
  apply (clarsimp simp: state_relation_def pspace_relation_def)
  apply (drule bspec, blast)
...
lemma ko_at'_pd:
  ...
  apply (clarsimp simp: obj_at'_def pde_at_def obj_at_def)
  apply (clarsimp simp: state_relation_def pspace_relation_def)
  apply (drule bspec, blast)
  apply (clarsimp simp: other_obj_relation_def pde_relation_def)
  apply (erule_tac x="(ucast (p && mask pd_bits >> pde_bits))" in allE)
...
lemma valid_pde_duplicates_at'_pde_obj:
  ...
  \<Longrightarrow> obj_at' (\<lambda>pde. isSuperSection pde \<and>
              ...
              (p && ~~ mask (pde_bits + 4)) s"
  apply (clarsimp simp: obj_at'_def valid_pde_duplicates_at'_def vspace_bits_defs)
  apply (drule (1) pspace_distinctD'[where x="p && ~~ mask 7"])
  apply (clarsimp simp: objBits_simps is_aligned_neg_mask vspace_bits_defs)
...
lemma get_master_pde_corres [@lift_corres_args, corres]:
  ...
  apply (case_tac "isSuperSection pde'")
   apply (drule valid_duplicates'_D)
     apply (clarsimp simp: obj_at'_def, assumption)
    apply assumption
  ...
  apply (clarsimp simp: obj_at'_def objBits_simps vspace_bits_defs)
...
lemma getObject_PTE_corres [@lift_corres_args, corres]:
  ...
  apply (drule_tac s'=ba in in_magnitude_check)
    apply (simp add: objBits_simps pte_bits_def, assumption, clarsimp)
...
lemma ko_at'_pt:
  ...
  apply (clarsimp simp: obj_at'_def pte_at_def obj_at_def)
  apply (clarsimp simp: state_relation_def pspace_relation_def)
  apply (drule bspec, blast)
  apply (clarsimp simp: other_obj_relation_def pde_relation_def)
  apply (erule_tac x="(ucast (p && mask pt_bits >> pte_bits))" in allE)
...
lemma valid_pte_duplicates_at'_pte_obj:
  ...
  \<Longrightarrow> obj_at' (\<lambda>pte. isLargePage pte \<and>
              ...
              (p && ~~ mask (pte_bits + 4)) s"
  apply (clarsimp simp: obj_at'_def valid_pte_duplicates_at'_def vspace_bits_defs)
  apply (drule (1) pspace_distinctD'[where x="p && ~~ mask 7"])
  apply (clarsimp simp: objBits_simps is_aligned_neg_mask vspace_bits_defs)
...
lemma setObject_PD_corres [corres]:
  ...
  apply (frule_tac s'=s'' in in_magnitude_check)
    apply (simp add: in_magnitude_check objBits_simps pde_bits_def, assumption)
...
lemma setObject_PT_corres [corres]:
  ...
  apply (rule corres_no_failI)
   ...
   apply (simp add: objBits_simps word_bits_def)
  apply (clarsimp simp: setObject_def in_monad split_def updateObject_default_def)
  apply (frule_tac s'=s'' in in_magnitude_check ; simp add: in_magnitude_check objBits_simps)
...
lemma page_table_at_state_relation:
  ...
  apply (clarsimp simp: objBits_simps pte_bits_def)
...
lemma page_directory_at_state_relation:
  ...
  apply (drule(1) pspace_distinctD')
  apply (clarsimp simp: objBits_simps pde_bits_def)
  apply (rule is_aligned_weaken)
   apply (erule aligned_add_aligned)
    apply (rule is_aligned_shiftl_self)
   apply simp
...
lemma getPDE_wp:
  ...
  by (clarsimp simp: getObject_def split_def loadObject_default_def
                     in_magnitude_check vspace_bits_defs
...
lemma find_pd_for_asid_corres [@lift_corres_args, corres]:
  ...
  subgoal for x ko xa
   ...
   apply (cut_tac asid_pool_at[of x, simplified obj_at_def])
     ...
     apply (clarsimp simp: other_aobj_relation_def obj_at'_def asid_pool_relation_def)
...
lemma setObject_ASID_valid_arch [wp]:
  ...
  apply (clarsimp simp: is_vcpu'_def ko_wp_at'_def obj_at'_def)
  done
...
lemma pspace_distinct_cross[ArchAcc_R_assms]:
  ...
  apply (case_tac "p = x")
   ...
   apply (erule (1) obj_relation_cutsE; clarsimp)
       ...
       apply (rule_tac n=pteBits in is_aligned_add_step_le';
              simp add: vspace_bits_defs vcpu_bits_def)
