...
primrec arch_irq_control_inv_valid' :: "Arch.irqcontrol_invocation \<Rightarrow> kernel_state \<Rightarrow> bool" where
  "arch_irq_control_inv_valid' (ARM_HYP_H.IssueIRQHandler irq ptr ptr' t) =
...
lemma arch_decodeIRQControlInvocation_corres[Interrupt_R_assms]:
  ...
  apply (clarsimp simp: arch_decode_irq_control_invocation_def
                        ARM_HYP_H.decodeIRQControlInvocation_def Let_def)
...
lemma arch_performIRQControl_corres[Interrupt_R_assms]:
  ...
  apply (cases ivk; simp add: ARM_HYP_H.performIRQControl_def invoke_irq_control.cases IRQ_def)
   apply corres (* unlessE haveSetTrigger *)
...
proof -
  ...
  show ?thesis
    ...
    apply (rule corres_guard_imp)
     ...
     apply (clarsimp simp: valid_arch_state_def valid_fault_def obj_at_def is_vcpu_def)
...
lemma vppiEvent_corres:
  ...
  apply (rule corres_guard_imp)
   ...
   apply (clarsimp simp: obj_at_def is_vcpu_def in_omonad)
  apply clarsimp
  apply (frule invs_arch_state')
  apply (rule conjI)
   apply (clarsimp simp: valid_arch_state'_def vcpu_at_is_vcpu')
