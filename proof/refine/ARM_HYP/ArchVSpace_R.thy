...
lemma find_pd_for_asid_valids:
  ...
  apply (auto simp: returnOk_def return_def pde_at_def pd_bits_def pdBits_def pageBits_def
...
lemma armKSNextASID_cross:
  "(s, s') \<in> state_relation \<Longrightarrow>
   armKSNextASID (ksArchState s') = arm_next_asid (arch_state s)"
...
lemma setVCPU_if_live[wp]:
  ...
  apply (wpsimp wp: setObject_iflive' [where P=\<top>]
         | simp add: objBits_simps vcpu_bits_def pageBits_def)+
    apply (clarsimp simp: updateObject_default_def in_monad)
...
lemma getObject_ko_at_vcpu [wp]:
  ...
  by (rule getObject_ko_at | simp add: objBits_simps vcpu_bits_def pageBits_def)+
...
lemma vcpuWriteReg_corres[corres]:
  "\<lbrakk> val' = val; r' = r \<rbrakk> \<Longrightarrow>
...
lemma dmo_maskInterrupt_corres[corres]:
  "vcpu_vppi_masked vcpu = vcpuVPPIMasked vcpu' \<Longrightarrow>
   corres dc \<top> \<top>
          (do_machine_op (maskInterrupt (vcpu_vppi_masked vcpu VPPIEventIRQ_VTimer) irqVTimerEvent))
          (doMachineOp (maskInterrupt (vcpuVPPIMasked vcpu' VPPIEventIRQ_VTimer) irqVTimerEvent))"
  by (corres corres: corres_machine_op corres_Id)
...
lemma empty_fail_setCurrentPD[simp, intro!]:
  "empty_fail (setCurrentPD pd)"
  by (simp add: setCurrentPD_def setCurrentPDPL2_empty_fail)
...
proof -
  have P: "corres dc \<top> \<top> set_global_pd setGlobalPD"
    apply (simp add: set_global_pd_def setGlobalPD_def setCurrentPD_def set_current_pd_def)
    apply (rule corres_underlying_split [where P=\<top> and P'=\<top> and r'="(=)"])
      ...
      apply (rule corres_rel_imp)
       apply (rule corres_underlying_trivial)
       apply (wpsimp wp: no_fail_dsb no_fail_isb no_fail_setCurrentPDPL2
                         no_fail_setHardwareASID)+
    done
  ...
  show ?thesis
    ...
    apply (rule_tac Q'="tcb_at' t" in corres_cross_add_guard)
     apply (fastforce dest!: tcb_at_cross)
          ...
          apply (rule corres_guard_imp)
            apply (rule corres_split_catch [where f=lfr])
               apply (rule corres_split_eqrE[OF find_pd_for_asid_corres])
                ...
                apply simp
                apply (wpsimp wp: find_pd_for_asid_pd_at_asid_again
...
lemma storePDE_ko_wp_vcpu_at'[wp]:
  ...
  apply (wp hoare_drop_imps setObject_ko_wp_at, simp, simp add: objBits_simps)
   apply (simp add: pde_bits_def)
  apply (clarsimp split del: if_split)
  apply (erule_tac P=P in rsubst)
  apply (clarsimp simp: ko_wp_at'_def obj_at'_def is_vcpu'_def)
  done
              ...
              apply (rule corres_split[OF storePTE_corres'])
                ...
                apply (rule corres_split[where r' = dc, OF corres_store_pte_with_invalid_tail])
                  ...
                  apply (rule corres_split[where r'=dc, OF corres_machine_op[OF corres_Id]])
                       apply (simp add: last_byte_pte_def objBits_simps)
                      apply simp
                     apply (rule no_fail_cleanCacheRange_PoU)
           ...
           apply (rule corres_split[OF pdeCheckIfMapped_corres])
             apply (rule corres_split[OF storePDE_corres'])
               ...
               apply (rule corres_split[where r'=dc, OF corres_store_pde_with_invalid_tail])
                 ...
                 apply (rule corres_split[where r'=dc,OF corres_machine_op[OF corres_Id]])
                      apply (simp add: last_byte_pde_def objBits_simps)
                     apply simp
                    apply (rule no_fail_cleanCacheRange_PoU)
                   apply (rule corres_split[where r'=dc, OF _ corres_return_eq_same[OF refl]])
...
lemma performPageTableInvocation_corres:
  ...
  apply (clarsimp simp: is_pt_cap_def split_def
                        vspace_bits_defs objBits_simps
                  cong: option.case_cong)
...
lemma setGlobalPD_invs'[wp]:
  ...
  apply (wpsimp wp: dmo_invs' no_irq_dsb no_irq_isb no_irq_setCurrentPD
                    no_irq_setHardwareASID no_irq)
  apply (drule_tac Q="\<lambda>_ m'. underlying_memory m' p = underlying_memory m p"
                   in use_valid)
    apply (clarsimp simp: setCurrentPD_def machine_op_lift_def writeTTBR0_def dsb_def isb_def
                          setCurrentPDPL2_def setHardwareASID_def machine_rest_lift_def split_def
           | wp)+
  done
...
lemma setGlobalPD_invs_no_cicd':
  ...
  apply (simp add: setGlobalPD_def)
  apply (wpsimp wp: dmo_invs_no_cicd' no_irq_dsb no_irq_isb no_irq_setCurrentPD
                    no_irq_setHardwareASID no_irq)
  apply (drule_tac Q="\<lambda>_ m'. underlying_memory m' p = underlying_memory m p"
                   in use_valid)
    apply (clarsimp simp: setCurrentPD_def machine_op_lift_def writeTTBR0_def dsb_def isb_def
...
lemma saveVirtTimer_invs_no_cicd'[wp]:
  ...
  by (wpsimp simp: saveVirtTimer_def get_cntv_off_64_def get_cntv_cval_64_def
             wp: dmo'_gets_wp)+
...
lemma restoreVirtTimer_invs_no_cicd'[wp]:
  ...
  by (wpsimp simp: restoreVirtTimer_def if_apply_def2 isIRQActive_def
...
lemma getObject_vcpu_ko_at':
  ...
  apply (drule use_valid, rule getObject_ko_at; clarsimp simp: vcpu_bits_def)
  done
...
lemma saveVirtTimer_invs'[wp]:
  ...
  by (wpsimp wp: dmo'_gets_wp setVCPU_vgic_invs' setVCPU_regs_invs' dmo_maskInterrupt_True
             simp: doMachineOp_bind get_cntv_off_64_def get_cntv_cval_64_def)
...
lemma restoreVirtTimer_invs'[wp]:
  ...
  by (wpsimp wp: maskInterrupt_invs' getIRQState_wp dmo_machine_op_lift_invs'
             simp: IRQ_def if_apply_def2 set_cntv_off_64_def set_cntv_cval_64_def
                   isIRQActive_def)
...
lemma setVMRoot_invs_no_cicd'[wp]:
  ...
  apply (simp add: setVMRoot_def getThreadVSpaceRoot_def)
   apply (wp hoare_drop_imps getObject_tcb_hyp_sym_refs
             ...
             dmo_setCurrentPD_invs_no_cicd' setGlobalPD_invs_no_cicd'
          | wpcw
          | simp add: if_apply_def2 checkPDNotInASIDMap_def split del: if_split)+
  done
...
lemma storePDE_inQ[wp]:
  ...
  apply (wp setObject_ko_wp_at hoare_drop_imp | simp add: objBits_simps vspace_bits_defs)+
  apply (clarsimp simp: obj_at'_def ko_wp_at'_def)
  done
...
lemma storePDE_iflive [wp]:
  ...
  apply (wpsimp simp: storePDE_def objBits_simps vspace_bits_defs
...
lemma storePTE_iflive [wp]:
  ...
  apply (wpsimp simp: storePTE_def objBits_simps vspace_bits_defs
                wp: hoare_drop_imp setObject_iflive' [where P=\<top>])
     apply (auto simp: updateObject_default_def in_monad live'_def hyp_live'_def arch_live'_def)
  done
...
lemma perform_aci_invs [wp]:
  ...
  apply (drule ko_at_valid_objs', fastforce, simp)
