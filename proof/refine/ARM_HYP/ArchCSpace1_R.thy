...
lemma sameRegionAs_SGISignalCap[simp]:
  ...
  by (auto simp add: global.sameRegionAs_def ARM_HYP_H.sameRegionAs_def isCap_simps split: if_splits)
...
lemma same_arch_region_as_relation:
  ...
  by (cases c; cases c')
     (auto simp: ARM_HYP_H.sameRegionAs_def global.sameRegionAs_def Let_def isCap_simps mask_def
...
lemma obj_size_relation:
  ...
  apply (case_tac arch_cap; simp add: objBits_def ARM_HYP_H.capUntypedSize_def bit_simps'
                                      pageBits_def)
  done
...
proof -
  ...
  { fix x :: machine_word
    define y where "y \<equiv> (x >> 8) && mask 18" (* guard_bits *)
    define z where "z \<equiv> unat ((x >> 3) && mask 5)" (* cnode_guard_size_bits *)
    have "of_bl (to_bl (y && mask z)) = (of_bl (replicate (32-z) False @ drop (32-z) (to_bl y))::machine_word)"
...
lemma capASID_update[simp]:
  ...
  apply (case_tac arch_capability,
         simp_all add: global.updateCapData_def ARM_HYP_H.updateCapData_def isCap_simps Let_def)
  done
...
lemma is_derived_eq[CSpace1_R_2_assms]:
  ...
  apply (rule conjI)
   ...
   apply (cases c, auto simp: isCap_simps cap_master_cap_def capMasterCap_def
                              vsCapRef_def vs_cap_ref_def)[1]
...
lemma maskedAsFull_revokable:
  ...
  apply (case_tac c'; simp add: maskedAsFull_def is_derived'_def isCap_simps badge_derived'_def
                                Retype_H.isCapRevocable_def ARM_HYP_H.isCapRevocable_def
...
lemma sameRegionAs_SGISignalCap2[simp]:
  ...
  by (auto simp: global.sameRegionAs_def ARM_HYP_H.sameRegionAs_def isCap_simps
...
lemma (in Arch_mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps ARM_HYP.isMDBParentOf_CTE is_derived'_def
...
lemma src_no_mdb_parent:
  ...
  apply (erule sameRegionAsE)
     ...
     subgoal by (cases src_cap; auto simp: capMasterCap_def arch_capMasterCap_def Retype_H.isCapRevocable_def ARM_HYP_H.isCapRevocable_def
