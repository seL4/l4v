...
(* Results about CNode Invocations, particularly the recursive revoke and delete operations. *)

theory CNodeInv_R
imports ArchIpc_R Invocations_R
begin

arch_requalify_consts (H) cteRightsBits

(* FIXME arch-split: this should be safe, as it'll be a ucast from a machine word, but if we want
   to keep this wrapped, lookupSlotForCNodeOp_corres will need to be wrapped too *)
arch_requalify_facts (A) data_to_nat_def
(* FIXME arch-split: from Machine_A, data is always machine_word so this is always a downcast to 16 *)
arch_requalify_facts (A) data_to_16_def

(* FIXME arch-split: ArchCNodeInv_AI: marked as interface but forgotten from locale? *)
arch_requalify_facts finalise_cap_makes_halted finalise_cap_replaceable

(* FIXME: move, gather with other _mw lemmas *)
lemmas unat_power_lower_mw = unat_power_lower[where 'a=machine_word_len, folded word_bits_def]
lemmas of_nat_mono_maybe_mw =
  of_nat_mono_maybe[rotated, where 'a=machine_word_len, folded word_bits_def]
lemmas unat_of_nat_mw = unat_of_nat_eq[where 'a=machine_word_len, folded word_bits_def]
...
(* FIXME consolidate to one name, preferably this one *)
lemmas cte_wp_at_ctes_ofI = ctes_of_eq_cte_wp_at'
...
lemma whenE_throwError_corres': (* FIXME: move *)
  ...
  apply (rule corres_req)
   apply (erule Q)
    apply simp
   apply simp
  apply (cases P; simp add: P)
  apply (erule corres_guard_imp[OF R]; simp)
  done
...
lemma rightsFromWord_correspondence:
  ...
  by (simp add: rightsFromWord_def rights_mask_map_def data_to_rights_def Let_def data_to_16_def)

primrec valid_cnode_inv' :: "Invocations_H.cnode_invocation \<Rightarrow> kernel_state \<Rightarrow> bool" where
...
primrec cnodeinv_relation ::
  "Invocations_A.cnode_invocation \<Rightarrow> Invocations_H.cnode_invocation \<Rightarrow> bool"
  where
...
lemma deriveCap_Null_helper:
  ...
  apply (cases "cap = NullCap")
   apply (simp add: deriveCap_def gen_isCap_simps)
   apply (wp | simp)+
  apply (rule hoare_strengthen_postE_R, rule assms)
  apply simp
  done

lemma hasCancelSendRights_isEndpointCap:
  "hasCancelSendRights cap \<Longrightarrow> isEndpointCap cap"
  by (clarsimp simp: hasCancelSendRights_def gen_isCap_simps split: capability.splits)
...
lemma untyped_derived_eq_updateCapData:
  ...
  apply (clarsimp simp: gen_isCap_simps)
  apply (clarsimp simp: updateCapData_def gen_isCap_simps)
  done
...
definition not_recursive_ctes :: "kernel_state \<Rightarrow> machine_word set" where
  "not_recursive_ctes s \<equiv>
     {ptr. \<exists>cap. cteCaps_of s ptr = Some cap \<and> \<not> (isZombie cap \<and> capZombiePtr cap = ptr)}"

definition
  "no_cte_prop P = (if \<forall>sl cte. \<lbrace>P\<rbrace> setCTE sl cte \<lbrace>\<lambda>_. P\<rbrace> then P else \<top>)"

definition capToRPO :: "capability \<Rightarrow> machine_word option \<times> nat" where
...
locale CNodeInv_R =
  fixes arch_finalise_prop_stuff :: "(kernel_state \<Rightarrow> bool) \<Rightarrow> bool"
  assumes arch_finalise_prop_stuff_top[simp]:
    "arch_finalise_prop_stuff \<top>"
  assumes acap_relation_arch_update_cap_data_NullCap:
    "\<And>acap acap' P x.
     acap_relation acap acap' \<Longrightarrow>
     (arch_update_cap_data P x acap = cap.NullCap) = (Arch.updateCapData P x acap' = NullCap)"
  assumes cnode_guard_size_bits_wordRadix:
    "cnode_guard_size_bits = wordRadix"
  assumes cteRightsBits_cnode_padding_bits:
  ...
  assumes valid_cnode_capI':
    "\<And>(s::det_state) n w g.
  ...
  assumes arch_capBadge_updateCapData_True:
    "\<And>x acap.
     Arch.updateCapData True x acap \<noteq> NullCap \<Longrightarrow>
     capBadge (Arch.updateCapData True x acap) = arch_capBadge acap"
  assumes prepareThreadDelete_ctes_of[wp]:
    "\<And>P t. prepareThreadDelete t \<lbrace>\<lambda>s. P (ctes_of s)\<rbrace>"
  assumes prepareThreadDelete_not_recursive_ctes:
    "\<And>P t. prepareThreadDelete t \<lbrace>\<lambda>s. P (not_recursive_ctes s)\<rbrace>"
  assumes sameRegionAs_Zombie[simp]:
    "\<And>p zb n cap. \<not> sameRegionAs (Zombie p zb n) cap"
  (* This is the expanded form of mdb_swap.cteSwap_valid_mdb_helper which needs Arch to prove
     (via mdb_swap_gen_CNodeInv_R).
     The original form contracts this significantly via internal locale variables from mdb_swap. *)
  assumes mdb_swap_cteSwap_valid_mdb_helper:
    "\<And>m src src_cap src_node dest dest_cap dest_node scap dcap dest2.
     \<lbrakk>mdb_swap m src src_cap src_node dest dest_cap dest_node scap dcap dest2;
      isUntypedCap src_cap \<Longrightarrow> scap = src_cap; isUntypedCap dest_cap \<Longrightarrow> dcap = dest_cap\<rbrakk>
     \<Longrightarrow> valid_mdb_ctes
        (modify_map
          (modify_map
            (modify_map
              (modify_map
                (modify_map
                  (modify_map
                    (modify_map (modify_map m src (cteCap_update (\<lambda>_. dcap))) dest
                      (cteCap_update (\<lambda>_. scap)))
                    (mdbPrev src_node) (cteMDBNode_update (mdbNext_update (\<lambda>_. dest))))
                  (mdbNext src_node) (cteMDBNode_update (mdbPrev_update (\<lambda>_. dest))))
  ...
  (* This is the expanded form of mdb_move.cteMove_valid_mdb_helper which needs Arch to prove
     (via mdb_move_gen_CNodeInv_R).
     The original form contracts this significantly via internal locale variables from mdb_move. *)
  assumes mdb_move_cteMove_valid_mdb_helper:
    "\<And>m src src_cap src_node dest cap' old_dest_node.
     \<lbrakk>mdb_move m src src_cap src_node dest cap' old_dest_node;
      isUntypedCap cap' \<Longrightarrow> cap' = src_cap\<rbrakk>
     \<Longrightarrow> valid_mdb_ctes
        (modify_map
          (modify_map
            (modify_map
              (modify_map
                (modify_map (modify_map m dest (cteCap_update (\<lambda>_. cap'))) src
                  (cteCap_update (\<lambda>_. capability.NullCap)))
                dest (cteMDBNode_update (\<lambda>m. src_node)))
              src (cteMDBNode_update (\<lambda>m. nullMDBNode)))
            (mdbPrev src_node) (cteMDBNode_update (mdbNext_update (\<lambda>_. dest))))
          (mdbNext src_node) (cteMDBNode_update (mdbPrev_update (\<lambda>_. dest))))"
  assumes mdb_chunked_update_final:
    "\<And>cap cap' m slot node.
     \<lbrakk>mdb_chunked m; m slot = Some (CTE cap node);
      \<And>x cte. \<lbrakk>m x = Some cte; x \<noteq> slot\<rbrakk> \<Longrightarrow> \<not> global.sameRegionAs cap (cteCap cte);
      \<And>x cte. \<lbrakk>m x = Some cte; x \<noteq> slot\<rbrakk> \<Longrightarrow> \<not> global.sameRegionAs cap' (cteCap cte);
      \<And>x cte. \<lbrakk>m x = Some cte; x \<noteq> slot; global.sameRegionAs (cteCap cte) cap\<rbrakk> \<Longrightarrow> isUntypedCap (cteCap cte);
      \<And>x cte. \<lbrakk>m x = Some cte; x \<noteq> slot; global.sameRegionAs (cteCap cte) cap'\<rbrakk> \<Longrightarrow> isUntypedCap (cteCap cte);
      capRange cap = capRange cap'\<rbrakk>
     \<Longrightarrow> mdb_chunked (m(slot \<mapsto> CTE cap' node))"
  assumes ztc_sameRegion:
    "\<And>cap cap'.
     \<lbrakk> isCNodeCap cap \<or> isThreadCap cap \<or> isZombie cap \<rbrakk>
     \<Longrightarrow> sameRegionAs cap cap' = sameObjectAs cap cap'"
  assumes sameRegionAs_ThreadCap_eq:
    "\<And>p p'. sameRegionAs (ThreadCap p) (ThreadCap p') = (p = p')"
  assumes sameRegionAs_CNodeCap_eq:
    "\<And>p p' b b' g g' gs gs'.
     sameRegionAs (CNodeCap p b g gs) (CNodeCap p' b' g' gs') = (p = p' \<and> b = b')"
  assumes sameRegionAs_IRQHandlerCap_eq:
    "\<And>irq irq'. sameRegionAs (IRQHandlerCap irq) (IRQHandlerCap irq') = (irq = irq')"
  assumes ztc_untyped_helper:
    "\<And>cap cap'.
  ...
  assumes valid_arch_badges_PhysicalClass:
    "\<And>cap cap' cap'' node'.
  ...
  assumes isFinal_Zombie:
    "\<And>(p::paddr) p' b n cs. isFinal (Zombie p' b n) p cs"
  assumes isFinal_notUntyped_capRange_disjoint:
    "\<And>cap cap' sl sl' s P.
     ...
     \<Longrightarrow> P"
  assumes sameRegion_ep:
    "\<And>cap cap'. \<lbrakk> sameRegionAs cap cap'; isEndpointCap cap \<rbrakk> \<Longrightarrow> isEndpointCap cap'"
  assumes sameRegion_ntfn:
    "\<And>cap cap'. \<lbrakk> sameRegionAs cap cap'; isNotificationCap cap \<rbrakk> \<Longrightarrow> isNotificationCap cap'"
  assumes arch_postCapDeletion_no_cte_prop:
    "\<lbrace>no_cte_prop P and K (arch_finalise_prop_stuff P)\<rbrace>
     Arch.postCapDeletion t
     \<lbrace>\<lambda>_. no_cte_prop P\<rbrace>"
  assumes post_cap_delete_pre'_IRQHandlerCap:
    "\<And>irq sl (cs :: paddr \<rightharpoonup> capability).
  ...
  assumes final_post_cap_delete_pre'_ArchObjectCap:
    "\<And>s sl acap.
  ...
  assumes prepareThreadDelete_rvk_prog':
    "\<And>t m. prepareThreadDelete t \<lbrace>\<lambda>s. revoke_progress_ord m (\<lambda>x. map_option capToRPO (cteCaps_of s x))\<rbrace>"
  assumes Arch_finaliseCap_rvk_prog':
    "\<And>acap b m. Arch.finaliseCap acap b \<lbrace>\<lambda>s. revoke_progress_ord m (\<lambda>x. map_option capToRPO (cteCaps_of s x))\<rbrace>"
  assumes prepareThreadDelete_irq_states'[wp]:
    "\<And>t. prepareThreadDelete t \<lbrace>valid_irq_states'\<rbrace>"
  assumes Arch_finaliseCap_irq_states'[wp]:
...
definition
  "finalise_prop_stuff P
    ...
    \<and> updateTimeStamp_independent P
    \<and> getCurrentTime_independent_H P
    \<and> time_state_independent_H P
    \<and> domain_time_independent_H P
    ...
    \<and> (\<forall>s f. P (s\<lparr>ksMachineState := ksMachineState s \<lparr>last_machine_time
                                                      := f (last_machine_time (ksMachineState s)) (time_state (ksMachineState s))\<rparr>\<rparr>)
             = P s)
    \<and> (\<forall>s f. P (ksMachineState_update (time_state_update f) s) = P s)
    \<and> (\<forall>s f. P (ksCurTime_update f s) = P s)
    \<and> (\<forall>s f. P (ksConsumedTime_update f s) = P s)
    \<and> (\<forall>s f. P (ksDomainTime_update f s) = P s)
    \<and> arch_finalise_prop_stuff P)"
...
lemma cap_relation_NullCap:
  ...
  apply (cases cap)
            apply (simp_all add: Let_def mask_cap_def cap_rights_update_def update_cap_data_closedform
                                       updateCapData_def[folded wordRadix_def] gen_isCap_simps
                      split del: if_split)
     apply simp+
   apply (clarsimp simp: word_bits_size cnode_guard_size_bits_wordRadix cteRightsBits_cnode_padding_bits)
  apply (clarsimp simp: gen_isCap_simps acap_relation_arch_update_cap_data_NullCap split del: if_split)
  done

lemma decodeCNodeInvocation_corres:
  "\<lbrakk> cap_relation (cap.CNodeCap w n list) cap'; list_all2 cap_relation cs cs';
     length list \<le> word_bits \<rbrakk> \<Longrightarrow>
   corres
   (ser \<oplus> cnodeinv_relation)
   (invs and cap_table_at n w and K (n \<noteq> 0) and (\<lambda>s. \<forall>x \<in> set cs. s \<turnstile> x)) (invs' and valid_cap' cap'
    and (\<lambda>s. \<forall>x \<in> set cs'. s \<turnstile>' x))
   (decode_cnode_invocation (mi_label mi) args (cap.CNodeCap w n list) cs)
   (decodeCNodeInvocation (mi_label mi) args cap' cs')"
  apply (rule decode_cnode_cases2[where args=args and exs=cs and label="mi_label mi"])
       ...
       apply (clarsimp simp: list_all2_Cons1 decode_cnode_invocation_def
                             ...
                             cnode_invok_case_cleanup data_to_nat_def
                  split del: if_split
      ...
      apply (simp add: decode_cnode_invocation_def decodeCNodeInvocation_def data_to_nat_def
                       gen_isCap_simps Let_def unlessE_whenE del: ser_def split del: if_split)
   ...
   apply (simp add: le_diff_conv2 split_def decode_cnode_invocation_def decodeCNodeInvocation_def
                    gen_isCap_simps Let_def unlessE_whenE whenE_whenE_body data_to_nat_def
  ...
  apply (elim disjE)
   ...
   apply fastforce
  apply (clarsimp simp: decode_cnode_invocation_def decodeCNodeInvocation_def
                        isCNodeCap_CNodeCap split_def unlessE_whenE data_to_nat_def
             split del: if_split cong: if_cong)
  apply (rule corres_guard_imp)
...
lemma capBadge_updateCapData_True:
  ...
  apply (simp add: updateCapData_def gen_isCap_simps Let_def
...
lemma decodeCNodeInv_wf[wp]:
  ...
  apply (rule decode_cnode_cases2[where label=label and args=args and exs=cs])
       ...
       apply (rule hoare_pre)
        apply (wp whenE_throwError_wp)
          ...
          apply (simp add: ctes_of_valid' valid_updateCapDataI
                           ...
                           untyped_derived_eq_maskCapRights
                           untyped_derived_eq_updateCapData
                           untyped_derived_eq_refl)
          apply (auto simp: gen_isCap_simps updateCapData_def[folded wordRadix_def])[1]
    ...
    apply (rule hoare_pre)
     ...
     apply (simp add: cte_wp_at_ctes_of imp_ex hasCancelSendRights_isEndpointCap)
   ...
   apply (rule hoare_pre)
    apply (wp whenE_throwError_wp getCTE_wp ensureEmptySlot_stronger
      ...
      apply (fastforce simp: gen_isCap_simps updateCapData_def)
     apply (wp lsfco_cte_at')+
   apply clarsimp
  \<comment> \<open>Errors\<close>
...
lemma decodeCNodeInvocation_inv[wp]:
  ...
  apply (rule decode_cnode_cases2[where label=label and args=args and exs=cs])
        apply (simp_all add: decodeCNodeInvocation_def isCNodeCap_CNodeCap split_def
                  ...
                  split del: if_split cong del: if_cong)[5]
...
text \<open>Various proofs about the two recursive deletion operations.
        ...
        termination of rec_del,
        desired properties about rec_del/cap_delete and recursiveDelete/cteDelete,
        termination of finaliseSlot.\<close>
...
lemma not_recursive_ctes_independent_simple[simp]:
  "not_recursive_ctes (ksCurTime_update f s) = not_recursive_ctes s"
  "not_recursive_ctes (ksConsumedTime_update f' s) = not_recursive_ctes s"
  "not_recursive_ctes (ksMachineState_update f'' s) = not_recursive_ctes s"
  by (simp add: not_recursive_ctes_def)+
...
lemma suspend_ctes_of_thread:
  ...
  apply (rule suspend_cte_wp_at'[where P="(=) (ThreadCap t)" and p=x])
    apply (clarsimp simp add: finaliseCap_def Let_def gen_isCap_simps)
   apply (clarsimp simp: cte_wp_at_ctes_of)
  apply (clarsimp simp: cte_wp_at_ctes_of)
...
lemma suspend_not_recursive_ctes:
  "suspend t \<lbrace>\<lambda>s. P (not_recursive_ctes s)\<rbrace>"
...
crunch schedContextUnbindTCB, schedContextCompleteYieldTo, unbindNotification, unbindFromSC
...
definition finaliseSlot_recset ::
  "((machine_word \<times> bool \<times> kernel_state) \<times> (machine_word \<times> bool \<times> kernel_state)) set"
  where
     ...
     wf_sum (\<lambda>(slot, exposed, state). exposed)
       (inv_image (less_than <*lex*> less_than)
          (\<lambda>(x, exp, s). case ctes_of s x of
                         ...
                         | Some (CTE (Zombie p zb n) node) \<Rightarrow> (if p = x then 1 else 2, n)
                         | _ \<Rightarrow> (3, 0)))
...
lemma case_Zombie_assert_fold:
  ...
  by (cases cap, simp_all add: gen_isCap_simps assertE_def)

context CNodeInv_R begin

(* these forms are used only for finaliseSlot' termination proof *)
lemmas unbindNotification_ctes_of_thread =
  unbindNotification_ctes_of[where param_a=t and P="\<lambda>ctes. \<exists>node. ctes x = Some (CTE (ThreadCap t) node)"
                             for t x]
lemmas prepareThreadDelete_ctes_of_thread =
  prepareThreadDelete_ctes_of[where t=t and P="\<lambda>ctes. \<exists>node. ctes x = Some (CTE (ThreadCap t) node)"
                              for t x]
...
termination finaliseSlot'
  ...
  apply (clarsimp simp: in_monad split: if_split_asm)
   ...
   apply (drule in_inv_by_hoareD [OF getCTE_inv])
   apply clarsimp
   apply (erule use_valid [OF _ preemptionPoint_not_recursive_ctes])
   ...
   apply (simp add: gen_isCap_simps, elim conjE disjE exE)
      apply simp
     apply (clarsimp simp: finaliseCap_def Let_def gen_isCap_simps in_monad
  ...
  apply (clarsimp split: if_split_asm
                   simp: in_monad
                  dest!: in_getCTE)
    apply (erule use_valid[OF _ preemptionPoint_ctes_of])
   ...
   apply (clarsimp simp: finaliseCap_def Let_def gen_isCap_simps in_monad fst_cte_ptrs_def)
  apply (erule use_valid[OF _ preemptionPoint_ctes_of])
  apply (frule use_valid [OF _ finaliseCap_cases], simp)
  apply (case_tac rv, case_tac ourCTE)
  apply (clarsimp simp: gen_isCap_simps cte_wp_at_ctes_of)
  apply (elim disjE conjE exE, simp_all)[1]
  done
    ...
    apply (rule hoare_pre_spec_validE)
     ...
     apply wp
        ...
        apply (wp "1.hyps")
          ...
          apply (wp wp preemptionPoint_inv| simp add: o_def indep)+
...
lemma next_trancl_xp:
  ...
  apply (erule next_rtrancl_tranclE; simp)
  done
    ...
    proof (cases rule: tranclE2')
      ...
      let ?goal = "mdb_chain_0 ?m'"
      {
        ...
        hence ?goal
          ...
          apply (auto simp: no_0 chain n3[OF d1])
          done
      } moreover
      {
        assume d1: "mdbPrev src_node \<notin> dom m" and "mdbNext dest_node \<in> dom m"
        hence ?goal
          by simp ((rule mdb_chain_0_modify_map_next)+, simp_all add: no_0 chain)
      } moreover
      {
        ...
        have ?goal using chain_n' d1 src dest
          ...
          apply (simp_all add: no_0 chain n3[OF d1])
          done
      } moreover
      {
  ...
  next
    ...
    proof (cases rule: tranclE2')
      ...
      let ?goal = "mdb_chain_0 ?m'"
      {
        ...
        hence ?goal
          by (intro mdb_chain_0_modify_map_next)
             (auto simp: no_0 chain n3[OF d1])
      } moreover
      {
      ...
      } moreover
      {
        ...
        have ?goal using chain_n' d1 src dest
          ...
          apply (rule  mdb_chain_0_modify_map_next)
          apply (rule  mdb_chain_0_modify_map_next [OF chain_n'])
          apply (simp_all add: no_0 n3[OF d1])
          done
      } moreover
      {
...
lemma swap_cases [case_names src_dest dest_src other]:
  assumes src_dest:
...
lemmas [intro?] = mdb_ptr_dest.prev_p_next mdb_ptr_src.prev_p_next
...
lemma (in mdb_swap) n_dest_prev:
  ...
  shows "\<exists>cte'. n (mdbPrev dest_node) = Some cte'
                \<and> mdbNext (cteMDBNode cte')
                  = (if dest = mdbNext src_node then mdbNext dest_node else src)
                \<and> mdbPrev (cteMDBNode cte')
                  = (if mdbNext src_node = mdbPrev dest_node \<or> dest = mdbNext src_node
                     then dest
                     else mdbPrev (cteMDBNode cte))"
proof -
...
lemma n_dest_next:
  ...
  shows "\<exists>cte'. n (mdbNext dest_node) = Some cte'
                \<and> mdbNext (cteMDBNode cte')
                  = (if src = mdbNext dest_node \<or> mdbNext dest_node = mdbPrev src_node
                     then dest
                     else mdbNext (cteMDBNode cte))
                \<and> mdbPrev (cteMDBNode cte')
                  = (if src = mdbNext dest_node then mdbPrev dest_node else src)"
proof -
...
lemma n_src_prev:
  ...
  shows "\<exists>cte'. n (mdbPrev src_node) = Some cte'
                \<and> mdbNext (cteMDBNode cte')
                  = (if src = mdbNext dest_node then mdbNext src_node else dest)
                \<and> mdbPrev (cteMDBNode cte')
...
lemma n_src_next:
  ...
  shows "\<exists>cte'. n (mdbNext src_node) = Some cte'
                \<and> mdbNext (cteMDBNode cte')
                  = (if dest = mdbNext src_node \<or> mdbNext src_node = mdbPrev dest_node
                     then src
                     else mdbNext (cteMDBNode cte))
                \<and> mdbPrev (cteMDBNode cte')
                  = (if dest = mdbNext src_node then mdbPrev src_node else dest)"
proof -
...
lemma other_dest_next_src_dest:
  fixes cte
  defines "p \<equiv> mdbNext (cteMDBNode cte)"
  assumes src_dest: "mdbNext src_node = dest"
...
lemma other_dest_prev_dest_src:
  fixes cte
  defines "p \<equiv> mdbPrev (cteMDBNode cte)"
  assumes dest_src: "mdbNext dest_node = src"
...
lemma swap_ptr_cases [case_names p_src_prev p_src p_src_next p_dest_prev p_dest p_dest_next p_other]:
  "\<lbrakk>p = mdbPrev src_node \<Longrightarrow> P; p = src \<Longrightarrow> P; p = mdbNext src_node \<Longrightarrow> P;
    ...
    \<lbrakk>p \<noteq> mdbPrev src_node; p \<noteq> src; p \<noteq> mdbNext src_node; p \<noteq> mdbPrev dest_node; p \<noteq> dest;
     p \<noteq> mdbNext dest_node\<rbrakk> \<Longrightarrow> P\<rbrakk>
   \<Longrightarrow> P"
  by auto

lemma prev_not0_into_dom:
...
proof
  ...
  have dest_src_pn:
         "\<lbrakk>mdbPrev src_node \<noteq> 0; mdbNext src_node = dest \<rbrakk>
  ...
  have src_dest_pn':
         "\<lbrakk> mdbPrev dest_node \<noteq> 0; mdbNext dest_node = src \<rbrakk>
  ...
  proof (cases rule: swap_ptr_cases [where p = p])
    ...
    hence psrc [intro?]: "m \<turnstile> p \<leadsto> src" using p0
      by (clarsimp intro!: mdb_ptr_src.prev_p_next)
  ...
  next
    ...
    hence pdest [intro?]: "m \<turnstile> p \<leadsto> dest" using p0
      by (clarsimp intro!: mdb_ptr_dest.prev_p_next)
  ...
  next
    ...
    proof (cases rule: swap_cases)
      ...
      hence "?mn = dest" using p_dest_next src np
        by (cases cte, clarsimp simp: dest2_node_def)
      thus ?thesis using p_dest_next mmn dest_src
...
(* there is already a mdb_swap_gen in CSpace1_R, use extra suffix to disambiguate *)
locale mdb_swap_gen_CNodeInv_R = mdb_swap +
  (* first four assumptions are shared with CNodeInv_R *)
  assumes sameRegionAs_eq_child:
    "\<And>cap c c'.
     \<lbrakk> sameRegionAs cap c; weak_derived' c c' \<rbrakk>
     \<Longrightarrow> sameRegionAs cap c'"
  assumes sameRegionAs_eq_parent:
    "\<And>cap c c'.
...
lemmas cap_simps =
  ep_simps
  isIRQControl_src isIRQHandler_src
  isIRQControl_dest isIRQHandler_dest

end (* mdb_swap *)

lemma (in mdb_swap_gen_CNodeInv_R) cteSwap_valid_badges:
  "valid_badges n"
proof -
  from valid
  have "valid_badges m" ..
...
lemma (in mdb_swap) n_cap_eq:
  ...
  by (auto simp: n_cap_eq' src dest s_d_swap_def)

lemma (in mdb_swap_gen_CNodeInv_R) cteSwap_chunked:
  "mdb_chunked n"
proof -
  from valid
  have "mdb_chunked m" ..
...
lemma weak_der_src:
  ...
  by unfold_locales
     (rule src_derived weak_derived_sym')+

lemma weak_der_dest:
  "weak_der' dest_cap dcap"
  by unfold_locales
     (rule dest_derived weak_derived_sym')+
...
lemma n_next:
  ...
  by (drule conjI[THEN exI[THEN next_m_n2[THEN iffD1, unfolded mdb_next_unfold]]])
     fastforce+
...
lemma n_prev:
  ...
  by (drule conjI[THEN exI[THEN n_prevD[unfolded mdb_prev_def]]])
     fastforce+
...
proof -
  ...
  thus ?thesis using dest_derived src_derived
    ...
    apply simp
    apply (case_tac n)
    apply (clarsimp simp: s_d_swap_def nullMDBNode_def nullPointer_def split: if_split_asm)
    done
qed
...
lemma scap_class[simp]:
  ...
  using src_derived
  by (fastforce simp: weak_derived'_def intro: master_eqI capClass_Master)
...
lemma (in mdb_swap_gen_CNodeInv_R) cteSwap_valid_mdb_helper:
  assumes untyped_eq: "isUntypedCap src_cap \<Longrightarrow> scap = src_cap"
                      "isUntypedCap dest_cap \<Longrightarrow> dcap = dest_cap"
...
lemma (in CNodeInv_R) cteSwap_valid_pspace'[wp]:
  ...
  apply (subgoal_tac "c2 \<in> dom (modify_map
   ...
   apply (case_tac cte)
   apply (case_tac cteb)
   apply (rule_tac dest_node = "cteMDBNode cteb"
                in mdb_swap_cteSwap_valid_mdb_helper[simplified const_def])
...
lemma weak_derived_cte_refs:
  ...
  by (fastforce intro: master_eqI cte_refs_Master)
...
lemma weak_derived_capRange_capBits:
  ...
  by (clarsimp simp: weak_derived'_def)
     (metis capRange_Master capBits_Master)
...
lemma weak_derived_untypedZeroRange:
  ...
  by (clarsimp simp: untypedZeroRange_def gen_isCap_simps weak_derived'_def)
...
lemma no_loops_tranclD:
  ...
  apply clarsimp
  apply (drule (1) trancl_trans)
  apply (fastforce dest: simp: no_loops_def)
  done
...
lemma ex_cte_cap_to'_cteCap:
  ...
  by (simp add: ex_cte_cap_to'_def cte_wp_at_ctes_of cteCaps_of_def)
     (fastforce del: ext intro!: ext)
...
lemma invalid_Thread_CNode:
  ...
  apply (clarsimp simp: valid_cap'_def gen_isCap_simps)
...
lemma distinct_zombies_seperate_if_zombiedE:
  "\<lbrakk> distinct_zombies m; m x = Some cte;
        ...
        \<And>y cte''. \<lbrakk> m y = Some cte''; x \<noteq> y;
                    ...
                    capUntypedPtr (cteCap cte'') = capUntypedPtr (cteCap cte);
                    capBits (cteCap cte'') = capBits (cteCap cte)\<rbrakk>
  ...
  apply (cases "isZombie (cteCap cte') \<and> \<not> isZombie (cteCap cte)")
   ...
   apply clarsimp
   apply (auto simp: gen_isCap_simps isArchFrameCap_non_arch)[1]
  apply clarsimp
...
lemma (in CNodeInv_R) ztc_replace_update_final:
...
proof (rule mdb_chunked_update_final [OF chunk, OF slot])
  ...
  have zombie_case_helper:
    "\<And>y cte. \<lbrakk> m y = Some cte; y \<noteq> x; isZombie cap \<rbrakk> \<Longrightarrow> \<not> sameObjectAs cap' (cteCap cte)"
    ...
    apply (rule distinct_zombiesD [OF slot distz], simp_all)[1]
     ...
     apply (cut_tac ztc2,
            fastforce simp: gen_isCap_simps isArchFrameCap_non_arch)
  ...
  show Fin2: "\<And>y cte. \<lbrakk> m y = Some cte; y \<noteq> x \<rbrakk> \<Longrightarrow> \<not> sameRegionAs cap' (cteCap cte)"
    ...
    apply clarsimp
    apply (frule(1) Fin1)
    apply (rule disjE[OF cases, rotated])
     (* Zombie cap or cap' *)
     apply (erule disjE)
    ...
    apply (clarsimp simp: ztc_sameRegion ztc1 ztc2)
    apply (erule sameObjectAsE; simp add: gen_isCap_simps isArchFrameCap_non_arch)
    using ztc2
    apply (fastforce simp: isArchFrameCap_non_arch ztc_sameRegion[symmetric] gen_isCap_simps
                           sameRegionAs_CNodeCap_eq sameRegionAs_ThreadCap_eq)
    done
...
lemma (in CNodeInv_R) make_zombie_invs':
    ...
    cte_wp_at' (\<lambda>cte. capClass (cteCap cte) = PhysicalClass \<and>
                      ...
                      (\<forall>p \<in> threadCapRefs (cteCap cte).
                            st_tcb_at' ((=) Inactive) p s
                             ...
                             \<and> obj_at' (\<lambda>tcb. tcbSchedNext tcb = None \<and> tcbSchedPrev tcb = None) p s
                             \<and> obj_at' (\<lambda>tcb. \<not> tcbInReleaseQueue tcb) p s)) sl s\<rbrace>
  updateCap sl cap
  \<lbrace>\<lambda>rv. invs'\<rbrace>"
  apply (simp add: invs'_def valid_pspace'_def valid_mdb'_def
                   valid_irq_handlers'_def irq_issued'_def)
  apply (wp updateCap_ctes_of_wp updateCap_ifunsafe' valid_irq_node_lift
            updateCap_untyped_ranges_zero_simple valid_dom_schedule'_lift
       | simp split del: if_split)+
  apply clarsimp
  apply (intro conjI[rotated])
          apply (clarsimp simp: cte_wp_at_ctes_of)
          apply (auto simp: untypedZeroRange_def gen_isCap_simps)[1]
         apply (clarsimp simp: modify_map_def ran_def split del: if_split
                        split: if_split_asm)
          apply (clarsimp simp: cteCaps_of_def cte_wp_at_ctes_of gen_isCap_simps)
         subgoal by auto
     ...
     subgoal by (auto elim: if_unsafe_then_capD' simp: gen_isCap_simps)
  ...
  apply (clarsimp simp: cte_wp_at_ctes_of disj_left_commute disj.commute
                 dest!: isCapDs)
  ...
  apply (rule conjI)
   ...
   apply (rule conjI; clarsimp)
    apply (clarsimp simp: gen_isCap_simps split: if_split_asm)
      subgoal by ((elim disjE | clarsimp simp: gen_isCap_simps)+)
     apply (thin_tac "All _" | thin_tac "Ball _ _")+ (* speed up auto *)
     subgoal by (auto dest: sameRegion_ep sameRegion_ntfn simp: gen_isCap_simps)[1]
    apply (clarsimp simp: mdb_next_unfold)
    apply (erule_tac x=p in allE)
    apply (erule_tac x="mdbNext node" in allE)
    subgoal by simp
     apply (clarsimp simp: gen_isCap_simps split: if_split_asm)
        apply (thin_tac "All _" | thin_tac "Ball _ _")+ (* speed up fastforce *)
        subgoal by (fastforce intro!: valid_arch_badges_not_arch simp: gen_isCap_simps)
       apply (erule_tac x=sl in allE)
       apply (clarsimp simp: valid_arch_badges_PhysicalClass mdb_next_unfold)
      apply (thin_tac "All _" | thin_tac "Ball _ _")+ (* speed up fastforce *)
      subgoal by (fastforce intro!: valid_arch_badges_not_arch simp: gen_isCap_simps)
     apply (erule_tac x=p in allE)
     apply (clarsimp simp: valid_arch_badges_PhysicalClass mdb_next_unfold)
    ...
    subgoal by (clarsimp simp: gen_isCap_simps)
   apply (clarsimp simp add: gen_isCap_simps)
  ...
  apply (rule conjI)
    ...
    apply (simp only: modify_map_apply)
      apply (rule use_update_ztc_two [OF descendants_of_update_ztc])
            apply (rule exEI, rule vmdb.isFinal_untypedParent)
              ...
              apply (clarsimp simp: gen_isCap_simps, fastforce) (* needs unfolding before fastforce *)
             apply assumption
            apply (clarsimp simp: gen_isCap_simps)
           apply assumption
   ...
   apply (clarsimp simp: modify_map_apply)
   apply (rule conjI, clarsimp simp: valid_nullcaps_def gen_isCap_simps)
   apply (rule conjI, clarsimp simp: ut_revocable'_def gen_isCap_simps)
   apply (rule conjI, clarsimp elim!: class_links_update)
   apply (rule conjI)
    apply (erule(1) distinct_zombies_seperate_if_zombiedE)
         apply (clarsimp simp: gen_isCap_simps)
        apply (fastforce simp: isArchFrameCap_non_arch gen_isCap_simps)
       apply simp
      apply simp
     apply simp
    apply (erule_tac sl'=y in isFinal_notUntyped_capRange_disjoint,
           simp add: cteCaps_of_def,
           simp_all add: disj_left_commute disj.commute)[1]
...
lemma (in CNodeInv_R) shrink_zombie_invs':
  ...
  apply (clarsimp simp: cte_wp_at_ctes_of isFinal_Zombie gen_isCap_simps final_matters'_def)
...
lemma valid_Zombie_cte_at':
  ...
  apply (clarsimp simp: valid_cap'_def split: zombie_type.split_asm)
   ...
   apply (subgoal_tac "tcb_cte_cases (of_nat n * 2^cteSizeBits) \<noteq> None")
    apply clarsimp
    apply (erule(2) cte_wp_at_tcbI'; fastforce)
   apply ((erule less_handy_casesE; clarsimp simp del: shiftl_1 simp flip: shiftl_t2n')+)[1]
  apply (drule spec[where x="of_nat n"])
  apply (subst (asm) less_mask_eq)
   apply (rule order_less_le_trans)
    apply (erule of_nat_mono_maybe_mw)
...
lemma cteSwap_cteCaps_of [wp]:
  ...
  unfolding cteSwap_def
  by (wpsimp wp: getCTE_cteCap_wp)
     (fastforce elim: rsubst[where P=P] simp: modify_map_def split: if_split_asm split: option.split)

lemma capSwap_cteCaps_of [wp]:
  "\<lbrace>\<lambda>s. P ((cteCaps_of s) \<circ> (id ( a := b, b := a )))\<rbrace>
   capSwapForDelete a b
   \<lbrace>\<lambda>rv s. P (cteCaps_of s)\<rbrace>"
  unfolding capSwapForDelete_def
  by (wpsimp wp: getCTE_wp getCTE_cteCap_wp)
     (fastforce elim: rsubst[where P=P] simp: cte_wp_at_ctes_of cteCaps_of_def comp_def)
...
lemma updateCap_cap_to':
  ...
  apply wp
  apply (fastforce simp: modify_map_def cte_wp_at_ctes_of cteCaps_of_def)
  done
...
lemma cteDeleteOne_cap_to'[wp]:
  ...
  apply (rule hoare_pre)
   ...
   apply (case_tac cap, simp_all add: finaliseCap_def Let_def gen_isCap_simps)[1]
  apply simp
  done
...
lemmas emptySlot_sch_act_simple[wp] = sch_act_simple_lift[OF emptySlot_nosch]
...
lemma cteDelete_delete_cases:
  ...
  unfolding cteDelete_def whenE_def split_def
  by (wpsimp wp: hoare_strengthen_post[OF emptySlot_deletes]
                 hoare_strengthen_postE_R[OF finaliseSlot_abort_cases]
             simp: cte_wp_at_ctes_of)

lemmas cteDelete_deletes = cteDelete_delete_cases[where e=True, simplified]

lemmas finalise_spec_induct2 =
  finaliseSlot'.induct[where P="\<lambda>sl exp s. P sl (finaliseSlot' sl exp) (\<lambda>P. exp \<or> P) s" for P]
...
lemma zombieCTEs_le:
  ...
  by (cases zb, simp_all add: tcbBlockSizeBits_def)
...
lemma ex_Zombie_to2:
  "\<lbrakk> ctes_of s p = Some cte; cteCap cte = Zombie p' b n; n \<noteq> 0; valid_objs' s \<rbrakk>
  ...
  apply (subgoal_tac "n \<in> unats word_bits")
   apply (simp add: word_less_nat_alt)
   apply (subst unat_minus_one)
    apply (simp add: word_neq_0_conv)
    apply (drule of_nat_mono_maybe[rotated, where 'a=machine_word_len, folded word_bits_def])
     apply (simp add: unats_def)
    apply simp
   apply (simp add: word_unat.Abs_inverse unat_of_nat_mw unats_def)
  apply (simp only: unats_def mem_simps unat_of_nat_mw)
...
lemma setInterruptState_no_cte_prop:
  ...
  unfolding setInterruptState_def
  by (wpsimp simp: no_cte_prop_def finalise_prop_stuff_def)
...
lemma dmo_maskInterrupt_no_cte_prop:
  "\<lbrace>no_cte_prop P and K (finalise_prop_stuff P)\<rbrace>
   ...
   \<lbrace>\<lambda>_. no_cte_prop P\<rbrace>"
  by (wpsimp wp: dmo_maskInterrupt simp: no_cte_prop_def finalise_prop_stuff_def)

lemma updateTrackedFreeIndex_no_cte_prop[wp]:
  "\<lbrace>no_cte_prop P and K (finalise_prop_stuff P)\<rbrace>
   updateTrackedFreeIndex ptr idx
   \<lbrace>\<lambda>_. no_cte_prop P\<rbrace>"
  unfolding updateTrackedFreeIndex_def getSlotCap_def
  by (wpsimp wp: getCTE_wp' simp: no_cte_prop_def finalise_prop_stuff_def)
...
crunch postCapDeletion
  ...
  (ignore: doMachineOp wp: dmo_maskInterrupt_no_cte_prop arch_postCapDeletion_no_cte_prop)

crunch emptySlot, capSwapForDelete
  for no_cte_prop[wp]: "no_cte_prop P"
  (ignore: doMachineOp wp: dmo_maskInterrupt_no_cte_prop)

lemma finalise_prop_stuff_archD:
  "finalise_prop_stuff Q \<Longrightarrow> arch_finalise_prop_stuff Q"
...
lemma reduceZombie_invs'':
  ...
  "\<And>s'' rv. \<lbrakk>\<not> (isZombie cap \<and> capZombieNumber cap = 0); \<not> (isZombie cap \<and> \<not> exposed); isZombie cap \<and> exposed;
             ...
             \<Longrightarrow> s'' \<turnstile> \<lbrace>\<lambda>s. no_cte_prop Q s \<and> invs' s
                                   \<and> cte_wp_at' (\<lambda>cte. isZombie (cteCap cte)) slot s
                                   \<and> ex_cte_cap_to' rv s\<rbrace>
                         finaliseSlot rv False
                \<lbrace>\<lambda>rva s. no_cte_prop Q s \<and> invs' s
                ...
                \<lbrace>\<lambda>rv s. no_cte_prop Q s \<and> invs' s\<rbrace>"
  assumes stuff:
    "finalise_prop_stuff Q"
  shows
  "st \<turnstile> \<lbrace>\<lambda>s.
      no_cte_prop Q s \<and> invs' s
  ...
  apply (rule hoare_pre_spec_validE)
   apply (wp hoare_vcg_disj_lift | simp)+
     ...
     apply (clarsimp simp: stuff[THEN finalise_prop_stuff_archD] stuff)
  ...
  apply (rule conjI)
   ...
   apply (rule conjI)
    ...
    apply clarsimp
   apply clarsimp
  apply (clarsimp dest!: isCapDs) (* doesn't combine with fastforce below *)
  apply (fastforce dest!: ex_Zombie_to2 simp: cteSizeBits_cte_level_bits right_diff_distrib')
  done

lemma sch_act_simple_only_ksSchedulerAction:
  "ksSchedulerAction (f s) = ksSchedulerAction s \<Longrightarrow> sch_act_simple (f s) =  sch_act_simple s"
...
lemma finaliseSlot_invs':
  ...
  "st \<turnstile> \<lbrace>\<lambda>s.
   ...
   \<lbrace>\<lambda>rv s. no_cte_prop Pr s \<and> invs' s\<rbrace>"
proof (induct arbitrary: P p rule: finalise_spec_induct2)
  note len_bit0[simp del] exp_eq_zero_iff[simp del] (* prevent leaking LENGTH for machine words *)
  ...
  have Q: "\<And>x y n. {x :: machine_word} = (\<lambda>x. y + (x << cteSizeBits)) ` {0 ..< n} \<Longrightarrow> n = 1"
    ...
    apply (case_tac "1 < n")
     ...
     apply (subst(asm) imageI, simp)
     using zero_one_less_2p_SizeBits apply force
  ...
  have final_IRQHandler_no_copy:
    "\<And>irq sl sl' s. \<lbrakk> isFinal (IRQHandlerCap irq) sl (cteCaps_of s); sl \<noteq> sl' \<rbrakk>
                    \<Longrightarrow> cteCaps_of s sl' \<noteq> Some (IRQHandlerCap irq)"
    by (fastforce simp: isFinal_def gen_isCap_simps sameObjectAs_def sameRegionAs_IRQHandlerCap_eq)
     ...
     apply (wp | simp)+
        ...
        apply (wp hyps)
          apply ((wp preemptionPoint_invE preemptionPoint_invR
              | clarsimp simp: sch_act_simple_def ex_cte_cap_wp_to'_def
       ...
       apply (erule disjE[where P="F \<and> G" for F G])
        ...
        apply (rule conjI, clarsimp)
        apply (case_tac b; case_tac "cteCap rv"; simp add: post_cap_delete_pre'_IRQHandlerCap; clarsimp)
          apply (clarsimp simp: final_IRQHandler_no_copy)
          apply (drule (1) ctes_of_valid'[OF _ invs_valid_objs'])
          apply (clarsimp simp: valid_cap'_def)
         apply (drule (1) ctes_of_valid'[OF _ invs_valid_objs'])
         apply (clarsimp simp: valid_cap'_def final_post_cap_delete_pre'_ArchObjectCap)
       apply (clarsimp dest!: isCapDs)
       apply (rule conjI)
        ...
        apply (rule conjI)
         ...
         apply (case_tac "cteCap rv",
                simp_all add: gen_isCap_simps removeable'_def
        ...
        apply (case_tac "cteCap rv",
               simp_all add: gen_isCap_simps removeable'_def
                             fun_eq_iff[where f="cte_refs' cap" for cap]
                             fun_eq_iff[where f=tcb_cte_cases]
                             tcb_cte_cases_neqs tcb_cte_cases_def)[1]
         apply (frule Q)
         apply clarsimp
        ...
        apply (subst(asm) R)
         apply (drule valid_capAligned [OF ctes_of_valid'])
          apply fastforce
         apply (clarsimp simp add: capAligned_def gen_objBits_simps)
         apply (metis exp_add_not_zero_imp_right less_exp not_less0 unat_eq_zero
                      unat_power_lower_mw)
        apply (frule Q)
        apply clarsimp
       ...
       apply (case_tac "cteCap rv",
              simp_all add: gen_isCap_simps final_matters'_def)[1]
...
lemma finaliseSlot_invs'':
  "\<lbrace>\<lambda>s. invs' s \<and> (\<not> exposed \<longrightarrow> ex_cte_cap_to' slot s)\<rbrace>
   finaliseSlot slot exposed
   \<lbrace>\<lambda>rv s. invs' s \<and> (fst rv \<longrightarrow> cte_wp_at' (\<lambda>cte. removeable' slot s (cteCap cte)) slot s)
                   \<and> (snd rv \<noteq> NullCap \<longrightarrow> post_cap_delete_pre' (snd rv) slot (cteCaps_of s))\<rbrace>,
...
lemma finaliseSlot_invs:
  "\<lbrace>\<lambda>s. invs' s \<and> (\<not> e \<longrightarrow> ex_cte_cap_to' slot s)\<rbrace> finaliseSlot slot e \<lbrace>\<lambda>rv. invs'\<rbrace>"
  by (fastforce intro!: validE_valid intro: hoare_strengthen_postE finaliseSlot_invs'')

lemma finaliseSlot_removeable:
  "\<lbrace>\<lambda>s. invs' s \<and> (\<not> e \<longrightarrow> ex_cte_cap_to' slot s)\<rbrace>
   finaliseSlot slot e
   \<lbrace>\<lambda>rv s. fst rv \<longrightarrow> cte_wp_at' (\<lambda>cte. removeable' slot s (cteCap cte)) slot s\<rbrace>,-"
  by (fastforce intro!: validE_validE_R intro: hoare_strengthen_postE finaliseSlot_invs'')
...
lemma finaliseSlot_cte_wp_at:
  "\<lbrakk> \<And>cap. P cap \<Longrightarrow> isZombie cap; p \<noteq> slot \<rbrakk> \<Longrightarrow>
   \<lbrace>\<lambda>s. invs' s \<and> ex_cte_cap_to' slot s \<and> cte_wp_at' (\<lambda>cte. P (cteCap cte)) p s\<rbrace>
...
lemmas reduceZombie_invs'
    = reduceZombie_invs''[where Q=\<top>, simplified no_cte_prop_top simp_thms
         finalise_prop_stuff_def irq_state_independent_H_def arch_finalise_prop_stuff_top updateTimeStamp_independent_def
         getCurrentTime_independent_H_def time_state_independent_H_def domain_time_independent_H_def,
...
lemma reduceZombie_invs:
  "\<lbrace>\<lambda>s. invs' s \<and> (\<not> exposed \<longrightarrow> ex_cte_cap_to' slot s)
...
lemma reduceZombie_cap_to:
  "\<lbrace>\<lambda>s. invs' s \<and> (\<not> exposed \<longrightarrow> ex_cte_cap_to' slot s)
           \<and> cte_wp_at' (\<lambda>cte. cteCap cte = cap) slot s\<rbrace>
   reduceZombie cap slot exposed
   \<lbrace>\<lambda>_ s. \<not> exposed \<longrightarrow> ex_cte_cap_to' slot s\<rbrace>, -"
...
lemma cteDelete_invs':
  "\<lbrace>invs' and K ex\<rbrace> cteDelete ptr ex \<lbrace>\<lambda>rv. invs'\<rbrace>"
...
lemma cteDelete_cte_wp_at_invs:
  "\<lbrakk> \<And>cap. P cap \<Longrightarrow> isZombie cap \<rbrakk> \<Longrightarrow>
   \<lbrace>\<lambda>s. invs' s \<and> ex_cte_cap_to' slot s \<and>
       cte_wp_at' (\<lambda>cte. P (cteCap cte)) p s\<rbrace>
   cteDelete slot False
   \<lbrace>\<lambda>rv. cte_at' slot and invs'
  ...
  apply (cases "p = slot")
   apply (cases "\<exists>cp. P cp")
    ...
    apply (rule_tac Q'="\<lambda>rv s. invs' s \<and>
      ...
      apply (wp finaliseSlot_invs finaliseSlot_removeable hoare_drop_impE_R[OF finaliseSlot_irqs])
...
crunch schedContextMaybeUnbindNtfn, cleanReply
...
lemma cteDelete_st_tcb_at':
  assumes x[simp]: "\<And>st. simple' st \<Longrightarrow> P st"
  shows "cteDelete slot ex \<lbrace>st_tcb_at' P t\<rbrace>"
...
lemma rvk_prog_modify_map:
  "\<lbrakk> \<And>x. Some x = m p \<Longrightarrow>
         ...
         \<or> rpo_measure p (Some (capToRPO (f x))) < rpo_measure p (Some (capToRPO x)) \<rbrakk>
...
crunch finaliseCap
  ...
  (wp: crunch_wps emptySlot_rvk_prog' threadSet_ctesCaps_of hoare_vcg_all_lift Arch_finaliseCap_rvk_prog'
   simp: crunch_simps
     ...
     apply wp
        ...
        apply (wp "1.hyps")
          ...
          apply (wp preemptionPoint_inv)[1]
            ...
            apply (clarsimp simp: getCurrentTime_independent_H_def)
           apply (clarsimp simp: time_state_independent_H_def)
          apply (clarsimp simp: domain_time_independent_H_def)
         apply (wp capSwap_rvk_prog | simp only: withoutPreemption_def)+
...
lemma spec_corres_locate_Zombie:
  ...
  unfolding spec_corres_def
  apply (simp add: locateSlot_conv stateAssert_def bind_assoc)
  apply (rule corres_symb_exec_r[OF _ get_sp])
    apply (rule corres_assume_pre, clarsimp simp: gen_isCap_simps)
    ...
    apply (subst(asm) cte_map_nat_to_cref)
      ...
      apply simp
     apply (clarsimp simp: valid_cap_def cap_aligned_def split: option.split_asm)
...
lemma spec_corres_req:
  ...
  unfolding spec_corres_def
  by (rule corres_assume_pre, erule meta_mp)
      simp
...
primrec rec_del_concrete :: "rec_del_call \<Rightarrow> (bool \<times> capability) kernel_p set" where
...
lemma cap_relation_removables:
  ...
  apply (cases cap'; simp add: gen_isCap_simps)
   (* Nullcap *)
   apply (clarsimp simp: capRemovable_def capCyclicZombie_def)
  (* Zombie *)
  apply (cases cap; clarsimp simp: gen_isCap_simps)
  apply (clarsimp simp: capRemovable_def capCyclicZombie_def cap_cyclic_zombie_def)
   ...
   spec_corres s (dc \<oplus> (case args of
      ...
      (einvs and valid_machine_time and simple_sched_action
            ...
            and (\<lambda>s. case args of ReduceZombieCall cap sl ex \<Rightarrow>
                                ...
                                | _ \<Rightarrow> True))
      (invs' and cte_at' (cte_map (slot_rdcall args)) and
  ...
  show ?case
    ...
    apply (rule spec_corres_guard_imp)
      apply (rule spec_corres_splitE)
       ...
       apply (wpsimp wp: rec_del_invs rec_del_valid_list rec_del_valid_sched rec_del_cte_at
                         finaliseSlot_invs hoare_drop_imps preemption_point_inv'
            | simp)+
    done
next
  case (2 slot exposed)
    ...
    apply (rule spec_corres_guard_imp)
      apply (rule spec_corres_splitE')
        ...
        apply (rule spec_corres_splitE')
          ...
          apply (rule spec_corres_splitE')
            ...
            apply (rule spec_corres_req[rotated])
             ...
             apply (rule spec_corres_splitE')
                ...
                apply (case_tac "fst rvb", auto simp: gen_isCap_simps is_cap_simps)[1]
               apply (rule spec_corres_splitE)
                  ...
                  apply (case_tac "fst rvb", simp_all add: gen_isCap_simps is_cap_simps)[1]
               ...
               apply ((wpsimp wp: reduceZombie_cap_to reduceZombie_invs
                       | strengthen invs_valid_objs')+)[1]
              apply simp
           ...
           apply (rule hoare_strengthen_post)
            apply (rule_tac Q="\<lambda>fin s. einvs s \<and> valid_machine_time s  \<and> simple_sched_action s
             ...
             apply (wp finalise_cap_invs finalise_cap_replaceable
                       finalise_cap_makes_halted finalise_cap_valid_list
          ...
          apply (case_tac "cteCap rv'",
                 auto simp add: gen_isCap_simps is_cap_simps final_matters'_def pred_tcb_at'_def
                                obj_at'_def)[1]
     ...
     apply clarsimp
     apply (frule(1) cte_wp_at_valid_objs_valid_cap[where P="(=) cap" for cap,
                                                    OF _ invs_valid_objs])
     supply split_paired_Ex[simp del]
...
next
  ...
  show ?case
      ...
      apply (rule spec_corres_splitE)
        ...
        apply (rule corres_split[OF get_cap_corres])
          ...
          apply (rule_tac P="invs and cte_wp_at (\<lambda>c. c = new_cap) slot
                     ...
                     and  P'="invs' and cte_wp_at' (\<lambda>c. c = ourCTE) (cte_map slot)
    ...
    apply (frule valid_Zombie_cte_at'[where n=n])
     apply (clarsimp simp: valid_cap'_def)
    apply (intro conjI)
     apply (fastforce simp: cte_wp_at_ctes_of cteSizeBits_cte_level_bits
                            mult.commute mult.left_commute)
    apply (clarsimp simp: ex_cte_cap_to'_def cte_wp_at_ctes_of)
    apply (rule_tac x="cte_map slot" in exI)
    apply (clarsimp simp: image_def)
    apply (rule_tac x="of_nat n" in bexI)
     apply (fastforce simp: cteSizeBits_cte_level_bits mult.commute mult.left_commute shiftl_t2n)
    apply simp
...
definition cteRevoke_recset ::
  "((machine_word \<times> kernel_state) \<times> (machine_word \<times> kernel_state)) set"
  where
  "cteRevoke_recset \<equiv>
     measure (\<lambda>(sl, s).
       (\<lambda>mp. \<Sum>x \<in> dom mp. rpo_measure x (mp x)) (option_map capToRPO \<circ> cteCaps_of s))"
...
lemma cteRevoke_invs':
  "cteRevoke ptr \<lbrace>invs'\<rbrace>"
  by (wpsimp wp: cteRevoke_preservation cteDelete_invs')
  ...
  show ?case
    ...
    apply (rule spec_corres_guard_imp)
      apply (rule spec_corres_splitE' [OF cteDelete_corres])
       ...
       apply (wpsimp wp: cteDelete_invs'
              | strengthen invs_valid_objs invs_cur_sc_tcb invs_psp_aligned invs_distinct
...
lemma updateCap_valid_objs [wp]:
  ...
  by (wpsimp wp: setCTE_valid_objs getCTE_wp)
     (erule cte_at_cte_wp_atD)
...
lemma m'_tranclD:
  ...
  by (erule trancl.induct;
      fastforce simp: m'_swp_eq intro: trancl_trans)
...
lemma m_cap:
  ...
  by (auto simp: m'_def n_def src dest modify_map_if neq)
...
lemma isIRQHandlerCap_cap'[simp]:
  ...
  using parency unfolding weak_derived'_def
  by (auto simp: weak_derived'_def gen_isCap_simps)

end (* mdb_move *)

(* there is already a mdb_move_gen in CSpace_R, use extra suffix to disambiguate *)
locale mdb_move_gen_CNodeInv_R = mdb_move +
  assumes sameRegion_cap'_src[simp]:
    "\<And>c. sameRegionAs cap' c = sameRegionAs src_cap c"
  assumes mdb_chunked_arch_assms_src[simp]:
    "mdb_chunked_arch_assms cap' = mdb_chunked_arch_assms src_cap"
  assumes sameRegionAs_parent_eq:
    "\<And>cap. sameRegionAs cap cap' = sameRegionAs cap src_cap"
  assumes valid_badges':
    "valid_badges m'"
begin
...
lemma chunked':
  ...
  apply (drule m'_cap)+
  apply (clarsimp simp: m'_trancl_eq split: if_split_asm)
    apply (erule_tac x=src in allE)
    apply (erule_tac x="s_d_swap p' src dest" in allE)
...
proof
  ...
  note dest_0[simp del] src_0[simp del] src_next[simp del] rtrancl0[simp del]
...
end (* mdb_move_gen_CNodeInv_R *)

lemma (in CNodeInv_R) cteMove_valid_pspace'[wp]:
  ...
  apply (rule_tac old_dest_node = "cteMDBNode cte" and src_cap = "cteCap ctea" in
                  mdb_move_cteMove_valid_mdb_helper)
  prefer 2
   apply (clarsimp simp: cte_wp_at_ctes_of weak_derived'_def gen_isCap_simps simp del: not_ex)
...
crunch cteMove
  ...
  and global_refs_noop[wp]: "\<lambda>s. P (global_refs' s)"
...
lemma (in CNodeInv_R) cteMove_invs' [wp]:
  ...
  apply (intro hoare_vcg_conj_lift_pre_fix;
         solves \<open>wpsimp wp: cteMove_urz cteMove_ifunsafe'
                 | wpsimp simp: cteMove_def o_def
                            wp: hoare_drop_imps valid_dom_schedule'_lift\<close>)
  done
...
lemma (in CNodeInv_R) invokeCNode_corres:
  ...
  apply (case_tac "has_cancel_send_rights prod",
   ...
   apply (clarsimp simp: when_def unless_def gen_isCap_simps)
   apply (rule corres_guard_imp)
...
lemma (in CNodeInv_R) invokeCNode_invs' [wp]:
  "\<lbrace>invs' and valid_cnode_inv' cinv\<rbrace>
   invokeCNode cinv
   \<lbrace>\<lambda>_. invs'\<rbrace>"
...
lemma finaliseSlot_irq_states':
  ...
  by (wpsimp wp: finaliseSlot_preservation)

lemma cteDelete_irq_states':
  "cteDelete x y \<lbrace>valid_irq_states'\<rbrace>"
