...
lemma suspend_not_recursive_ctes:
  ...
  apply (auto simp: gen_isCap_simps finaliseCap_def Let_def)
  done

lemma unbindNotification_not_recursive_ctes:
  "unbindNotification t \<lbrace>\<lambda>s. P (not_recursive_ctes s)\<rbrace>"
...
lemma (in CNodeInv_R) make_zombie_invs':
  ...
  apply (intro conjI[rotated])
     ...
     apply (auto elim: if_unsafe_then_capD' simp: gen_isCap_simps)[1]
   ...
   apply (subgoal_tac "st_tcb_at' ((=) Inactive) p' s
    ...
    apply (clarsimp simp: pred_tcb_at'_def obj_at'_def ko_wp_at'_def live'_def)
...
lemmas reduceZombie_invs'
    = reduceZombie_invs''[where Q=\<top>, simplified no_cte_prop_top simp_thms
         finalise_prop_stuff_def irq_state_independent_H_def arch_finalise_prop_stuff_top,
...
crunch finaliseCap
  ...
  (wp: crunch_wps emptySlot_rvk_prog' threadSet_ctesCaps_of Arch_finaliseCap_rvk_prog'
   simp: crunch_simps
    ...
    apply (rule spec_corres_guard_imp)
      apply (rule spec_corres_splitE')
        ...
        apply (rule spec_corres_splitE')
          ...
          apply (case_tac "cteCap rv'",
                 auto simp add: gen_isCap_simps is_cap_simps final_matters'_def)[1]
...
crunch updateCap
  ...
  and rdyq_projs[wp]:
        "\<lambda>s. P (ksReadyQueues s) (tcbSchedNexts_of s) (tcbSchedPrevs_of s)
...
lemma (in CNodeInv_R) invokeCNode_corres:
  ...
  apply (cases ci, simp_all)
   ...
   apply (simp add: getThreadCallerSlot_def locateSlot_conv)
   apply (rule corres_guard_imp)
     apply (rule corres_split[OF getCurThread_corres])
       ...
       (is_reply_cap cap \<longrightarrow>
        ...
        P'="\<lambda>s. (isReplyCap rv' \<and> \<not> capReplyMaster rv') \<longrightarrow> (invs' and
         ...
         apply (case_tac cap, simp_all add: gen_isCap_simps is_cap_simps split: bool.split)[1]
         apply clarsimp
         apply (rule corres_guard_imp)
  ...
  apply (rename_tac c)
  apply clarsimp
  apply (case_tac "has_cancel_send_rights c",
...
lemma (in CNodeInv_R) invokeCNode_invs' [wp]:
  ...
  apply (cases cinv)
   ...
   apply (clarsimp simp: cte_wp_at_ctes_of)
   apply (fastforce simp: gen_isCap_simps weak_derived'_def) (* ~10s *)
...
proof (induct rule: finalise_spec_induct)
  ...
  show ?case
    apply (rule hoare_pre_spec_validE)
     apply (subst finaliseSlot'_simps_ext)
     apply (simp only: split_def)
     apply (wp "1.hyps" withoutPreemption_lift)
            apply (unfold Let_def split_def fst_conv snd_conv
                          case_Zombie_assert_fold haskell_fail_def)
            apply (wp getCTE_wp' preemptionPoint_invR | simp add: o_def)+
...
proof (induct rule: cteRevoke.induct)
  ...
  show ?case
    apply (subst cteRevoke.simps)
    apply (wp "1.hyps" unlessE_wp whenE_wp preemptionPoint_IRQInactive_spec withoutPreemption_lift
...
lemma inv_cnode_IRQInactive:
  "\<lbrace>valid_irq_states'\<rbrace>
   invokeCNode cnode_inv
   -, \<lbrace>\<lambda>rv s. intStateIRQTable (ksInterruptState s) rv \<noteq> irqstate.IRQInactive\<rbrace>"
  unfolding invokeCNode_def
  by (wpsimp wp: cteRevoke_IRQInactive finaliseSlot_IRQInactive cteDelete_IRQInactive
                 whenE_wp withoutPreemption_lift
           simp:  split_def)
