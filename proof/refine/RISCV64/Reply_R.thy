...
lemma replyRemoveTCB_st_tcb_at'_sym_ref:
  "\<lbrace>(\<lambda>s. tcb_at' tptr s \<longrightarrow>
   ...
   replyRemoveTCB tptr
   \<lbrace>\<lambda>_. st_tcb_at' test tptr\<rbrace>"
  unfolding replyRemoveTCB_def cleanReply_def updateReply_def updateSchedContext_def
...
lemma replyUnlink_valid_replies'[wp]:
  ...
  apply (clarsimp simp: pred_tcb_at'_def obj_at'_def replyUnlink_assertion_def)
  by (auto simp: opt_map_def split: option.splits)

lemma replyUnlink_valid_pspace'[wp]:
  "\<lbrace>\<lambda>s. valid_pspace' s \<and> (replyTCBs_of s rptr = Some tptr \<longrightarrow> \<not> is_reply_linked rptr s)\<rbrace>
   replyUnlink rptr tptr
...
lemma setSchedContext_valid_tcbs'[wp]:
  "setSchedContext scPtr sc \<lbrace>valid_tcbs'\<rbrace>"
...
lemma setThreadState_not_queued_valid_sched_pointers:
  "\<lbrace>\<lambda>s. valid_sched_pointers s \<and> \<not> (inIPCQueueThreadState |< tcbStates_of' s) t\<rbrace>
   setThreadState st t
   \<lbrace>\<lambda>_. valid_sched_pointers\<rbrace>"
  by (wpsimp wp: setThreadState_valid_sched_pointers)

lemma replyRemove_valid_sched_pointers[wp]:
  "replyRemoveTCB tptr \<lbrace>valid_sched_pointers\<rbrace>"
  apply (clarsimp simp: replyRemoveTCB_def replyUnlink_def)
  apply (rule bind_wp[OF _ stateAssert_sp])
  apply (wpsimp wp: setThreadState_not_queued_valid_sched_pointers
                    gts_wp' hoare_vcg_all_lift hoare_drop_imps)
  apply (clarsimp simp: st_tcb_at'_def)
  apply normalise_obj_at'
  apply (rename_tac tcb, case_tac "tcbState tcb";
         clarsimp simp: isReply_def obj_at'_def opt_pred_def opt_map_def)
  done
...
lemma set_reply_corres: (* for reply update that doesn't touch the reply stack *)
  "reply_relation ae ae' \<Longrightarrow>
   corres dc \<top> (obj_at' (\<lambda>ko. replyPrev_same ae' ko) ptr)
     (set_reply ptr ae) (setReply ptr ae')"
  apply (rule_tac Q="reply_at ptr" in corres_cross_add_abs_guard)
   apply (fastforce intro: reply_at'_cross simp: obj_at'_def)
  apply (rule corres_underlying_from_rcorres)
   apply (wpsimp simp: setReply_def obj_at'_def gen_objBits_simps)
  ...
  apply (rule rcorres_conj_lift_fwd)
    \<comment>\<open>replies_relation\<close>
    apply wpsimp
   apply (rule rcorres_from_valid_det)
    apply wpsimp
   apply (drule in_set_reply)
   apply (wpsimp simp: setReply_def)
   apply (fastforce intro: map_relation_fun_upd simp flip: fun_upd_def)
  apply (rcorres_conj_lift fastforce)+
  apply (rule rcorres_conj_lift_fwd)
    \<comment>\<open>sc_replies_relation\<close>
    apply wpsimp
   apply (rule rcorres_weaken_pre)
    apply (rule_tac Q="\<lambda>s s'. reply_at ptr s \<and> obj_at' (replyPrev_same ae') ptr s'"
                 in sc_replies_relation_lift_rcorres;
           wpsimp)
    apply (rename_tac P s s')
    apply (erule_tac P=P in rsubst)
    apply (fastforce simp: obj_at'_def replyPrev_same_def opt_map_def)
   apply fastforce
  apply (rcorres_conj_lift fastforce)+
  done

lemma not_in_replies_with_sc_not_in_set:
  "\<lbrakk>ptr \<notin> fst ` replies_with_sc s; sc_replies_of s p = Some replies\<rbrakk> \<Longrightarrow> ptr \<notin> set replies"
   by (fastforce simp: sc_replies_of_scs_def map_project_simps scs_of_kh_def
                       sc_at_pred_n_def obj_at_def replies_with_sc_def image_def opt_map_def
                split: option.splits)
...
lemma setReply_not_queued_corres: (* for reply updates on replies not in fst ` replies_with_sc *)
  ...
  apply (rule rcorres_conj_lift_fwd)
   ...
   apply (rule rcorres_from_valid_det)
    apply wpsimp
   apply (drule in_set_reply)
   apply (wpsimp simp: setReply_def wp: set_reply'.setObject_wp)
   apply (clarsimp simp: sc_replies_relation_def)
   apply (fold fun_upd_def)
   apply (rename_tac s s' p replies)
   apply (prop_tac "((scs_of' s')(ptr := sc_of' (KOReply r2)) |> scReply) p = scReplies_of s' p")
    apply (clarsimp simp: obj_at'_def opt_map_def split: option.splits)
   apply (prop_tac "sc_replies_of s p = Some replies")
    subgoal
      by (fastforce simp: sc_replies_of_scs_def map_project_simps scs_of_kh_def split: if_splits)
   apply (fastforce elim!: heap_path_heap_upd_not_in not_in_replies_with_sc_not_in_set)
  apply (rcorres_conj_lift fastforce)+
  done
...
defs valid_bound_reply'_asrt_def:
  "valid_bound_reply'_asrt replyOpt \<equiv> valid_bound_reply' replyOpt"

declare valid_bound_reply'_asrt_def[simp]

lemma bindScReply_corres:
  "corres dc
     (reply_at rptr and sc_at scptr and (\<lambda>s. rptr \<notin> fst ` replies_with_sc s)
      and pspace_aligned and pspace_distinct and valid_objs
      and valid_replies and (\<lambda>s. sym_refs (state_refs_of s)))
     (reply_at' rptr)
     (bind_sc_reply scptr rptr) (bindScReply scptr rptr)"
   ...
   apply (rule corres_guard_imp)
    ...
    (* This command is the lynch-pin. This does all useful state-relation lifting
       ...
       apply (rule_tac Q'="reply_at' rptr and ko_at' sc' scptr
                ...
                and K (rptr \<notin> set (sc_replies sc))
         ...
         apply (rule corres_stateAssert_ignore)
          apply (clarsimp simp: valid_bound_reply'_def split: option.splits)
         apply (rule corres_guard_imp)
           apply (rule corres_split [where r'=dc])
              ...
              apply clarsimp
              apply (frule pspace_aligned_cross, fastforce)
              apply (frule (1) pspace_distinct_cross, fastforce)
             ...
             apply (rule corres_split[OF updateReply_replyPrev_corres])
               ...
               apply (rule corres_symb_exec_r')
                  ...
                  apply (rule corres_split[OF update_sc_reply_stack_update_ko_at'_corres])
                    apply (rule updateReply_replyPrev_same_corres[unfolded dc_def])
                    apply (clarsimp simp: reply_relation_def)
                   apply (wpsimp wp: updateReply_reply_projs get_sc_inv')+
                apply (rename_tac sc sc' n)
                apply (rule_tac Q'="\<lambda>_. reply_at' rptr and ko_at' sc' scptr
                                        and (\<lambda>s. heap_ls (replyPrevs_of s) (replyPrevs_of s rptr)
                                                         (sc_replies sc))
                                        and K (rptr \<notin> set (sc_replies sc))"
            ...
            apply (rule_tac Q'="\<lambda>_. reply_at' rptr and ko_at' sc' scptr
                                    and (\<lambda>s. heap_ls (replyPrevs_of s) (Some y) (sc_replies sc))
                                    and K (rptr \<notin> set (sc_replies sc))"
                         in hoare_strengthen_post[rotated])
             apply clarsimp
...
lemma updateReply_replyTCB_invs':
  "\<lbrace>invs' and case_option \<top> (\<lambda>t. tcb_at' t) p and
  ...
  apply (wpsimp wp: updateReply_valid_objs' updateReply_list_refs_of_replies'_inv
                    updateReply_valid_replies'
...
lemma bindScReply_valid_objs'[wp]:
  "\<lbrace>valid_objs' and reply_at' replyPtr\<rbrace>
   ...
   \<lbrace>\<lambda>_. valid_objs'\<rbrace>"
  unfolding bindScReply_def updateSchedContext_def
  ...
  apply (wpsimp wp: updateReply_valid_objs')
        ...
        apply wpsimp
       apply (rule_tac Q'="\<lambda>_. valid_objs' and reply_at' replyPtr and ko_at' sc scp"
                    in hoare_strengthen_post)
        apply (wpsimp wp: updateReply_valid_objs')
       apply clarsimp
       apply (rule conjI)
        apply (clarsimp simp: valid_obj'_def refillSize_def valid_sched_context'_def
                       dest!: sc_ko_at_valid_objs_valid_sc' split: if_splits)
       apply fastforce
      apply (rule_tac Q'="\<lambda>_ s. valid_objs' s \<and> reply_at' replyPtr s \<and> ko_at' sc scp s
                                \<and> scReply sc = scReplyOpt
                                \<and> (scReplyOpt \<noteq> None \<longrightarrow> reply_at' (the scReplyOpt) s)"
                   in hoare_strengthen_post)
       apply (wpsimp wp: updateReply_valid_objs')
      apply (clarsimp simp: valid_reply'_def  valid_sched_context'_def valid_bound_obj'_def
                     dest!: sc_ko_at_valid_objs_valid_sc' split: if_splits option.splits)
     apply wpsimp+
  apply (clarsimp simp: valid_reply'_def)
  done
...
lemma bindScReply_valid_replies'[wp]:
  "\<lbrace>\<lambda>s. valid_replies' s \<and> pspace_distinct' s \<and> pspace_aligned' s \<and> pspace_bounded' s
   ...
   \<lbrace>\<lambda>_. valid_replies'\<rbrace>"
  apply (solves wp
         | simp (no_asm_use) add: bindScReply_def updateSchedContext_def
                       split del: if_split cong: conj_cong
  ...
  apply (intro conjI impI allI; clarsimp?)
      ...
      apply ((erule impCE)?; fastforce simp: obj_at'_def elim!: opt_mapE)+
  done
...
lemma bindScReply_obj_at'_scTCB[wp]:
  ...
  unfolding bindScReply_def updateSchedContext_def
  apply (wpsimp wp: set_sc'.obj_at' hoare_vcg_all_lift | wp (once) hoare_vcg_imp_lift')+
...
lemma sym_refs_replySCs_of_None:
  ...
  apply (clarsimp simp: obj_at'_def elim!: opt_mapE)
...
lemma prev_reply_in_sc_replies:
  "\<lbrakk>sc_replies_relation s s'; pspace_relation (kheap s) (ksPSpace s');
    sc_with_reply rp s = Some scp; sym_refs (list_refs_of_replies' s');
...
lemma sc_with_reply_replyPrev_None:
  ...
  apply (prop_tac "reply_at rp s")
   ...
   apply (erule (1) obj_relation_cutsE)
         apply ((clarsimp simp: is_reply obj_at_def is_other_obj_relation_type_def
                                other_aobj_relation_def
                         split: Structures_A.kernel_object.split_asm if_split_asm
                                RISCV64_A.arch_kernel_obj.split_asm)+)[9]
...
lemma sc_with_reply_replyNext_None:
  ...
  apply (frule pspace_relation_pspace_bounded')
  apply (frule (8) prev_reply_in_sc_replies)
...
lemma pspace_relation_reply_at:
  ...
  apply (erule (1) pspace_dom_relatedE)
  apply (erule (1) obj_relation_cutsE;
         clarsimp simp: is_reply obj_at_def is_other_obj_relation_type_def other_aobj_relation_def
                 split: Structures_A.kernel_object.split_asm if_split_asm
...
lemma updateReply_sr_inv_next:
  ...
  apply (clarsimp simp: caps_of_state_after_update cte_wp_at_after_update
                        swp_def fun_upd_def obj_at_def map_to_ctes_upd_other
              simp del: fun_upd_def)
  apply (prop_tac "typ_at' (koTypeOf (injectKO (replyNext_update (\<lambda>_. None) reply))) rp s'")
   apply (clarsimp simp: typ_at'_def ko_wp_at'_def obj_at'_def)
  apply (frule tcbs_of'_non_tcb_update)
   apply clarsimp
  apply (extract_conjunct \<open>match conclusion in "ep_queues_relation_2 _ _ _ _" \<Rightarrow> -\<close>)
...
lemma sc_replies_update_takeWhile_sc_with_reply:
  "\<lbrace>(\<lambda>s. sc_with_reply rp s = Some scp) and valid_replies\<rbrace>
   ...
   \<lbrace>\<lambda>_ s. sc_with_reply rp s = None\<rbrace>"
  ...
  apply (rename_tac sc n)
  apply (prop_tac "\<not> rp \<in> set (takeWhile ((\<noteq>) rp) (sc_replies sc))")
  ...
  show ?thesis using assms
    ...
    apply (rule_tac Q'="reply_at' rp and reply_at' nrp and sc_at' scp
     ...
     apply (intro conjI, simp)
        ...
        apply (rule valid_objs_sc_replies_reply_at[where sc_ptr=scp])
         apply fastforce
        ...
        apply (elim impE)
         apply (clarsimp simp: sc_replies_of_scs_def map_project_def scs_of_kh_def opt_map_def)
        apply (frule (1) heap_ls_prev_cases[where np=rp and p=nrp])
          apply (fastforce simp: sym_refs_replyNext_replyPrev_sym)
         apply (erule reply_sym_heap_Prev_Next)
      ...
      apply (fastforce dest!: sym_refs_cross)
     apply (fastforce dest: state_relationD simp: sc_replies_relation_sc_with_reply_cross_eq)
    (* corres proof *)
    apply (clarsimp simp: corres_underlying_def)
    apply (rename_tac s s')
    apply (rule conjI)
     apply (clarsimp simp: update_sched_context_def get_sched_context_def
                           updateReply_def getReply_def
     ...
     apply (extract_conjunct \<open>match conclusion in "sc_replies_relation_2 _ _ _" \<Rightarrow> -\<close>)
      ...
      (* p =  scp *)
       ...
       apply (rule heap_path_heap_upd_not_in[rotated])
        using set_takeWhileD apply (metis (mono_tags, lifting))
      ...
      apply (prop_tac "tcbs_of' s' nrp = None")
       apply (clarsimp simp: opt_map_def projectKO_opts_defs split: option.splits)
      apply (extract_conjunct \<open>match conclusion in "ep_queues_relation_2 _ _ _ _" \<Rightarrow> -\<close>)
     ...
     apply (rule conjI)
      ...
      apply (rule conjI)
       ...
       apply (erule heap_ls_cong)
         subgoal by (fastforce simp: opt_map_def projectKO_opts_defs)
        apply fastforce
       apply fastforce
      apply (clarsimp simp: prev_queue_head_def opt_map_def inQ_def opt_pred_def split: option.splits)
      apply blast
     apply (clarsimp simp: release_queue_relation_def Let_def list_queue_relation_def)
     apply (rule conjI)
      apply (fastforce intro: rsubst3[where P=heap_ls] simp: opt_map_def split: option.splits)
     apply (clarsimp simp: prev_queue_head_def opt_map_def inQ_def opt_pred_def split: option.splits)
     apply blast
