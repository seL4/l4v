...
lemma valid_sched_init[simp]:
  ...
  apply (clarsimp simp: init_kheap_def obj_at_def idle_thread_ptr_def not_queued_def
                        init_irq_node_ptr_def riscv_global_pt_ptr_def init_global_pt_def
                        ct_not_in_q_def valid_sched_action_def is_activatable_def
...
<<<<<<< HEAD
  ...
  apply (rule_tac Q'="\<lambda>s'. event \<noteq> Interrupt \<longrightarrow> schedulable' (ksCurThread s') s'"
  ...
  apply (rule corres_guard_imp)
    ...
    apply (wpsimp wp: he_invs')
      apply (wpsimp simp: mcsPreemptionPoint_def wp: getSchedulable_wp)
     apply (wpsimp wp: dmo_getirq_inv)
     apply (clarsimp simp: schedulable'_def active_sc_tcb_at'_def)
...
=======
  ...
  apply (corres simp: irq_state_independent_def
                corres: handleEvent_corres maybeHandleInterrupt_corres
         | corres_cases_both)+
      ...
      apply (wpsimp wp: handle_interrupt_valid_sched[unfolded non_kernel_IRQs_def, simplified]
                        schedule_invs' hoare_vcg_if_lift2 hoare_drop_imps handle_spurious_irq_invs
  ...
  apply (erule_tac P="a \<and> (\<exists>x. b x)" for a b in disjE)
   ...
   apply (drule use_valid[OF _ valid_corres_combined])
     ...
     apply (intro conjI)
      ...
      apply (rule conjI)
       apply (clarsimp simp: ct_in_state_def pred_tcb_at_def obj_at_def)
      apply (rule schact_is_rct_ct_active_sc)
        apply fastforce
