...
lemma threadGet_rcorres:
  ...
  apply (clarsimp simp: gets_the_thread_read' threadGet_def)
...
\<comment> \<open>A form that is more useful when used alongside orderedInsert_rcorres\<close>
lemmas threadGet_rcorres' =
  threadGet_rcorres[simplified gets_the_thread_read' threadGet_def comp_def]
...
lemma tcbEPAppend_corres:
  ...
  apply (rcorres_conj_lift fastforce intro: det_wp_tcb_append_set_endpoint simp: tcbAppend_def)+
  ...
  apply (rule rcorres_conj_lift_fwd; (solves ipc_append)?)
   ...
   apply (rule_tac p=release_queue in rcorres_lift_abs)
    ...
    apply wpsimp
   apply wpsimp
  by (rcorres_conj_lift fastforce intro: det_wp_tcb_append_set_endpoint simp: tcbAppend_def)+

end (* anonymous context block for the private method ipc_append *)
...
lemma tcbReleaseEnqueue_corres:
  ...
  apply (rcorres_conj_lift fastforce
                           intro: ready_times_append_rules
  ...
  apply (rule rcorres_conj_lift_fwd; (solves rlq_append)?)
   ...
   apply (rule rcorres_conj_lift_fwd; (solves rlq_append)?)
    ...
    apply (rule_tac p="\<lambda>s. t' \<in> set (ready_queues s d p)" in rcorres_lift_abs)
     ...
     apply (wpsimp wp: threadSet_inQ_unchanged)
    apply wpsimp
  ...
  by (rcorres_conj_lift fastforce intro: ready_times_append_rules)+
  ...
  apply (case_tac "ntfn_obj ntfn"; clarsimp)
   ...
   apply (rule corres_assert_gen_asm_cross_forwards)
    apply (fastforce dest!: st_tcb_at_coerce_concrete
                      simp: thread_state_relation_def st_tcb_at'_def obj_at'_def isBlockedOnNtfn_def)
   apply (rule stronger_corres_guard_imp)
              ...
              apply (case_tac ep)
               ...
               apply (rule corres_guard_imp)
                 apply (rule corres_split[OF tcbEPDequeue_corres], simp, simp)
                   ...
                   apply (rule corres_split[OF getThreadState_corres])
                     apply (rule_tac F="\<exists>data. sender_state = Structures_A.BlockedOnSend epptr data"
                                  in corres_gen_asm)
...
lemma tcbNTFNAppend_corres:
  ...
  apply (rule_tac Q="\<lambda>s s'. (\<forall>t \<in> set (ntfn_queue (ntfn_obj ntfn)). tcb_at t s)
   ...
   apply (clarsimp simp: state_relation_def pspace_relation_heap_pspace_relation
                         heap_pspace_relation_def ghost_relation_heap_ghost_relation)
  apply (rcorres_conj_lift fastforce
                           intro: det_wp_tcb_append_set_notification
                           simp: tcbAppend_def)+
  ...
  apply (rule rcorres_conj_lift_fwd; (solves ipc_ntfn_append)?)
   ...
   apply (rule_tac p=release_queue in rcorres_lift_abs)
    ...
    apply wpsimp
   apply wpsimp
  by (rcorres_conj_lift fastforce
                        intro: det_wp_tcb_append_set_notification
                        simp: tcbAppend_def)+
...
lemma ri_invs' [wp]:
  ...
  apply (fastforce simp: opt_map_def obj_at'_def split: if_splits)
  done
