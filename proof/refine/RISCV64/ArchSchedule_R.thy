...
lemma arch_switchToThread_corres:
  ...
  unfolding arch_switch_to_thread_def RISCV64_H.switchToThread_def
...
lemma Arch_switchToThread_invs[Schedule_R_assms, wp]:
  ...
  unfolding RISCV64_H.switchToThread_def by (wpsimp wp: getObject_tcb_hyp_sym_refs)
...
lemma Arch_switchToThread_invs_no_cicd'[Schedule_R_assms]:
  ...
  by (wpsimp wp: getObject_tcb_hyp_sym_refs setVMRoot_invs_no_cicd' simp: RISCV64_H.switchToThread_def)
...
lemmas [Schedule_R_assms] =
   (* part of DetSchedSchedule_AI_assms but not interfaced (RISCV64 only) *)
