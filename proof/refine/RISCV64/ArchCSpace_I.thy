...
interpretation CSpace_I?: CSpace_I RISCV64.arch_capMasterCap
proof goal_cases
  interpret Arch  .
...
(* no SGI or IO port caps on this arch, but keep this lemma name occupied for future-proofing and
   similarity to other arches  *)
lemmas arch_isCap_Master = isArchFrameCap_capMasterCap
...
lemma sameRegionAs_def2:
  ...
  apply (cases "isUntypedCap cap")
   apply (clarsimp simp: global.sameRegionAs_def Let_def
                         gen_isCap_Master capRange_Master capClass_Master)
...
interpretation CSpace_I_2?: CSpace_I_2 RISCV64.arch_capMasterCap
proof goal_cases
  interpret Arch  .
...
definition arch_mdb_preservation :: "capability \<Rightarrow> capability \<Rightarrow> bool" where
  "arch_mdb_preservation cap cap' \<equiv> True"

declare arch_mdb_preservation_def[simp]
