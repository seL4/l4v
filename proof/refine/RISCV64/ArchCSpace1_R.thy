...
lemma same_arch_region_as_relation:
  ...
  by (cases c; cases c')
     (auto simp: RISCV64_H.sameRegionAs_def global.sameRegionAs_def Let_def isCap_simps mask_def
...
lemma capASID_update[simp]:
  ...
  apply (case_tac arch_capability,
         simp_all add: global.updateCapData_def RISCV64_H.updateCapData_def isCap_simps Let_def)
  done
...
lemma obj_relation_cuts_in_obj_range[CSpace1_R_assms]:
  ...
  apply (case_tac arch_kernel_obj; simp)
   apply (clarsimp simp only: obj_range_def atLeastAtMost_iff bit_simps
...
(* there are no arch caps on this architecture for which there is a safe parent *)
definition
  "safe_parent_for_arch' m cap parent \<equiv> False"
...
lemma (in Arch_mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps RISCV64.isMDBParentOf_CTE is_derived'_def
...
lemma src_no_mdb_parent:
  ...
  apply (erule sameRegionAsE)
     ...
     subgoal by (cases src_cap; auto simp: capMasterCap_def arch_capMasterCap_def Retype_H.isCapRevocable_def RISCV64_H.isCapRevocable_def
