...
theorem call_kernel_serial:
  "\<lbrakk> (einvs and (\<lambda>s. event \<noteq> Interrupt \<longrightarrow> ct_running s) and (ct_running or ct_idle) and
      schact_is_rct and current_time_bounded and consumed_time_bounded and valid_machine_time and
      ct_not_in_release_q and cur_sc_active and (\<lambda>s. cur_sc_offset_ready (consumed_time s) s) and
     ...
     \<exists>s'. (s, s') \<in> state_relation \<and>
          (invs' and (\<lambda>s. event \<noteq> Interrupt \<longrightarrow> schedulable' (ksCurThread s) s) and
