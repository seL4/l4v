...
definition schedulable' :: "machine_word \<Rightarrow> kernel_state \<Rightarrow> bool" where
  "schedulable' tcbPtr s' \<equiv>
   ...
   \<and> ((\<lambda>sc. 0 < scRefillMax sc) |< (tcbs_of' s' |> tcbSchedContext |> scs_of' s')) tcbPtr
...
lemma sc_active_cross_eq:
  "\<lbrakk>(s, s') \<in> state_relation; pspace_aligned s; pspace_distinct s; tcb_at t s; valid_tcbs' s'\<rbrakk>
   \<Longrightarrow> (sc_active |< (tcbs_of s |> tcb_sched_context |> scs_of s)) t
       = ((\<lambda>sc. 0 < scRefillMax sc) |< (tcbs_of' s' |> tcbSchedContext |> scs_of' s')) t"
  apply (frule state_relation_pspace_relation)
  apply (frule (3) tcb_at_cross)
  apply normalise_obj_at'
...
lemma threadSet_schedulable':
  "\<lbrace>\<lambda>s. runnable' st
      \<and> ((\<lambda>sc. 0 < scRefillMax sc) |< (tcbs_of' s |> tcbSchedContext |> scs_of' s)) t
      \<and> (Not \<circ> tcbInReleaseQueue |< tcbs_of' s) t\<rbrace>
  ...
  apply (fastforce simp: opt_pred_def opt_map_def obj_at'_def split: option.splits)
  done
