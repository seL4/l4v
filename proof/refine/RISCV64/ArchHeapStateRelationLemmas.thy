...
(* FIXME arch-split: this proof and many in this file use obj_relation_cuts_def2 and
   arch_kernel_obj.split, so cannot be generic. *)
  ...
  apply (rule conjI)
   ...
   apply (intro conjI impI)
    ...
    apply (case_tac ako; clarsimp simp: ko_relations split: if_splits)
            ...
            apply (subgoal_tac "cte_map (ptr, y) \<in> dom (cnode_ctes_of' s')")
