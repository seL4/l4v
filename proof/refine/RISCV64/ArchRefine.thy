...
lemma pointerInUserData_relation[Refine_assms]:
  ...
  apply (drule_tac sz=sz and n="(p && mask (pageBitsForSize sz)) >> pageBits"
      ...
      apply (fastforce simp: and_mask_pbfs_shiftr_pageBits_limit)+
...
lemma pointerInDeviceData_relation[Refine_assms]:
  ...
  apply (drule_tac sz=sz and n="(p && mask (pageBitsForSize sz)) >> pageBits"
      ...
      apply (fastforce simp: invs'_def and_mask_pbfs_shiftr_pageBits_limit)+
  apply (erule arg_cong[where f="\<lambda>p. typ_at' _ p s'", THEN iffD1, rotated])
  apply (simp add: p_and_not_mask_pbfs_add_mask_pbfs_eq)
  done
...
lemma valid_sched_init[Refine_assms, simp]:
  ...
  apply (clarsimp simp: init_kheap_def obj_at_def idle_thread_ptr_def not_queued_def
                        init_irq_node_ptr_def init_global_pt_def riscv_global_pt_ptr_def
