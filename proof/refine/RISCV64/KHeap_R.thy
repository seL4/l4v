...
method readObject_obj_at'_method
  =  clarsimp simp: readObject_def obind_def omonad_defs split_def loadObject_default_def
                    obj_at'_def objBits_simps' scBits_pos_power2
...
lemma koType_objBitsKO:
  ...
  by (auto simp: objBitsKO_def archObjSize_def
          split: Structures_H.kernel_object.splits
                 RISCV64_H.arch_kernel_object.splits)
...
lemma getObject_def2:
  ...
  apply (rule monad_state_eqI)
    apply (force simp: getObject_def readObject_def gets_the_def exec_gets obind_def split_def
                       omonad_defs assert_opt_def fail_def return_def in_monad RISCV64_H.fromPPtr_def
                split: option.splits)+
...
lemma pspace_relation_tcb_at:
  ...
  by  (fastforce elim!: pspace_dom_relatedE obj_relation_cutsE
                  simp: other_obj_relation_def obj_at_def projectKOs is_tcb_def opt_map_def
...
method setObject_easy_cases =
  ...
  clarsimp simp: updateObject_cte updateObject_default_def in_monad
                 typeError_def opt_map_def opt_pred_def projectKO_opts_defs projectKO_eq
          split: if_split_asm
...
lemma ctes_of'_after_update:
  ...
  apply (rule if_cong)
   ...
   apply (cases val; clarsimp simp: tcb_cte_cases_def cteSizeBits_def)
  apply simp
  done
...
lemma pspace_aligned'[wp]: "f p v \<lbrace>pspace_aligned'\<rbrace>"
  ...
  and pspace_in_kernel_mappings'[wp]: "f p v \<lbrace>pspace_in_kernel_mappings'\<rbrace>"
...
lemma readObject_wp:
  "\<lblot>\<lambda>s. \<forall>ko :: 'a. ko_at' ko p s \<longrightarrow> P ko s\<rblot>
   readObject p
   \<lblot>P\<rblot>"
  apply (wpsimp simp: default_load RISCV64_H.fromPPtr_def loadObject_default_def
...
lemma getObject_sp:
  ...
  apply (clarsimp simp: getObject_def loadObject_default_def default_load
                        ...
                        split_def RISCV64_H.fromPPtr_def readObject_def omonad_defs
...
lemma aligned'_distinct'_ko_wp_at'I:
  ...
  apply (cases ko; force simp: valid_sz_simps simp: pspace_bounded'_def)
  done
...
lemma valid_idle'_cross:
  ...
  apply (intro conjI)
   ...
   apply (clarsimp simp: obj_at_simps idle_tcb'_def tcb_relation_def tcb_relation_cut_def)
...
lemma is_active_sc'_cross:
  ...
  apply (clarsimp simp: projectKOs is_active_sc2_def is_active_sc'_def opt_pred_def
...
lemma no_fail_setSchedContext[wp]:
  ...
  unfolding setSchedContext_def by (wpsimp simp: opt_map_def obj_at'_def projectKOs opt_pred_def)
...
lemma updateSchedContext_sc_obj_at'_inv:
  ...
  apply (wpsimp wp: set_sc'.obj_at')
  by (clarsimp split: if_splits simp: obj_at'_real_def ko_wp_at'_def)
...
lemma setObject_tcb_tcbs_of':
  ...
  by (setObject_easy_cases, clarsimp simp: RISCV64_H.fromPPtr_def)
