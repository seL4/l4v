...
definition APIType_map2 :: "kernel_object + RISCV64_H.object_type \<Rightarrow> Structures_A.apiobject_type"
  where
...
definition APIType_capBits :: "RISCV64_H.object_type \<Rightarrow> nat \<Rightarrow> nat" where
...
definition makeObjectKO :: "bool \<Rightarrow> nat \<Rightarrow> domain \<Rightarrow> (kernel_object + RISCV64_H.object_type) \<rightharpoonup> kernel_object"
  ...
  "makeObjectKO dev us d ty \<equiv> case ty of
    ...
    | Inr (APIObjectType api) \<Rightarrow> makeObjectKO_gen us d api
...
lemma makeObjectKO_eq[Retype_R_assms]:
  ...
  "(v = KOReply reply) =
       (tp = Inr (APIObjectType ArchTypes_H.ReplyObject) \<and> reply = makeObject)"
  "(v = KOEndpoint endpoint) =
       (tp = Inr (APIObjectType ArchTypes_H.EndpointObject) \<and> endpoint = makeObject)"
  "(v = KONotification ntfn) =
       (tp = Inr (APIObjectType ArchTypes_H.NotificationObject) \<and> ntfn = makeObject)"
  using x
  ...
  next
    ...
    proof(cases a)
      case Untyped with Some cover ct show ?thesis
        ...
        apply (intro conjI; simp add: APIType_capBits_gen_def)
    ...
    next
      case CapTableObject with Some cover ct show ?thesis
        ...
        apply (simp_all add: RISCV64_H.toAPIType_def
                      ...
                      split: RISCV64_H.object_type.splits)
        apply wp
         apply (clarsimp simp: RISCV64_H.toAPIType_def APIType_capBits_gen_def objBits_simps
    ...
    next
      case ReplyObject with Some cover ct show ?thesis
        including no_pre
        apply (clarsimp simp: Arch_createNewCaps_def createNewCaps_def)
        apply (simp_all add: RISCV64_H.toAPIType_def fromIntegral_def toInteger_nat fromInteger_nat
                      split: RISCV64_H.object_type.splits)
        apply wp
        apply (rule hoare_post_imp)
         prefer 2
...
lemmas makeObjectKO_simps =
  makeObjectKO_def[split_simps RISCV64_H.object_type.split apiobject_type.split sum.split
                               kernel_object.split ]
...
lemma valid_untyped'_helper_arch_cap[Retype_R_assms]:
  ...
  by (clarsimp simp: valid_arch_cap'_def valid_arch_cap_ref'_def retype_obj_at_disj'
        ...
        apply (case_tac apiobject_type; simp split del: if_split)
           ...
           apply (wp createObjects_orig_cte_wp_at'[where sz = sz] mapM_x_wp'
                  ...
                  | clarsimp simp: objBits_simps field_simps mult_2_right APIType_capBits_def
                                   createObjects_def curDomain_def bit_simps scBits_simps
                  | intro conjI impI
   ...
   apply (simp add: APIType_capBits_def toAPIType_def Arch_createNewCaps_def
        ...
        apply ((wpsimp wp: mapM_x_wp' createObjects_state_refs_of'' simp: createObjects_def,
               force simp: not_0 zero_less_iff_neq_zero refs_of'_def valid_pspace'_def
                               objBits_simps mult_2_right field_simps bit_simps
                     split: if_split)+)[4]
  (* generic *)
  using cover tysc
  apply (clarsimp)
  apply (wpsimp simp: createObjects_def curDomain_def wp: createObjects_state_refs_of'')
  apply (clarsimp simp: not_0 zero_less_iff_neq_zero APIType_capBits_gen_def tcb_bound_refs'_def
                        makeObject_notification makeObject_tcb makeObject_endpoint
                        makeObject_sc makeObject_reply scBits_simps)
...
lemma createNewCaps_list_refs_of_replies'[Retype_R_assms]:
  ...
  apply (cases ty; simp add: createNewCaps_def Arch_createNewCaps_def
        ...
        apply (case_tac apiobject_type; simp split del: if_split)
           ...
           by (wp mapM_x_wp' createObjects_list_refs_of_replies''[simplified o_def]
               | simp add: not_0 pspace_no_overlap'_def APIType_capBits_def
                           valid_pspace'_def makeObject_tcb makeObject_endpoint
                           makeObject_notification makeObject_reply makeObject_sc objBits_simps
                           createObjects_def curDomain_def o_def scBits_simps bit_simps
               | intro conjI impI )+
      ...
      apply (case_tac apiobject_type, simp_all split del: if_split)[1]
            apply (wp mapM_x_threadSet_createNewCaps_futz mapM_x_wp' createObjects_obj_at
                   ...
                   | simp add: makeObjectKO_def objBitsKO_def archObjSize_def makeObjectKO_gen_def
                               APIType_capBits_def APIType_capBits_gen_def
                               objBits_def curDomain_def field_simps mult_2_right bit_simps scBits_simps
                          split del: if_split
...
lemma createNewCaps_valid_bitmaps[Retype_R_assms]:
  ...
  apply (cases ty; simp add: createNewCaps_def Arch_createNewCaps_def
      ...
      apply (rename_tac apiobject_type)
      apply (case_tac apiobject_type; simp split del: if_split)
            apply (wpsimp wp: createObjects_valid_bitmaps[simplified o_def]
...
lemma createNewCaps_valid_sched_pointers[Retype_R_assms]:
  ...
  apply (cases ty; simp add: createNewCaps_def Arch_createNewCaps_def
      ...
      apply (rename_tac apiobject_type)
      apply (case_tac apiobject_type; simp split del: if_split)
            apply (wpsimp wp: createObjects_valid_sched_pointers[simplified o_def]
                   | simp add: makeObject_tcb objBits_def createObjects_def
                   | intro conjI impI)+
  done
  ...
  apply (rule hoare_pre)
   apply (wpc
         | wp hoare_vcg_const_Ball_lift hoare_vcg_disj_lift
           ...
           hoare_vcg_all_lift
           doMachineOp_mapM_x_wp dmo_lift' mapM_x_wp' threadSet_ko_wp_at2'
...
lemma retype_state_relation[Retype_R_2_assms]:
  ...
  proof (rule state_relation_null_filterE[OF sr refl _ _ _ _ _ _ _ _ _ _ _ _ vs'],
  ...
  have epqsrel: "ep_queues_relation s s'"
    using sr by (simp add: state_relation_def)

  thus "ep_queues_relation_2 (eps_of_kh ?ps ||> ep_queue) (?ps' |> ep_of' ||> epQueue)
                             (?ps' |> tcb_of' |> tcbSchedNext) (?ps' |> tcb_of' |> tcbSchedPrev)"
    using retype_ep_queues_relation[OF _ pspr vs' pn' ko tysc cover orr num_r]
    by (clarsimp simp: ep_queues_relation_def)

  have ntfnqsrel: "ntfn_queues_relation s s'"
    using sr by (simp add: state_relation_def)

  thus "ntfn_queues_relation_2 (?ps |> ntfn_of ||> ntfn_obj ||> ntfn_queue)
                               (?ps' |> ntfn_of' ||> ntfnQueue)
                               (?ps' |> tcb_of' |> tcbSchedNext) (?ps' |> tcb_of' |> tcbSchedPrev)"
    using retype_ntfn_queues_relation[OF _ pspr vs' pn' ko tysc cover orr num_r]
    by (clarsimp simp: ep_queues_relation_def)
  ...
  show ?thesis using vo tysc
    ...
    apply (case_tac obj; simp add: valid_obj'_def)
       ...
       apply (intro conjI ballI)
        ...
        apply (case_tac thread_state, simp_all add: valid_bound_obj'_def
  ...
  apply (cases ty; simp add: createNewCaps_def Arch_createNewCaps_def
        ...
        apply (case_tac apiobject_type; simp split del: if_split)
           ...
           apply (wpsimp wp: mapM_x_wp' createObjects_sched_queues
                       simp: curDomain_def)
           by (wp mapM_x_wp' createObjects_sched_queues[simplified o_def]
               | simp add: APIType_capBits_def valid_pspace'_def makeObject_tcb
...
lemma createNewCaps_null_filter'[Retype_R_2_assms]:
  ...
  apply (cases ty, simp_all split del: if_split)
          ...
          apply (case_tac apiobject_type, simp_all split del: if_split)
              apply (rule hoare_pre, wp,simp)
             by (simp add: createObjects_def makeObjectKO_def APIType_capBits_def
                           objBits_def scBits_simps APIType_capBits_gen_def pageBits_def
...
lemma createObjects_no_cte_invs:
  ...
  apply (rule hoare_vcg_conj_lift)
    apply (simp add: createObjects_def)
    apply (wp createObjects_valid_bitmaps)
   apply (rule hoare_vcg_conj_lift)
    apply (simp add: createObjects_def)
   ...
   apply (wp createObjects_no_cte_ifunsafe' irqs_masked_lift
             createObjects_no_cte_valid_global
...
lemma createNewCaps_valid_pspace[Retype_R_3_assms]:
  ...
  using gen_valid_obj_makeObject_rules sz_limit ptr_cn ptr_km tysc
  apply (clarsimp simp: RISCV64_H.toAPIType_def
             split del: if_split cong: option.case_cong)
  apply (cases ty, simp_all split del: if_split)
      apply (rename_tac apiobject_type)
      apply (case_tac apiobject_type, simp_all split del: if_split)
           ...
           apply (wpsimp wp: createObjects_valid_pspace_untyped'[of _ _ _ "Inr ty", where ptr=ptr]
                             mapM_x_wp'
                  split_del: if_split
                       simp: createObjects_def makeObjectKO_def objBits_def objBitsKO_def
                             ...
                             objBits_simps bit_simps scBits_simps
                  | simp add: power_add)+
  done
