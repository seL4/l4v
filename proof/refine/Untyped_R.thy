...
locale invokeUntyped_proofs =
  ...
  assumes misc: "ct_active' s" "invs' s" "sym_refs (state_refs_of' s)"
begin
...
lemma (in Untyped_R) resetUntypedCap_invs_etc:
  ...
  apply (rule hoare_pre, rule hoare_strengthen_postE,
    ...
    and valid_untyped_inv_wcap' ?ui
     ...
     apply (clarsimp simp: cte_wp_at_ctes_of gen_isCap_simps ex_cte_cap_wp_to'_def
                           conj_comms)
...
lemma (in Untyped_R) whenE_reset_resetUntypedCap_invs_etc:
  "\<lbrace>invs' and (\<lambda>s. sym_refs (state_refs_of' s))
    and valid_untyped_inv_wcap' ui (Some (UntypedCap dev ptr sz idx))
    and ct_active'
...
crunch insertNewCap
  ...
  (wp: crunch_wps irqs_masked_lift valid_replies'_lift)
...
lemma (in Untyped_R_2) insertNewCap_invs':
  ...
  apply (rule hoare_pre)
   apply (wp insertNewCap_valid_pspace' valid_dom_schedule'_lift
             ...
             sym_heap_sched_pointers_lift valid_irq_node_lift insertNewCap_valid_irq_handlers)
  apply (clarsimp simp: cte_wp_at_ctes_of)
...
lemma invokeUntyped_invs'':
 ...
 shows "\<lbrace>invs' and (\<lambda>s. sym_refs (state_refs_of' s)) and valid_untyped_inv' ui
  ...
  proof -
    ...
    note range_simps[simp del] =
           untyped_range.simps usable_untyped_range.simps atLeastAtMost_simps
