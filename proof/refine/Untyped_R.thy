...
(* FIXME arch-split: duplicate: also requalified in CNodeInv_R which is not in this import chain *)
arch_requalify_facts (A) data_to_nat_def

arch_requalify_facts (H)
  fromAPIType_def (* FIXME arch-split: ArchTypes_H *)
  toAPIType_def (* FIXME arch-split: safe, only checks whether type is APIObjectType *)

(* FIXME arch-split: in Machine_AI, on some architectures it's word_size_bits and on others a
   magic number; for arch-split we need a stable version *)
lemma (in Arch) no_fail_clearMemory_gen:
  "no_fail (\<lambda>_. is_aligned p word_size_bits) (clearMemory p b)"
  using no_fail_clearMemory
  by (simp add: word_size_bits_def)

requalify_facts Arch.no_fail_clearMemory_gen
declare no_fail_clearMemory_gen[wp, simp]

arch_requalify_facts (* FIXME arch-split: Retype_AI *)
  clearMemory_irq_masks_inv
  init_arch_objects_pspace_aligned
  init_arch_objects_pspace_distinct
  init_arch_objects_valid_objs
  init_arch_objects_valid_mdb
  init_arch_objects_cte_wp_at
  init_arch_objects_cap_table
  init_arch_objects_valid_cap
  retype_region_plain_invs
...
arch_requalify_facts (* FIXME arch-split: Deterministic_AI *)
  retype_region_valid_list
  reset_untyped_cap_valid_list
  init_arch_objects_valid_list
...
(* FIXME: move, gather with other _mw lemmas, this is duplicated from CNodeInv_R *)
...
(* FIXME: move *)
lemma corres_returnOk_no_sr:
  "rrel rv rv' \<Longrightarrow> corres_underlying sr  nf nf' (frrel \<oplus> rrel) P P' (returnOk rv) (returnOk rv')"
  by (rule corres_returnOk, simp)

(* FIXME: move *)
lemma the_index_last_length_eq:
  "\<lbrakk> xs \<noteq> []; distinct xs \<rbrakk> \<Longrightarrow> the_index xs (last xs) = length xs - 1"
  by (induct xs; auto)

(* FIXME: move *)
lemma the_index_enum_last_length_eq:
  "the_index enum (last (enum :: object_type list)) = length (enum :: object_type list) - 1"
  by (simp add: the_index_last_length_eq)

(* FIXME: move *)
schematic_goal enum_apiobject_type_length:
  "length (enum :: apiobject_type list) = numeral ?n"
  by (simp add: enum_apiobject_type flip: One_nat_def)

(* FIXME arch-split: move to Lib *)
lemma nth_drop_sub:
  "\<lbrakk> k \<le> n; k \<le> length xs \<rbrakk> \<Longrightarrow> (drop k xs ! (n - k)) = (xs ! n)"
  by simp
...
primrec untypinv_relation ::
  "Invocations_A.untyped_invocation \<Rightarrow> Invocations_H.untyped_invocation \<Rightarrow> bool"
  where
  "untypinv_relation
...
primrec valid_untyped_inv_wcap' ::
  "Invocations_H.untyped_invocation \<Rightarrow> capability option \<Rightarrow> kernel_state \<Rightarrow> bool"
  where
  "valid_untyped_inv_wcap' (Invocations_H.Retype slot reset ptr_base ptr ty us slots d)
   = (\<lambda>co s. \<exists>sz idx.
          (cte_wp_at' (\<lambda>cte. cteCap cte = UntypedCap d ptr_base sz idx
          ...
          \<and> (ty = APIObjectType CapTableObject \<longrightarrow> us > 0)
          \<and> (ty = APIObjectType Untyped \<longrightarrow> minUntypedSizeBits \<le> us \<and> us \<le> maxUntypedSizeBits)
          ...
          \<and> (d \<longrightarrow> ty = APIObjectType Untyped \<or> isFrameType ty)
...
lemma alignUp_H[simp]:
  ...
  by (auto simp: Untyped_H.alignUp_def More_Word_Operations.alignUp_def mask_def
           del: ext intro!: ext)

lemma mapM_locate_eq:
    "isCNodeCap cap
    \<Longrightarrow> mapM (\<lambda>x. locateSlotCap cap x) xs
        = (do stateAssert (\<lambda>s. case gsCNodes s (capUntypedPtr cap) of
                                 None \<Rightarrow> xs = []
                               | Some n \<Rightarrow> \<forall>x \<in> set xs. n = capCNodeBits cap \<and> x < 2 ^ n) [];
              return (map (\<lambda>x. (capCNodePtr cap) + 2 ^ cte_level_bits * x) xs) od)"
  apply (clarsimp simp: gen_isCap_simps)
  apply (simp add: locateSlot_conv gen_objBits_simps
                   liftM_def[symmetric] mapM_liftM_const gen_isCap_simps)
  apply (simp add: liftM_def mapM_discarded mapM_x_stateAssert)
  apply (intro bind_cong refl arg_cong2[where f=stateAssert] ext)
  apply (simp add: gen_isCap_simps split: option.split)
  done
...
lemma decodeUntyped_inv[wp]:
  "decodeUntypedInvocation label args slot (UntypedCap d w n idx) cs \<lbrace>P\<rbrace>"
  apply (simp add: decodeUntypedInvocation_def whenE_def split_def unlessE_def Let_def
              split del: if_split cong: if_cong list.case_cong)
  apply (wp mapME_x_inv_wp hoare_drop_imps constOnFailure_wp mapM_wp'
         | wpcw
...
(* shared assumptions between Untyped_R and mdb_insert_again_all locales *)
locale mdb_insert_again_all_common =
  assumes mdb_chunked_arch_assms_non_arch:
    "\<And>cap. \<not> isArchObjectCap cap \<Longrightarrow> mdb_chunked_arch_assms cap"
  assumes sameRegionAs_def_untyped:
    "\<And>cap cap'.
  ...
  assumes untyped_inc_mdbD:
    "\<And>cap cap' node node' ctes p p'.
...
locale Untyped_R = mdb_insert_again_all_common +
  assumes length_enum_apiobject_less_enum_object_type:
    "length (enum :: apiobject_type list) < length (enum :: object_type list)"
  assumes nth_enum_object_type_gen_eq:
    "\<And>n.
     n < length (enum :: apiobject_type list)
     \<Longrightarrow> ((enum :: object_type list) ! n) = APIObjectType ((enum :: apiobject_type list) ! n)"
  assumes APIType_map2_CapTable[simp]:
    "\<And>ty. (APIType_map2 ty = Structures_A.CapTableObject)
          = (ty = Inr (APIObjectType ArchTypes_H.CapTableObject))"
  assumes updateFreeIndex_valid_irq_states'[wp]:
    "\<And>slot idx. updateFreeIndex slot idx \<lbrace>valid_irq_states'\<rbrace>"
  assumes freeMemory_irq_masks_inv[wp]:
    "\<And>ptr bits P. freeMemory ptr bits \<lbrace>\<lambda>s. P (irq_masks s)\<rbrace>"
  assumes deleteGhost_valid_irq_states'[wp]:
    "\<And>ptr bits. Arch.deleteGhost ptr bits \<lbrace>valid_irq_states'\<rbrace>"
  assumes deleteGhost_ksInterruptState[wp]:
    "\<And>ptr bits P. Arch.deleteGhost ptr bits \<lbrace>\<lambda>s. P (ksInterruptState s)\<rbrace>"
  assumes deleteGhost_ksIdleThread[wp]:
    "\<And>ptr bits P. Arch.deleteGhost ptr bits \<lbrace>\<lambda>s. P (ksIdleThread s)\<rbrace>"
  assumes deleteGhost_ksCurDomain[wp]:
    "\<And>ptr bits P. Arch.deleteGhost ptr bits \<lbrace>\<lambda>s. P (ksCurDomain s)\<rbrace>"
  assumes  deleteGhost_ksCurThread[wp]:
    "\<And>ptr bits P. Arch.deleteGhost ptr bits \<lbrace>\<lambda>s. P (ksCurThread s)\<rbrace>"
  assumes deleteGhost_gsMaxObjectSize[wp]:
    "\<And>ptr bits P. Arch.deleteGhost ptr bits \<lbrace>\<lambda>s. P (gsMaxObjectSize s)\<rbrace>"
  assumes arch_data_to_obj_type_invalid:
    "\<And>n.
  ...
  assumes arch_data_to_obj_type_valid:
    "\<And>n.
  ...
  assumes APIType_map2_arch_data_to_obj_type:
    "\<And>n.
     \<lbrakk>n < length (enum :: object_type list); length (enum :: apiobject_type list) \<le> n\<rbrakk>
     \<Longrightarrow> APIType_map2 (Inr ((enum :: object_type list) ! n))
        = ArchObject (the (arch_data_to_obj_type (n - length (enum :: apiobject_type list))))"
  assumes obj_bits_api_APIType_map2:
    "\<And>x y. (obj_bits_api (APIType_map2 (Inr x)) y) = (getObjectSize x y)"
  assumes length_nat_to_cref:
    "\<And>bits x. bits < word_bits \<Longrightarrow> length (nat_to_cref bits x) = bits"
  assumes is_frame_type_isFrameType_eq[simp]:
    "\<And>arg0 :: machine_word.
     (is_frame_type (APIType_map2 (Inr (toEnum (unat arg0))))) =
     (isFrameType (toEnum (unat arg0)))"
  assumes ctes_of_ko_arch:
    "\<And>cap s.
  ...
  assumes irq_nodes_global:
    "\<And>s irq. irq_node' s + (ucast (irq :: irq) << cteSizeBits) \<in> global_refs' s"
  assumes createNewCaps_range_helper:
    "\<And>ptr sz tp us n d.
  ...
  (* FIXME arch-split: these might be better in Bits_R *)
  assumes word_size_bits_le_untyped_min_bits:
  ...
  assumes minUntypedSizeBits_le_resetChunkBits:
  ...
  assumes createObject_nosch[wp]:
    "\<And>t regionBase userSize isDevice P.
     createObject t regionBase userSize isDevice \<lbrace>\<lambda>s. P (ksSchedulerAction s)\<rbrace>"
...
crunch deleteObjects, updateFreeIndex
  ...
  (wp: doMachineOp_irq_states' crunch_wps
   simp: no_irq_storeWord unless_def ignore: Arch.deleteGhost)

lemma data_to_obj_type_invalid:
  "unat v \<ge> length (enum :: object_type list)
   \<Longrightarrow> data_to_obj_type v = throwError (ExceptionTypes_A.InvalidArgument 0)"
  apply (frule arch_data_to_obj_type_invalid)
  using length_enum_apiobject_less_enum_object_type
  apply (simp add: data_to_obj_type_def enum_apiobject_type_length data_to_nat_def)
  done

lemma data_to_obj_type_APIType_map2:
  "unat v < length (enum :: object_type list)
   \<Longrightarrow> data_to_obj_type v = returnOk (APIType_map2 (Inr ((enum :: object_type list) ! unat v)))"
  supply if_split[split del] data_to_nat_def[simp]
  (* arch object types are in the enum after api objects *)
  apply (case_tac "unat v \<ge> (length (enum :: apiobject_type list))"; clarsimp?)
   apply (simp add: data_to_obj_type_def)
   apply (subst APIType_map2_arch_data_to_obj_type; simp?)
   using arch_data_to_obj_type_valid[where n="unat v", simplified enum_apiobject_type_length]
   apply clarsimp
   apply (clarsimp simp add: enum_apiobject_type_length split: option.splits)
  (* otherwise, we have a generic object type *)
  apply (simp add: data_to_obj_type_def not_le nth_enum_object_type_gen_eq
                   enum_apiobject_type APIType_map2_gen_def
              split: if_split)
  done
...
next
  ...
  have Q: "\<And>v. corres (ser \<oplus> (\<lambda>a b. APIType_map2 (Inr (toEnum (unat v))) = a)) \<top> \<top>
                  (data_to_obj_type v)
                  (whenE (fromEnum (maxBound :: object_type) < unat v)
                         (throwError (Fault_H.syscall_error.InvalidArgument 0)))"
    supply if_split[split del]
    apply (clarsimp simp add: maxBound_def fromEnum_def whenE_def toEnum_def
                              the_index_enum_last_length_eq)
    apply (case_tac "unat v \<ge> length (enum :: object_type list)")
     (* completely outside enumeration range *)
     apply (simp add: Suc_le_eq[symmetric] data_to_obj_type_invalid, simp)
    (* within enumeration range *)
    apply (simp add: not_le data_to_obj_type_APIType_map2)
    apply (prop_tac "\<not> (length (enum :: object_type list)) - Suc 0 < unat v")
     apply (simp add: not_less)
    apply (simp add: not_less corres_returnOk_no_sr)
    done
  ...
  have S: "\<And>x bits.
           \<lbrakk> bits < word_bits; x < 2 ^ bits \<rbrakk> \<Longrightarrow> toEnum x = (of_nat x :: machine_word)"
    (* avoid unfolding LENGTH *)
    by (rule toEnum_of_nat[where 'a=machine_word_len, folded word_bits_def])
       (fastforce elim: order_less_trans)

  have arg_4_5_cte_map_fold:
    "\<And>ref bits.
     \<lbrakk> is_aligned ref bits; Suc (unat arg4 + unat arg5 - Suc 0) \<le> 2 ^ bits;
       bits < word_bits; 1 \<le> arg4 + arg5;  arg4 \<le> arg4 + arg5 \<rbrakk>
     \<Longrightarrow> (map (\<lambda>x. ref + 2 ^ cte_level_bits * x) [arg4 .e. arg4 + arg5 - 1])
        = map cte_map (map (Pair ref) (map (nat_to_cref bits) xs))"
    apply (prop_tac "Suc (unat (arg4 + arg5 - 1)) = unat arg4 + unat arg5")
     apply unat_arith
    apply (clarsimp simp: upto_enum_def cte_map_def of_bl_nat_to_cref xs
                          S cte_level_bits_def shiftl_t2n)
    done

  have pow2_sub_of_nat_mw:
  ...
  show ?thesis
    supply data_to_nat_def[simp]
    apply (rule corres_name_pre)
    apply clarsimp
    apply (prop_tac "cte_wp_at' (\<lambda>cte. cteCap cte = (capability.UntypedCap d w n idx)) (cte_map slot) s'")
    ...
    (* more restriction of the returnOk_bindE stuff needs to be done in order to give you a single
       target to do the word proof against or else it needs repeating.
    ...
    apply (rule corres_guard_imp)
      apply (rule corres_splitEE[OF Q])
        ...
        apply (rule whenE_throwError_corres)
          apply (simp add: maxUntypedSizeBits_untyped_max_bits)
         apply (clarsimp simp: not_le wordBits_word_bits maxUntypedSizeBits_untyped_max_bits
                               obj_bits_api_APIType_map2)
        apply (rule whenE_throwError_corres, simp)
         apply (clarsimp simp: fromAPIType_def)
        apply (rule whenE_throwError_corres, simp)
         apply (clarsimp simp: fromAPIType_def minUntypedSizeBits_untyped_min_bits)
        apply (rule_tac r' = "\<lambda>cap cap'. cap_relation cap cap'"
          ...
          apply (rule_tac corres_split_norE)
            ...
            apply (subgoal_tac "is_aligned (obj_ref_of rva) (bits_of rva) \<and> bits_of rva < word_bits")
             prefer 2
            ...
            apply (rule_tac P'="valid_cap rva" in corres_stateAssert_implied)
             apply (frule_tac bits1 = "bits_of rva" in arg_4_5_cte_map_fold)
                 apply (simp)
                apply (simp)
             ...
             apply (rule corres_split_norE)
                ...
                apply (rule_tac P = "valid_cap (cap.CNodeCap r bits g) and invs" in corres_guard_imp [where P' = invs'])
                 ...
                 apply (simp add: length_nat_to_cref)
                apply simp
               ...
               apply (rule corres_split_eqr[OF corres_check_no_children])
                 ...
                 apply (rule whenE_throwError_corres)
                   apply (clarsimp simp:shiftL_nat word_less_nat_alt shiftr_div_2n'
                              split del: if_split)+
                  apply (simp add: word_of_nat_le pow2_sub_of_nat_mw)
                  apply (drule_tac x = "if_res reset" in unat_of_nat_eq_mw[OF le_less_trans])
                   apply (simp add: obj_bits_api_APIType_map2 shiftR_nat)+
                  apply (simp add: unat_of_nat_eq_mw le_less_trans[OF div_le_dividend]
                                  le_less_trans[OF diff_le_self])
                 apply (rule whenE_throwError_corres)
                   apply clarsimp
                  apply (clarsimp simp: fromAPIType_def)
                 apply (rule corres_returnOkTT)
                 apply (clarsimp simp: obj_bits_api_APIType_map2 getFreeRef_def get_free_ref_def is_cap_simps)
                apply simp
            ...
            apply (clarsimp simp: is_cap_simps valid_cap_simps
                                  cap_table_at_gsCNodes bits_of_def linorder_not_less)
          ...
          apply (rule hoare_strengthen_post[where Q'="\<lambda>r. invs and valid_cap r and cte_at slot"])
           apply wp+
          apply (clarsimp simp: is_cap_simps cap_aligned_def valid_cap_def)
...
lemma ctes_of_ko:
  ...
  apply (case_tac cap; simp add: gen_isCap_simps capRange_def)
    ...
    apply (case_tac zombie_type)
     apply (clarsimp simp: valid_cap'_def obj_at'_def)
     apply (intro exI conjI, assumption)
     apply (clarsimp simp: mask_def add_ac gen_objBits_simps obj_range'_def p_assoc_help
    ...
    apply (clarsimp simp: less_mask_eq cteSizeBits_cte_level_bits)
    apply (fastforce simp: obj_range'_def gen_objBits_simps mask_def field_simps
...
lemma cte_wp_at_caps_descendants_range_inI':
  "\<lbrakk>invs' s; cte_wp_at' (\<lambda>c. cteCap c = UntypedCap d (ptr && ~~ mask sz) sz idx) cref s;
   ...
   \<Longrightarrow> descendants_range_in' {ptr .. (ptr && ~~ mask sz) + mask sz} cref (ctes_of s)"
...
lemma checkFreeIndex_wp:
  "\<lbrace>\<lambda>s. if descendants_of' slot (ctes_of s) = {} then Q y s else Q x s\<rbrace>
   ...
   \<lbrace>Q\<rbrace>"
  by (wpsimp simp: constOnFailure_def const_def wp: ensureNoChildren_wp)
...
lemma cte_cap_in_untyped_range:
  "\<lbrakk> ptr \<le> x; x \<le> ptr + mask bits; cte_wp_at' (\<lambda>cte. cteCap cte = UntypedCap d ptr bits idx) cref s;
     ...
     ex_cte_cap_to' x s; valid_global_refs' s \<rbrakk>
...
lemma decodeUntyped_wf[wp]:
  ...
  apply (subgoal_tac "args ! 4 \<le> 2 ^ capCNodeBits nodeCap")
   ...
   apply (subst (asm) le_m1_iff_lt[THEN iffD1])
    apply (subst p2_gt_0[where 'a=machine_word_len, folded word_bits_def]) (* prevent leaking LENGTH *)
  ...
  apply (subgoal_tac
   ...
   apply (rule comp_inj_on)
    ...
    apply (simp add: unats_def)
   apply (rule inj_onI)
   apply (clarsimp simp: toEnum_of_nat[OF less_Suc_unat_less_bound] gen_isCap_simps)
   apply (erule(2) inj_bits)
    apply (simp add: obj_sizeBits_less_word_bits less_imp_le flip: cteSizeBits_cte_level_bits)
   ...
   apply (subst unat_power_lower_mw[symmetric], simp)
   apply (simp add: word_less_nat_alt[symmetric])
   (* prevent leaking LENGTH *)
   apply (rule two_power_increasing[where 'a=machine_word_len, folded word_bits_def])
    apply (clarsimp dest!: valid_capAligned
                     simp: capAligned_def objBits_def objBitsKO_def cteSizeBits_cte_level_bits)
    apply arith
   apply simp
  apply (clarsimp simp: fromAPIType_def)
  ...
  apply (subgoal_tac "(\<forall>x. (args ! 4) \<le> x \<and> x \<le> (args ! 4) + (args ! 5) - 1 \<longrightarrow>
   ...
   apply (erule disjE)
    apply (erule bspec)
    apply (clarsimp simp: gen_isCap_simps image_def shiftl_t2n mult_ac)
   ...
   apply (case_tac cte)
   apply (clarsimp simp: cte_wp_at_ctes_of gen_isCap_simps image_def shiftl_t2n)
   apply (rule_tac x = x in bexI,simp)
   apply (simp add: mask_def)
   apply (erule order_trans)
   apply (frule(1) le_plus)
  ...
  apply (rule conjI)
   ...
   apply (clarsimp simp: image_def gen_isCap_simps nullPointer_def field_simps)
   apply (intro conjI)
      apply (clarsimp simp: image_def gen_isCap_simps nullPointer_def field_simps)
      apply (drule_tac x=x in spec)+
      apply simp
     apply (clarsimp simp add: APIType_capBits_gen_def)
  ...
  apply (intro conjI; clarsimp simp add: image_def)
   apply (clarsimp simp: image_def gen_isCap_simps nullPointer_def)
   apply (drule_tac x=x in spec)+
   apply simp
...
lemma next_wont_bite:
  ...
  apply (clarsimp simp: isMDBParentOf_CTE_gen gen_isCap_simps)
  apply (rule conjI)
...
definition is_end_chunk :: "cte_heap \<Rightarrow> capability \<Rightarrow> machine_word \<Rightarrow> bool" where
...
definition mdb_chunked2 :: "cte_heap \<Rightarrow> bool" where
...
lemma chunk_sameRegionAs_step1:
  "\<lbrakk> ctes \<turnstile> p' \<leadsto>\<^sup>* p''; ctes p'' = Some cte; is_chunk ctes (cteCap cte) p p'';
     mdb_chunked2 ctes; valid_dlist ctes \<rbrakk>
   \<Longrightarrow> \<forall>cte'. ctes p' = Some cte' \<longrightarrow> ctes \<turnstile> p \<leadsto>\<^sup>+ p' \<longrightarrow> sameRegionAs (cteCap cte') (cteCap cte)"
...
lemma descendants_of'_D:
  "p' \<in> descendants_of' p ctes \<Longrightarrow> ctes \<turnstile> p \<rightarrow> p'"
...
lemma valid_nullcaps_n' [simp]:
  ...
  apply (clarsimp simp: valid_nullcaps_def n'_def n modify_map_if new_site_def
                        new_parent_def gen_isCap_simps)
  apply (erule allE)+
  apply (erule (1) impE)
...
lemma capRange_c':
  "capRange c' \<subseteq> capRange parent_cap"
  by (intro sameRegion_capRange_sub same_region)
...
lemma capRange_ut:
  ...
  by (clarsimp simp: gen_isCap_simps del: subsetI)
...
lemma no_default_zombie:
  ...
  by (cases tp, auto simp: gen_isCap_simps)
...
lemma insertNewCap_ranges:
  ...
  apply (subst mdb_insert_again_child.descendants)
   apply unfold_locales[1]
       ...
       apply (frule sameRegionAs_classes, clarsimp simp: gen_isCap_simps)
...
locale mdb_insert_again_all_gen = mdb_insert_again_all + mdb_insert_again_all_common +
...
lemma mdb_chunked_n'[simp]:
  ...
  apply (simp split: if_split_asm)
    ...
    apply (clarsimp simp add: mdb_chunked_arch_assms_non_arch gen_isCap_simps)
    apply (drule_tac x=p'' in spec)
    apply clarsimp
   ...
   apply (case_tac "untypedRange parent_cap \<subseteq> untypedRange cap")
    apply (erule impE)
     apply (clarsimp simp only: gen_isCap_simps untypedRange.simps)
     ...
     apply (drule valid_capI')+
     apply (drule valid_capAligned)+
     apply (clarsimp simp: sameRegionAs_def_untyped capAligned_def is_aligned_no_overflow)
    apply clarsimp
    apply (erule disjE)
     apply simp
     apply (rule conjI)
...
lemma valid_n':
  ...
  by (auto simp: valid_badges_n')

end (* mdb_insert_again_all_gen *)
...
lemma descendants_range_caps_no_overlapI':
  ...
  apply simp
  apply (drule untyped_incD')
      apply ((simp add: gen_isCap_simps del:usableUntypedRange.simps untypedRange.simps)+)[4]
...
locale invokeUntyped_proofs =
  ...
  assumes misc: "ct_active' s" "invs' s"
begin

lemma
  cte_wp_at':
    "cte_wp_at' (\<lambda>cte. cteCap cte = capability.UntypedCap dev (ptr && ~~ mask sz) sz idx) cref s" and
  cover:
    "range_cover ptr sz (APIType_capBits tp us) (length (slots::machine_word list))" and
  misc2:
    "distinct slots"
    "slots \<noteq> []"
    "\<forall>slot\<in>set slots. cte_wp_at' (\<lambda>c. cteCap c = capability.NullCap) slot s"
    "\<forall>x\<in>set slots. ex_cte_cap_wp_to' \<top> x s"
  using vui
...
abbreviation(input)
  "retype_range \<equiv> {ptr..ptr + of_nat (length slots) * 2 ^ APIType_capBits tp us - 1}"

(* FIXME: consider {ptr .. ptr || mask sz} instead *)
abbreviation(input)
  "usable_range \<equiv> {ptr..(ptr && ~~ mask sz) + mask sz}"
...
crunch updateNewFreeIndex
  for ghost_relation_wrapper'[wp]: "\<lambda>s'. P (ghost_relation_wrapper s s')"
  and aobjs_of'[wp]: "\<lambda>s. P (aobjs_of' s)"
  (wp: ghost_relation_wrapper_lift')
    ...
    apply (rule corres_caps_decomposition)
     ...
     apply (intro conjI; (solves \<open>simp add: state_relation_def\<close>)?)
          ...
          apply (subst bluhr_descendants_of')
           ...
           apply (clarsimp simp: gen_isCap_simps isArchMDBParentOf_non_arch)
           apply (clarsimp simp: valid_mdb'_def valid_mdb_ctes_def
                                 ut_revocable'_def)
...
locale Untyped_R_2 = Untyped_R +
  (* This is the expanded form of mdb_insert_again_all.valid_n' which needs Arch to prove.
     The original form contracts this significantly via internal locale variables. *)
  assumes mdb_insert_again_all_valid_n':
    "\<And>m parent parent_cap parent_node site site_cap site_node c' s.
     \<lbrakk>mdb_insert_again_all m parent parent_cap parent_node site site_cap site_node c' s;
      untypedRange c' \<inter> usableUntypedRange parent_cap = {}\<rbrakk>
     \<Longrightarrow> valid_mdb_ctes
          (modify_map
            (modify_map
              (\<lambda>x. if x = site then Some (CTE c' (MDB (mdbNext parent_node) parent True True)) else m x)
              parent (cteMDBNode_update (mdbNext_update (\<lambda>x. site))))
            (mdbNext parent_node) (cteMDBNode_update (mdbPrev_update (\<lambda>a. site))))"
  (* This is the expanded form of invokeUntyped_proofs.descendants_range which needs lemmas from
     Untyped_R to prove.
     The original form contracts this significantly via internal locale variables. *)
  assumes invokeUntyped_proofs_descendants_range:
    "\<And>s cref reset ptr_base ptr tp us slots sz idx dev.
     invokeUntyped_proofs s cref reset ptr_base ptr tp us slots sz idx dev \<Longrightarrow>
     descendants_range_in' {ptr..(ptr && ~~ mask sz) + mask sz} cref (ctes_of s)"
    "\<And>s cref reset ptr_base ptr tp us slots sz idx dev.
     invokeUntyped_proofs s cref reset ptr_base ptr tp us slots sz idx dev \<Longrightarrow>
     descendants_range_in' {ptr..ptr + word_of_nat (length slots) * 2 ^ APIType_capBits tp us - 1}
       cref (ctes_of s)"
  assumes invokeUntyped_proofs_ex_cte_no_overlap':
    "\<And>s cref reset ptr_base ptr tp us slots sz idx dev p P.
     \<lbrakk>invokeUntyped_proofs s cref reset ptr_base ptr tp us slots sz idx dev; ex_cte_cap_wp_to' P p s\<rbrakk>
     \<Longrightarrow> p \<notin> {ptr..(ptr && ~~ mask sz) + mask sz}"
  assumes invokeUntyped_proofs_cref_inv:
    "\<And>s cref reset ptr_base ptr tp us slots sz idx dev.
     invokeUntyped_proofs s cref reset ptr_base ptr tp us slots sz idx dev
     \<Longrightarrow> cref \<notin> {ptr..(ptr && ~~ mask sz) + mask sz}"
  assumes invokeUntyped_proofs_slots_invD:
    "\<And>s cref reset ptr_base ptr tp us slots sz idx dev x.
     \<lbrakk>invokeUntyped_proofs s cref reset ptr_base ptr tp us slots sz idx dev; x \<in> set slots\<rbrakk>
     \<Longrightarrow> x \<noteq> cref \<and> x \<notin> {ptr..(ptr && ~~ mask sz) + mask sz} \<and> ex_cte_cap_wp_to' (\<lambda>_. True) x s"
begin
...
lemma insertNewCap_valid_mdb:
  ...
  apply (clarsimp simp: nullPointer_def)
  apply (rule mdb_insert_again_all_valid_n')
   apply unfold_locales[1]
                apply (assumption|rule refl)+
        apply (frule sameRegionAs_classes, clarsimp simp: gen_isCap_simps)
       apply (erule (1) ctes_of_valid_cap')
     ...
     apply simp
    apply (clarsimp simp: isMDBParentOf_CTE_gen gen_isCap_simps)
    apply (clarsimp simp: gen_isCap_simps valid_mdb_ctes_def ut_revocable'_def)
   apply assumption
...
lemmas [simp] = map_fst_zip_prefix map_snd_zip_prefix
...
lemma createNewCaps_range_helper2:
  ...
  apply (clarsimp simp: capRange_def ptr_add_def
...
lemma createNewCaps_children:
  ...
  apply clarsimp
  apply (drule(1) bspec)
  apply (clarsimp simp: sameRegionAs_def_untyped gen_isCap_simps)
...
lemma createNewCaps_parent_helper:
  ...
  apply (auto simp:cte_wp_at'_def gen_isCap_simps)
  done
...
lemma createNewCaps_valid_cap':
  "\<lbrace>\<lambda>s. pspace_no_overlap' ptr sz s \<and>
        ...
        ptr \<noteq> 0 \<and> sz \<le> maxUntypedSizeBits \<and> canonical_address (ptr && ~~ mask sz) \<and>
        ptr && ~~ mask sz \<in> kernel_mappings\<rbrace>
   createNewCaps ty ptr n us d
   \<lbrace>\<lambda>r s. \<forall>cap\<in>set r. s \<turnstile>' cap\<rbrace>"
  by (rule hoare_assume_pre, fastforce elim: createNewCaps_valid_cap)
...
lemma retype_region_caps_overlap_reserved_ret:
  ...
  apply (intro conjI; simp)
   apply fastforce
  apply (clarsimp simp: obj_bits_api_def ptr_add_def APIType_map2_gen_def
                  split: Structures_A.apiobject_type.splits)
  apply (drule (1) range_cover_subset, auto)
  done
...
lemma updateFreeIndex_updateCap_descendants_range_in':
  ...
  apply (rule hoare_pre)
   apply (wp descendants_range_in_lift'
    ...
    apply (clarsimp simp: cte_wp_at_ctes_of swp_def gen_isCap_simps)
   apply (simp add:updateCap_def)
   apply (wp setCTE_weak_cte_wp_at getCTE_wp)
   apply (fastforce simp:cte_wp_at_ctes_of gen_isCap_simps)
  apply (clarsimp)
  done
...
lemma updateFreeIndex_mdb_simple':
  ...
  apply (wp getCTE_wp)
  apply (clarsimp simp:cte_wp_at_ctes_of gen_isCap_simps simp del:fun_upd_apply)
...
lemma pspace_no_overlap_valid_untyped':
  ...
  apply (erule disjE)
   (* avoid leaking LENGTH *)
   apply (drule base_member_set[where 'a=machine_word_len, folded word_bits_def,
                                simplified field_simps add_mask_fold])
    apply simp
   apply blast
  apply (simp split: if_split_asm)
...
lemma updateFreeIndex_valid_pspace_no_overlap':
  ...
  apply (clarsimp simp:cte_wp_at_ctes_of valid_pspace'_def)
  apply (case_tac cte, simp add: gen_isCap_simps)
...
lemma updateFreeIndex_clear_invs':
  ...
  apply (clarsimp simp: untypedZeroRange_def cteCaps_of_def gen_isCap_simps)
  done

lemma (in Untyped_R) descendants_range_ex_cte':
  "\<lbrakk>descendants_range_in' S p (ctes_of s'); ex_cte_cap_wp_to' P q s'; S \<subseteq> capRange (cteCap cte);
    invs' s'; ctes_of s' p = Some cte; isUntypedCap (cteCap cte)\<rbrakk>
   \<Longrightarrow> q \<notin> S"
  ...
  apply (case_tac "\<exists>irq. cteCap ctea = IRQHandlerCap irq")
   ...
   apply (erule(1) in_empty_interE[OF _ _ subsetD,rotated -1])
    apply (fastforce simp: irq_nodes_global)
   apply (case_tac ctea)
   apply clarsimp
  apply (case_tac ctea)
...
(* some proofs in invokeUntyped_proofs require lemmas proved in Untyped_R locale *)
locale invokeUntyped_proofs_gen = invokeUntyped_proofs +
  assumes cte_wp_at_caps_descendants_range_inI':
    "\<And>s d ptr sz idx cref.
     \<lbrakk>invs' s; cte_wp_at' (\<lambda>c. cteCap c = UntypedCap d (ptr && ~~ mask sz) sz idx) cref s;
      idx \<le> unat (ptr && mask sz); sz < word_bits\<rbrakk>
     \<Longrightarrow> descendants_range_in' {ptr .. (ptr && ~~ mask sz) + mask sz} cref (ctes_of s)"
  assumes descendants_range_ex_cte':
    "\<And>S p s' P q cte.
     \<lbrakk>descendants_range_in' S p (ctes_of s'); ex_cte_cap_wp_to' P q s'; S \<subseteq> capRange (cteCap cte);
      invs' s'; ctes_of s' p = Some cte; isUntypedCap (cteCap cte)\<rbrakk>
     \<Longrightarrow> q \<notin> S"
...
lemma ex_cte_no_overlap':
  ...
  apply (drule_tac cte = cte in descendants_range_ex_cte'[OF descendants_range(1)])
      apply (clarsimp simp: word_and_le2 gen_isCap_simps add_mask_fold)+
  done
...
lemma resetUntypedCap_corres:
  ...
  apply (rule corres_guard_imp)
    apply (rule corres_split[OF getSlotCap_corres])
      ...
      apply (rule corres_split[OF deleteObjects_corres])
         ...
         apply (clarsimp simp: valid_cap_def cap_aligned_def
                               order_trans[OF word_size_bits_le_untyped_min_bits])
        apply (rule corres_if)
          apply simp
        ...
        apply (rule_tac P="\<lambda>x. valid_objs and pspace_aligned and pspace_distinct
           ...
           apply (rule corres_guard_imp)
             apply (rule corres_split_nor)
               ...
               apply (rule corres_split_nor[OF updateFreeIndex_corres])
                  ...
                  apply clarify
                  apply (subst unat_mult_simple[folded word_bits_def]) (* avoid leaking LENGTH *)
                   apply (subst unat_of_nat_eq_mw)
                    apply (rule order_less_trans[rotated],
                           rule_tac n=sz in power_strict_increasing; simp?)
                    apply (erule order_less_le_trans; simp)
                   (* avoid leaking LENGTH *)
                   apply (subst unat_p2[where 'a=machine_word_len, folded word_bits_def])
                    apply (simp add: resetChunkBits_le_word_bits)
                   apply (rule order_less_trans[rotated],
                         rule_tac n=sz in power_strict_increasing; simp?)
                  apply (subst unat_of_nat_eq_mw)
                   apply (rule order_less_trans[rotated],
         ...
         apply (rule hoare_pre)
          ...
          apply (wp updateFreeIndex_valid_pspace_no_overlap'
                    ...
                    hoare_vcg_ex_lift
                    | simp)+
         apply (clarsimp simp add: cte_wp_at_ctes_of exI gen_isCap_simps valid_pspace'_def)
  ...
  apply (frule(1) descendants_range_ex_cte'[OF empty_descendants_range_in' _ order_refl],
         (simp add: gen_isCap_simps add_mask_fold)+)
  using word_size_bits_le_untyped_min_bits[folded minUntypedSizeBits_untyped_min_bits]
  apply (auto simp: descendants_range_in'_def valid_untyped'_def elim: is_aligned_weaken)
  done
...
lemma deleteObjects_ex_cte_cap_wp_to':
  ...
  apply (frule descendants_range_ex_cte'[rotated, OF _ order_refl, where p=p],
         (simp add: gen_isCap_simps empty_descendants_range_in')+)
...
lemma updateFreeIndex_cte_cap_wp_to'[wp]:
  ...
  apply (clarsimp simp: cte_wp_at_ctes_of)
  apply (clarsimp simp: gen_isCap_simps ex_cte_cap_wp_to'_def split: option.split)
  done
...
lemma (in Untyped_R) resetUntypedCap_invs_etc:
  ...
  apply (rule_tac Q'="\<lambda>_. invs' and valid_untyped_inv_wcap' ?ui (Some ?cap) and ct_active' and ?psp"
   ...
   apply (clarsimp simp: gen_isCap_simps empty_descendants_range_in' add_mask_fold)
   apply auto[1]
  ...
  apply (rule hoare_pre, rule hoare_strengthen_postE,
    rule_tac P="\<lambda>i. invs' and ?psp and ct_active' and valid_untyped_inv_wcap' ?ui
      ...
      apply (subgoal_tac "resetChunkBits < word_bits \<and> sz < word_bits")
       ...
       apply (subst nat_less_power_trans2[THEN order_less_imp_le])
         ...
         apply (subst unat_minus_one)
          ...
          (* avoid leaking LENGTH *)
          apply (drule of_nat_0[where 'a=machine_word_len, folded word_bits_def])
        ...
        apply simp
       apply (auto simp: minUntypedSizeBits_le_resetChunkBits)[1]
     ...
     apply (subgoal_tac "resetChunkBits < word_bits")
      ...
      apply (simp only: unat_lt2p[where 'a=machine_word_len, folded word_bits_def])
...
lemma (in Untyped_R) whenE_reset_resetUntypedCap_invs_etc:
  "\<lbrace>invs' and valid_untyped_inv_wcap' ui (Some (UntypedCap dev ptr sz idx))
    and ct_active'
...
abbreviation (input)
  "createNewCaps_valid_pspace_extra_pre n ptr sz ty us \<equiv> (\<lambda>s.
     n \<noteq> 0 \<and> ptr \<noteq> 0 \<and> range_cover ptr sz (APIType_capBits ty us) n
...
lemma createNewCaps_valid_pspace_extras:
  "\<lbrace>createNewCaps_valid_pspace_extra_pre n ptr sz ty us\<rbrace>
   createNewCaps ty ptr n us d
  ...
  by (all \<open>rule hoare_grab_asm\<close>)+
     (all \<open>rule hoare_pre, rule hoare_strengthen_post[OF createNewCaps_valid_pspace],
              (simp add: valid_pspace'_def)+\<close>)
...
defs canonicalAddressAssert_def:
  "canonicalAddressAssert \<equiv> canonical_address"

lemma (in Untyped_R_2) inv_untyped_corres:
  ...
  let ?ui' = "Invocations_H.untyped_invocation.Retype
    ...
    have ptr_sz_simp:
      "ptr_base = ptr && ~~ mask sz \<and> sz' = sz \<and> idx' = idx"
    ...
    have obj_bits_low_bound[simp]:
      ...
      apply (simp add: minUntypedSizeBits_untyped_min_bits)
      done
    ...
    have delete_objects_rewrite:
      "ptr && ~~ mask sz = ptr \<Longrightarrow>
       (delete_objects ptr sz :: (unit, det_ext) s_monad) =
       do y \<leftarrow> modify (clear_um {ptr + of_nat k |k. k < 2 ^ sz});
              modify (detype {ptr && ~~ mask sz..ptr + 2 ^ sz - 1})
       od"
      supply obj_bits_low_bound[simp del]
      using cover obj_bits_low_bound
            word_size_bits_le_untyped_min_bits[folded minUntypedSizeBits_untyped_min_bits]
      apply (prop_tac "is_aligned (ptr && ~~ mask sz) sz")
       apply (fastforce intro!: is_aligned_neg_mask)
      apply (simp add: delete_objects_def)
      apply (subst dmo_freeMemory_clear_um;
             clarsimp simp: range_cover_def[where 'a=machine_word_len, folded word_bits_def])
      done

    have of_nat_length:
       "of_nat (length slots) - (1::machine_word) < of_nat (length slots)"
       using vslot
       using range_cover.range_cover_le_n_less(1)[OF cover, where p="length slots", folded word_bits_def]
       apply -
    ...
    have size_eq[simp]: "APIType_capBits ao' us = obj_bits_api (APIType_map2 (Inr ao')) us"
      by (simp add: obj_bits_api_APIType_map2)
    ...
    have vc'[simp] : "s' \<turnstile>' capability.UntypedCap dev (ptr && ~~ mask sz) sz idx"
      ...
      apply (clarsimp simp:cte_wp_at_ctes_of)
      apply (case_tac cte)
      apply (clarsimp simp: ptr_sz_simp)
      apply (erule ctes_of_valid_cap')
      apply (simp add:invs_valid_objs')
      done

    have nidx[simp]:
      "ptr + (of_nat (length slots) * 2^obj_bits_api (APIType_map2 (Inr ao')) us) - (ptr && ~~ mask sz)
       = (ptr && mask sz) + (of_nat (length slots) * 2^obj_bits_api (APIType_map2 (Inr ao')) us)"
      by (subst word_plus_and_or_coroll2[symmetric, where w = "mask sz" and t = ptr])
         simp

    have idx_compare'[simp]:
      "unat ((ptr && mask sz) + (of_nat (length slots)<< obj_bits_api (APIType_map2 (Inr ao')) us)) \<le> 2 ^ sz"
      apply (rule le_trans[OF unat_plus_gt])
      apply (simp add: range_cover.unat_of_nat_n_shift[OF cover] range_cover_unat)
      apply (insert range_cover.range_cover_compare_bound[OF cover])
      apply simp
    ...
    have idx_compare''[simp]:
       "unat ((ptr && mask sz) + (of_nat (length slots) *
                                  (2::machine_word) ^ obj_bits_api (APIType_map2 (Inr ao')) us))
        < 2 ^ sz
   ...
   from vc' have sz_le_word_bits[simp]:
     "sz \<le> word_bits"
     using maxUntypedSizeBits_less_word_bits
     by (clarsimp simp: valid_cap'_def)
   ...
   have overlap_ranges:
     ...
     apply (cases reset; simp add: no_overflow)
    ...
    have canonical_ptr[simp]:
      "canonical_address ptr"
      using range_cover_canonical_address[OF cover _ ptr_cn sz_limit, where p=0]
      by (simp add: vslot)
    ...
    note set_cap_free_index_invs_spec =
      set_free_index_invs[where cap = "cap.UntypedCap dev (ptr && ~~ mask sz) sz (if reset then 0 else idx)",
        unfolded free_index_update_def free_index_of_def, simplified]

    note range_simps[simp del] =
    ...
    show " corres (dc \<oplus> (=)) ((=) s) ((=) s')
      ...
      apply (rule corres_guard_imp)
        apply (rule corres_split_norE)
          ...
          apply (rule corres_split[OF updateFreeIndex_corres])
             ...
             apply (clarsimp simp:getFreeIndex_def bits_of_def shiftL_nat shiftl_t2n
                                  free_index_of_def)
            apply (insert range_cover.range_cover_n_less[OF cover, folded word_bits_def] vslot)
            ...
            apply hypsubst
            apply (case_tac c, clarsimp simp: gen_isCap_simps)
           ...
           apply (clarsimp simp:conj_comms bits_of_def ptr_sz_simp)
         ...
         apply (intro conjI impI)
          ...
          apply (simp add: range_simps word_and_le2)
        ...
        apply (clarsimp simp: cte_wp_at_ctes_of
                              ...
                              invokeUntyped_proofs_descendants_range
        ...
        apply (intro conjI)
         ...
         apply (drule invokeUntyped_proofs.subset_stuff, simp,
             erule order_trans, simp add: range_simps word_and_le2 add_mask_fold)
     ...
     apply (clarsimp simp: ui cte_wp_at_ctes_of if_apply_def2 ui' ptr_sz_simp)
     done
qed
...
lemma sts_valid_untyped_inv':
  "setThreadState st t \<lbrace>valid_untyped_inv' ui\<rbrace>"
  apply (cases ui, simp add: ex_cte_cap_to'_def)
  apply (rule hoare_pre)
   apply (rule hoare_use_eq_irq_node'[OF setThreadState_ksInterruptState])
   apply (wpsimp wp: hoare_vcg_const_Ball_lift hoare_vcg_ex_lift)+
  done
...
crunch deleteObjects
  ...
  (simp: unless_def wp: crunch_wps ignore: Arch.deleteGhost)
...
lemma (in Untyped_R_2) insertNewCap_valid_pspace':
...
lemma insertNewCap_ct_idle_or_in_cur_domain'[wp]:
  ...
  apply (rule_tac Q'="\<lambda>_. obj_at' (\<lambda>tcb. tcbState tcb \<noteq> Structures_H.thread_state.Inactive) t
                          and obj_at' (\<lambda>tcb. d = tcbDomain tcb) t"
...
lemma capRange_subset_capBits:
  "\<lbrakk>capAligned cap; capAligned cap'; capRange cap \<subseteq> capRange cap'; capRange cap \<noteq> {}\<rbrakk>
...
lemma insertNewCap_urz[wp]:
  ...
  apply (intro conjI impI; clarsimp simp: gen_isCap_simps)
    apply (auto simp add: cteCaps_of_def untypedZeroRange_def gen_isCap_simps)
  done
...
lemma (in Untyped_R_2) insertNewCap_invs':
...
lemma (in Untyped_R_2) zipWithM_x_insertNewCap_invs'':
...
lemma createNewCaps_not_isZombie[wp]:
  ...
  apply (wpsimp wp: undefined_valid simp: gen_isCap_simps)
  done
...
lemma (in Untyped_R) createNewCaps_idlethread_ranges[wp]:
 ...
 shows "\<lbrace>invs' and valid_untyped_inv' ui
  ...
  proof -
    ...
    note range_simps[simp del] =
           ...
           atLeastatMost_subset_iff atLeastLessThan_iff Int_atLeastAtMost
           atLeastatMost_empty_iff split_paired_Ex usableUntypedRange.simps
    note descendants_range[simp] = invokeUntyped_proofs_descendants_range[OF pf]
    ...
    note ex_cte_no_overlap' = invokeUntyped_proofs_ex_cte_no_overlap'[OF pf]
    note cref_inv = invokeUntyped_proofs_cref_inv[OF pf]
    note slots_invD = invokeUntyped_proofs_slots_invD[OF pf]
    ...
    apply (clarsimp simp: cte_wp_at_ctes_of gen_isCap_simps getFreeIndex_def
    ...
    apply (intro conjI; assumption?)
          ...
          apply (simp add: APIType_capBits_lower_bound)
         apply (cases reset)
          apply clarsimp
     ...
     apply (simp add: range_simps word_and_le2 add_mask_fold)
    apply (frule valid_global_refsD2', clarsimp)
    apply (drule orthD1[OF _ idle_is_global])
    apply (clarsimp simp: range_simps not_le)
    apply (force dest!: less_le_trans[OF _ word_and_le2])
    done
qed
...
lemma inv_untyp_st_tcb_at'[wp]:
  ...
  apply (cases ui, clarsimp simp: cte_wp_at_ctes_of gen_isCap_simps)
