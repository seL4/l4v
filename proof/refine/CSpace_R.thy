...
lemma cteInsert_invs:
 "\<lbrace>invs' and cte_wp_at' (\<lambda>c. cteCap c=NullCap) dest and valid_cap' cap and
  ...
  apply (wpsimp wp: valid_bitmaps_lift valid_irq_node_lift irqs_masked_lift
                    cteInsert_norq sym_heap_sched_pointers_lift valid_dom_schedule'_lift)
...
lemma arch_update_setCTE_invs:
  ...
  apply (wp arch_update_setCTE_mdb setCTE_pspace_in_kernel_mappings' valid_bitmaps_lift
            ...
            sym_heap_sched_pointers_lift setCTE_valid_arch valid_dom_schedule'_lift
          ...
          apply (rule_tac R="\<lambda>r. ?P and cte_at dest and
           ...
           and (\<lambda>s. safe_parent_for' (ctes_of s) src' c')"
            ...
            apply (clarsimp simp: exec_gets update_cdt_def bind_assoc set_cdt_def
                   ...
                   | (rule bind_execI[where f="cap_insert_ext x y z x' y'" for x y z x' y'],
                      clarsimp simp: mdb_insert_abs.cap_insert_ext_det_def2 update_cdt_list_def set_cdt_list_def put_def
                           simp del: fun_upd_apply)
                   | rule refl)+
...
lemma cteInsert_simple_invs:
 "\<lbrace>invs' and cte_wp_at' (\<lambda>c. cteCap c=NullCap) dest and valid_cap' cap and
  ...
  apply (auto simp: invs'_def valid_pspace'_def
