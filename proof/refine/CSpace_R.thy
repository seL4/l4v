...
arch_requalify_facts set_cap_aobjs_of (* FIXME arch-split: ArchCSpaceInvPre_AI/ArchMove_R *)
lemmas [wp] = set_cap_aobjs_of
...
locale mdb_move =
  ...
  fixes m'
  defines "m' \<equiv> modify_map n (mdbNext src_node)
...
locale Arch_mdb_move = mdb_move + Arch

locale mdb_move_gen = mdb_move +
  assumes parent_preserved:
    "\<And>cte'. isMDBParentOf cte' (CTE cap' src_node) = isMDBParentOf cte' (CTE src_cap src_node)"
  assumes children_preserved:
    "\<And>cte'. isMDBParentOf (CTE cap' src_node) cte' = isMDBParentOf (CTE src_cap src_node) cte'"

context mdb_move_gen begin
...
locale CSpace_R =
  assumes mdb_move_parent_preserved: (* mdb_move.parent_preserved, needs Arch *)
    "\<And>m src src_cap src_node dest cap' old_dest_node cte'.
     mdb_move m src src_cap src_node dest cap' old_dest_node \<Longrightarrow>
     isMDBParentOf cte' (CTE cap' src_node) = isMDBParentOf cte' (CTE src_cap src_node)"
  assumes mdb_move_children_preserved: (* mdb_move.children_preserved, needs Arch *)
    "\<And>m src src_cap src_node dest cap' old_dest_node cte'.
    mdb_move m src src_cap src_node dest cap' old_dest_node \<Longrightarrow>
    isMDBParentOf (CTE cap' src_node) cte' = isMDBParentOf (CTE src_cap src_node) cte'"
  (* This is the expanded form of mdb_insert.chunked_n which needs Arch to prove.
     The original form contracts this significantly via internal locale variables from mdb_insert. *)
  assumes mdb_insert_chunked_n:
    "\<And>m src src_cap src_node dest dest_cap dest_node c'.
     \<lbrakk>mdb_insert m src src_cap src_node dest dest_cap dest_node; badge_derived' c' src_cap\<rbrakk>
     \<Longrightarrow> mdb_chunked
        (modify_map
          (modify_map (modify_map m dest (cteCap_update (\<lambda>_. c'))) dest
            (cteMDBNode_update
              (\<lambda>m. mdbFirstBadged_update (\<lambda>_. global.isCapRevocable c' src_cap)
                    (mdbRevocable_update (\<lambda>_. global.isCapRevocable c' src_cap)
                      (mdbPrev_update (\<lambda>_. src) src_node)))))
          src (cteMDBNode_update (mdbNext_update (\<lambda>_. dest))))"
  (* This is the expanded form of mdb_insert_sib.untyped_inc_n which needs Arch to prove.
     The original form contracts this significantly via internal locale variables from mdb_insert. *)
  assumes mdb_insert_sib_untyped_inc_n:
    "\<And>m src src_cap src_node dest dest_cap dest_node c'.
     \<lbrakk>mdb_insert_sib m src src_cap src_node dest dest_cap dest_node c'; capAligned c'\<rbrakk>
     \<Longrightarrow> untyped_inc'
        (modify_map
          (modify_map (modify_map m dest (cteCap_update (\<lambda>_. c'))) dest
            (cteMDBNode_update
              (\<lambda>m. mdbFirstBadged_update (\<lambda>_. global.isCapRevocable c' src_cap)
                    (mdbRevocable_update (\<lambda>_. global.isCapRevocable c' src_cap)
                      (mdbPrev_update (\<lambda>_. src) src_node)))))
          src (cteMDBNode_update (mdbNext_update (\<lambda>_. dest))))"
  assumes capAligned_master:
    "\<And>cap ncap. \<lbrakk>capAligned cap; capMasterCap cap = capMasterCap ncap\<rbrakk> \<Longrightarrow> capAligned ncap"
  assumes maskedAsFull_derived':
    "\<And>m src cap s_cap s_node ptr b c.
  ...
  assumes valid_arch_badges_mdbPrev_update[simp]:
    "\<And>cap cap' f node.
  ...
  assumes badge_derived'_capRange:
    "\<And>cap src_cap. badge_derived' cap src_cap \<Longrightarrow> capRange cap = capRange src_cap"
  assumes capMaster_untypedRange:
    "\<And>c c'. capMasterCap c = capMasterCap c' \<Longrightarrow> untypedRange c = untypedRange c'"
  assumes capMaster_capClass:
    "\<And>c c'. capMasterCap c = capMasterCap c' \<Longrightarrow> capClass c = capClass c'"
  assumes valid_arch_badges_non_arch:
    "\<And>c c' node. \<lbrakk> \<not>isArchObjectCap c; \<not>isArchObjectCap c' \<rbrakk> \<Longrightarrow> valid_arch_badges c c' node"
  assumes capMasterCap_valid_arch_badges_isCapRevocable:
    "\<And>src_cap cap word1 src.
  ...
  assumes capMaster_capRange:
    "capMasterCap c = capMasterCap c' \<Longrightarrow> capRange c = capRange c'"
  assumes cteInsert_pspace_in_kernel_mappings'[wp]:
    "\<And>cap src dest. cteInsert cap src dest \<lbrace>pspace_in_kernel_mappings'\<rbrace>"
  assumes setCTE_global_refs[wp]:
    "\<And>p c P. setCTE p c \<lbrace>\<lambda>s. P (global_refs' s)\<rbrace>"
  assumes cteInsert_arch_state'[wp]:
    "\<And>cap src dest. cteInsert cap src dest \<lbrace>valid_arch_state'\<rbrace>"
  assumes arch_deriveCap_corres:
   "\<And>c c' slot.
  ...
  assumes arch_deriveCap_inv:
    "\<And>arch_cap u P. Arch.deriveCap arch_cap u \<lbrace>P\<rbrace>"
  assumes arch_deriveCap_valid:
    "\<And>arch_cap u.
  ...
  assumes isArchMDBParentOf_non_arch:
    "\<And>cap cap' b. \<not>isArchObjectCap cap \<Longrightarrow> isArchMDBParentOf cap cap' b"
    "\<And>cap cap' b. \<not>isArchObjectCap cap' \<Longrightarrow> isArchMDBParentOf cap cap' b"
begin

(* this locale should satisfy all the assumptions of mdb_move_gen, so we can treat it like the
   non-gen locale *)
lemma mdb_move_convert:
  "mdb_move_gen = mdb_move"
  unfolding mdb_move_gen_def mdb_move_gen_def mdb_move_gen_axioms_def
  by (auto del: ext intro!: ext
           elim!: mdb_move_parent_preserved mdb_move_children_preserved)
...
lemma cteMove_corres:
  ...
  apply (thin_tac "ghost_relation_wrapper s t" for s t)+
  ...
  apply (rule conjI)
   ...
   apply clarsimp
   apply (subst mdb_move_gen.descendants[simplified mdb_move_convert], assumption)
  ...
  apply(case_tac ab)
  apply(frule mdb_move_gen.m'_next[simplified mdb_move_convert])
...
lemma untyped_c':
  ...
  apply -
   apply (case_tac "isUntypedCap src_cap")
    by (clarsimp dest!: is_derived'_genD
                 simp: gen_isCap_simps freeIndex_update_def badge_derived'_def capMasterCap_def
...
lemma capRange_c':
  ...
  apply -
  apply (case_tac "isUntypedCap src_cap"; clarsimp simp:untypedCapRange)
  apply (rule master_eqI, rule capRange_Master)
  apply (drule is_derived'_genD)
  apply (auto simp: gen_isCap_simps freeIndex_update_def badge_derived'_def capMasterCap_def
              split: if_splits capability.splits)
  done
...
lemma untyped_no_parent:
  ...
  using partial_is_derived' untyped_c'
  by (clarsimp dest!: is_derived'_genD simp: gen_isCap_simps freeIndex_update_def descendants_of'_def)
...
lemma irq_control_n:
  ...
  apply (clarsimp split: if_split_asm)
   apply (drule is_derived'_genD, simp add: gen_isCap_simps)
...
lemma is_derived_badge_derived':
  ...
  by (drule is_derived'_genD, simp)
...
lemma cteInsert_untyped_mdb:
  ...
  apply (wp getCTE_wp)+
  apply (clarsimp simp:cte_wp_at_ctes_of simp del:fun_upd_apply)
  apply (prop_tac "src \<noteq> 0", fastforce simp: valid_mdb_ctes_def no_0_def)
  apply (prop_tac "dest \<noteq> 0", fastforce simp: valid_mdb_ctes_def no_0_def)
  apply (rule conjI, fastforce simp: valid_mdb_ctes_def no_0_def)
  apply (case_tac cte)
  apply (rename_tac s_cap s_node)
  apply (case_tac cteb)
  apply (rename_tac d_cap d_node)
  apply (simp add: nullPointer_def)
  apply (prop_tac "mdb_insert_der (ctes_of s) src s_cap s_node dest NullCap d_node cap")
  ...
  apply (case_tac "isMDBParentOf (CTE s_cap s_node) (CTE cap
   ...
   apply (prop_tac "mdb_insert_child (ctes_of s) src s_cap s_node dest NullCap d_node cap")
  ...
  apply (prop_tac "mdb_insert_sib (ctes_of s) src s_cap s_node dest NullCap d_node cap")
...
lemma valid_mdb_ctes_maskedAsFull:
  ...
  apply (frule mdb_inv_preserve_updateCap
   ...
   apply (rule mdb_inv_preserve.untyped_inc')
    ...
    apply (clarsimp split:if_split_asm simp: gen_isCap_simps max_free_index_def)
   apply simp
...
lemma capAligned_maskedAsFull:
  ...
  by (case_tac s_cap;
      clarsimp simp: gen_isCap_simps capAligned_def maskedAsFull_def max_free_index_def)
...
lemma maskedAsFull_usable_empty:
  ...
  by (simp add: gen_isCap_simps maskedAsFull_def max_free_index_def split: if_split_asm;
      fastforce)

lemma (in CSpace_R) cteInsert_mdb_chunked:
  "\<lbrace>valid_mdb' and pspace_aligned' and pspace_distinct' and (\<lambda>s. src \<noteq> dest) and
    (\<lambda>s. cte_wp_at' (is_derived' (ctes_of s) src cap \<circ> cteCap) src s)\<rbrace>
   cteInsert cap src dest
  ...
  apply (wp hoare_vcg_imp_lift hoare_vcg_all_lift setUntypedCapAsFull_ctes_of
            setUntypedCapAsFull_ctes_of_no_0 mdb_inv_preserve_modify_map
            setUntypedCapAsFull_mdb_chunked mdb_inv_preserve_fun_upd,
         simp)
  apply (wp getCTE_wp)+
  apply (clarsimp simp:cte_wp_at_ctes_of simp del:fun_upd_apply)
  apply (prop_tac "src \<noteq> 0", fastforce simp: valid_mdb_ctes_def no_0_def)
  ...
  apply (subgoal_tac "mdb_insert (ctes_of s) src s_cap s_node dest NullCap d_node")
   apply (drule mdb_insert_chunked_n, erule is_derived_badge_derived')
...
lemma (in CSpace_R) cteInsert_untyped_inc':
  ...
  apply (subgoal_tac "mdb_insert_sib
   ...
   prefer 2
   apply (simp add: mdb_insert_sib_def mdb_insert_sib_axioms_def)
  apply (drule mdb_insert_sib_untyped_inc_n)
   apply (rule capAligned_master[OF valid_capAligned])
   apply (erule(1) ctes_of_valid_cap')
    apply (clarsimp dest!: is_derived'_genD simp: badge_derived'_def)
  apply (clarsimp simp: modify_map_apply untyped_inc_prev_update maskedAsFull_revokable
                        descendants_of_prev_update)
  done
...
lemma setUntypedCapAsFull_valid_dlist:
  ...
  apply (clarsimp simp:setUntypedCapAsFull_def split: if_splits,
         intro conjI impI)
   apply (wpsimp wp: updateCapFreeIndex_dlist simp: preserve cte_wp_at_ctes_of)+
  done
...
lemma cteInsert_no_0:
  ...
  apply (wp updateMDB_ctes_of_no_0 getCTE_wp')
      ...
      apply (wp hoare_vcg_imp_lift hoare_vcg_all_lift setUntypedCapAsFull_ctes_of
                ...
                setUntypedCapAsFull_valid_dlist mdb_inv_preserve_fun_upd
             | simp)+
  apply (intro conjI impI)
  apply (clarsimp simp:cte_wp_at_ctes_of)
...
lemma cteInsert_valid_dlist:
  ...
  apply (rule valid_dlistI)
   ...
   apply (frule(2) valid_dlist_prevD)
    apply simp
    apply (prop_tac "mdbPrev (cteMDBNode ctea) \<noteq> mdbNext (cteMDBNode ctea)")
     apply (clarsimp)
...
lemma cteInsert_mdb'[wp]:
  "\<lbrace>valid_mdb' and pspace_aligned' and pspace_distinct' and (\<lambda>s. src \<noteq> dest) and K (capAligned cap)
    and valid_objs' and (\<lambda>s. cte_wp_at' (is_derived' (ctes_of s) src cap \<circ> cteCap) src s) \<rbrace>
   cteInsert cap src dest
  ...
  apply (rule_tac Q'="\<lambda>r s. valid_dlist (ctes_of s) \<and> irq_control (ctes_of s) \<and>
                            ...
                            mdb_chunked (ctes_of s) \<and> untyped_mdb' (ctes_of s) \<and>
                            untyped_inc' (ctes_of s) \<and>
                            Q s" for Q
...
proof -
  ...
  from is_der have notZomb1: "\<not> isZombie cap"
    by (clarsimp dest!: is_derived'_genD simp: gen_isCap_simps badge_derived'_def)

  from is_der have notZomb2: "\<not> isZombie src_cap"
    by (clarsimp dest!: is_derived'_genD simp: gen_isCap_simps)
  ...
  show "valid_badges ?C"
    ...
    apply (elim disjE)
      ...
      apply (case_tac cap;
             clarsimp simp: gen_isCap_simps valid_arch_badges_non_arch
                            capMasterCap_valid_arch_badges_isCapRevocable)
    ...
    apply (rule conjI, clarsimp)
     subgoal by (auto simp: gen_isCap_simps sameRegionAs_def) (* slowish *)
    apply clarsimp
  ...
  have isUntyped_eq: "isUntypedCap cap = isUntypedCap src_cap"
   apply (clarsimp simp: badge_derived'_def)
   apply (case_tac cap, auto simp: gen_isCap_simps)
   done

  note badge_derived'_capRange[simp]

  have [simp]: "untypedRange cap = untypedRange src_cap"
     using badge
     by (clarsimp simp:badge_derived'_def dest!: capMaster_untypedRange)
  ...
  apply (clarsimp simp: modify_map_def dest!: is_derived'_genD)
  apply (rule conjI)
    apply (clarsimp simp: badge_derived'_def)+
  apply (drule_tac x = word1 in spec)
  apply (case_tac z)
  ...
  apply (rule conjI)
   apply clarsimp
   apply (clarsimp simp: Retype_H.isCapRevocable_def gen_isCap_simps)+
   apply auto
  apply (drule_tac x= src in spec)
   apply clarsimp
  apply (case_tac z)
 ...
 show "reply_masters_rvk_fb ?C"
   ...
   apply (clarsimp simp:modify_map_def split:if_split_asm)
    apply fastforce+
   apply (clarsimp dest!: is_derived'_genD simp: gen_isCap_simps)
   apply fastforce
 done
qed
...
lemma (in CSpace_R) cteInsert_valid_pspace:
  ...
  by (wpsimp wp: cteInsert_valid_objs,
      fastforce elim: valid_capAligned)

(* same derivation on all architectures: changing caps never affects anything VCPU-related *)
lemma (in Arch) hyp_live'_tcb_cap_update[simp]:
  "hyp_live' (KOTCB (tcbIPCBufferFrame_update (\<lambda>_. v) tcb)) = hyp_live' (KOTCB tcb)"
  "hyp_live' (KOTCB (tcbCaller_update (\<lambda>_. v) tcb)) = hyp_live' (KOTCB tcb)"
  "hyp_live' (KOTCB (tcbReply_update (\<lambda>_. v) tcb)) = hyp_live' (KOTCB tcb)"
  "hyp_live' (KOTCB (tcbVTable_update (\<lambda>_. v) tcb)) = hyp_live' (KOTCB tcb)"
  "hyp_live' (KOTCB (tcbCTable_update (\<lambda>_. v) tcb)) = hyp_live' (KOTCB tcb)"
  by (auto simp: hyp_live'_def)

requalify_facts Arch.hyp_live'_tcb_cap_update
lemmas [simp] = hyp_live'_tcb_cap_update
...
lemma updateMDB_global_refs[wp]:
  "updateMDB p m \<lbrace>valid_global_refs'\<rbrace>"
  apply (clarsimp simp add: updateMDB_def)
  apply (rule hoare_pre)
   apply (wp getCTE_wp)
...
lemma cteInsert_valid_irq_handlers'[wp]:
  ...
  apply (intro conjI impI)
   ...
   apply (drule bspec)
    apply fastforce
   apply (clarsimp simp: gen_isCap_simps modify_map_def split:if_splits)
  apply (clarsimp simp:ran_dom modify_map_dom)
  apply (drule bspec)
   apply fastforce
  apply (clarsimp simp:modify_map_def split:if_splits)
...
lemma setCTE_arch_ctes_of_wp [wp]:
  ...
  apply (elim exE conjE disjE rsubst[where P="P (ksArchState s)" for s])
   ...
   apply (subst map_to_ctes_upd_tcb; assumption?)
    apply (clarsimp simp: mask_def field_simps ps_clear_def3)
...
lemma setUntypedCapAsFull_valid_refs'[wp]:
  ...
  apply (clarsimp simp:modify_map_def cte_wp_at_ctes_of gen_isCap_simps split:if_splits)
  done
...
lemma (in CSpace_R) setUntypedCapAsFull_valid_global_refs'[wp]:
...
lemma (in CSpace_R) cteInsert_valid_globals[wp]:
 "\<lbrace>valid_global_refs' and (\<lambda>s. cte_wp_at' (is_derived' (ctes_of s) src cap \<circ> cteCap) src s)\<rbrace>
  ...
  apply (rule hoare_pre)
   apply (wp getCTE_wp)
  apply (clarsimp simp: cte_wp_at_ctes_of badge_derived'_def dest!: is_derived'_genD)
...
definition untyped_derived_eq :: "capability \<Rightarrow> capability \<Rightarrow> bool" where
  "untyped_derived_eq cap cap' \<equiv> (isUntypedCap cap \<longrightarrow> cap = cap')"
...
lemma usableUntypedRange_empty:
  ...
  apply (rule order_trans, rule word_plus_mono_right)
    ...
    apply simp
    (* avoid LENGTH(64/32) to keep generic proof *)
    apply (subst unat_2p_sub_1[where 'a=machine_word_len, simplified word_bits_def[symmetric]])
     apply assumption
    apply simp
...
lemma cteInsert_untyped_ranges_zero[wp]:
 "\<lbrace>untyped_ranges_zero' and (\<lambda>s. src \<noteq> dest) and valid_mdb'
  ...
  apply (intro impI conjI allI; erule
   ...
   apply (clarsimp simp: gen_isCap_simps untypedZeroRange_def
...
lemma deriveCap_corres:
 "\<lbrakk>cap_relation c c'; cte = cte_map slot \<rbrakk> \<Longrightarrow>
  ...
  apply (case_tac c)
            apply (simp_all add: returnOk_def Let_def is_zombie_def gen_isCap_simps
                          split: sum.splits)
...
lemma deriveCap_valid[wp]:
  "\<lbrace>\<lambda>s. s \<turnstile>' c\<rbrace> deriveCap slot c \<lbrace>\<lambda>rv s. s \<turnstile>' rv\<rbrace>,-"
...
lemma maskCapRightsNull [simp]:
  ...
  by (simp add: maskCapRights_def gen_isCap_defs)

lemma maskCapRightsUntyped [simp]:
  "maskCapRights R (UntypedCap d r n f) = UntypedCap d r n f"
  by (simp add: maskCapRights_def gen_isCap_defs Let_def)
...
lemma untyped_derived_eq_ArchObjectCap:
  ...
  by (rule ext, simp add: untyped_derived_eq_def gen_isCap_simps)

lemma setCTE_corres:
  "cap_relation cap (cteCap cte) \<Longrightarrow>
   corres_underlying {(s::det_state, s'). pspace_relation (kheap s) (ksPSpace s')} False True dc
  ...
  "safe_parent_for' m p cap \<equiv>
     \<exists>parent node. m p = Some (CTE parent node) \<and> sameRegionAs parent cap \<and>
                   ((\<exists>irq. cap = IRQHandlerCap irq) \<and> parent = IRQControlCap \<and>
                    ...
                    \<or> safe_parent_for_arch' m cap parent
...
(* Parts of mdb_insert_simple which need Arch to prove, but can be assumed in order to make the
   rest generic. These are duplicated in CSpace_R_2 so we can pretend we have mdb_insert_simple
   there. *)
locale mdb_insert_simple_gen = mdb_insert_simple +
  assumes dest_no_parent_n: "n \<turnstile> dest \<rightarrow> p = False"
  assumes new_child[simp]: "isMDBParentOf new_src new_dest"

(* These assumptions needed to make proofs generic in mdb_insert_simple'_gen later in this theory,
   but since there is overlap with CSpace_R_2, we consolidate them here. This also lets us overload
   CSpace_R_2_assms to discharge the assumptions of mdb_insert_simple'_gen once they are proved in
   Arch. *)
locale mdb_insert_simple'_gen_Arch_assms =
  assumes safe_parent_for_untypedRange:
    "\<And>m p cap cte.
     \<lbrakk> safe_parent_for' m p cap; m p = Some cte \<rbrakk> \<Longrightarrow> untypedRange cap \<subseteq> untypedRange (cteCap cte)"
  assumes safe_parent_for_capRange_capBits:
    "\<And>m p cap cte.
     \<lbrakk> safe_parent_for' m p cap; m p = Some cte \<rbrakk>
     \<Longrightarrow> capRange cap \<subseteq> capRange (cteCap cte) \<and> capBits cap \<le> capBits (cteCap cte)"
  assumes safe_parent_for_capUntypedRange:
    "\<And>m p cap cte.
     \<lbrakk> safe_parent_for' m p cap; m p = Some cte \<rbrakk> \<Longrightarrow> capRange cap \<subseteq> untypedRange (cteCap cte)"
  assumes safe_parent_for_descendants':
    "\<And>m p cap pcap n.
     \<lbrakk> safe_parent_for' m p cap; m p = Some (CTE pcap n); isUntypedCap pcap \<rbrakk>
     \<Longrightarrow> descendants_of' p m = {}"
  assumes is_simple_cap'_genD:
    "\<And>cap.
  ...
  assumes safe_parent_capClass:
    "\<And>m p cap src_cap n.
...
lemma corres_caps_decomposition:
  ...
  assumes updates:
             ...
             "\<And>P s'. \<lbrace>\<lambda>s. P (ghost_relation_wrapper (new_gs s) s')\<rbrace> f \<lbrace>\<lambda>rv s. P (ghost_relation_wrapper s s')\<rbrace>"
             "\<And>P s. \<lbrace>\<lambda>s'. P (ghost_relation_wrapper s (new_gs' s'))\<rbrace> g \<lbrace>\<lambda>rv s'. P (ghost_relation_wrapper s s')\<rbrace>"
  assumes updated_relations:
    "\<And>s s'. \<lbrakk> P s; P' s'; (s, s') \<in> state_relation \<rbrakk>
              \<Longrightarrow> cdt_relation ((\<noteq>) None \<circ> new_caps s) (new_mdb s) (new_ctes s')
                  ...
                  \<and> ghost_relation_wrapper (new_gs s) (new_gs' s')"
...
lemma setupReplyMaster_corres:
  ...
  apply (rule_tac F="t + 2^cte_level_bits*2 = cte_map (t, tcb_cnode_index 2)" in corres_req)
   apply (simp add: cte_map_def tcb_cnode_index_def shiftl_t2n)
  apply simp
...
locale CSpace_R_2 = CSpace_R +
  mdb_insert_simple'_gen_Arch_assms +
  assumes mdb_insert_simple_gen_new_child: (* mdb_insert_simple.new_child, needs Arch *)
    "\<And>m src src_cap src_node dest dest_cap dest_node c'.
     mdb_insert_simple m src src_cap src_node dest dest_cap dest_node c' \<Longrightarrow>
     isMDBParentOf (mdb_insert.new_src src_cap src_node dest)
       (mdb_insert.new_dest src src_cap src_node c')"
  (* This is the expanded form of mdb_insert_simple.dest_no_parent_n which needs Arch to prove.
     The original form contracts this significantly via internal locale variables from mdb_insert. *)
  assumes mdb_insert_simple_dest_no_parent_n:
    "\<And>m src src_cap src_node dest dest_cap dest_node c'.
     mdb_insert_simple m src src_cap src_node dest dest_cap dest_node c' \<Longrightarrow>
     modify_map
      (modify_map (modify_map m dest (cteCap_update (\<lambda>_. c'))) dest
        (cteMDBNode_update
          (\<lambda>m. mdbFirstBadged_update (\<lambda>a. global.isCapRevocable c' src_cap)
                (mdbRevocable_update (\<lambda>a. global.isCapRevocable c' src_cap)
  ...
  assumes zobj_refs_Master:
    "\<And>cap. zobj_refs' (capMasterCap cap) = zobj_refs' cap"
  assumes setCTE_pspace_in_kernel_mappings':
    "\<And>ptr val. setCTE ptr val \<lbrace>pspace_in_kernel_mappings'\<rbrace>"
  assumes arch_update_setCTE_mdb:
    "\<And>cap p oldcte.
     ...
     \<lbrace>\<lambda>_. valid_mdb'\<rbrace>"
  assumes setUntypedCapAsFull_safe_parent_for':
    "\<And>slot a srcCTE.
  ...
  assumes setUntypedCapAsFull_archMDBAssertions[wp]:
    "\<And>src_cap cap p. setUntypedCapAsFull src_cap cap p \<lbrace>archMDBAssertions\<rbrace>"
  assumes maskedAsFull_revokable_safe_parent:
    "\<And>m p c' cte src_cap' a.
  ...
  assumes sameRegion_capRange_sub:
    "\<And>cap cap'. sameRegionAs cap cap' \<Longrightarrow> capRange cap' \<subseteq> capRange cap"
  assumes capRange_sameRegionAs:
    "\<And>x y s.
...
(* this locale should satisfy all the assumptions of mdb_insert_simple_gen, so we can treat it like
   the non-gen locale *)
lemma mdb_insert_simple_convert:
  "mdb_insert_simple_gen = mdb_insert_simple"
...
lemma setupReplyMaster_valid_objs[wp]:
  ...
  apply (clarsimp)
  apply (frule obj_at_aligned', simp add: gen_objBits_simps)
  apply (simp add: valid_cap'_def capAligned_def obj_sizeBits_less_word_bits gen_objBits_simps)
  done
...
lemma setupReplyMaster_ifunsafe'[wp]:
  "slot = t + (tcbReplySlot << cteSizeBits) \<Longrightarrow>
  ...
  apply (clarsimp simp: ex_cte_cap_to'_def cte_wp_at_ctes_of cteCaps_of_def
                        cteSizeBits_cte_level_bits gen_objBits_simps)
  apply (prop_tac " 2 ^ cte_level_bits * tcbReplySlot = tcbReplySlot << cte_level_bits")
   apply (solves \<open>simp add: shiftl_t2n\<close>)
...
lemma setupReplyMaster_urz[wp]:
  ...
  apply (clarsimp simp: cteCaps_of_def untypedZeroRange_def Let_def gen_isCap_simps)
  done
...
lemma arch_update_descendants':
  ...
  apply (auto simp: is_arch_update'_def gen_isCap_simps)
  done
...
lemma arch_update_setCTE_invs:
  ...
  apply (subst untyped_ranges_zero_delta[where xs="[p]"], assumption, simp_all)
   apply (clarsimp simp: ran_restrict_map_insert cteCaps_of_def
                         ...
                         gen_isCap_simps[where v="ArchObjectCap ac" for ac])
...
lemma arch_update_updateCap_invs:
  ...
  unfolding updateCap_def
  by (wpsimp wp: arch_update_setCTE_invs getCTE_wp')

end (* CSpace_R_2 *)

context mdb_insert_simple_gen begin
...
end (* mdb_insert_simple_gen *)

lemma (in CSpace_R_2) cteInsert_simple_corres:
  ...
  apply (rule conjI)
   ...
   apply (drule_tac src_cap' = src_cap' in maskedAsFull_revokable_safe_parent[symmetric])
      apply simp+
   apply (subst mdb_insert_simple_gen.descendants[simplified mdb_insert_simple_convert])
    apply simp
...
lemma notUntypedRange:
  ...
  by (cases cap) (auto simp: gen_isCap_simps)
...
lemma n'_badge:
  "n' p = Some (CTE cap node) \<Longrightarrow>
   \<exists>node'. if p = dest then mdbFirstBadged node = isCapRevocable c' src_cap
                            \<and> m p = Some (CTE dest_cap node')
...
end (* mdb_insert_simple' *)

(* due to locale hierarchy, we need to access results of both mdb_insert_simple_gen and
   mdb_insert_simple' *)
locale mdb_insert_simple'_gen =
  mdb_insert_simple_gen +
  mdb_insert_simple' +
  mdb_insert_simple'_gen_Arch_assms +
  assumes ut_capRange_non_empty: "isUntypedCap src_cap \<Longrightarrow> capRange c' \<noteq> {}"

context mdb_insert_simple'_gen begin
...
lemma not_ut_c'[simp]:
  "\<not>isUntypedCap c'"
  using simple
  by (simp add: is_simple_cap'_genD)
...
lemma c_not_ep[simp]:
  "\<not>isEndpointCap c'"
  using simple by (simp add: is_simple_cap'_genD)
...
lemma notArchPage:
  ...
  using simple
  by (clarsimp simp: gen_isCap_simps is_simple_cap'_genD)
...
lemma irq'[simp]:
  ...
  apply (drule n'_rev)
  apply (clarsimp split: if_split_asm)
   apply (clarsimp dest!: is_simple_cap'_genD)
  apply (frule irq_revocable, rule irq_control)
  apply clarsimp
  apply (drule n'_cap)
  apply (clarsimp split: if_split_asm)
...
lemma reply_masters_rvk_fb'[simp]:
  "reply_masters_rvk_fb n'"
   ...
   apply (drule is_simple_cap'_genD)
   apply (subst ball_ran_modify_map_eq)
    apply (clarsimp simp: modify_map_def m_p)
   apply (simp add: ball_ran_modify_map_eq m_p dest_cap gen_isCap_simps)
   done

end (* mdb_insert_simple'_gen *)
...
lemma locateSlot_cap_to'[wp]:
  ...
  apply (clarsimp dest!: isCapDs valid_capAligned
                   simp: gen_objBits_simps mult.commute capAligned_def shiftl_t2n
...
proof (induct arbitrary: s rule: resolveAddressBits.induct)
  ...
  show ?case
    ...
    apply (simp add: Let_def split_def cap_case_CNodeCap[unfolded gen_isCap_simps]
               split del: if_split cong: if_cong)
...
lemma usableUntypedRange_mono2:
  ...
  apply (clarsimp simp only: gen_isCap_simps capability.sel del: subsetI)
...
locale CSpace_R_3 = CSpace_R_2 +
  (* This is the expanded form of mdb_insert_simple'.mdb which needs Arch to prove.
     The original form contracts this significantly via internal locale variables from mdb_insert. *)
  assumes mdb_insert_simple'_mdb:
    "\<And>m src src_cap src_node dest dest_cap dest_node c'.
     mdb_insert_simple' m src src_cap src_node dest dest_cap dest_node c' \<Longrightarrow>
     valid_mdb_ctes
      (modify_map
  ...
  assumes updateCap_valid_arch_state'[wp]:
    "\<And>src c. updateCap src c \<lbrace>valid_arch_state'\<rbrace>"
  assumes ghost_relation_wrapper_set_cap_setCTE:
    "\<And>a c a' c' cap cap' rv slot.
...
lemma cteInsert_simple_mdb':
  ...
  apply (subst modify_map_eq)
     apply simp+
   apply (clarsimp simp:maskedAsFull_def dest!: is_simple_cap'_genD)
  ...
  apply (drule mdb_insert_simple'_mdb)
  apply (clarsimp simp:valid_mdb_ctes_def)
  done
...
lemma updateCap_same_master:
  ...
  apply (rule corres_guard_imp)
    apply (rule_tac Q="?P" and R'="\<lambda>cte. ?P' and (\<lambda>s. ctes_of s (cte_map slot) = Some cte)"
       ...
       apply (rule conjI)
        apply (erule (2) ghost_relation_wrapper_set_cap_setCTE)
       apply (subst conj_assoc[symmetric])
       apply (rule conjI)
        prefer 2
        apply (rule conjI)
...
crunch updateFreeIndex, updateCap
...
lemma updateFreeIndex_forward_invs':
  ...
  apply (clarsimp simp: cte_wp_at_ctes_of fun_upd_def[symmetric])
  apply (clarsimp simp: gen_isCap_simps valid_pspace'_def)
