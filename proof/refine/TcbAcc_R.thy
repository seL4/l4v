...
lemma compareVals_not_None:
  ...
  by (fastforce simp: compareVals_def runReaderT_def split: if_splits option.splits)
...
lemma findInsertionPoint_beforePtr_afterPtr:
  ...
  apply (case_tac ys'; force dest!: spec antisympD)
  done
...
lemma tcbQueueInsert_rewrite:
  ...
  (* step over left and right hand sides until we get to the threadSets *)
  apply monadic_rewrite_symb_exec_l
    apply monadic_rewrite_symb_exec_l
     apply monadic_rewrite_symb_exec_l
      apply monadic_rewrite_symb_exec_l
       apply monadic_rewrite_symb_exec_l
        apply monadic_rewrite_symb_exec_r
         apply monadic_rewrite_symb_exec_r
          (* repeatedly apply threadSet_rewrite to the head of both sides *)
          apply ((rule monadic_rewrite_bind)?, rule threadSet_rewrite)+
  ...
  apply (rule_tac P="\<lambda>ptrOpt s. (\<forall>t \<in> set ts. f t s \<noteq> None \<and> tcb_at' t s)
    ...
    apply (rule_tac C'="\<lambda>r _. r \<noteq> None" in whileLoop_terminates_weaken_cond)
     ...
     apply (rule_tac P'="\<lambda>r r' s'. (\<exists>end. heap_path (tcbSchedNexts_of s') (Some (hd ts)) r end
             ...
             apply clarsimp
             apply (frule (1) heap_path_head)
             apply (force intro: distinct_prefix' elim!: prefix_order.trans simp: prefix_def)
            apply force
           apply wpsimp
           apply (elim disjE)
...
crunch tcbQueueRemove, tcbQueuePrepend, tcbQueueAppend, tcbQueueInsert
  ...
  (wp: crunch_wps threadSet_inQ_unchanged threadSet_field_inv threadSet_field_opt_pred)
