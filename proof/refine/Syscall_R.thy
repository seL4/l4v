...
theory Syscall_R
imports ArchTcb_R ArchArch_R ArchInterrupt_R SchedContextInv_R
begin
...
lemma gets_comp_eq_return: (* FIXME: move to Monads *)
  "gets (f \<circ> g) = do x \<leftarrow> gets g; return (f x) od"
  by monad_eq

lemma fst_snd_pairI: (* FIXME: move *)
...
lemma setObject_F_st_tcb_at':
  "\<lbrakk>\<And>tcb f. tcbState (F f tcb) = tcbState tcb \<rbrakk> \<Longrightarrow>
   \<lbrace>st_tcb_at' P t' and obj_at' ((=) tcb) t\<rbrace>
   setObject t (F f tcb)
...
lemma doNormalTransfer_non_null_cte_wp_at':
  ...
  by (wp transferCaps_non_null_cte_wp_at' | simp add:PUC)+
...
lemma doIPCTransfer_non_null_cte_wp_at':
  ...
  by (wpsimp wp: doNormalTransfer_non_null_cte_wp_at' hoare_drop_imp hoare_allI simp:PUC)
...
lemma doIPCTransfer_non_null_cte_wp_at2':
  ...
  shows "doIPCTransfer sender endpoint badge grant receiver \<lbrace>cte_wp_at' (\<lambda>cte. P (cteCap cte)) ptr\<rbrace>"
  proof -
    ...
    show ?thesis
      apply (rule hoare_chain[OF doIPCTransfer_non_null_cte_wp_at'])
        apply (erule PUC)
       apply (fastforce elim: PimpQ cte_wp_at_weakenE' dest: PNN)+
      done
  qed

(* FIXME: move *)
lemma getSlotCap_eq [wp]:
  "\<lbrace>\<top>\<rbrace> getSlotCap slot \<lbrace>\<lambda>cap. cte_wp_at' ((=) cap \<circ> cteCap) slot\<rbrace>"
...
lemma contract_all_imp_strg:
  "P \<and> P' \<and> P'' \<and> (\<forall>x. R x \<longrightarrow> Q x) \<Longrightarrow> \<forall>x. R x \<longrightarrow> P \<and> P' \<and> Q x \<and> P''"
  by blast

locale Syscall_R =
  assumes prepareSetDomain_corres[corres]:
    "\<And>tptr new_dom.
  ...
  assumes prepareSetDomain_invs'[wp]:
    "\<And>t d. prepareSetDomain t d \<lbrace>invs'\<rbrace>"
  assumes prepareSetDomain_sch_act_simple[wp]:
    "\<And>t d. prepareSetDomain t d \<lbrace>sch_act_simple\<rbrace>"
  assumes prepareSetDomain_tcb_at'[wp]:
    "\<And>t d p. prepareSetDomain t d \<lbrace>tcb_at' p\<rbrace>"
  assumes prepareSetDomain_ct_in_state'[wp]:
    "\<And>t d P. prepareSetDomain t d \<lbrace>ct_in_state' P\<rbrace>"
  assumes prepareSetDomain_ksCurThread[wp]:
    "\<And>t d P. prepareSetDomain t d \<lbrace>\<lambda>s. P (ksCurThread s)\<rbrace>"
  assumes postSetFlags_typ_at'[wp]:
    "\<And>t flags P T p. postSetFlags t flags \<lbrace>\<lambda>s. P (typ_at' T p s)\<rbrace>"
  assumes postSetFlags_sc_at'_n[wp]:
    "\<And>t flags P n p. postSetFlags t flags \<lbrace>\<lambda>s.  P (sc_at'_n n p s)\<rbrace>"
  assumes postSetFlags_ksCurThread[wp]:
    "\<And>t flags P. postSetFlags t flags \<lbrace>\<lambda>s.  P (ksCurThread s)\<rbrace>"
  assumes arch_performIRQControl_typ_at'[wp]:
    "\<And>inv P T p. Arch.performIRQControl inv \<lbrace>\<lambda>s. P (typ_at' T p s)\<rbrace>"
  assumes arch_performIRQControl_sc_at'_n[wp]:
    "\<And>inv P n p. Arch.performIRQControl inv \<lbrace>\<lambda>s. P (sc_at'_n n p s)\<rbrace>"
  assumes arch_performIRQControl_ksCurThread[wp]:
    "\<And>inv P. Arch.performIRQControl inv \<lbrace>\<lambda>s. P (ksCurThread s)\<rbrace>"
  assumes Arch_performIRQControl_inv_EE[wp]:
    "\<And>irqc P. \<lbrace>\<top>\<rbrace> Arch.performIRQControl irqc -, \<lbrace>P\<rbrace>"
  assumes arch_performInvocation_inv:
    "\<And>invocation P. \<lbrace>\<top>\<rbrace> Arch.performInvocation invocation -, \<lbrace>P\<rbrace>"
  assumes arch_invokeIRQHandler_typ_at'[wp]:
    "\<And>inv P T p. Arch.invokeIRQHandler inv \<lbrace>\<lambda>s. P (typ_at' T p s)\<rbrace>"
  assumes arch_invokeIRQHandler_sc_at'_n[wp]:
    "\<And>inv P n p. Arch.invokeIRQHandler inv \<lbrace>\<lambda>s. P (sc_at'_n n p s)\<rbrace>"
  assumes arch_invokeIRQHandler_ksCurThread[wp]:
    "\<And>inv P. Arch.invokeIRQHandler inv \<lbrace>\<lambda>s. P (ksCurThread s)\<rbrace>"
  assumes setThreadState_irq_control_inv_valid'[wp]:
    "\<And>inv st t. setThreadState st t \<lbrace>irq_control_inv_valid' inv\<rbrace>"
  (* FIXME arch-split: consider moving to where other msgRegisters stuff goes... Tcb_R? Ipc_R? *)
  assumes len_msg_registes_le_max_length:
    "length msg_registers \<le> msg_max_length"
  assumes capRegister_cap_register:
    "capRegister = cap_register"
  assumes hv_invs'[wp]:
    "\<And>t' vptr. \<lbrace>invs' and tcb_at' t'\<rbrace> handleVMFault t' vptr \<lbrace>\<lambda>r. invs'\<rbrace>"
  assumes handleVMFault_st_tcb_at'[wp]:
    "\<And>t' vptr P t. handleVMFault t' vptr \<lbrace>st_tcb_at' P t\<rbrace>"
  assumes handleVMFault_ex_nonz_cap_to'[wp]:
    "\<And>t' vptr t. handleVMFault t' vptr \<lbrace>ex_nonz_cap_to' t\<rbrace>"
  assumes handleVMFault_ksit[wp]:
    "\<And>t' vptr P. handleVMFault t' vptr \<lbrace>\<lambda>s. P (ksIdleThread s)\<rbrace>"
  assumes handleVMFault_nosch[wp]:
    "\<And>t ft P. handleVMFault t ft \<lbrace>\<lambda>s. P (ksSchedulerAction s)\<rbrace>"
  assumes handleSpuriousIRQ_corres[corres]:
  ...
  assumes handleHypervisorFault_corres:
    "\<And>thread fault.
     corres dc
  ...
  assumes handleSpuriousIRQ_invs'[wp]:
...
This identifies all possible errors before actually performing the syscall.
*)

lemma syscall_corres:
  assumes corres:
    "corres (fr \<oplus> r_flt_rel) P P' m_flt m_flt'"
    "\<And>flt flt'. flt' = fault_map flt \<Longrightarrow> corres r (P_flt flt) (P'_flt flt') (h_flt flt) (h_flt' flt')"
...
lemma decodeInvocation_corres:
  ...
  apply (case_tac cap, simp_all only: cap.simps)
   \<comment> \<open>dammit, simp_all messes things up, must handle cases manually\<close>
             \<comment> \<open>Null\<close>
              ...
              apply (simp add: gen_isCap_simps Let_def o_def split del: if_split)
             ...
             apply (simp add: gen_isCap_simps returnOk_def)
            apply (simp add: gen_isCap_simps)
            apply (clarsimp simp: returnOk_def neq_Nil_conv)
           \<comment> \<open>ReplyCap\<close>
           apply (simp add: gen_isCap_simps Let_def returnOk_def)
          \<comment> \<open>CNodeCap\<close>
          apply (rename_tac word nat list)
          apply (simp add: gen_isCap_simps Let_def CanModify_def
        ...
        apply (clarsimp simp: gen_isCap_simps o_def)
        apply (rule corres_guard_imp)
     ...
     apply (simp add: gen_isCap_simps o_def)
  ...
  apply (clarsimp simp add: gen_isCap_simps Let_def o_def)
...
arch_requalify_facts
  arch_prepare_set_domain_valid_tcbs arch_prepare_set_domain_in_correct_ready_q
  arch_prepare_set_domain_ready_qs_distinct arch_prepare_set_domain_ep_queues_blocked
  arch_prepare_set_domain_ntfn_queues_blocked arch_prepare_set_domain_ready_queues_runnable
...
lemma update_valid_tcb2[simp]:
  ...
  by (auto simp: valid_tcb_def valid_tcb_state_def valid_bound_obj_def RISCV64.valid_arch_tcb_def (* FIXME rt arch-split *)
...
crunch restart, bindNotification, performTransfer, setFlags, doReplyTransfer, sendIPC,
...
crunch invokeTCB, performIRQControl, invokeIRQHandler
  for typ_at'[wp]: "\<lambda>s. P (typ_at' T p s)"
  and sc_at'_n[wp]: "\<lambda>s. P (sc_at'_n n p s)"
  (simp: crunch_simps
...
crunch decodeSchedContextInvocation, decodeSchedControlInvocation
  for inv[wp]: P
  (wp: crunch_wps simp: crunch_simps)
...
crunch decodeInvocation
  for inv[wp]: P
  (simp: crunch_simps wp: crunch_wps mapME_x_inv_wp)
...
lemma decode_inv_wf'[wp]:
  ...
  apply (case_tac cap,
         simp_all add: decodeInvocation_def Let_def gen_isCap_simps uncurry_def split_def
            split del: if_split
                 cong: if_cong)
             apply ((rule hoare_pre,
...
lemma doReplyTransfer_invs'[wp]:
  "doReplyTransfer sender replyPtr grant \<lbrace>invs'\<rbrace>"
  ...
  apply (wpsimp wp: handleTimeout_invs' postpone_invs' threadGet_wp hoare_vcg_all_lift
             ...
             apply (rename_tac receiver state rv scOpt rv' faultOpt tcb)
             apply (rule_tac Q'="\<lambda>_. invs' and (st_tcb_at' (\<lambda>st. st = Inactive) receiver)"
                          in hoare_post_imp)
            ...
            apply wpsimp
                ...
                apply (rule_tac Q'="\<lambda>_ s. invs' s \<and> st_tcb_at' (Not \<circ> is_BlockedOnReply) receiver s
                                          \<and> st_tcb_at' (Not \<circ> inIPCQueueThreadState) receiver s"
                             in hoare_post_imp)
                 apply (clarsimp simp: opt_pred_def opt_map_red gen_obj_at_simps st_tcb_at'_def
  ...
  apply (fastforce elim!: pred_tcb'_weakenE simp: isReply_def split: thread_state.splits)
  done
...
lemma threadSet_tcbDomain_update_invs':
  ...
  apply (simp add: tcb_cte_cases_def tcb_cte_cases_neqs)+
   apply (wp valid_irq_node_lift valid_irq_handlers_lift'' threadSet_ctes_ofT
             threadSet_valid_dom_schedule' threadSet_ifunsafe'T
...
lemma schedContextBindNtfn_invs':
  "schedContextBindNtfn scPtr ntfnPtr \<lbrace>invs'\<rbrace>"
  apply (clarsimp simp: schedContextBindNtfn_def updateSchedContext_def updateNotification_def)
...
lemma contextYieldToUpdateQueues_invs'_helper:
  "\<lbrace>\<lambda>s. invs' s \<and> tcb_at' ctPtr s\<rbrace>
  ...
  apply (wp threadSet_valid_objs' threadSet_mdb' threadSet_cap_to
         ...
         | clarsimp simp: tcb_cte_cases_def tcb_cte_cases_neqs cteCaps_of_def)+
  apply normalise_obj_at'
  apply (frule (1) sc_ko_at_valid_objs_valid_sc')
  by (fastforce simp: valid_tcb'_def tcb_cte_cases_def tcb_cte_cases_neqs comp_def
...
crunch schedContextResume, schedContextCompleteYieldTo
  for ct_in_state'[wp]: "ct_in_state' P"
  (wp: crunch_wps threadSet_st_tcb_at2 setConsumed_pred_tcb_at'
   simp: ct_in_state'_def crunch_simps bitmap_fun_defs ignore: setConsumed)

lemma schedContextYiedTo_invs':
  "\<lbrace>invs' and ct_active'\<rbrace> schedContextYieldTo scPtr \<lbrace>\<lambda>_. invs'\<rbrace>"
  unfolding schedContextYieldTo_def returnConsumed_def
  by (wpsimp wp: contextYieldToUpdateQueues_invs' hoare_drop_imps)+
...
lemma invokeSchedContext_invs':
  ...
  apply (cases iv; clarsimp)
     ...
     apply (case_tac cap; clarsimp)
      ...
      apply (clarsimp simp: pred_tcb_at'_def gen_obj_at_simps)
...
lemma invokeSchedControlConfigureFlags_invs':
  ...
  apply (rule_tac Q'="\<lambda>_ s. ?pre s \<and> sc_at' sc_ptr s \<and> ex_nonz_cap_to' sc_ptr s"
   ...
   apply (wpsimp wp: hoare_vcg_if_lift refillNew_invs' refillUpdate_invs' commitTime_invs'
          ...
          | wps)+
  by (fastforce simp: st_tcb_at'_def valid_refills_number'_def)
...
defs sch_act_rct'_asrt_def:
  "sch_act_rct'_asrt \<equiv> \<lambda>s. ksSchedulerAction s = ResumeCurrentThread"

declare sch_act_rct'_asrt_def[simp]

defs schedulable'_asrt_def:
  "schedulable'_asrt \<equiv> schedulable'"

declare schedulable'_asrt_def[simp]
...
lemma get_mrs_length_rv[wp]:
  ...
  supply upt.simps[simp del]
  apply (wpsimp wp: mapM_length)
  using len_msg_registes_le_max_length
  apply clarsimp
  done

crunch threadSet, setThreadState
  for valid_duplicates'[wp]: "\<lambda>s. vs_valid_duplicates' (ksPSpace s)"
  (wp: setObject_ksInterrupt updateObject_default_inv cong: if_cong)
...
lemma handleInvocation_corres:
  ...
  supply opt_mapE[elim!]
  apply add_cur_tcb'
  apply (rule_tac Q'=ct_not_inQ in corres_cross_add_guard)
   apply (fastforce intro!: ct_not_inQ_cross)
  ...
  apply (rule corres_stateAssertE_add_assertion[rotated], solves simp)+
  apply (rule corres_stateAssertE_add_assertion[rotated])
   apply (force dest!: state_relation_sched_act_relation
                 simp: sched_act_relation_def schact_is_rct_def)
  apply (rule corres_split_forwards'[OF _ gets_sp getCurThread_sp])
   apply corres
  apply (rule corres_stateAssertE_add_assertion[rotated])
   apply (frule curthread_relation)
   apply (force intro!: schedulable_schedulable'_eq[THEN iffD1]
                  simp: schedulable_def2 ct_in_state_def runnable_eq_active)
  apply (clarsimp, rename_tac thread)
  apply (rule stronger_corres_guard_imp)
    apply (rule corres_split [OF getMessageInfo_corres])
      apply (rule syscall_corres)
            ...
            apply (rule corres_splitEE)
              ...
              apply (rule corres_split [OF getThreadState_corres])
                ...
                apply (rule corres_split)
                   apply (rule corres_when [OF refl replyFromKernel_corres[OF refl refl]])
          ...
          apply ((wpsimp wp: sts_sym_refs' setThreadState_nonqueued_state_update setThreadState_st_tcb
                             setThreadState_rct sts_bound_sc_tcb_at'
                  | wps)+)[1]
         apply clarsimp
  ...
  apply (clarsimp simp: schedulable'_def cong: conj_cong)
  apply (intro conjI impI allI)
      apply fastforce
     apply fastforce
    apply (clarsimp simp: ct_in_state_def)
    apply (frule (1) st_tcb_at_coerce_concrete)
      apply fastforce
     apply fastforce
    subgoal by (fastforce elim!: pred_tcb'_weakenE)
   apply (clarsimp simp: opt_pred_def opt_map_red pred_tcb_at'_def obj_at'_def)
   apply (rename_tac tcb scPtr, case_tac "tcbState tcb"; clarsimp)
  apply (force simp: opt_pred_def cur_tcb'_def opt_map_def split: option.splits)
  done
...
lemma handleInvocation_invs'[wp]:
  ...
  apply (wp syscall_valid' setThreadState_nonqueued_state_update rfk_invs'
        ...
        apply (wpsimp wp: sts_invs_minor' sts_sym_refs' setThreadState_st_tcb setThreadState_rct
                    ...
                    simp: stateAssertE_def
               | wps)+
  by (fastforce simp: gen_obj_at_simps pred_tcb_at'_def schedulable'_def opt_pred_def opt_map_def
                      runnable_eq_active' cur_tcb'_def)
...
lemma getCapReg_corres:
  ...
  apply (simp add: get_cap_reg_def getCapReg_def capRegister_cap_register)
  apply (rule corres_guard_imp)
...
lemma handleSend_corres:
  ...
  apply (rule_tac Q'=ct_active' in corres_cross_add_guard, fastforce intro!: ct_active_cross)
  apply add_cur_tcb'
...
lemma hs_invs'[wp]:
  "\<lbrace>invs' and (\<lambda>s. schedulable' (ksCurThread s) s) and
...
lemma lookupReply_corres:
  ...
  apply (rule_tac Q'=cur_tcb' in corres_cross_add_guard, fastforce intro!: cur_tcb_cross)
...
crunch lookupReply
  for inv[wp]: "P"
  (simp: crunch_simps wp: crunch_wps RISCV64.getRegister_inv) (* FIXME rt arch-split *)
...
lemma lookupReply_valid [wp]:
  ...
  unfolding lookupReply_def getCapReg_def stateAssertE_def
  apply (wpsimp wp: get_cap_wp hoare_vcg_imp_liftE_R)
       apply (rule hoare_FalseE_R)
      apply wpsimp+
  done
...
lemma handleRecv_isBlocking_corres':
  "corres dc
     (einvs and ct_active and current_time_bounded
  ...
  supply if_split[split del]
  apply (clarsimp simp: handle_recv_def handleRecv_def Let_def sch_act_sane_asrt_def)
  apply add_cur_tcb'
  ...
  apply (rule corres_stateAssert_ignore)
   apply (fastforce intro: weak_sch_act_wf_cross)
  apply (rule corres_stateAssert_ignore)
   apply (fastforce intro: scheduler_act_sane_cross)
  apply (rule corres_split_forwards'[OF _ gets_sp getCurThread_sp])
   apply corres
  ...
  apply (simp add: capRegister_cap_register liftM_bind
             cong: if_cong cap.case_cong capability.case_cong)
  apply (rule corres_guard_imp)
      ...
      apply (rule corres_split_catch)
         apply (rule corres_splitEE[OF corres_cap_fault[OF lookupCap_corres]])
           apply (rule_tac P="?pre1 and tcb_at thread
                              ...
                              and valid_cap rv"
                       and P'="?pre2 and tcb_at' thread and valid_cap' epCap" in corres_inst)
           apply (case_tac "is_ep_cap rv"; clarsimp simp: is_cap_simps)
               ...
               apply (rule corres_splitEE[OF lookupReply_corres])
                 ...
                 apply (rule corres_guard_imp)
                   apply (rule receiveIPC_corres)
                    ...
                    apply simp+
                apply (wpsimp wp: RISCV64.lookup_reply_valid_list)+ (* FIXME rt arch-split *)
              apply (clarsimp simp: ct_in_state_def invs_def valid_state_def valid_pspace_def)
             apply (simp add: invs'_def valid_pspace'_def)
            apply (clarsimp simp: lookup_failure_map_def split: if_splits)
            apply (rule corres_guard_imp)
              apply (rule receiveIPC_corres)
                 apply ((clarsimp simp: cap_relation_def ct_in_state_def)+)[6]
           apply (case_tac "is_ntfn_cap rv"; clarsimp simp: is_cap_simps)
            apply (rename_tac rights)
            apply (simp add: bool.case_eq_if if_swap[where P="AllowRead \<in> x" for x, symmetric])
            apply (case_tac "AllowRead \<in> rights")
             ...
             apply (rule stronger_corres_guard_imp)
              ...
              apply (subst runnable_eq_active)+
            ...
            apply (clarsimp simp: lookup_failure_map_def)
           (* neither endpoint nor notification cap, so clear up throwError case *)
           apply (case_tac rv; clarsimp simp: lookup_failure_map_def)
          apply wpsimp+
        apply (rule handleFault_corres, clarsimp)
       apply (wpsimp wp: get_sk_obj_ref_wp RISCV64.lookup_reply_valid_list)+ (* FIXME rt arch-split *)
   ...
   apply (clarsimp simp: invs_def cur_tcb_def valid_state_def valid_pspace_def ct_in_state_def
                  ...
                  dest!: get_tcb_SomeD)
   apply (fastforce simp: runnable_eq_active elim: objs_valid_tcb_ctable)
  apply (clarsimp simp: invs'_def cur_tcb'_def valid_pspace'_def sch_act_sane_def ct_in_state'_def)
  done

lemma handleRecv_isBlocking_corres:
  "corres dc
     (einvs and ct_active and scheduler_act_sane and current_time_bounded
...
lemma handleRecv_invs'[wp]:
  "handleRecv isBlocking canReply \<lbrace>invs'\<rbrace>"
  supply if_split[split del]
  apply (simp add: handleRecv_def)
  apply (intro bind_wp[OF _ stateAssert_sp] bind_wp[OF _ getCurThread_sp])
  ...
  apply (rule_tac P=P and
                  Q'="\<lambda>rv. P and (\<lambda>s. \<forall>r\<in>zobj_refs' rv. ex_nonz_cap_to' r s)
                           and (\<lambda>s. st_tcb_at' runnable' (ksCurThread s) s)"
         for P in bindE_wp_fwd)
   apply wpsimp
   apply (fastforce simp: runnable_eq_active')
  apply (rename_tac epCap)
  apply (case_tac epCap; clarsimp; (wpsimp; fail)?)
...
lemma endTimeslice_corres: (* called when ct_schedulable *)
  ...
  apply (rule corres_guard_imp)
   ...
   apply (intro conjI impI allI; fastforce?)
      ...
      apply (fastforce simp: runnable_eq_active get_tcb_timeout_handler_ptr_def cte_wp_at_cases)
...
lemma chargeBudget_corres:
  ...
  apply (rule_tac Q'="\<lambda>s. is_active_sc' (ksCurSc s) s" in corres_cross_add_guard)
   apply (fastforce intro: valid_objs_valid_sched_context_size
                     simp: sc_at_pred_n_def obj_at_def is_sc_obj_def state_relation_def vs_all_heap_simps
                           is_active_sc_rewrite[symmetric]
  ...
  apply (rule_tac Q="\<lambda>_. ?pred"
    ...
    apply (rule corres_guard_imp)
      apply (rule corres_split[OF setConsumedTime_corres], simp)
            ...
            apply (rule corres_split[OF endTimeslice_corres])
             ...
             apply (rule hoare_strengthen_post
                              [where Q'="\<lambda>_. invs and active_scs_valid and valid_sched_action
                                             ...
                                             and sorted_ipc_queues", rotated])
              apply clarsimp
              apply (frule invs_sym_refs)
              apply (fastforce simp: invs_def valid_state_def valid_pspace_def valid_objs_valid_tcbs
                                     valid_sched_action_def)
             apply (wpsimp wp: end_timeslice_invs end_timeslice_valid_sched_action
                               end_timeslice_sorted_ipc_queues end_timeslice_valid_ready_qs)
       ...
       apply wpsimp
      apply (rule hoare_strengthen_post
                     [where Q'="\<lambda>_. invs'", rotated])
       apply (frule invs_valid_objs')
       apply (frule valid_objs'_valid_tcbs')
       apply (clarsimp simp: ct_in_state'_def st_tcb_at'_def schedulable'_def opt_pred_def opt_map_def
                      split: option.splits)
       apply (erule aligned'_distinct'_obj_at'_propI, fastforce+)[1]
   ...
   apply wpsimp
     ...
     apply ((wpsimp wp: refill_budget_check_released_ipc_queues
            | strengthen valid_sc_strengthen[OF invs_valid_objs'])+)[1]
   ...
   apply (prop_tac "sc_scheduler_act_not (cur_sc s) s")
    ...
    apply (fastforce simp: schedulable_def3 vs_all_heap_simps)
  ...
  apply (wpsimp wp: updateSchedContext_invs' isRoundRobin_wp
         | strengthen valid_sc_strengthen[OF invs_valid_objs'])+
...
lemma checkBudget_corres: (* called when ct_schedulable or in checkBudgetRestart *)
  ...
  apply (clarsimp simp: check_budget_def checkBudget_def)
  apply (rule_tac Q'="\<lambda>s'. is_active_sc' (ksCurSc s') s'" in corres_cross_add_guard)
   apply (fastforce intro: is_active_sc'_cross simp: is_active_sc_rewrite state_relation_def)
  apply (rule corres_stateAssert_ignore)
...
lemma maybeHandleInterrupt_corres:
  ...
  apply (corres corres: corres_machine_op handleInterrupt_corres[@lift_corres_args]
    ...
    apply (rule_tac Q'="\<lambda>rv s. (\<forall>irq. rv = Some irq \<longrightarrow> intStateIRQTable (ksInterruptState s) irq \<noteq> IRQInactive)
                             \<and> invs' s"
...
lemma hvmf_invs_etc:
  "\<lbrace>invs' and sch_act_not t and st_tcb_at' simple' t and
    ex_nonz_cap_to' t\<rbrace>
    handleVMFault t f
   \<lbrace>\<lambda>_ _. True\<rbrace>,
   \<lbrace>\<lambda>_. invs' and sch_act_not t and st_tcb_at' simple' t and ex_nonz_cap_to' t\<rbrace>"
...
lemma checkBudgetRestart_invs'[wp]:
  ...
  apply (clarsimp simp: checkBudgetRestart_def)
  apply (intro bind_wp[OF _ stateAssert_inv])
  apply (rule bind_wp_fwd_skip, wpsimp)
  apply (wpsimp wp: setThreadState_Restart_invs')
  apply (clarsimp simp: pred_tcb_at'_def obj_at'_real_def ko_wp_at'_def is_BlockedOnReply_def)
  apply (rename_tac tcb, case_tac "tcbState tcb"; clarsimp)
  done
...
crunch updateTimeStamp
  for tcbs_of'_ct[wp]: "\<lambda>s. P (tcbs_of' s) (ksCurThread s)"
...
lemma ksCurThread_schedulable'_ct_active':
  "\<lbrakk>schedulable' (ksCurThread s) s; invs' s\<rbrakk> \<Longrightarrow> ct_active' s"
...
lemma non_kernel_IRQs_strg:
  "invs' s \<and> irq \<notin> Some ` non_kernel_IRQs \<Longrightarrow>
    (\<forall>x. irq = Some x \<longrightarrow> invs' s \<and> (x \<in> non_kernel_IRQs \<longrightarrow> P))"
  by auto
...
lemma he_invs'[wp]:
  "\<lbrace>invs'
    and (\<lambda>s. vs_valid_duplicates' (ksPSpace s))
    and (\<lambda>s. event \<noteq> Interrupt \<longrightarrow> schedulable' (ksCurThread s) s)
 ...
 apply (case_tac event, simp_all add: handleEvent_def)
       ...
       apply (case_tac syscall,
              (wpsimp wp: checkBudgetRestart_true checkBudgetRestart_false hoare_vcg_if_lift2
               ...
               | clarsimp dest!: ksCurThread_schedulable'_ct_active' intro!: pred_tcb'_weakenE
                          simp:  cur_tcb'_def runnable_eq_active')+)
                by (wp hoare_vcg_if_lift2 checkBudgetRestart_true
                       checkBudgetRestart_false hoare_vcg_all_lift
                       updateTimeStamp_ct_in_state'[simplified ct_in_state'_def]
                       updateTimeStamp_cur_tcb'[unfolded cur_tcb'_def]
                    | clarsimp dest!: ksCurThread_schedulable'_ct_active'
                                simp: ct_in_state'_def runnable_eq_active'
                    | wpc)+
...
lemma handleInv_handleRecv_corres:
  "corres (dc \<oplus> dc)
     ...
     (invs' and ct_running' and (\<lambda>s. ksSchedulerAction s = ResumeCurrentThread) and
...
abbreviation (input)
  "c_pre \<equiv> (invs' and ct_running' and (\<lambda>s. ksSchedulerAction s = ResumeCurrentThread) and
            (\<lambda>s. vs_valid_duplicates' (ksPSpace s)))"
...
proof -
  ...
  show ?thesis
    ...
    apply (rule corres_underlying_split)
     ...
     apply wpsimp
     apply (fastforce simp: ct_in_state_def intro: runnable_nonz_cap_to elim: st_tcb_weakenE)
    apply wp
