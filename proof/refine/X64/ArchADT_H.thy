  ...
  "absHeapArch h pt_types a \<equiv> \<lambda>ako.
     case ako of
       KOASIDPool (X64_H.ASIDPool ap) \<Rightarrow>
         Some (X64_A.ASIDPool (\<lambda>w. map_option abs_asid_entry (ap (ucast w))))
     | KOPTE _ \<Rightarrow>
...
definition mdata_map' ::
  "(asid \<times> vspace_ref) option \<Rightarrow> (Machine_A.X64_A.asid \<times> vspace_ref) option" where
...
primrec ArchFaultMap :: "Fault_H.arch_fault \<Rightarrow> ExceptionTypes_A.arch_fault" where
  "ArchFaultMap (X64_H.VMFault p m) = X64_A.VMFault p m"
| "ArchFaultMap (X64_H.VCPUFault w) = X64_A.VCPUFault w"
| "ArchFaultMap (X64_H.VGICMaintenance m) = X64_A.VGICMaintenance m"
| "ArchFaultMap (X64_H.VPPIEvent irq) = X64_A.VPPIEvent irq"
