...
lemma
  ...
  apply (rule conjI)
   ...
   apply (case_tac arch_kernel_object)
      ...
      apply (simp add: pt_bits_def bit_simps)
      ...
      apply (erule_tac x=y in allE)
      apply (simp add: objBitsKO_def archObjSize_def
  ...
  apply (case_tac arch_kernel_object)
      apply (clarsimp split: asidpool.splits)
      apply (simp add: other_aobj_relation_def asid_pool_relation_def o_def inv_def)
     apply simp
...
lemma TCB_implies_KOTCB:
  ...
  apply (clarsimp simp add: pspace_relation_def pspace_dom_def dom_def UNION_eq Collect_eq)
  apply (erule_tac x=a in allE)+
  apply (clarsimp simp: other_obj_relation_def tcb_relation_cut_def
