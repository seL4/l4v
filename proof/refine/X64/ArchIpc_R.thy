...
lemma maskCapRights_vs_cap_ref'[simp]:
  ...
  apply (case_tac arch_capability;
         simp add: X64_H.maskCapRights_def isCap_simps Let_def)
  done
...
lemma sanitiseRegister_sanitise_register[Ipc_R_assms]:
  ...
  by (rule ext)+
     (clarsimp simp: sanitiseRegister_def sanitise_register_def
                     sanitiseOrFlags_def sanitise_or_flags_def
...
lemma isIRQControlCap_mask[simp]:
  ...
  by (case_tac c;
      clarsimp simp: isCap_simps maskCapRights_def X64_H.maskCapRights_def Let_def)
