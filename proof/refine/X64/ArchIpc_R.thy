...
lemma unbindNotification_valid_objs'_strengthen:
  ...
  by (simp_all add: valid_tcb'_def valid_ntfn'_def valid_bound_tcb'_def valid_tcb_state'_def
                    tcb_cte_cases_def tcb_cte_cases_neqs split: ntfn.splits)
       ...
       apply (case_tac rv)
         ...
         apply (rule corres_guard_imp)
           apply (rule corres_split[OF setThreadState_corres])
             ...
             apply (simp add: ep_relation_def)
            apply wp+
          apply (clarsimp simp: st_tcb_at_tcb_at valid_tcb_state_def invs_psp_aligned invs_distinct)
         apply clarsimp
...
lemma setThreadState_nonqueued_state_update:
  ...
  apply (simp add: invs'_def valid_state'_def)
  apply (rule hoare_pre, wp valid_irq_node_lift setThreadState_ct_not_inQ valid_dom_schedule'_lift)
  ...
  apply (case_tac "ntfnObj nTFN", simp_all)
    ...
    apply (wp hoare_convert_imp [OF asUser_nosch]
     ...
     apply (wp valid_irq_node_lift sts_valid_objs' setThreadState_ct_not_inQ
               set_ntfn_valid_objs' cur_tcb_lift sts_st_tcb' valid_dom_schedule'_lift
        ...
        apply (rule corres_guard_imp)
          apply (case_tac isBlocking; simp)
           apply (rule corres_split[OF setThreadState_corres])
             ...
             apply (simp add: ntfn_relation_def)
            apply wp+
          apply (rule corres_guard_imp, rule doNBRecvFailedTransfer_corres, fastforce+)
       \<comment> \<open>WaitingNtfn\<close>
       apply (simp add: ntfn_relation_def)
       apply (rule corres_guard_imp)
         apply (case_tac isBlocking; simp)
          apply (rule corres_split[OF setThreadState_corres])
           ...
           apply wp+
         apply (rule corres_guard_imp)
           apply (rule doNBRecvFailedTransfer_corres, fastforce+)
      \<comment> \<open>ActiveNtfn\<close>
      apply (simp add: ntfn_relation_def)
      apply (rule corres_guard_imp)
        apply (simp add: badgeRegister_def badge_register_def)
      ...
      apply (rule corres_splitEE)
        ...
        apply (case_tac handler_cap,
         ...
         apply (rule corres_guard_imp)
          ...
          apply (fastforce simp: valid_cap_def)
         apply (fastforce simp: valid_cap'_def inQ_def)
        apply (fastforce simp: lookup_failure_map_def)
       apply wp+
...
lemma completeSignal_invs:
  ...
  apply (rule hoare_pre)
   apply (wp set_ntfn_minor_invs' | wpc | simp)+
    ...
    apply (clarsimp simp: obj_at'_def state_refs_of'_def typ_at'_def ko_wp_at'_def projectKOs live'_def
                    split: option.splits)
...
lemma rai_invs'[wp]:
  ...
  apply (case_tac "ntfnObj ep")
    ...
    apply (simp add: invs'_def valid_state'_def)
    apply (rule hoare_pre)
     apply (wp valid_irq_node_lift sts_sch_act' typ_at_lifts valid_dom_schedule'_lift
               setThreadState_ct_not_inQ
               asUser_urz
  ...
  apply (rule hoare_pre)
   apply (wp hoare_vcg_const_Ball_lift valid_irq_node_lift sts_sch_act'
             setThreadState_ct_not_inQ typ_at_lifts valid_dom_schedule'_lift
             asUser_urz
        | simp add: valid_ntfn'_def doNBRecvFailedTransfer_def live'_def | wpc)+
  apply (clarsimp simp: valid_tcb_state'_def)
  ...
  apply (case_tac epa)
    ...
    apply (rule hoare_pre)
     ...
     apply (wp valid_irq_node_lift
               ...
               possibleSwitchTo_sch_act_not setThreadState_ct_not_inQ valid_dom_schedule'_lift
               possibleSwitchTo_ct_not_inQ hoare_vcg_all_lift
               hoare_convert_imp [OF doIPCTransfer_sch_act doIPCTransfer_ct']
               hoare_convert_imp [OF setEndpoint_nosch setEndpoint_ksCurThread]
  ...
  apply (rule corres_guard_imp)
    apply (subst return_bind [symmetric],
      ...
      apply (rule corres_split_catch)
       ...
       apply wp+
      apply (rule hoare_strengthen_postE, rule sfi_invs_plus', simp_all)[1]
      apply clarsimp
     apply wp+
