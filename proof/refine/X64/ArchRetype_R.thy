    ...
    proof(cases a)
      case Untyped with Some cover ct show ?thesis
        ...
        apply (simp_all add: X64_H.toAPIType_def fromIntegral_def
                             toInteger_nat fromInteger_nat APIType_capBits_gen_def
                      split: X64_H.object_type.splits)
        apply wp
        apply (intro ballI)
...
crunch copyGlobalMappings
  ...
  (ignore: storePML4E wp: crunch_wps valid_bitmaps_lift)
        ...
        apply (case_tac apiobject_type; simp split del: if_split)
           ...
           apply (wp mapM_x_wp' createObjects_state_hyp_refs_of'' threadSet_state_hyp_refs_of'
                  | simp add: not_0 pspace_no_overlap'_def objBitsKO_def APIType_capBits_def
                              ...
                              createObjects_def curDomain_def mult_2_right
                  | intro conjI impI)+
  done
...
proof (rule hoare_gen_asm)
  ...
  show
    ...
    apply (wp mapM_x_wp' setObject_ko_wp_at)
         apply (simp add: objBits_simps cong: if_cong split del: if_split)+
         ...
         apply (case_tac apiobject_type, simp_all split del: if_split)[1]
            ...
            by (wp mapM_x_threadSet_createNewCaps_futz
                 ...
                 | simp add: makeObjectKO_def objBitsKO_def archObjSize_def makeObjectKO_gen_def
                             APIType_capBits_def APIType_capBits_gen_def bit_simps
...
lemma createNewCaps_pspace_domain_valid[Retype_R_assms, wp]:
  ...
  apply (rule hoare_pre)
   apply (wp createObjects_pspace_domain_valid[where sz=sz] copyGlobalMappings_ko_wp_at
            mapM_x_wp'
...
lemma createNewCaps_idle'[Retype_R_2_assms, wp]:
  ...
  by (wpsimp wp: createObjects_idle'[where sz=sz] mapM_x_wp' split_del: if_split
      ...
      | simp add: projectKO_opt_tcb projectKO_opt_cte mult_2 makeObject_cte makeObject_tcb
                  ...
                  bit_simps objBits_def createObjects_def tcb_cte_cases_neqs)+

(* weaker preconditions to assist createNewCaps_valid_arch_state *)
  ...
  apply (cases ty, simp_all split del: if_split)
          ...
          apply (case_tac apiobject_type, simp_all split del: if_split)
             ...
             apply (simp add: createObjects_def makeObjectKO_def APIType_capBits_def
                              ...
                              curDomain_def objBits_if_dev bit_simps
                       split del: if_split
...
lemma createObjects_pml4_at:
  ...
  apply (rule hoare_strengthen_post, rule hoare_weaken_pre)
    apply (rule hoare_vcg_conj_lift)
     apply (rule createObjects_ko_at[where sz=sz and val="makeObject :: pml4e"]; fastforce)
    apply (rule createObjects_aligned)
      ...
      apply (rule le_less_trans[OF range_cover.range_cover_n_le(2) power_strict_increasing])
       ...
       apply arith+
    apply (simp add: objBits_simps range_cover_def word_bits_def)
   apply simp
  apply (clarsimp; rename_tac p)
  apply (drule (1) bspec)+
  apply (clarsimp simp: page_map_l4_at'_def objBits_simps bit_simps)
