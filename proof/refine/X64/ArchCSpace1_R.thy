...
lemma same_arch_region_as_relation:
  ...
  by (cases c; cases c')
     (auto simp: X64_H.sameRegionAs_def global.sameRegionAs_def Let_def isCap_simps mask_def
...
lemma capASID_update[simp]:
  ...
  apply (case_tac arch_capability,
         simp_all add: global.updateCapData_def X64_H.updateCapData_def isCap_simps Let_def)
  done
...
lemma updateCapData_Master:
  ...
  apply (cases cap, simp_all add: global.updateCapData_def isCap_simps Let_def
                           split: if_split_asm)
  apply (rename_tac arch_capability)
...
lemma updateCapData_Reply:
  ...
  apply (cases "updateCapData P x c = NullCap")
   ...
   apply (simp add: global.updateCapData_def gen_isCap_simps Let_def)
...
lemma derived_sameRegionAs:
  ...
  apply (clarsimp simp: isCap_simps valid_cap'_def
                        is_aligned_no_overflow capRange_def valid_arch_cap'_def
                 split: capability.splits arch_capability.splits option.splits)
  done
...
lemma parentOf_preserve_oneway[CSpace1_R_2_assms]:
  ...
  apply (drule_tac x=x in misc)
   apply assumption
  apply ((simp only: isMDBParentOf_def split_def
...
definition
  "safe_parent_for_arch' m cap parent \<equiv>
     isArchIOPortCap cap \<and> isIOPortControlCap' parent \<and> (\<forall>p n'. m p \<noteq> Some (CTE cap n'))"
...
lemma (in Arch_mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps X64.isMDBParentOf_CTE is_derived'_def
...
lemma src_no_mdb_parent:
  ...
  apply (erule sameRegionAsE)
     ...
     subgoal by (cases src_cap; auto simp: capMasterCap_def arch_capMasterCap_def Retype_H.isCapRevocable_def X64_H.isCapRevocable_def
...
lemma isIOPortControlCap'[simp]:
  ...
  by (simp add: capMasterCap_def arch_capMasterCap_def isIOPortControlCap'_def
           del: isNull isDomain
           split: capability.splits arch_capability.splits)
