...
definition ioport_state_independent_H :: "(kernel_state \<Rightarrow> bool) \<Rightarrow> bool" where
  "ioport_state_independent_H P \<equiv>
     \<forall>s f. P s \<longrightarrow> P (s\<lparr>ksArchState := ksArchState s\<lparr>
                                        x64KSAllocatedIOPorts :=
                                          f (x64KSAllocatedIOPorts (ksArchState s))\<rparr>\<rparr>)"
...
lemma isFinal_notUntyped_capRange_disjoint[CNodeInv_R_assms]:
  ...
  apply (elim disjE isCapDs[elim_format])
   apply (clarsimp simp: valid_cap'_def valid_arch_cap'_def vspace_table_at'_defs bit_simps
...
lemma setIOPortMask_no_cte_prop[wp]:
  "\<lbrace>no_cte_prop P and K (arch_finalise_prop_stuff P)\<rbrace> setIOPortMask f l b \<lbrace>\<lambda>_. no_cte_prop P\<rbrace>"
  supply fun_upd_apply[simp del]
  apply (clarsimp simp: setIOPortMask_def)
  apply wpsimp
  apply (clarsimp simp: no_cte_prop_def arch_finalise_prop_stuff_def foldl_map foldl_fun_upd_value)
  apply auto
  done
...
lemma final_IOPort_no_copy:
  "\<lbrakk> isFinal (ArchObjectCap (IOPortCap f l)) sl (cteCaps_of s); sl \<noteq> sl' \<rbrakk>
...
lemmas storePDPTE_cteCaps_of[wp] = cteCaps_of_ctes_of_lift [OF storePDPTE_ctes]
lemmas storePML4E_cteCaps_of[wp] = cteCaps_of_ctes_of_lift [OF storePML4E_ctes]
...
lemma arch_recycleCap_improve_cases:
   "\<lbrakk> \<not> isPageCap cap; \<not> isPageTableCap cap; \<not> isPageDirectoryCap cap; \<not> isPDPointerTableCap cap;
      ...
      \<not> isASIDControlCap cap \<rbrakk>
...
lemmas arch_assms =
  X64.sameRegionAs_eq_child
  X64.sameRegionAs_eq_parent
  X64.sameRegion_ep
  X64.sameRegion_ntfn
