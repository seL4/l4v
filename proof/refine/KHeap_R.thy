...
declare fromPPtr_def[simp] (* FIXME arch-split: move this somewhere high up *)
...
(* FIXME arch-split: true on all arches, move *)
lemma zero_less_tcbBlockSizeBits[simp]:
  "0 < tcbBlockSizeBits"
  unfolding tcbBlockSizeBits_def by simp

(* same derivation on all arches *) (* FIXME arch-split: move *)
lemma (in Arch) not_is_other_obj_relation_type:
  "is_other_obj_relation_type AEndpoint"
  "is_other_obj_relation_type ANTFN"
  unfolding is_other_obj_relation_type_def
  by auto

requalify_facts Arch.not_is_other_obj_relation_type
lemmas [simp] = not_is_other_obj_relation_type
...
lemma no_fail_getObject_tcb[wp]:
  ...
  apply (rule ps_clear_lookupAround2, assumption+)
    apply simp+
  apply (fastforce split: option.split_asm simp: gen_objBits_simps)
  done
...
lemma getObject_tcb_at':
  ...
  by (fastforce simp: valid_def getObject_def in_monad
...
lemma setObject_ep_pre:
  ...
  apply (clarsimp simp: valid_def setObject_def in_monad
                        split_def updateObject_default_def in_magnitude_check gen_objBits_simps)
  apply (drule spec, drule mp, erule conjI)
   apply (simp add: obj_at'_def gen_objBits_simps)
...
lemma getObject_ep_inv:
  "\<lbrace>P\<rbrace> (getObject addr :: endpoint kernel) \<lbrace>\<lambda>rv. P\<rbrace>"
  by (rule getObject_inv, simp add: loadObject_default_inv)
...
declare diff_neg_mask[simp del] (* FIXME arch-split: do we want to keep this? *)
...
lemma map_to_ctes_upd_tcb:
  ...
  apply (subgoal_tac "p && ~~ (mask tcbBlockSizeBits) = p")
   ...
   apply (clarsimp simp: tcb_cte_cases_def gen_objBits_simps mask_def field_simps
                  split: if_split_asm)
...
lemma no_fail_getEndpoint[wp]:
  ...
  apply (rule no_fail_pre)
   apply wp
  apply (clarsimp simp add: obj_at'_def gen_objBits_simps lookupAround2_known1)
  apply (erule(1) ps_clear_lookupAround2)
    apply simp
   apply (simp add: is_aligned_no_overflow)
  apply (clarsimp split: option.split_asm simp: gen_objBits_simps)
  done
...
lemma getEndpoint_corres:
  ...
  apply (clarsimp simp: loadObject_default_def in_monad in_magnitude_check gen_objBits_simps)
  apply (clarsimp simp add: state_relation_def pspace_relation_def)
  apply (drule bspec)
   apply blast
...
lemma set_ep_state_refs_of'[wp]:
  ...
  by (wp setObject_state_refs_of',
      simp_all add: gen_objBits_simps fun_upd_def[symmetric])
...
lemma setObject_gen_obj_at:
  ...
  using o
  apply (clarsimp simp: setObject_def valid_def in_monad split_def split_paired_Ball
                        R updateObject_default_def project_inject objBits_def[symmetric] n
                        in_magnitude_check [OF _ m])
  apply (erule allE, erule impE)
   apply (fastforce simp: obj_at'_def objBits_def[symmetric] n project_inject)
  apply (auto simp: project_inject objBits_def[symmetric] n in_magnitude_check [OF _ m])
  done

lemma set_ep_state_hyp_refs_of'[wp]:
  "setEndpoint epptr ep \<lbrace>\<lambda>s. P (state_hyp_refs_of' s)\<rbrace>"
  unfolding setEndpoint_def
  apply (rule setObject_gen_obj_at, simp, simp add: gen_objBits_simps, simp)
...
lemma setEndpoint_idle'[wp]:
  ...
  apply (wp setObject_idle')
       apply (simp add: gen_objBits_simps updateObject_default_inv idle_tcb_ps_def)+
  done
...
lemma get_ntfn_sp':
  ...
  by (clarsimp simp: getNotification_def getObject_def loadObject_default_def
                     in_monad valid_def obj_at'_def gen_objBits_simps in_magnitude_check split_def)
...
locale KHeap_R =
  assumes koType_objBitsKO:
    "\<And>k k'. koTypeOf k = koTypeOf k' \<Longrightarrow> objBitsKO k = objBitsKO k'"
  ...
  (* can't quantify over a generic 'a::storable in a locale (setObject_other_corres),
     so assume it for the two objects we require for this theory, and requalify the lemma later *)
...
locale KHeap_R_2 = KHeap_R +
  assumes set_ntfn_global_refs'[wp]:
    "\<And>ptr val. \<lbrace>valid_global_refs'\<rbrace> setNotification ptr val \<lbrace>\<lambda>_. valid_global_refs'\<rbrace>"
  assumes setEndpoint_valid_arch'[wp]:
...
lemma setEndpoint_cte_wp_at'[wp]:
...
lemma setEndpoint_corres:
  ...
  by (fastforce simp: is_ep gen_obj_at_simps partial_inv_def)
