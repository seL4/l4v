...
abbreviation caps_relation :: "'z state \<Rightarrow> kernel_state \<Rightarrow> bool" where
  "caps_relation s s' \<equiv> caps_relation_2 (cnode_sizes_of s) (cnode_contents_of s) (cnode_ctes_of' s')"
...
lemma heap_pspace_relation_KOKernelData:
...
lemma caps_relation_lift_rcorres[rcorres_lift]:
  "\<lbrakk>\<And>s'. no_fail (\<lambda>s. Q s s') f; empty_fail f;
    ...
    \<And>P s. \<lbrace>\<lambda>s'. P (cnode_ctes_of' s') \<and> Q s s'\<rbrace> f' \<lbrace>\<lambda>_ s'. P (cnode_ctes_of' s')\<rbrace>\<rbrakk>
   \<Longrightarrow> rcorres (\<lambda>s s'. caps_relation s s' \<and> Q s s') f  f' (\<lambda>_ _. caps_relation)"
  apply (rule rcorres_lift_conc[where p=cnode_ctes_of'])
   apply (rule rcorres_lift_abs)
    apply (rule rcorres_prop_fwd)
      apply (fastforce intro: no_fail_pre)
     apply fastforce
...
lemma abs_conc_rel_lift_rcorres:
  "\<lbrakk>\<And>s'. no_fail (\<lambda>s. Q s s') f; empty_fail f;
    \<And>P s'. \<lbrace>\<lambda>s. P (p s) \<and> Q s s'\<rbrace> f \<lbrace>\<lambda>_ s. P (p s)\<rbrace>;
    \<And>P s. \<lbrace>\<lambda>s'. P (p' s') \<and> Q s s'\<rbrace> f' \<lbrace>\<lambda>_ s'. P (p' s')\<rbrace>\<rbrakk>
   \<Longrightarrow> rcorres (\<lambda>s s'. R (p s) (p' s') \<and> Q s s') f  f' (\<lambda>_ _ s s'. R (p s) (p' s'))"
  apply (rule rcorres_lift_conc)
   apply (rule rcorres_lift_abs)
    apply (rule rcorres_prop_fwd)
      by (fastforce intro: no_fail_pre hoare_weaken_pre)+
...
lemma idle_sc_relation_lift_rcorres[rcorres_lift]:
  ...
  apply (erule (1) abs_conc_rel_lift_rcorres[where p="\<lambda>_. idle_sc_ptr" and p'=ksIdleSC and R="(=)"])
   apply wpsimp
  apply wpsimp
  done

lemmas cur_thread_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=cur_thread and p'=ksCurThread and R="(=)"]

lemmas idle_thread_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=idle_thread and p'=ksIdleThread and R="(=)"]

lemmas machine_state_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=machine_state and p'=ksMachineState and R="(=)"]

lemmas work_units_completed_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=work_units_completed and p'=ksWorkUnitsCompleted and R="(=)"]
...
lemmas domain_time_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=domain_time and p'=ksDomainTime and R="(=)"]

lemmas consumed_time_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=consumed_time and p'=ksConsumedTime and R="(=)"]

lemmas cur_time_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=cur_time and p'=ksCurTime and R="(=)"]

lemmas cur_sc_relation_lift_rcorres[rcorres_lift] =
  abs_conc_rel_lift_rcorres[where p=cur_sc and p'=ksCurSc and R="(=)"]

lemmas reprogram_timer_relation_lift_rcorres[rcorres_lift] =
