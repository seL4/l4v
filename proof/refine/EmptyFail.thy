...
(* Collect empty_fail lemmas here. naming convention is empty_fail_NAME.
   Unless there is a good reason, they should all be [intro!, wp] *)
...
declare empty_fail_stateAssert[wp] (* FIXME: tag original instead *)
...
lemma empty_fail_getObject:
  ...
  apply (simp add: getObject_def split_def fromPPtr_def)
...
locale EmptyFail_R =
  assumes empty_fail_lookupIPCBuffer[intro!, wp, simp]:
    "\<And>r t. empty_fail (lookupIPCBuffer r t)"
