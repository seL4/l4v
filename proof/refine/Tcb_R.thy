...
crunch setup_reply_master, Tcb_A.restart
...
lemma invokeTCB_WriteRegisters_corres:
  ...
  apply (corres corres: arch_getSanitiseRegisterInfo_corres asUser_corres)
             apply (simp add: zipWithM_mapM)
             apply (rule corres_Id)
               apply (simp add: sanitiseRegister_sanitise_register)+
             apply (wpsimp wp: no_fail_mapM)+
...
proof -
  have Q: "\<And>src src' des des' r r'. \<lbrakk> src = src'; des = des' \<rbrakk> \<Longrightarrow>
    ...
    apply (corres corres: asUser_getRegister_corres asUser_corres)
          ...
          apply (wpsimp simp: invs_distinct invs_psp_aligned)+
    done
  ...
  apply (simp add: invokeTCB_def performTransfer_def)
    apply (rule corres_guard_imp)
      ...
      apply (rule corres_split_eqr[OF asUser_getRestartPC_corres])
       ...
       apply (wpsimp wp: suspend_nonz_cap_to_tcb hoare_weak_lift_imp simp: if_apply_def2
...
lemma readreg_invs':
  ...
  unfolding invokeTCB_def performTransfer_def
  by (simp | wp (* somehow much faster than wpsimp *)
...
lemma threadcontrol_corres_helper4:
  ...
  \<lbrace>invs' and cte_wp_at' (\<lambda>cte. cteCap cte = capability.NullCap) (cte_map (a, tcb_cnode_index 4))
         and valid_cap' ac and (\<lambda>_. capBadge ac = None)\<rbrace>
  ...
  by (case_tac ac;
      clarsimp simp: capBadge_def gen_isCap_simps tcb_cnode_index_def cte_map_def cte_wp_at'_def
                     tcb_cte_cases_neqs cteSizeBits_cte_level_bits[symmetric])
...
lemma threadSet_invs_tcbIPCBuffer_update:
  "\<lbrace>invs' and K (\<forall>tcb. is_aligned (tcbIPCBuffer (tcbIPCBuffer_update f tcb)) msg_align_bits)\<rbrace>
   threadSet (tcbIPCBuffer_update f) t
   \<lbrace>\<lambda>_. invs'\<rbrace>"
  supply tcbIPCBuffer_tcbIPCBuffer_update[simp del]
  by (rule hoare_gen_asm)
     (wp threadSet_invs_trivial; fastforce)
...
lemma (in Tcb_R_2) transferCaps_corres:
  ...
  "corres (dc \<oplus> (=))
    ...
    (invs' and sch_act_simple and case_option \<top> (valid_cap' o fst) e' and
     ...
     K (case_option True (case_option True ((\<lambda>cap. isArchObjectCap cap \<and>
                                                   capBadge cap = None) o fst) o snd) g') and
...
proof -
  ...
  have T2: "corres (dc \<oplus> dc)
     ...
     (invs' and sch_act_simple and tcb_at' a and
       (\<lambda>s. \<forall>cp \<in> (case g' of None \<Rightarrow> {} | Some (x, v) \<Rightarrow> (case v of
        ...
        \<and> (case v of None \<Rightarrow> True | Some (ac, _) \<Rightarrow> isArchObjectCap ac \<and> capBadge ac = None)) )
        ...
        apply (rule corres_split_norE[OF cteDelete_corres])
          ...
          apply (rule corres_split_nor)
           ...
           apply (wp thread_set_tcb_ipc_buffer_cap_cleared_invs
                     ...
                     thread_set_ipc_buffer_no_cap_to_obj_dr_emp thread_set_valid_cap
    ...
    apply (clarsimp simp: cte_map_def tcb_cnode_index_def obj_at'_def gen_objBits_simps)
    apply (erule (2) cte_wp_at_tcbI')
     apply (auto simp: tcb_cte_cases_neqs cteSizeBits_cte_level_bits[symmetric])
    done
  ...
  have V: "getThreadVSpaceRoot a = return (cte_map (a, tcb_cnode_index 1))"
    ...
    apply (simp add: cte_map_def tcb_cnode_index_def to_bl_1 gen_objBits_simps
                     tcb_cte_cases_neqs cteSizeBits_cte_level_bits[symmetric])
    done
  ...
  note bind_wp[wp]
  show ?thesis
    supply word_bl_Rep'[simp del] word_bl_Rep'_mw[simp] (* avoid revealing LENGTH of machine word *)
    ...
    apply (rule corres_guard_imp)
        ...
        apply (rule corres_split_nor[OF S', simplified])
          apply (rule corres_split_norE[OF T [OF x U], simplified])
            apply (rule corres_split_norE[OF T [OF y V], simplified])
              apply (rule corres_split_norE)
               ...
               apply (wpsimp wp: hoare_vcg_const_imp_liftE_R hoare_vcg_const_imp_lift
                                 ...
                                 cap_delete_valid_cap checked_insert_tcb_invs_gen
                                 thread_set_ipc_buffer_no_cap_to_obj_dr_emp thread_set_valid_cap
             ...
             apply (rule_tac Q'="\<lambda>_. ?T2_pre" in hoare_strengthen_postE_R[simplified validE_R_def, rotated])
              apply (clarsimp simp: valid_ipc_buffer_cap_is_nondevice_page_cap)
             apply ((wp stuff checkCap_wp assertDerived_wp_weak cap_insert_objs'
                        checked_insert_tcb_invs_gen
                     | simp add: ran_tcb_cap_cases split_def U V emptyable_def
                     | wpc | strengthen tcb_cap_always_valid_strg)+)[1]
            ...
            apply (rule_tac Q'="\<lambda>_. ?T2_pre' and (\<lambda>s. valid_option_prio p_auth)"
             ...
             apply (case_tac g'; clarsimp simp: gen_isCap_simps ; clarsimp cong: imp_cong)
     ...
     apply (clarsimp simp: tcb_at_cte_at_0 tcb_at_cte_at_1[simplified] tcb_at_st_tcb_at[symmetric]
                           tcb_cap_valid_def invs_valid_objs emptyable_def
                           is_valid_vtable_root_is_cnode_or_valid_arch
                           no_cap_to_obj_with_diff_ref_Null
                           arch_cap_fun_lift_def
                           invs_psp_aligned invs_distinct
                           is_cnode_cap_is_cnode_or_valid_arch
                     cong: conj_cong imp_cong
                    split: option.split_asm)
    apply (clarsimp simp: invs'_def valid_state'_def valid_pspace'_def gen_objBits_simps
                          cte_map_tcb_0 cte_map_tcb_1[simplified] tcb_at_cte_at'
                          cte_at_tcb_at_2p_cteSizeBits
                          gen_isCap_simps domIff valid_tcb'_def tcb_cte_cases_def tcb_cte_cases_neqs
                   split: option.split_asm
 ...
 "tcbinv_relation ti ti' \<Longrightarrow>
  ...
  apply (case_tac ti, simp_all only: tcbinv_relation.simps valid_tcb_invocation_def)
       ...
       apply (rule corres_guard_imp [OF transferCaps_corres], clarsimp+)
        apply (clarsimp split: option.split option.split_asm)
       apply clarsimp
...
lemma decodeWriteRegisters_corres:
  ...
  apply (simp add: word_less_nat_alt unat_of_nat_mw)
...
lemma decodeSetIPCBuffer_corres:
  ...
  apply (clarsimp simp: list_all2_Cons1 liftME_def[symmetric]
                        is_cap_simps
             split del: if_split)
  apply (clarsimp simp add: returnOk_def newroot_rel_def VPtr_def data_to_vref_def)
  apply (rule corres_guard_imp)
...
lemma decodeSetIPC_wf[wp]:
  ...
  apply (simp   add: decodeSetIPCBuffer_def Let_def whenE_def VPtr_def
