...
lemma setThreadState_runnable_simp:
  ...
  apply (drule use_valid[OF _ threadSet_st_tcb_at_state[where p=t and P="op = ts"]])
   apply simp
...
lemma setCTE_weak_sch_act_wf[wp]:
  ...
  apply (wp hoare_vcg_all_lift hoare_convert_imp setCTE_st_tcb_at' setCTE_tcb_in_cur_domain')
  done
...
lemma ipcCancel_nonz_cap_to'[wp]:
  ...
  apply (simp add: ipcCancel_def getThreadReplySlot_def Let_def
                   capHasProperty_def)
  apply (wp threadSet_cap_to' | simp | wpc
       | rule_tac Q="\<lambda>rv. ex_nonz_cap_to' p" in hoare_post_imp, simp)+
  done
...
lemma finaliseCapTrue_standin_not_rct[wp]:
  "\<lbrace>\<lambda>s. ksSchedulerAction s \<noteq> ResumeCurrentThread \<rbrace>
...
lemma restart_invs':
  ...
  apply (clarsimp simp: st_tcb_at' invs'_def valid_state'_def
                        ct_in_state'_def)
...
lemma
  no_fail_setRegister: "no_fail \<top> (setRegister r v)"
...
lemma isRunnable_wp':
  ...
  apply wp
  apply (clarsimp simp: getObject_def valid_def in_monad
...
lemma setP_vq'[wp]:
  ...
  apply (simp add: setPriority_def)
  apply (wp | clarsimp simp: st_tcb_at'_def o_def)+
...
lemma threadSet_iflive'T2:
  "\<forall>tcb. \<forall>(getF, setF)\<in>ran tcb_cte_cases. getF (F tcb) = getF tcb \<Longrightarrow>
   \<lbrace>\<lambda>s. if_live_then_nonz_cap' s \<and>
     \<not> (\<exists>tcb. (tcbState tcb = Structures_H.thread_state.Inactive \<or> idle' (tcbState tcb)) \<and>
             tcbState (F tcb) \<noteq> Structures_H.thread_state.Inactive \<and>
             \<not> idle' (tcbState (F tcb)))
       \<and> \<not> (\<exists>tcb. \<not> tcbQueued tcb \<and> tcbQueued (F tcb))\<rbrace>
     threadSet F t
   \<lbrace>\<lambda>rv. if_live_then_nonz_cap'\<rbrace>"
  apply (rule hoare_pre, erule threadSet_iflive'T)
  apply auto
  done
...
proof -
  ...
  apply (wp rescheduleRequired_all_invs_but_ct_not_inQ)
   ...
   apply (rule_tac Q="\<lambda>_. invs'" in hoare_post_imp,
          clarsimp simp: st_tcb_at'_def comp_def)
  ...
  apply (wp_trace tcbSchedDequeue_not_queued)
    apply (clarsimp)+
  done
qed
...
crunch valid_cap[wp]: setPriority "valid_cap' c"
...
definition valid_tcb_invocation :: "tcbinvocation \<Rightarrow> bool" where
   "valid_tcb_invocation i \<equiv> case i of
        ThreadControl _ _ _ p _ _ _ \<Rightarrow> valid_option_prio p
      | _                           \<Rightarrow> True"

lemma tc_corres:
  assumes x: "newroot_rel e e'" and y: "newroot_rel f f'" and p: "p = p'"
          ...
          apply (rule corres_split_norE [OF _ T [OF x U], simplified])
             apply (rule corres_split_norE [OF _ T [OF y V], simplified])
               ...
               apply (wp stuff hoare_vcg_all_lift_R hoare_vcg_all_lift
                         ...
                         typ_at_lifts [OF setPriority_typ_at'] setP_invs'
                         assertDerived_wp threadSet_cap_to' out_st_tcb_at_preserved
...
| "tcbinv_relation (tcb_invocation.ThreadControl a sl b d e f g) x
    = (\<exists>b' e' f' sl' g'. b = option_map to_bl b' \<and>
                        newroot_rel e e' \<and> newroot_rel f f' \<and>
                        ({e, f, option_map undefined g} \<noteq> {None}
                           \<longrightarrow> sl' = cte_map sl) \<and>
                        (case g of None \<Rightarrow> g' = None
                          | Some (vptr, g'') \<Rightarrow> \<exists>g'''. g' = Some (vptr, g''')
                               \<and> newroot_rel g'' g''') \<and>
       x = tcbinvocation.ThreadControl a sl' b' d e' f' g')"
...
lemma tcbinv_invs':
  ...
  apply (case_tac ti, simp_all only:)
     ...
     apply (wp writereg_invs' readreg_invs' copyreg_invs' | simp)+
  done
...
lemma OR_choice_decode_sp_simp[simp]:
  "OR_choice (decode_set_priority_error_choice (ucast a) cur) f g
    = do rv \<leftarrow> decode_set_priority_error_choice (ucast a) cur;
         if rv then f else g od"
  apply (rule ext)
  apply (clarsimp simp: OR_choice_def select_f_def decode_set_priority_error_choice_def
               wrap_ext_bool_det_ext_ext_def get_def bind_def ethread_get_def split_def
               gets_the_def assert_opt_def return_def gets_def fail_def mk_ef_def
               split: option.splits)
  done
...
lemma decode_set_priority_corres:
  ...
  apply (simp    add: decode_set_priority_def decodeSetPriority_def
           split del: split_if)
  apply (cases args)
   apply simp
  apply (simp add: liftE_bindE)
...
lemma threadGet_tcbPriority_maxPriority:
  "\<lbrace> valid_objs' \<rbrace> threadGet tcbPriority t \<lbrace> \<lambda>p s. p \<le> maxPriority\<rbrace>"
...
lemma decodeSetPriority_wf[wp]:
  ...
  proof -
    have prio_maxPrio:
    "\<And>P t. \<lbrakk> \<And>s. P s \<Longrightarrow> valid_objs' s \<rbrakk> \<Longrightarrow> \<lbrace> P \<rbrace> threadGet tcbPriority t \<lbrace> \<lambda>p s. P s \<and> p \<le> maxPriority \<rbrace>"
    apply (wp threadGet_tcbPriority_maxPriority)
    apply (simp)+
    done
  show ?thesis
  ...
  apply (rule hoare_pre)
   apply (case_tac "args")
    defer
    apply simp
    apply (rule validE_validE_R, rule seqE)
     apply (rule_tac Q="\<lambda>_. invs' and ex_nonz_cap_to' t and tcb_at' t" in liftE_wp)
     apply (wp_trace getCurThread_invs' getCurThread_cap_to')[1]
    apply (rule seqE, rule liftE_wp)
     apply (rule prio_maxPrio, clarsimp simp: invs'_def valid_state'_def valid_pspace'_def)
    apply (wp_trace)[1]
    apply (wp whenE_throwError_wp)
    apply fastforce+
  apply (simp, wp throwError_validE_R)
  done
qed
...
lemma isThreadControl_def2:
  "isThreadControl tinv = (\<exists>a b c d e f g. tinv = ThreadControl a b c d e f g)"
...
lemma decode_tcb_inv_corres:
 "\<lbrakk> c = Structures_A.ThreadCap t; cap_relation c c';
  ...
  corres (ser \<oplus> tcbinv_relation) (einvs and tcb_at t and (\<lambda>s. \<forall>x \<in> set extras. cte_at (snd x) s))
                                 (invs' and tcb_at' t and (\<lambda>s. \<forall>x \<in> set extras'. cte_at' (snd x) s))
  ...
  apply (clarsimp simp: list_all2_map1 list_all2_map2
             elim!: list_all2_mono)
  done

lemma decodeTCBInv_inv:
  "\<lbrace>P\<rbrace> decodeTCBInvocation label args (capability.ThreadCap t) slot extras \<lbrace>\<lambda>rv. P\<rbrace>"
  apply (simp add: decodeTCBInvocation_def Let_def
              cong: if_cong invocation_label.case_cong split del: split_if)
  apply (rule hoare_vcg_precond_imp)
   apply (wpc, wp decodeTCBConf_inv decodeReadReg_inv
             decodeWriteReg_inv decodeCopyReg_inv)
  apply simp
  done
...
lemma decodeTCBInv_wf:
  ...
  apply (rule hoare_vcg_precond_impE_R)
   apply (wpc, wp decodeTCBConf_wf decodeReadReg_wf
             decodeWriteReg_wf decodeCopyReg_wf)
...
lemma setPriority_st_tcb_at'[wp]:
  ...
  apply (simp add: setPriority_def)
  apply (wp threadSet_st_tcb_no_state | simp)+
  done
...
lemma st_tcb_at_ksArch_update[simp]:
  ...
  by (simp add: st_tcb_at'_def obj_at'_def projectKOs ps_clear_def)
...
crunch irq_states' [wp]: getThreadVSpaceRoot valid_irq_states'
crunch irq_states' [wp]: setPriority valid_irq_states'
  (simp: crunch_simps)
