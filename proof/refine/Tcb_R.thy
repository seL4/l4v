...
lemma isRunnable_wp':
  ...
  apply (clarsimp simp: pred_tcb_at'_def obj_at'_def projectKOs objBitsKO_def)
  done
...
lemma threadSet_tcbBoundAEP_state_refs_of':
  ...
  apply (simp add: threadSet_def)
  apply (wp getObject_tcb_wp)
  apply (auto simp: fun_upd_def obj_at'_def projectKOs state_refs_of'_def)
  done
       ...
       apply (rule corres_split_eqrE)
          apply (rule corres_split_norE)
             apply (rule corres_splitEE'[where r'="\<lambda>rv rv'. ((fst rv) = (fst rv')) \<and> ((snd rv') = (AllowRead \<in> (snd rv)))"])
              ...
              apply (wp | wpc | simp)+
            apply (rule corres_trivial, simp split: option.splits)
           apply (wp | wpc | simp)+
...
lemma decodeBindAEP_wf:
  ...
  apply (fastforce simp: valid_cap'_def[where c="capability.ThreadCap t", standard]
  ...
  apply (cases tinv, simp_all add: invokeTCB_def Let_def split_def
   ...
   apply (rule hoare_pre)
    apply ((simp split del: split_if
             ...
             | (wp option_cases_weak_wp)[1])+)[2]
  apply (rule hoare_pre)
   apply (wp | wpc | simp)+
  done

crunch irq_states'[wp]: getThreadBufferSlot, setPriority
                         valid_irq_states'
  (simp: crunch_simps)
