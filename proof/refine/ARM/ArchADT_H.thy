  ...
  "absHeapArch h pt_types a \<equiv> \<lambda>ako.
     case ako of
       KOASIDPool (ARM_H.ASIDPool ap) \<Rightarrow>
         Some (ARM_A.ASIDPool (\<lambda>w. map_option abs_asid_entry (ap (ucast w))))
     | KOPTE _ \<Rightarrow>
...
definition mdata_map' ::
  "(asid \<times> vspace_ref) option \<Rightarrow> (Machine_A.ARM_A.asid \<times> vspace_ref) option" where
...
| "ArchFaultMap (ARM_H.VCPUFault w) = ARM_A.VCPUFault w"
| "ArchFaultMap (ARM_H.VGICMaintenance m) = ARM_A.VGICMaintenance m"
| "ArchFaultMap (ARM_H.VPPIEvent irq) = ARM_A.VPPIEvent irq"
