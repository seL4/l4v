...
lemma capMasterCap_valid_arch_badges_isCapRevocable[CSpace_R_assms]:
  ...
  apply (clarsimp simp: ARM_H.isCapRevocable_def)
...
(* FIXME arch-split: cleaner way of doing this? can't use obj_at_setObject2 on CTEs *)
lemma setCTE_ko_at'_pde[wp]:
  "setCTE p c \<lbrace> \<lambda>s. P (ko_at' (pde::pde) p' s) \<rbrace>"
  apply (simp add: setCTE_def)
...
lemma arch_deriveCap_untyped_derived[wp]:
  ...
  apply (wpsimp simp: ARM_H.deriveCap_def Let_def untyped_derived_eq_ArchObjectCap
           split_del: if_split
...
lemma ex_nonz_tcb_cte_caps':
  ...
  apply (subgoal_tac "s \<turnstile>' cteCap cte")
   ...
   apply (clarsimp simp: valid_cap'_def obj_at'_def dom_def ARM.typ_at_to_obj_at_arches
...
lemma maskedAsFull_revokable_safe_parent[CSpace_R_2_assms]:
  ...
  apply (clarsimp simp: Retype_H.isCapRevocable_def ARM_H.isCapRevocable_def maskedAsFull_def
...
definition createNewCaps_arch_ko_pre :: "(kernel_object \<Rightarrow> bool) \<Rightarrow> bool" where
  "createNewCaps_arch_ko_pre P \<equiv>
     \<forall>pde pde'. P (KOArch (KOPDE pde)) = P (KOArch (KOPDE pde'))"

definition createNewCaps_arch_ko_type_pre :: "kernel_object_type \<Rightarrow> bool" where
  "createNewCaps_arch_ko_type_pre ty \<equiv> ty \<noteq> koType(TYPE(pde))"
