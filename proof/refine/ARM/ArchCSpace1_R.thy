...
lemma sameRegionAs_SGISignalCap[simp]:
  ...
  by (auto simp add: global.sameRegionAs_def ARM_H.sameRegionAs_def isCap_simps split: if_splits)
...
lemma same_arch_region_as_relation:
  ...
  by (cases c; cases c')
     (auto simp: ARM_H.sameRegionAs_def global.sameRegionAs_def Let_def isCap_simps mask_def
...
lemma obj_size_relation:
  ...
  apply (case_tac arch_cap; simp add: objBits_def ARM_H.capUntypedSize_def asid_low_bits_def
...
lemma capASID_update[simp]:
  ...
  apply (case_tac arch_capability,
         simp_all add: global.updateCapData_def ARM_H.updateCapData_def isCap_simps Let_def)
  done
...
lemma sameRegionAs_SGISignalCap2[simp]:
  ...
  by (auto simp: global.sameRegionAs_def ARM_H.sameRegionAs_def isCap_simps
...
lemma (in Arch_mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps ARM.isMDBParentOf_CTE is_derived'_def
