...
definition sgi_invocation_map :: "sgi_signal_invocation \<Rightarrow> sgisignal_invocation" where
  "sgi_invocation_map sgii \<equiv>
     case sgii of ARM_A.SGISignalGenerate irq targets \<Rightarrow> SGISignalGenerate irq (ucast targets)"
...
crunch inv [wp]: decodeARMMMUInvocation "P"
...
shows
  ...
  apply (cases arch_cap)
       ...
       prefer 6
       apply (in_case "arch_cap.SGISignalCap ?irq ?targets")
...
lemma arch_decodeInvocation_wf[wp]:
  ...
  apply (cases arch_cap)
       prefer 6
       apply (in_case "SGISignalCap ?irq ?targets")
...
lemma ipiSendTarget_underlying_memory[wp]: (* FIXME SGI: move to Machine_R *)
  "ipiSendTarget irq targets \<lbrace>\<lambda>ms. P (underlying_memory ms)\<rbrace>"
...
lemma performSGISignalInvocation_invs[wp]:
  ...
  by (wpsimp wp: dmo_invs'_simple simp: no_irq_ipiSendTarget)
