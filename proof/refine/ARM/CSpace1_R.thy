...
lemma ps_clear_16:
  ...
  apply (simp add: ps_clear_def mask_def)
...
lemma capRange_cap_relation:
  ...
  by (simp add: capRange_def objBits_simps' cte_level_bits_def
                asid_low_bits_def pageBits_def zbits_map_def word_size_bits_def
         split: cap_relation_split_asm arch_cap.split_asm
...
lemma valid_badges_def2:
  ...
  apply (case_tac cap; clarsimp simp: gen_isCap_simps)
      by (fastforce simp: ARM.sameRegionAs_def3 ARM.isCap_simps)+  (* FIXME arch-split *)
...
lemma (in mdb_insert_der) dest_no_parent_n:
  ...
  apply (clarsimp simp: gen_isCap_simps isMDBParentOf_CTE is_derived'_def
   ...
   apply (erule notE, rule direct_parent)
     apply (simp add: mdb_next_unfold)
    apply simp
   apply (simp add: parentOf_def src isMDBParentOf_CTE ARM.isCap_simps (* FIXME arch-split *)
  ...
  apply (erule ARM.sameRegionAsE, simp_all) (* FIXME arch-split *)
   ...
   apply (clarsimp simp add: gen_isCap_simps capMasterCap_def
                   simp del: not_ex
                      split: capability.splits)
  apply (clarsimp simp: ARM.isCap_simps)+ (* FIXME arch-split *)
  done
...
lemma mdb_inv_preserve_updateCap:
  ...
  apply (intro conjI impI allI)
      apply fastforce
     apply (simp add: ARM.isArchFrameCap_def) (* FIXME arch-split: UntypedCap is not isArchFrameCap *)
...
lemma SGISignal_is_capMasterCap[simp]:
  "(capMasterCap cap = ArchObjectCap (SGISignalCap irq target)) =
   (cap = ArchObjectCap (SGISignalCap irq target))"
  "(ArchObjectCap (SGISignalCap irq target) = capMasterCap cap) =
   (cap = ArchObjectCap (SGISignalCap irq target))"
  by (auto simp: capMasterCap_def arch_capMasterCap_def
           split: capability.splits arch_capability.splits)
