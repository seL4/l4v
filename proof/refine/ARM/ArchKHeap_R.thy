...
(* FIXME arch-split: check with other arches what is actually in common, standardise *)
...
(* FIXME arch-split: possible interface lemma, but can't be done via locale *)
...
arch_requalify_facts setObject_other_corres (* FIXME arch-split: move *)
...
lemma valid_pde_mappings'_def2:
  ...
  apply (auto simp add: objBits_simps)
  done
