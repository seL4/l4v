...
(* ARM_HYP and ARM: PT entries can be duplicated 2^4 times for large pages and supersections,
...
lemma arch_deletionIsSafe:
  ...
  supply [simp del] =  atLeastatMost_subset_iff atLeastLessThan_iff
    ...
    apply (subgoal_tac "obj_bits koa < 32")
     ...
     apply (rename_tac arch_kernel_obj)
     apply (case_tac arch_kernel_obj,simp_all add:vspace_bits_defs word_bits_def)
     apply (simp add:pageBitsForSize_def split:vmpage_size.splits)
    apply (subgoal_tac "6 \<le> obj_bits koa")
     apply simp
    apply (case_tac koa, simp_all add: other_obj_relation_def vspace_bits_defs
...
sublocale detype_locale'_gen by (unfold_locales; fact ARM.arch_deletionIsSafe)
...
lemma valid_arch_state':
  ...
  apply (simp add: valid_arch_state'_def valid_pde_mappings'_def
                   global_refs'_def valid_asid_table'_def
                   page_directory_at'_def page_directory_refs'_def)
  apply (simp add: valid_global_pts'_def subset_iff
                   page_table_at'_def page_table_refs'_def)
  by fastforce
...
lemma createObject_cte_wp_at'[Detype_R_2_assms]:
  ...
  apply (wpsimp wp: createObjects_orig_cte_wp_at'[where sz = "(Types_H.getObjectSize ty us)"]
                    threadSet_cte_wp_at'
         | clarsimp simp: ARM_H.createObject_def placeNewDataObject_def
...
lemma copyGlobalMappings_setCTE_commute:
  ...
  apply (clarsimp simp: valid_arch_state'_def page_directory_at'_def
                        objBits_simps pdBits_def pageBits_def)
...
lemma createObject_gsUntypedZeroRanges_commute[Detype_R_2_assms]:
  "monad_commute
     ...
     (modify (\<lambda>s. s \<lparr> gsUntypedZeroRanges := f (gsUntypedZeroRanges s) \<rparr> ))"
  apply (simp add: global.createObject_def ARM_H.createObject_def
   ...
   apply (case_tac ty)
    ...
    apply (clarsimp simp: apiGetObjectSize_def  curDomain_def
           ...
           apply (intro conjI range_cover_le[where n = "Suc n"] | simp)+
            apply ((simp add:objBits_simps vspace_bits_defs range_cover_def word_bits_def)+)[5]
...
lemma doMachineOp_ksPSpace_commute:
...
lemma doMachineOp_assert_opt_commute:
...
lemma doMachineOp_assert_commute:
...
lemma doMachineOp_projectKO_pde_commute:
...
lemma doMachineOp_alignCheck_commute:
...
lemma doMachineOp_magnitudeCheck_commute:
...
lemma doMachineOp_storePDE_commute_T:
  ...
  apply (rule monad_commute_guard_imp)
   apply (rule monad_commute_split [OF _ doMachineOp_ksPSpace_commute])
    apply (rule monad_commute_split [OF _ doMachineOp_assert_opt_commute])
      apply (rule monad_commute_split [OF _ doMachineOp_assert_commute])
        apply (rule monad_commute_split [OF _ doMachineOp_projectKO_pde_commute])
          apply (rule monad_commute_split [OF _ doMachineOp_alignCheck_commute])
            apply (rule monad_commute_split [OF _ doMachineOp_magnitudeCheck_commute])
...
lemma createObjects'_page_directory_at':
  ...
  apply (simp add:obj_at'_real_def objBits_simps pdeBits_def)
...
proof -
  ...
  show ?thesis
    ...
    apply (case_tac ty)
          apply (simp add: ARM_H.toAPIType_def Arch_createNewCaps_def)
          apply (rename_tac apiobject_type)
          apply (case_tac apiobject_type)
       ...
       apply (simp add: Arch_createNewCaps_def Retype_H.createObject_def createObjects_def bind_assoc
                        ARM_H.toAPIType_def ARM_H.createObject_def placeNewDataObject_def)
       apply (intro conjI impI)
        (* device *)
        apply (subst monad_eq, rule createObjects_Cons)
              apply (simp_all add: field_simps shiftl_t2n pageBits_def
      ...
      apply (simp add: Arch_createNewCaps_def Retype_H.createObject_def createObjects_def bind_assoc
                       toAPIType_def ARM_H.createObject_def placeNewDataObject_def)
      apply (intro conjI impI)
       (* device *)
       apply (subst monad_eq, rule createObjects_Cons)
             apply (simp_all add: field_simps shiftl_t2n pageBits_def
      ...
      apply (subst monad_eq, rule createObjects_Cons)
            apply (simp_all add: field_simps shiftl_t2n pageBits_def
                                 ARM_H.getObjectSize_def objBits_simps)[6]
      apply (simp add: bind_assoc placeNewObject_def2 objBits_simps ARM_H.getObjectSize_def
                       pageBits_def add.commute append mapM_x_append mapM_x_singleton)
     apply (subst gsUserPages_update gsCNodes_update
                  gsUserPages_upd_createObjects'_comm
                  monad_commute_simple'[OF map_dmo'_createObjects'_comm]
     ...
     apply (simp add: bind_assoc placeNewObject_def2 objBits_simps pageBits_def
                      ARM_H.getObjectSize_def add.commute append mapM_x_append mapM_x_singleton)
    apply (subst gsUserPages_update gsCNodes_update
                 gsUserPages_upd_createObjects'_comm
                 monad_commute_simple'[OF map_dmo'_createObjects'_comm]
                 monad_commute_simple'[OF dmo'_gsUserPages_upd_map_commute]
   ...
   apply (simp add: Arch_createNewCaps_def toAPIType_def bind_assoc
                    createObjects_def global.createObject_def ARM_H.createObject_def)
   apply (subst monad_eq, rule createObjects_Cons; simp?)
    apply (simp add: objBits_simps getObjectSize_def pdeBits_def pdBits_def)
   apply (simp add: getObjectSize_def placeNewObject_def2 objBits_simps append mapM_x_append
                    mapM_x_singleton bind_assoc pdBits_def pdeBits_def)
   apply (subst monad_commute_simple'[OF map_dmo'_createObjects'_comm]
          | simp add: modify_modify_bind o_def
          | simp only: o_def cong: if_cong)+
  ...
  apply (rule_tac Q = "\<lambda>r s. valid_arch_state' s \<and>
    ...
    apply (subst monad_commute_simple)
     ...
     apply (clarsimp simp:objBits_simps pdBits_def pageBits_def word_bits_conv)
  ...
  apply (clarsimp simp: makeObject_pde)
  apply (drule range_cover_le[where n = "Suc n"], simp)
  apply (rule conjI, assumption)
  apply (clarsimp simp: objBits_simps pdeBits_def word_bits_def cong: conj_cong)
...
lemma createObject_def2[Detype_R_3_assms]:
  ...
  apply (case_tac apiobject_type)
      apply (clarsimp simp: Arch_createNewCaps_def createObjects_def shiftL_nat
                            ARM_H.createObject_def placeNewObject_def2 objBits_simps bind_assoc
