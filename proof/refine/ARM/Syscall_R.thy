...
lemma cur_sc_not_idle_imp_ct_not_idle:
  "\<lbrakk>invs s; cur_sc_tcb_are_bound s; cur_sc s \<noteq> idle_sc_ptr\<rbrakk> \<Longrightarrow> \<not> ct_idle s"
  apply (frule (1) cur_sc_tcb_are_bound_sym[OF _ invs_sym_refs])
  apply (clarsimp simp: invs_def valid_state_def ct_in_state_def valid_pspace_def
                 dest!: only_idleD)
  apply (clarsimp simp: pred_tcb_at_def obj_at_def valid_idle_def sc_tcb_sc_at_def vs_all_heap_simps)
  done

lemma cur_sc_tcb_are_bound_cross:
  "\<lbrakk>(s, s') \<in> state_relation; pspace_aligned s; pspace_distinct s;
    cur_sc_tcb_are_bound s\<rbrakk>
  \<Longrightarrow> obj_at' (\<lambda>sc. tcbSchedContext sc = Some (ksCurSc s')) (ksCurThread s') s'"
  apply (clarsimp simp: obj_at_def is_tcb pred_map_eq opt_map_red vs_all_heap_simps
                 dest!: state_relationD)
  apply (frule (1) pspace_relation_absD)
  apply clarsimp
  apply (clarsimp simp: other_obj_relation_def)
  apply (rename_tac z; case_tac z; simp)
  apply (drule (3) aligned_distinct_ko_at'I[where 'a=tcb], simp)
  apply (clarsimp simp: obj_at'_def tcb_relation_def projectKOs)
  done

lemma cur_sc_tcb_are_bound_sym_cross:
  "\<lbrakk>(s, s') \<in> state_relation; pspace_aligned s; pspace_distinct s;
    cur_sc_tcb_are_bound s; sym_refs (state_refs_of s)\<rbrakk>
  ...
  apply (clarsimp simp: cur_tcb_def obj_at_def is_tcb pred_map_eq opt_map_red vs_all_heap_simps
                 dest!: state_relationD)
  apply (frule (1) pspace_relation_absD)
  apply clarsimp
