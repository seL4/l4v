...
(* FIXME RT: Corres_UL *)
definition cross_rel where
  "cross_rel P Q \<equiv> \<forall>s s'. P s \<longrightarrow> (s, s') \<in> state_relation \<longrightarrow> Q s' "

lemma cross_rel_True_right[simp]:
  "cross_rel P \<top>"
  by (clarsimp simp: cross_rel_def)

lemma cross_rel_False_left[simp]:
  "cross_rel \<bottom> P"
  by (clarsimp simp: cross_rel_def)

lemma cross_rel_imp_right:
  "cross_rel Q P' \<Longrightarrow> (\<And>x. P' x \<Longrightarrow> P x) \<Longrightarrow> cross_rel Q P"
  by (fastforce simp: corres_underlying_def cross_rel_def)

lemma cross_rel_imp_left:
  "cross_rel Q P' \<Longrightarrow> (\<And>x. Q' x \<Longrightarrow> Q x) \<Longrightarrow> cross_rel Q' P'"
  by (fastforce simp: corres_underlying_def cross_rel_def)

lemma cross_rel_and_right:
  "cross_rel P P' \<Longrightarrow> cross_rel P P'' \<Longrightarrow> cross_rel P (P' and P'')"
  by (fastforce simp: corres_underlying_def cross_rel_def)

lemma corres_imp_right:
  "cross_rel P (\<lambda>s'. P' s' \<longrightarrow> Q' s') \<Longrightarrow> corres a P Q' f g \<Longrightarrow> corres a P P' f g"
  by (auto simp: corres_underlying_def cross_rel_def)

lemma corres_cross:
  "cross_rel Q Q' \<Longrightarrow> corres a P (P' and Q') f g \<Longrightarrow> (\<And>x. P x \<Longrightarrow> Q x) \<Longrightarrow> corres a P P' f g"
  by (fastforce simp: corres_underlying_def cross_rel_def)

lemma cross_rel_aug:
  "cross_rel P' Q' \<Longrightarrow> cross_rel P (\<lambda>x. Q' x \<longrightarrow> Q x) \<Longrightarrow> (\<And>x. P x \<Longrightarrow> P' x) \<Longrightarrow> cross_rel P Q"
  unfolding cross_rel_def
  by fastforce
