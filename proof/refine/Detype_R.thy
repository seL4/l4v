...
lemma deleteObjects_def2:
  ...
  apply (rule arg_cong2[where f=gsUserPages_update])
   apply (simp add: NOT_eq[symmetric] mask_in_range ext)
  apply (rule arg_cong[where f="\<lambda>f. ksPSpace_update f s", standard])
...
lemma obj_relation_cuts_eqv_base_in_detype_range:
  "\<lbrakk> (y, P) \<in> obj_relation_cuts ko x; kheap s x = Some ko;
      ...
      valid_untyped (cap.UntypedCap base bits idx) s \<rbrakk>
...
lemma detype_pspace_relation:
  ...
  and      vu: "valid_untyped (cap.UntypedCap base bits idx) s"
...
lemma objBits_bounded[simp]:
  "objBitsKO obj < word_bits"
  apply (cases obj,
         simp_all add: objBitsKO_def word_bits_def
                       pageBits_def archObjSize_def)
  apply (case_tac arch_kernel_object, simp_all add: pageBits_def)
  done
...
lemma cte_wp_at_delete':
  "\<lbrakk> s \<turnstile>' UntypedCap base magnitude idx; pspace_distinct' s \<rbrakk> \<Longrightarrow>
...
lemma map_to_ctes_delete:
  assumes vc: "s \<turnstile>' UntypedCap base magnitude idx"
  ...
  using cte_wp_at_delete' [where P="op = cte", standard, OF vc vs]
...
lemma word_range_card:
  "base \<le>base + h \<Longrightarrow>
  card {base..base + (h::word32)} = (unat h) + 1"
  proof (induct h)
   case 1
   show ?case
    by simp
...
lemma (in detype_locale') deletionIsSafe:
  ...
  and    cap: "cap = cap.UntypedCap base magnitude idx"
  and      vs: "valid_pspace s" 
  and      al: "is_aligned base magnitude"
  and      vu: "valid_untyped (cap.UntypedCap base magnitude idx) s"
...
proof -
  ...
  hence "\<forall>ptr t m. cte_wp_at (op = (cap.ReplyCap t m)) ptr s
         \<longrightarrow> t \<notin> {base .. base + 2 ^ magnitude - 1}"
    by (fastforce simp del: split_paired_All split_paired_Ex)
  ...
  have arch: "\<And> ko p. \<lbrakk> ksPSpace s' p = Some (KOArch ko); p \<in> {base..base + 2 ^ magnitude - 1} \<rbrakk>
    ...
    apply (case_tac arch_kernel_obj,
      simp_all add:arch_kobj_size_def pageBits_def
      pageBitsForSize_def)+
    done
  thus ?thesis
    using cte
    apply (auto simp: deletionIsSafe_def)
    done
qed
...
lemma ekheap_relation_detype: "ekheap_relation ekh kh \<Longrightarrow> ekheap_relation (\<lambda>x. if P x then None else (ekh x)) (\<lambda>x. if P x then None else (kh x))"
  apply (fastforce simp add: ekheap_relation_def split: split_if_asm)
  done
...
lemma detype_corres:
  "is_aligned base magnitude \<Longrightarrow> magnitude \<ge> 2 \<Longrightarrow>
   corres dc
      (\<lambda>s. einvs s
           \<and> s \<turnstile> (cap.UntypedCap base magnitude idx)
           \<and> (\<exists>cref. cte_wp_at (op = (cap.UntypedCap base magnitude idx)) cref s
                     \<and> descendants_range (cap.UntypedCap base magnitude idx) cref s)
           \<and> untyped_children_in_mdb s \<and> if_unsafe_then_cap s
           \<and> valid_mdb s \<and> valid_global_refs s \<and> ct_active s)
      (\<lambda>s. s \<turnstile>' (UntypedCap base magnitude idx)
           \<and> valid_pspace' s)
  ...
  apply (rule corres_stateAssert_implied[where P'=\<top>, simplified])
   prefer 2
   apply clarsimp
   apply (rule_tac cap="cap.UntypedCap base magnitude idx" and ptr="(a,b)" and
  ...
  apply (rule_tac Q="\<lambda>_ s. valid_objs s \<and> valid_list s \<and>
           (\<exists>cref. cte_wp_at (op = (cap.UntypedCap base magnitude idx)) cref s \<and>
                   descendants_range (cap.UntypedCap base magnitude idx) cref s ) \<and>
           s \<turnstile> cap.UntypedCap base magnitude idx \<and> pspace_aligned s \<and>
    ...
    apply (rule state_relation_null_filterE, assumption,
           ...
           apply (simp add: detype_def, rule state.equality,(simp add: detype_ext_def wrap_ext_det_ext_ext_def)+)
...
locale delete_locale =  
  fixes s and base and bits and ptr and idx
  assumes cap: "cte_wp_at' (\<lambda>cte. cteCap cte = UntypedCap base bits idx) ptr s"
  and  nodesc: "descendants_range' (UntypedCap base bits idx) ptr (ctes_of s)"
  and    invs: "invs' s"
...
lemma valid_untyped[simp]:
  "s \<turnstile>' UntypedCap base bits idx"
...
lemma pre_helper2:
    ...
    apply (drule word_mult_less_mono1[where k="2 ^ 2"])
      apply simp
     apply (simp only: unat_power_lower32 power_add[symmetric])
   ...
   (* proves that the bits of caps they need for validity argument
      are within their capRanges *)
lemma valid_cap_ctes_pre:
    "\<And>p c. s \<turnstile>' c \<Longrightarrow> case c of CNodeCap ref bits g gs
    ...
    apply (simp split: capability.split zombie_type.split arch_capability.split, safe)
       apply (clarsimp simp add: capRange_def capAligned_def objBits_simps
                                 pre_helper field_simps
                       simp del: atLeastAtMost_iff)
...
lemma replycap_argument:
    "\<And>p t m. cte_wp_at' (\<lambda>cte. cteCap cte = ReplyCap t m) p s
...
proof -
  from cap obtain node
    where ctes_of: "ctes_of s ptr = Some (CTE (UntypedCap base bits idx) node)"
...
lemma ctes_of_valid[elim!]:
    "\<And>p cte. ctes_of s p = Some cte \<Longrightarrow> s \<turnstile>' cteCap cte"
    by (case_tac cte, simp add: ctes_of_valid_cap' [OF _ valid_objs])

lemma valid_cap2:
    "\<And>p c. \<lbrakk> cte_wp_at' (\<lambda>cte. cteCap cte = c) p s \<rbrakk>
...
lemma ex_nonz_cap_notRange: "\<And>p. ex_nonz_cap_to' p s \<Longrightarrow> p \<notin> base_bits"
...
lemma live_notRange: "\<And>(p :: word32) P. \<lbrakk> ko_wp_at' P p s; \<And>ko. P ko \<Longrightarrow> live' ko \<rbrakk>
...
lemma refs_notRange: "\<And>x y tp. (x, tp) \<in> state_refs_of' s y
                                  \<Longrightarrow> y \<notin> base_bits"
...
lemma non_null_present:
    "\<And>p. cte_wp_at' (\<lambda>c. cteCap c \<noteq> NullCap) p s \<Longrightarrow> p \<notin> base_bits"
    apply (drule if_unsafe_then_capD' [OF _ ifunsafe])
     apply assumption
...
lemma cte_cap: "\<And>p. ex_cte_cap_to' p s \<Longrightarrow> ex_cte_cap_to' p state'"
...
lemma map_to_ctesE[elim!]:
    "\<And>P x cte. \<lbrakk> ctes' x = Some cte;
                  \<lbrakk> ctes_of s x = Some cte; x \<notin> base_bits \<rbrakk> \<Longrightarrow> P
                \<rbrakk> \<Longrightarrow> P"
    by (clarsimp simp: tree_to_ctes split: split_if_asm)


lemma not_nullMDBNode:
    "\<lbrakk> ctes_of s x = Some cte; cteCap cte = NullCap;
                 cteMDBNode cte = nullMDBNode \<Longrightarrow> P \<rbrakk>
             \<Longrightarrow> P" using nullcaps
    apply (cases cte)
    apply (simp add: valid_nullcaps_def)
    done

lemma mdb_src: "\<And>x y. \<lbrakk> ctes_of s \<turnstile> x \<leadsto> y; y \<noteq> 0 \<rbrakk> \<Longrightarrow> x \<notin> base_bits"
...
lemma mdb_dest: "\<And>x y. \<lbrakk> ctes_of s \<turnstile> x \<leadsto> y; y \<noteq> 0 \<rbrakk> \<Longrightarrow> y \<notin> base_bits"
    apply (case_tac "x = 0")
...
lemma trancl_next[elim]:
    "\<And>x y. \<lbrakk> ctes_of s \<turnstile> x \<leadsto>\<^sup>+ y; x \<notin> base_bits \<rbrakk> \<Longrightarrow> ctes' \<turnstile> x \<leadsto>\<^sup>+ y"
    apply (erule rev_mp,
           erule converse_trancl_induct)
     apply clarsimp
...
lemma mdb_parent_notrange:
    "\<And>x y. ctes_of s \<turnstile> x \<rightarrow> y \<Longrightarrow> x \<notin> base_bits \<and> y \<notin> base_bits"
    apply (erule subtree.induct)
     apply (frule(1) mdb_src, drule(1) mdb_dest, simp)
    apply (drule(1) mdb_dest, simp)
    done

lemma mdb_parent: "\<And>x y. ctes_of s \<turnstile> x \<rightarrow> y
                           \<Longrightarrow> ctes' \<turnstile> x \<rightarrow> y"
    apply (erule subtree.induct)
...
lemma trancl_next_rev:
    "\<And>x y. ctes' \<turnstile> x \<leadsto>\<^sup>+ y \<Longrightarrow> ctes_of s \<turnstile> x \<leadsto>\<^sup>+ y"
...
lemma is_chunk[elim!]:
    "\<And>cap x y. is_chunk (ctes_of s) cap x y \<Longrightarrow>
               is_chunk ctes' cap x y"
...
lemma mdb_parent_rev: "\<And>x y. ctes' \<turnstile> x \<rightarrow> y
                              \<Longrightarrow> ctes_of s \<turnstile> x \<rightarrow> y"
    apply (erule subtree.induct)
     apply (rule subtree.direct_parent)
...
proof (simp add: invs'_def valid_state'_def valid_pspace'_def
  ...
  show "valid_irq_node' (irq_node' s) ?s"
    ...
    by (simp add: valid_irq_node'_def mult_ac ucast_ucast_mask_8)
  ...
  thus "valid_machine_state' ?state''"
    ...
    using valid_untyped[simplified valid_cap'_def capability.cases]
...
lemma deleteObjects_null_filter:
  "\<lbrace>cte_wp_at' (\<lambda>c. cteCap c = UntypedCap ptr bits idx) p
     ...
     and (\<lambda>s. descendants_range' (UntypedCap ptr bits idx) p (ctes_of s))
...
proof -
  ...
  apply (subst 1)
  apply (simp add: bind_assoc[where f="\<lambda>_. modify f", standard, symmetric])
...
lemma deleteObjects_cap_to':
  ...
  apply (subgoal_tac "delete_locale s ptr bits p idx")
   ...
   apply (subgoal_tac
    ...
    apply (simp add: field_simps ex_cte_cap_wp_to'_def
      cong:if_cong)
   apply simp
  apply (simp add: delete_locale_def)
  done
...
lemma valid_untyped_no_overlap:
  "\<lbrakk> valid_untyped' ptr bits idx s; is_aligned ptr bits; valid_pspace' s \<rbrakk>
...
lemma pspace_no_overlap'_gsUserPages_update[simp]:
  "pspace_no_overlap' p b (gsUserPages_update f s') =
   pspace_no_overlap' p b s'"
  by (simp add: pspace_no_overlap'_def)
...
lemma deleteObject_no_overlap[wp]:
  "\<lbrace>valid_cap' (UntypedCap ptr bits idx) and valid_pspace'\<rbrace>
     deleteObjects ptr bits
...
lemma setCTE_pspace_no_overlap':
  "\<lbrace>pspace_aligned' and pspace_distinct' and pspace_no_overlap' ptr sz\<rbrace>
   ...
   apply (rule pspace_no_overlap'_lift)
   apply (wp setCTE_typ_at')
   apply auto
   done
...
lemma cte_wp_at_top:
  ...
  apply (case_tac b,simp_all)
    ...
    apply (simp add:loadObject_cte cte_check_def 
      ...
      tcbCTableSlot_def tcbVTableSlot_def
      loadObject_tcb objBits_simps cteSizeBits_def)
...
lemma locateCTE_monad:
  ...
  proof -
  have neq_out:
    "\<And>a b c. \<lbrakk>a \<noteq> b; b \<notin> {a..a + c - 1} - {a} \<rbrakk> \<Longrightarrow> b \<notin> {a..a + c - 1}"
    by fastforce

  have rule_out:
    "\<And>s a b obj obj'. \<lbrakk> ksPSpace s a = Some obj; ksPSpace s b = Some obj'; pspace_distinct' s; a\<noteq>b \<rbrakk> 
  ...
  have src_in_range:
   "\<And>obj src a m s'. \<lbrakk>cte_check obj src a m;ksPSpace s' a = Some obj\<rbrakk> \<Longrightarrow> src \<in> {a..a + 2 ^ objBitsKO obj - 1}"
    proof -
    fix obj src a m s'
    show "\<lbrakk>cte_check obj src a m;ksPSpace s' a = Some obj\<rbrakk> \<Longrightarrow> src \<in> {a..a + 2 ^ objBitsKO obj - 1}"
    apply (case_tac obj)
  ...
  (r, s') \<in> fst (f s);pspace_aligned' s;pspace_distinct' s;(P1 and P2 and P3 and P4) s\<rbrakk> \<Longrightarrow>
  (ptr,s') \<in> fst (locateCTE src s')"
  ...
  apply (clarsimp simp:locateCTE_def gets_def split_def
    ...
    apply (frule_tac dest1 = src
      in use_valid[OF _ cte_wp_at])
    apply simp
    apply (subst cte_wp_at_top)
    apply simp
  apply (clarsimp simp add:cte_wp_at_top)
  apply (clarsimp simp:lookupAround2_char1)
  apply (frule_tac dest1 = a and  Q1 = "\<lambda>x. x = objBitsKO b" in use_valid[OF _ ko_wp_at])
   apply (frule(1) pspace_alignedD')
   apply (frule(1) pspace_distinctD')
   apply (fastforce simp add:ko_wp_at'_def)
  ...
  apply (frule use_valid[OF _ psp_aligned])
   apply simp
  apply (frule_tac x = aa in pspace_distinctD')
   apply simp
  apply (frule_tac s = s' and a = a in rule_out[rotated])
   apply simp+
  apply (frule_tac s = s' and b = a and a = aa in rule_out)
   apply simp+
  apply (thin_tac "\<forall>x. ?P x \<longrightarrow>?Q x")+
  apply (drule_tac p = a and p' = aa in aligned_ranges_subset_or_disjoint)
   apply (erule(1) pspace_alignedD')
  apply (drule(1) src_in_range)+
  apply (drule base_member_set[OF pspace_alignedD'])
    apply simp
   apply (simp add:objBits_bounded[unfolded word_bits_def,simplified])
  apply (drule base_member_set[OF pspace_alignedD'])
    apply simp
   apply (simp add:objBits_bounded[unfolded word_bits_def,simplified])
...
lemma fail_empty_locateCTE:
  ...
  apply (clarsimp simp:locateCTE_def bind_assoc 
    return_def split_def split_def gets_def
    get_def bind_def assert_opt_def image_def)
  apply (elim disjE)
   apply (clarsimp simp:assert_def fail_def return_def
     split:option.splits split_if_asm)+
  done
...
proof -
  ...
  apply (intro conjI iffI set_eqI)
     apply (clarsimp dest!:singletonD)
     apply (frule same)
     apply (clarsimp)
     apply (rule bexI[rotated], assumption)
     apply (frule singleton_locateCTE[THEN iffD1])
     apply (frule locateCTE_monad
      [OF ko_wp_at cte_wp_at psp_distinct psp_aligned])
        apply assumption+
      apply simp
     apply (clarsimp)
  ...
  apply (rule hoare_pre)
   apply (wpc
        | wp createWordObjects_orig_cte_wp_at[where sz = "(Types_H.getObjectSize ty us)"]
             ...
             threadSet_cte_wp_at'
        | simp add: ArchRetype_H.createObject_def createPageObject_def placeNewObject_def2 createPageObject_def curDomain_def)+
  apply (case_tac ty)
...
lemma createObject_getCTE_commute:
  "monad_commute
     ...
     (RetypeDecls_H.createObject ty ptr us) (getCTE dests)"
...
lemma simpler_placeNewObject_def:
  ...
  apply (clarsimp simp:placeNewObject_def2)
   ...
   apply (clarsimp simp:return_def bind_def gets_def assert_def fail_def 
     get_def split_def split:option.splits)
   ...
   apply (drule_tac x = a in in_empty_interE)
    ...
    apply (simp add:is_aligned_no_overflow objBits_bounded)
   apply (clarsimp simp: is_aligned_neg_mask_eq shiftL_nat p_assoc_help)
  apply simp
  done

lemma snd_fail: "snd (fail s)"
...
lemma placeNewObject_modify_commute:
  ...
  apply (clarsimp simp: simpler_modify_def dest!:singletonD)
...
lemma getPDE_det:
  ...
  apply (clarsimp simp:SUP1_iff fail_def return_def lookupAround2_known1)
   apply (simp add:ArchObjInsts_H.loadObject_pde loadObject_default_def)
...
lemma modify_pde_pde_at':
  ...
  apply wp
  apply (clarsimp simp del:fun_upd_apply simp:
    typ_at'_def ko_wp_at'_def objBits_simps archObjSize_def)
...
lemma doMachineOp_storePDE_commute:
  ...
  apply (rule monad_commute_guard_imp)
   ...
   apply (clarsimp simp:monad_commute_def simpler_modify_def return_def bind_def)
    apply (intro conjI iffI set_eqI)
       apply (clarsimp simp:SUP1_iff dest!:singletonD)
...
lemma storePDE_setCTE_commute:
  ...
  apply (rule monad_commute_guard_imp)
   ...
   apply wp 
    apply (thin_tac "cte_wp_at' ?P src ?s")+
    apply (clarsimp simp: typ_at'_def
       cte_wp_at_obj_cases_mask obj_at'_real_def)
...
lemma setCTE_gets_globalPD_commute:
  ...
  apply (rule monad_commute_guard_imp)
   apply (rule commute_commute[OF monad_commute_split[where Q = "\<lambda>r. \<top>"]])
     apply (clarsimp simp:monad_commute_def gets_def simpler_modify_def
...
lemma placeNewObject_pd_at':
  ...
  apply (clarsimp simp:pdBits_def pageBits_def 
    new_cap_addrs_def objBits_simps archObjSize_def image_def)
...
lemma threadSet_det: 
  ...
  apply (cut_tac ko = "KOTCB obj" in magnitudeCheck_det)
   apply (simp add:objBits_simps objBitsKO_def is_aligned_mask)+
...
lemma createObject_setCTE_commute:
  ...
  apply (case_tac ty,
         simp_all add: Types_H.toAPIType_def ArchTypes_H.toAPIType_def)
...
lemma createObject_updateMDB_commute:
  "monad_commute
     ((\<lambda>s. src \<noteq> 0 \<longrightarrow> cte_wp_at' (\<lambda>_. True) src s) and
      ...
      K (is_aligned ptr (Types_H.getObjectSize ty us)) and
      K ((Types_H.getObjectSize ty us)< word_bits))
     (updateMDB src f) (RetypeDecls_H.createObject ty ptr us)"
...
lemma updateMDB_pspace_no_overlap':
  ...
  apply (rule hoare_pre)
  apply (clarsimp simp:updateMDB_def split del:if_splits)
...
lemma new_cap_object_comm_helper:
  "monad_commute
     ...
     (RetypeDecls_H.createObject ty ptr us) (insertNewCap parent slot cap)"
  ...
  apply (rule monad_commute_guard_imp)
   ...
   apply (wp updateMDB_weak_cte_wp_at updateMDB_pspace_no_overlap'
             getCTE_wp valid_arch_state'_updateMDB
                CSpace_R.setCTE_weak_cte_wp_at setCTE_pspace_no_overlap')
...
lemma createNewCaps_not_nc:
  "\<lbrace>\<top>\<rbrace>
   createNewCaps ty ptr (Suc (length as)) us 
...
lemma createObjects'_psp_distinct:
  ...
  apply (clarsimp simp:createObjects'_def split_def)
    apply (subst new_cap_addrs_fold')
     ...
     apply unat_arith
    apply (rule hoare_pre)
    apply (wpc|wp|simp add:unless_def alignError_def haskell_assert_def
           del:fun_upd_apply)+
  apply clarsimp
  apply (subst data_map_insert_def[symmetric])+
...
lemma createNewCaps_pspace_no_overlap':
  ...
  apply (subgoal_tac "pspace_no_overlap' 
    (ptr + (1 + of_nat n << (Types_H.getObjectSize ty us)))
  ...
  apply (case_tac ty)
    apply (simp_all add: apiGetObjectSize_def Types_H.getObjectSize_def
     Types_H.toAPIType_def ArchTypes_H.toAPIType_def tcbBlockSizeBits_def
      ArchTypes_H.getObjectSize_def objBits_simps epSizeBits_def aepSizeBits_def
      cteSizeBits_def pageBits_def ptBits_def archObjSize_def pdBits_def
      createObjects_def haskell_fail_def createWordObjects_def)
    apply (rule hoare_pre)
    apply wpc
    apply (clarsimp simp: apiGetObjectSize_def Types_H.getObjectSize_def
     Types_H.toAPIType_def ArchTypes_H.toAPIType_def tcbBlockSizeBits_def
      ArchTypes_H.getObjectSize_def objBits_simps epSizeBits_def aepSizeBits_def
      cteSizeBits_def pageBits_def ptBits_def archObjSize_def pdBits_def
      createObjects_def haskell_fail_def createWordObjects_def Arch_createNewCaps_def curDomain_def
      split:ArchTypes_H.apiobject_type.splits
    ...
    apply ((clarsimp simp: apiGetObjectSize_def Types_H.getObjectSize_def
     Types_H.toAPIType_def ArchTypes_H.toAPIType_def tcbBlockSizeBits_def
      ArchTypes_H.getObjectSize_def objBits_simps epSizeBits_def aepSizeBits_def
      cteSizeBits_def pageBits_def ptBits_def archObjSize_def pdBits_def
      createObjects_def haskell_fail_def createWordObjects_def Arch_createNewCaps_def
      split:ArchTypes_H.apiobject_type.splits
      | wp doMachineOp_psp_no_overlap createObjects'_pspace_no_overlap 
      createObjects'_psp_aligned createObjects'_psp_distinct
      copyGlobalMappings_pspace_aligned' mapM_x_wp_inv
      copyGlobalMappings_pspace_no_overlap' | assumption | clarsimp simp:word_bits_def
...
lemma createNewCaps_ret_len:
  ...
  apply clarsimp
  apply (case_tac ty)
   apply (simp_all add:createNewCaps_def Types_H.toAPIType_def
...
lemma createObjects_Cons:
  ...
  prefer 2
   ...
   apply (rule aligned_add_aligned[OF range_cover.aligned],assumption)
    apply (rule is_aligned_shiftl_self)
   apply (simp add:range_cover_def)
  apply (rule monad_eq_split[where Q ="\<lambda>x s'. s' = s \<and>
    ptr && mask (objBitsKO val + us) = 0"])
...
lemma placeNewObject_doMachineOp_commute:
  ...
  apply (rule commute_name_pre_state)
  apply (rule monad_commute_guard_imp)
  apply (rule commute_rewrite
    [where Q = "pspace_no_overlap' ptr ((objBitsKO (injectKOS ty)) +  us) and pspace_aligned'"])
   apply (rule simpler_placeNewObject_def)
     apply simp+
    apply (wp doMachineOp_psp_no_overlap)
    apply (simp add:modify_specify)
    apply (simp add:modify_mapM_x)
   apply (rule commute_commute)
   apply (rule mapM_x_commute[where f = id])
   apply (rule doMachineOp_upd_heap_commute)
   apply wp
...
lemma doMachineOp_ksArchState_commute:
  ...
  apply (clarsimp simp:monad_commute_def gets_def 
    return_def get_def bind_def)
  apply (intro conjI set_eqI iffI)
      apply (clarsimp simp:SUP1_iff doMachineOp_def select_f_def gets_def
        get_def bind_def return_def simpler_modify_def)
       apply (erule bexI[rotated])
        apply clarsimp
     apply (clarsimp simp:SUP1_iff doMachineOp_def select_f_def gets_def
      get_def bind_def return_def simpler_modify_def)
...
lemma createNewCaps_Cons:
  ...
  and "ptr \<noteq> 0"
  shows "createNewCaps ty ptr (Suc (Suc n)) us s
 = (do x \<leftarrow> createNewCaps ty ptr (Suc n) us;
      r \<leftarrow> RetypeDecls_H.createObject ty
             (((1 + of_nat n) << Types_H.getObjectSize ty us) + ptr) us;
      return (x @ [r])
    od) s"
proof -
  note if_weak_cong[cong]
  ...
  apply (frule(3) pspace_no_overlap'_tail)
   apply simp
  apply (drule_tac ptr = "ptr + ?x"
         in pspace_no_overlap'_le[where sz' = "Types_H.getObjectSize ty us"])
    apply (simp add:range_cover_def word_bits_def)
   apply (erule range_cover.sz(1)[where 'a=32, folded word_bits_def])
  apply (simp add: createNewCaps_def)
  apply (case_tac ty)
        apply (simp add: Types_H.toAPIType_def ArchTypes_H.toAPIType_def
                         Arch_createNewCaps_def)
        apply (case_tac apiobject_type)
            apply (simp_all add: bind_assoc Types_H.toAPIType_def
                                 ArchTypes_H.toAPIType_def)
            -- Untyped
            apply (simp add: cteSizeBits_def pageBits_def
              ...
              createObjects_def ArchRetype_H.createObject_def
       ...
       apply (subst gsUserPages_update gsCNodes_update
              ...
              | simp add: empty_fail_mapM_x empty_fail_clearMemory
                          modify_modify_bind o_def)+
       apply (rule fun_cong[where x=s])
       apply (rule arg_cong2[where f=bind, OF refl ext])+
       apply (simp add: bind_assoc[symmetric])
...
-- "PageDirectoryObject"
         ...
         apply (subst monad_eq,rule createObjects_Cons)
               apply ((simp add: field_simps shiftl_t2n pageBits_def archObjSize_def
                 getObjectSize_def ArchTypes_H.getObjectSize_def pdBits_def
                 objBits_simps ptBits_def)+)[6]
         ...
         apply (rule_tac Q = "\<lambda>r s. valid_arch_state' s \<and> 
           (\<forall>x\<le>of_nat n. page_directory_at' (ptr + (x << 14)) s) \<and> ?Q s" in monad_eq_split)
           apply (rule sym)
...
lemma createNewObjects_def2:
  "\<lbrakk>dslots \<noteq> []; length ( dslots ) < 2^word_bits;
   ...
   \<Longrightarrow> createNewObjects ty parent dslots ptr us s =
       insertNewCaps ty parent dslots ptr us s"
  ...
  show "zipWithM_x
        (\<lambda>num slot.
            RetypeDecls_H.createObject ty ((num << Types_H.getObjectSize ty us) + ptr) us >>=
            insertNewCap parent slot)
        [0.e.of_nat (length ys)] (y # ys) s =
       (createNewCaps ty ptr (Suc (length ys)) us >>= zipWithM_x (insertNewCap parent) (y # ys)) s"
    ...
    next
      ...
      show ?case
       ...
       apply (rule_tac Q = "(\<lambda>r s. 
         (\<forall>cap\<in>set r. cap \<noteq> capability.NullCap) \<and>
         ...
         (Types_H.getObjectSize ty us) s
         \<and> valid_pspace' s \<and> valid_arch_state' s \<and> ?Q1 r s)" in monad_eq_split)
       apply (subst append_Cons[symmetric])
       apply (subst zipWithM_x_append1)
        apply clarsimp
        apply assumption
  ...
  apply (frule range_cover_le[where n = "Suc (length as)"])
   apply simp+
  using psp_no_overlap caps_r valid_psp unt_at
    caps_no_overlap valid_arch_state
...
assumes check: "distinct dslots"
  ...
  and       c: "corres r P P' f (insertNewCaps ty parent dslots ptr us)"
 ...
 shows "corres r P P' f (createNewObjects ty parent dslots ptr us)"
...
lemma createNewObjects_wp_helper:
  ...
  and   cover: "range_cover ptr sz (Types_H.getObjectSize ty us) (length dslots)"
  and   not_0: "ptr \<noteq> 0" "length dslots \<noteq> 0"
  shows "\<lbrace>P\<rbrace> insertNewCaps ty parent dslots ptr us \<lbrace>Q\<rbrace>
  ...
  \<rbrace> (createNewObjects ty parent dslots ptr us) \<lbrace>Q\<rbrace>"
  using assms
  apply (clarsimp simp:valid_def)
  apply (drule_tac x = s in spec)
  apply (frule range_cover.range_cover_le_n_less(1)[where 'a=32, folded word_bits_def, OF _ le_refl])
...
lemma createObject_def3:
  "createObject =
   (\<lambda>ty ptr us. createNewCaps ty ptr (Suc 0) us >>= (\<lambda>m. return (hd m)))"
  apply (rule ext)+
...
lemma ArchCreateObject_pspace_no_overlap':
  "\<lbrace>\<lambda>s. pspace_no_overlap'
   ...
   ArchRetypeDecls_H.createObject ty
     (ptr + (of_nat n << APIType_capBits ty userSize)) userSize 
  ...
  apply (frule pspace_no_overlap'_le2)
   ...
   apply (clarsimp simp:add_assoc[symmetric])
  ...
  apply (clarsimp simp:field_simps pageBits_def
    pdBits_def
    word_bits_conv archObjSize_def ptBits_def
...
lemma createObject_pspace_no_overlap':
  "\<lbrace>\<lambda>s. pspace_no_overlap'
   ...
   createObject ty (ptr + (of_nat n << APIType_capBits ty userSize)) userSize 
  ...
  apply (rule hoare_pre)
   ...
   apply wpc
      ...
      apply (simp add:APIType_capBits_def objBits_simps
        pageBits_def toAPIType_def ArchTypes_H.toAPIType_def
...
lemma createObject_pspace_aligned_distinct':
  ...
  apply (rule hoare_pre)
  apply (wp placeNewObject_pspace_aligned'
      placeNewObject_pspace_distinct'
...
lemma createNewObjects_Cons:
  ...
  shows "createNewObjects ty src (dest @ [lt]) ptr us = 
  do createNewObjects ty src dest ptr us;
     (RetypeDecls_H.createObject ty ((of_nat (length dest) << APIType_capBits ty us) + ptr) us
       >>= insertNewCap src lt)
  od"
  proof -
    from dlength
...
lemma createNewObjects_pspace_no_overlap':
  ...
  and K (ptr \<noteq> 0) 
  and K (ty = APIObjectType ArchTypes_H.apiobject_type.CapTableObject \<longrightarrow> us < 28)\<rbrace>
  createNewObjects ty src dests ptr us 
   ...
   show ?case
     ...
     apply (rule hoare_pre)
      ...
      apply (rule hoare_vcg_conj_lift)
       ...
       apply (simp add:snoc)+
      apply wp
     apply (simp add:conj_ac field_simps)
