...
lemma  valid_objs: "valid_objs' s'"
  ...
  using invs
  by (auto simp: invs'_def valid_pspace'_def valid_mdb'_def valid_mdb_ctes_def)

abbreviation base_bits :: "obj_ref set" where
...
abbreviation replies' :: "obj_ref \<rightharpoonup> reply" where
  "replies' \<equiv> pspace' |> reply_of'"

lemma ko_wp_at'[simp]:
  "\<And>P p. (ko_wp_at' P p state') = (ko_wp_at' P p s' \<and> p \<notin> base_bits)"
  by (fastforce simp: ko_wp_at_delete'[OF pd])
...
lemma global_refs:
  ...
  apply (fastforce simp: field_simps mask_def)
  done
...
proof -
  ...
  have arch:
    "\<And> ko p. \<lbrakk> ksPSpace s' p = Some (KOArch ko); p \<in> mask_range base bits \<rbrakk> \<Longrightarrow> 6 \<le> bits"
    using sr vs vu
    ...
    apply (subst (asm) word_range_card)
     ...
     apply (erule(1) pspace_alignedD)
    apply (subst (asm) word_range_card)
     apply (rule is_aligned_no_overflow_mask[OF al])
    apply (rule ccontr)
    apply (simp add:not_le)
    apply (prop_tac "obj_bits koa < word_bits")
    ...
    apply (case_tac koa, simp_all add: ep_relation_cut_def ntfn_relation_cut_def
  ...
  thus ?thesis
    ...
    apply (intro conjI; blast?)
     apply (fastforce simp: add_mask_fold dest!: live_notRange)
    by (auto simp: add_mask_fold)
qed
end
...
lemma live_idle_untyped_range':
  ...
  apply (case_tac "ko_wp_at' live' p s'")
   ...
   apply (erule ex_nonz_cap_notRange)
  apply clarsimp
  apply (insert invs_valid_global'[OF invs] cap valid_idle' idle_is_global)
  apply (clarsimp simp: cte_wp_at_ctes_of)
...
lemma list_refs_of_replies_live':
  ...
  apply (clarsimp simp: ko_wp_at'_def list_refs_of_replies'_def list_refs_of_reply'_def pspace_bounded'_def
                        pspace_aligned'_def pspace_distinct'_def get_refs_def
                 elim!: opt_mapE
                 split: option.splits)
    by (intro conjI impI allI | force simp: live'_def live_reply'_def)+
...
lemma replyPrev_list_refs_of_replies:
  ...
  by (clarsimp simp: list_refs_of_replies'_def list_refs_of_reply'_def opt_map_def
                     obj_at'_def
              split: option.splits)
...
lemma valid_obj':
  ...
  apply (case_tac obj, simp_all add: valid_obj'_def)
     ...
     apply (clarsimp simp: valid_tcb'_def valid_arch_tcb'_def ko_wp_at'_def gen_objBits_simps)
     apply (rule conjI)
     ...
     apply (intro conjI)
         subgoal
           by (clarsimp simp: valid_bound_tcb'_def tcb_bound_refs'_def
                       split: option.splits thread_state.splits)
        apply (clarsimp simp: valid_bound_ntfn'_def tcb_bound_refs'_def split: option.splits)
       apply (clarsimp simp: valid_bound_sc'_def tcb_bound_refs'_def split: option.splits)
      apply (clarsimp simp: none_top_bool_cases)
      apply (rename_tac prev)
      apply (cut_tac P=live' and p=prev in live_notRange; fastforce?)
      apply (fastforce dest: sym_heapD2[where p'=p] simp: opt_map_def ko_wp_at'_def obj_at'_def live'_def)
     apply (clarsimp simp: none_top_bool_cases)
     apply (rename_tac "next")
     apply (cut_tac P=live' and p="next" in live_notRange; fastforce?)
     apply (fastforce dest!: sym_heapD1[where p=p] simp: opt_map_def ko_wp_at'_def obj_at'_def live'_def)
    apply (clarsimp simp: valid_cte'_def)
    apply (rule_tac p=p in valid_cap2)
    apply (clarsimp simp: ko_wp_at'_def gen_objBits_simps cte_level_bits_def[symmetric])
  ...
  apply (rule conjI; (solves \<open>clarsimp split: option.splits\<close>)?)+
   ...
   using sym_refs_def live_notRange list_refs_of_replies_live' subgoal by fastforce
  ...
  apply (frule replyNext_list_refs_of_replies[rotated], simp)
   apply (simp add: obj_at'_def)
  using sym_refs_def live_notRange list_refs_of_replies_live' by fastforce
...
lemma sc_tcb_not_idle_thread':
  "\<lbrakk> pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s';  ksPSpace s' scp = Some (KOSchedContext sc);
...
lemma thread_not_idle_implies_sc_not_idle':
  ...
  apply (drule if_live_then_nonz_capE'[where p=idle_sc_ptr])
   apply (fastforce simp: ko_wp_at'_def live'_def live_sc'_def state_refs_of'_def
...
lemma state_refs:
  ...
  apply (case_tac ko; simp)
     apply (fastforce simp: ko_wp_at'_def live'_def live_ntfn'_def state_refs_of'_def
                     split: ntfn.splits)
    ...
    apply (fastforce simp: ko_wp_at'_def tcb_bound_refs'_def live'_def)
   apply (frule (6) sc_tcb_not_idle_thread')
     apply (fastforce simp: state_refs_of'_def)
    apply (fastforce simp: state_refs_of'_def)
   apply (clarsimp simp: ko_wp_at'_def live'_def live_sc'_def  valid_idle'_def)
  apply (clarsimp simp: ko_wp_at'_def live'_def live_reply'_def)
  done
...
proof (simp add: invs'_def valid_pspace'_def
  ...
  show "valid_objs' ?s" using valid_objs sym_sched assms
  ...
  show "valid_replies' ?s" using vreplies assms
    ...
    apply (prop_tac "tptr \<notin> base_bits")
     ...
     apply (fastforce simp: ko_wp_at'_def pred_tcb_at'_def obj_at'_def live'_def)
  ...
  show "sym_heap (pspace' |> tcb_of' |> tcbSchedNext) (pspace' |> tcb_of' |> tcbSchedPrev)"
    using pa pd bd pspace_distinct'_state'
  ...
  from vsp show "valid_sched_pointers_2
                   (pspace' |> tcb_of' |> tcbSchedPrev)
                   (pspace' |> tcb_of' |> tcbSchedNext)
                   (\<lambda>t. (tcbQueued |< (pspace' |> tcb_of')) t
...
lemma detype_ReplyPrevs_of:
  ...
  apply (prop_tac "\<And>p reply_ptr. (replyPrevs_of s' p = Some reply_ptr) \<Longrightarrow> p \<notin> S")
   apply (clarsimp simp: opt_map_def split: option.splits)
   apply (drule_tac x=p in spec)
   apply (clarsimp simp: ko_wp_at'_def pred_neg_def live'_def live_reply'_def
                  split: kernel_object.splits)
   using pspace_alignedD' pspace_distinctD' pspace_boundedD' apply clarsimp
  by (force simp: vs_all_heap_simps opt_map_def in_opt_map_eq
           split: option.splits)
...
lemma detype_tcbInReleaseQueue:
  "\<lbrakk>pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s'; \<forall>p. p \<in> S \<longrightarrow> \<not> ko_wp_at' live' p s'\<rbrakk>
   \<Longrightarrow> (tcbInReleaseQueue |< ((\<lambda>x. if x \<in> S then None else ksPSpace s' x) |> tcb_of'))
       = (tcbInReleaseQueue |< tcbs_of' s')"
  using pspace_alignedD' pspace_distinctD' pspace_boundedD'
  by (fastforce simp: opt_pred_def opt_map_def ko_wp_at'_def live'_def split: option.splits)

lemma detype_sc_replies_relation:
  "\<lbrakk>pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s';
    \<forall>p. p \<in> {lower..upper} \<longrightarrow> \<not> ko_wp_at' live' p s';
    sc_replies_relation s s'; upper = upper'\<rbrakk>
...
lemma detype_ep_queues_relation:
  "\<lbrakk>pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s';
    \<forall>p. p \<in> {lower..upper} \<longrightarrow> \<not> ko_wp_at' live' p s';
    ep_queues_relation s s'; upper = upper'\<rbrakk>
   \<Longrightarrow> ep_queues_relation_2 (ep_queues_of (detype {lower..upper'} s))
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> ep_of' ||> epQueue)
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> tcb_of' |> tcbSchedNext)
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> tcb_of' |> tcbSchedPrev)"
  apply (clarsimp simp: ep_queues_relation_def detype_def)
  apply (frule detype_tcbSchedNexts_of[where S="{lower..upper}"]; simp)
  apply (frule detype_tcbSchedPrevs_of[where S="{lower..upper}"]; simp)
  apply (clarsimp simp: vs_all_heap_simps opt_map_def
                 split: if_splits option.splits)
  done

lemma detype_ntfn_queues_relation:
  "\<lbrakk>pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s';
    \<forall>p. p \<in> {lower..upper} \<longrightarrow> \<not> ko_wp_at' live' p s';
    ntfn_queues_relation s s'; upper = upper'\<rbrakk>
   \<Longrightarrow> ntfn_queues_relation_2 (ntfn_queues_of (detype {lower..upper'} s))
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> ntfn_of' ||> ntfnQueue)
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> tcb_of' |> tcbSchedNext)
                         ((\<lambda>x. if lower \<le> x \<and> x \<le> upper
                               then None else ksPSpace s' x) |> tcb_of' |> tcbSchedPrev)"
  apply (clarsimp simp: ntfn_queues_relation_def detype_def)
  apply (frule detype_tcbSchedNexts_of[where S="{lower..upper}"]; simp)
  apply (frule detype_tcbSchedPrevs_of[where S="{lower..upper}"]; simp)
  apply (clarsimp simp: vs_all_heap_simps opt_map_def
                 split: if_splits option.splits ntfn.splits)
  done
...
lemma detype_ready_queues_relation:
  ...
  apply (clarsimp simp: detype_def ready_queues_relation_def Let_def)
  apply (frule (2) detype_tcbSchedNexts_of[where S="{lower..upper}"]; simp)
  apply (frule (2) detype_tcbSchedPrevs_of[where S="{lower..upper}"]; simp)
  apply (frule (2) detype_inQ[where S="{lower..upper}"]; simp)
  done

lemma detype_release_queue_relation:
  "\<lbrakk>pspace_aligned' s'; pspace_distinct' s'; pspace_bounded' s';
    \<forall>p. p \<in> {lower..upper} \<longrightarrow> \<not> ko_wp_at' live' p s';
    release_queue_relation s s'; upper = upper'\<rbrakk>
   \<Longrightarrow> release_queue_relation_2
        (release_queue (detype {lower..upper'} s))
        (ksReleaseQueue s')
        ((\<lambda>x. if lower \<le> x \<and> x \<le> upper then None
              else ksPSpace s' x) |>
         tcb_of' |>
        ...
        ((\<lambda>x. if lower \<le> x \<and> x \<le> upper then None
         ...
         tcb_of' |>
         tcbSchedPrev)
        (tcbInReleaseQueue |< ((\<lambda>x. if lower \<le> x \<and> x \<le> upper then None else ksPSpace s' x) |> tcb_of'))"
  apply (clarsimp simp: detype_def release_queue_relation_def)
  apply (frule (2) detype_tcbSchedNexts_of[where S="{lower..upper}"]; simp)
  apply (frule (2) detype_tcbSchedPrevs_of[where S="{lower..upper}"]; simp)
  apply (frule (2) detype_tcbInReleaseQueue[where S="{lower..upper}"]; simp)
  done
...
lemma valid_untyped_no_overlap:
  ...
  apply (elim disjE)
    ...
    apply (clarsimp simp:p_assoc_help mask_def)
   apply (force simp: mask_def add_diff_eq valid_pspace'_def pspace_bounded'_def)+
  done
...
lemma pspace_no_overlap'_lift:
  ...
  assumes ps :"\<lbrace>Q and pspace_aligned' and pspace_distinct' and pspace_bounded'\<rbrace>
               f
               \<lbrace>\<lambda>_ s. pspace_aligned' s \<and> pspace_distinct' s \<and> pspace_bounded' s\<rbrace>"
  shows "\<lbrace>Q and pspace_aligned' and pspace_distinct' and pspace_bounded' and pspace_no_overlap' ptr sz\<rbrace>
         f
         \<lbrace>\<lambda>_. pspace_no_overlap' ptr sz\<rbrace>"
...
lemma simpler_updateObject_def:
  ...
  by (intro conjI;
      clarsimp simp: alignCheck_def unless_def when_def not_less[symmetric]
                     ...
                     cte_update_def return_def tcbSlot_defs gen_objBits_simps cteSizeBits_def
...
lemma cte_wp_at_top:
  ...
  by (case_tac b;
      simp add: typeError_def read_typeError_def obind_def omonad_defs cte_check_def
                ...
                gen_objBits_simps tcbSlot_defs cteSizeBits_def objBits_simps'
         split: option.split;
      fastforce)
...
proof -
  ...
  note [simp del] = usableUntypedRange.simps atLeastAtMost_simps
...
lemma locateCTE_ko_wp_at':
  ...
  apply (rename_tac ko, case_tac ko)
    apply (simp_all add: cte_check_def)
    apply (clarsimp simp: lookupAround2_char1 gen_objBits_simps cte_update_def)
    apply (drule(1) pspace_distinctD')+
    apply (simp add: objBits_simps' word_bits_def)
  apply (clarsimp simp: gen_objBits_simps cte_update_def)
  apply (drule(1) pspace_distinctD')+
  apply (simp add: objBits_simps' word_bits_def)
  done
...
lemma getTCB_det:
  ...
  by (clarsimp simp: ko_wp_at'_def getObject_def split_def gets_the_def
                     ...
                     no_ofailD[OF no_ofail_tcb_at'_readObject] obj_at'_def
...
lemma threadSet_det:
  ...
  apply (clarsimp simp: threadSet_def bind_def obj_at'_def)
  ...
  apply (clarsimp simp: read_alignCheck_def omonad_defs alignCheck_def unless_def when_def
                        gets_the_def is_aligned_mask gen_objBits_simps)
...
proof -
    ...
    have hint:
      ...
      apply (clarsimp simp add: monad_commute_def bind_def simpler_modify_def return_def)
      apply (clarsimp simp: threadSet_det simpler_modify_def)
      apply (prop_tac "tcb_at' ptr (ksPSpace_update (\<lambda>ps. ps(a \<mapsto> cte_update cte (the (ps a)) src a)) s)")
       apply (clarsimp simp:obj_at'_def)
       apply (intro conjI impI)
            apply simp
           apply (clarsimp split: Structures_H.kernel_object.split_asm)
...
lemma placeNewObject_tcb_at':
  ...
  apply wpsimp
  apply (clarsimp simp: obj_at'_def lookupAround2_None1 gen_objBits_simps
...
lemma createObject_updateMDB_commute:
  "monad_commute
     ((\<lambda>s. src \<noteq> 0 \<longrightarrow> cte_wp_at' (\<lambda>_. True) src s) and
      ...
      K (ty = APIObjectType ArchTypes_H.apiobject_type.SchedContextObject \<longrightarrow> sc_size_bounds us))
...
lemma new_cap_object_commute:
  ...
  apply (rule monad_commute_guard_imp)
   ...
   apply (wpsimp wp: updateMDB_weak_cte_wp_at updateMDB_pspace_no_overlap' getCTE_wp
                     updateMDB_valid_arch_state' setCTE_weak_cte_wp_at setCTE_pspace_no_overlap')
...
proof -
  note no_simps [simp del] = untyped_range.simps usable_untyped_range.simps atLeastAtMost_simps
...
lemma createObjects_setDomain_commute:
  ...
  apply (clarsimp simp: gen_objBits_simps typ_at'_def obj_at'_def ko_wp_at'_def
  ...
  next
    ...
    show ?case
      ...
      apply (auto simp: kscd min_sched_bits[unfolded sc_size_bounds_def]
                  dest: object_type_inject[THEN iffD1])
      done
  qed
qed
...
lemma createNewObjects_wp_helper:
  ...
  and K (ty = APIObjectType ArchTypes_H.apiobject_type.SchedContextObject \<longrightarrow> sc_size_bounds us))
...
lemma createObject_pspace_no_overlap':
  ...
  by (auto simp: word_shiftl_add_distrib field_simps shiftl_t2n elim: range_cover_le)
     (auto simp: gen_objBits_simps APIType_capBits_generic APIType_capBits_gen_def scBits_simps)
