 ...
 *)
theory Ipc_R
imports ArchFinalise_R Reply_R
begin
...
lemma doNormalTransfer_corres:
  ...
  (tcb_at' sender and tcb_at' receiver and valid_objs' and valid_replies'
   ...
   and pspace_in_kernel_mappings' and valid_mdb' and no_0_obj'
   and (\<lambda>s. case ep of Some x \<Rightarrow> ep_at' x s | _ \<Rightarrow> True)
...
context begin interpretation Arch . (*FIXME: rt arch-split, needs make_fault_msg lemmmas*)
...
crunch update_sk_obj_ref, set_simple_ko, update_sched_context, update_reply
...
lemma cap_in_tcbFaultHandlerSlot:
  ...
  apply (rule conjI)
   ...
   apply (rule disjI2)
   apply (rule_tac x="tcbFaultHandlerSlot << cteSizeBits" in exI)
   apply (fastforce simp: shiftl_t2n[symmetric] obj_at'_def gen_objBits_simps
                          cteSizeBits_cte_level_bits[symmetric])
  ...
  apply (rule_tac x="tcbFaultHandlerSlot << cteSizeBits" in bexI)
   apply (fastforce elim: cte_wp_at_tcbI'
                    simp: obj_at'_def objBitsKO_def)
  apply fastforce
  done
...
lemma cap_in_tcbTimeoutHandlerSlot:
  ...
  apply (rule conjI)
   apply (simp add: cte_wp_at_cases')
   apply (rule disjI2)
   apply (rule_tac x="tcbTimeoutHandlerSlot << cteSizeBits" in exI)
   apply (fastforce simp: shiftl_t2n[symmetric] obj_at'_def gen_objBits_simps
                          cteSizeBits_cte_level_bits[symmetric])
  apply (frule (1) ko_at'_valid_tcbs'_valid_tcb')
  apply (rule conjI)
...
lemma ko_at_cte_tcbTimeoutHandler:
  ...
  apply (erule (2) cte_wp_at_tcbI')
   apply (fastforce simp: shiftl_t2n[symmetric] cteSizeBits_cte_level_bits[symmetric])
  apply simp
  done
...
crunch maybe_donate_sc
  ...
  and ntfn_queues_blocked[wp]: ntfn_queues_blocked
  (wp: ep_queues_blocked_lift ntfn_queues_blocked_lift crunch_wps cong: if_cong)
...
lemma valid_tcb'_SchedContext_update_empty[elim!]:
  ...
  by (auto simp: valid_tcb'_def valid_cap'_def tcb_cte_cases_def tcb_cte_cases_neqs)
...
lemma replyCanGrant_update_valid_replies'_sc_asrt[wp]:
  "updateReply ptr (replyCanGrant_update f) \<lbrace>valid_replies'_sc_asrt ptr\<rbrace>"
...
crunch update_reply
  ...
  and ready_queues_runnable[wp]: ready_queues_runnable
  and release_q_runnable[wp]: release_q_runnable
  (wp: ep_queued_lift ntfn_queued_lift ep_queues_blocked_lift ntfn_queues_blocked_lift
       ready_queues_runnable_lift release_q_runnable_lift)

lemma valid_reply'_grant_update[simp]:
  "valid_reply' (replyCanGrant_update f r) s = valid_reply' r s"
  by (simp add: valid_reply'_def)
...
lemma receiveIPCBlocked_corres:
  "corres dc
     ...
     (receive_ipc_blocked isBlocking thread epptr replyOpt canGrant)
     (receiveIPCBlocked isBlocking thread epptr replyOpt canGrant)"
  ...
  apply (cases isBlocking; clarsimp)
   apply (rule corres_guard_imp)
     ...
     apply (wpsimp wp: hoare_TrueI hoare_vcg_if_lift2 hoare_vcg_imp_lift')
...
lemma receiveIPC_corres:
  ...
  apply (rule corres_assert_gen_asm_cross_forwards)
   apply (cases reply_cap; clarsimp simp: is_reply_cap_def gen_isCap_simps)
  apply (rule stronger_corres_guard_imp)
        ...
        apply (rule corres_split[OF getBoundNotification_corres])
          apply (rule_tac r'="ntfn_relation" in corres_split)
            ...
            apply (rule corres_split[where r'=dc])
                   ...
                   apply (rule corres_split[OF getThreadState_corres])
                     ...
                     apply (rule corres_split[OF doIPCTransfer_corres])
                      ...
                      apply (wpsimp wp: do_ipc_transfer_tcb_caps hoare_vcg_ball_lift
                                        valid_bound_obj_lift do_ipc_transfer_valid_arch) (* FIXME: make do_ipc_transfer_valid_arch wp *)
                     apply (rule_tac Q'="\<lambda>_ s. tcb_at' sender s \<and> \<not> is_sched_linked sender s
                                               ...
                                               \<and> valid_bound_reply' replyOpt s"
                                  in hoare_post_imp)
             ...
             apply (wpsimp wp: hoare_vcg_ball_lift RISCV64.maybe_return_sc_valid_list) (* FIXME: rt arch-split *)
...
lemma receiveSignalBlocked_corres:
  ...
  apply (cases isBlocking; simp)
   apply (rule stronger_corres_guard_imp)
    ...
    apply clarsimp
    apply (frule invs_sym_refs)
    apply (clarsimp cong: conj_cong if_cong)
    apply (intro conjI impI allI; fastforce?)
...
lemma schedContextUpdateConsumed_scReplies_of[wp]:
  ...
  apply (clarsimp simp: opt_map_def if_distrib cong: if_cong)
...
crunch maybeReturnSc, cancelIPC
  ...
  (wp: crunch_wps hoare_drop_imps simp: crunch_simps cong: if_cong)
...
lemma cancelIPC_replyTCBs_of_None:
  ...
  apply (case_tac state; clarsimp)
       ...
       apply (all \<open>wpsimp simp: cancelSignal_def wp: hoare_drop_imps\<close>)
...
lemma cleanReply_sym_heap_scReplies :
  ...
  apply (wpsimp wp: hoare_drop_imp  cong: if_cong | wps)+
  done
...
lemma replyTCB_is_not_ksIdleThread:
  ...
  apply (frule sym_ref_replyTCB_Receive_or_Reply)
    apply blast
   apply fastforce
  apply (clarsimp simp: st_tcb_at'_def gen_obj_at_simps valid_idle'_def idle_tcb'_def)
  done
...
lemma hf_invs'[wp]:
  "\<lbrace>invs' and st_tcb_at' runnable' t\<rbrace>
   handleFault t f
   \<lbrace>\<lambda>_. invs'\<rbrace>"
  apply (simp add: handleFault_def handleNoFaultHandler_def sendFaultIPC_def debugPrint_def)
  apply (rule bind_wp[OF _ stateAssert_sp])
...
lemma doReplyTransfer_corres:
  ...
  apply add_cur_tcb'
  apply add_sym_refs
  supply if_split [split del] subst_all[simp del] opt_mapE[elim!] if_cong[cong]
              ...
              apply (rule corres_split [OF threadget_fault_corres])
                apply (rule corres_split)
                 ...
                 apply (wpsimp wp: set_thread_state_valid_sched_action
                    ...
                    apply (wpsimp wp: hoare_drop_imp RISCV64.handle_fault_reply_valid_list) (* FIXME: rt arch-split *)
                   apply wpsimp
                  apply wpsimp
                 apply wpsimp
