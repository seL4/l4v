...
| "translate_irq_control_invocation (Invocations_A.ArchIRQControl arch_label) =
    (case arch_label of
       ArchIRQControlIssue irq p slot t \<Rightarrow>
         IssueIrqHandler irq (transform_cslot_ptr slot) (transform_cslot_ptr p)
     | IssueSGISignal irq targets ctrl_slot sgi_slot \<Rightarrow>
         ArchIssueIrqHandler (ARMIssueSGISignal irq targets
...
lemma dcorres_arch_invoke_irq_control:
  "dcorres (\<lambda>_. isRight) \<top> (invs and arch_irq_control_inv_valid arch_irq_control_invocation and valid_etcbs)
...
lemma dcorres_invoke_irq_control:
  "dcorres (\<lambda>_. isRight) \<top> (invs and irq_control_inv_valid irq_control_invocation and valid_etcbs)
