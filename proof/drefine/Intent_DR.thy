...
lemma transform_objects_tcb:
  ...
  by (clarsimp dest!: get_tcb_SomeD get_etcb_SomeD
                  simp: transform_objects_def)
...
lemma caps_of_object_update_boundaep [simp]:
  "(\<lambda>n. map_option (\<lambda>(f, _). f (tcb_bound_aep_update stf tcb)) (tcb_cap_cases n)) =
   (\<lambda>n. map_option (\<lambda>(f, _). f tcb) (tcb_cap_cases n))"
  apply (rule ext)
  apply (simp add: tcb_cap_cases_def split: split_if)
  done
...
where
  "generates_pending st \<equiv> case st of
          ...
          | Structures_A.BlockedOnAsyncEvent ptr \<Rightarrow> True
...
apply (auto | drule append_butlast_last_id)+
done
...
lemma not_idle_thread[wp]:
  "\<lbrace>not_idle_thread y\<rbrace> as_user y (set_register a v) \<lbrace>\<lambda>rv. not_idle_thread y\<rbrace>"
  by (simp add:not_idle_thread_def,wp)
