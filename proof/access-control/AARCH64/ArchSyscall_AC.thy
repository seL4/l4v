...
lemma invs_irq_state_update[Syscall_AC_assms, simp]:
  ...
  apply (rule iffI)
   ...
   apply (subst AARCH64.fold_congs(2); fastforce)
...
lemma set_thread_state_restart_to_running_respects[Syscall_AC_assms]:
  ...
  apply (clarsimp simp: integrity_def integrity_asids_def obj_at_def st_tcb_at_def opt_map_def)
  apply (clarsimp dest!: get_tcb_SomeD)
  apply (rule_tac tro_tcb_activate[OF refl refl])
    apply (simp add: tcb_bound_notification_reset_integrity_def arch_tcb_get_registers_def
