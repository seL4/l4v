...
(* Axioms cannot be defined inside the Arch locale. To circumvent this, we define Arch-specific
   copies of the axioms, then hide the original axioms from the generic context. *)

context Arch begin arch_global_naming

(* Define new copies of the axioms that are only visible in the Arch locale *)
lemmas dmo_getESR_reads_respects = dmo_getESR_reads_respects
lemmas dmo_getFAR_reads_respects = dmo_getFAR_reads_respects
lemmas dmo_doSMC_mop_reads_respects = dmo_doSMC_mop_reads_respects
lemmas dmo_addressTranslateS1_reads_respects = dmo_addressTranslateS1_reads_respects

end

(* Hide the original axioms *)
hide_fact
  dmo_getESR_reads_respects
  dmo_getFAR_reads_respects
...
lemma pt_walk_reads_equiv:
  ...
  apply (subgoal_tac "ptes_of s (level_type level) (pt_slot_offset level pt vptr) =
   ...
   apply (drule meta_mp)
    apply (subst (asm) vs_lookup_split_Some[OF order_less_imp_le], rule bit_pred)
      apply fastforce+
...
definition vcpu_save_lrs where
  "vcpu_save_lrs vr \<equiv> do
     lrs <- do num_list_regs <- gets (arm_gicvcpu_numlistregs \<circ> arch_state);
        return [0..<num_list_regs]
     od;
     mapM_x (vcpu_save_lr vr) lrs
   od"
...
locale_abbrev states_equiv_valid_2 ::
  "'l PAS \<Rightarrow> ('l \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> (det_state \<Rightarrow> bool) \<Rightarrow> (det_state \<Rightarrow> bool) \<Rightarrow>
   (det_state,'a) nondet_monad \<Rightarrow> (det_state,'b) nondet_monad \<Rightarrow> bool"
  where
  "states_equiv_valid_2 aag L \<equiv> equiv_valid_2 (\<lambda>_ _. True) (states_equiv_for_labels aag L)
                                                           (states_equiv_for_labels aag L)"

locale_abbrev states_equiv_valid ::
  "'l PAS \<Rightarrow> ('l \<Rightarrow> bool) \<Rightarrow> (det_state \<Rightarrow> bool) \<Rightarrow> (det_state,'a) nondet_monad \<Rightarrow> bool"
  where
  "states_equiv_valid aag L \<equiv> equiv_valid_inv (\<lambda>_ _. True) (states_equiv_for_labels aag L)"

lemma states_equiv_valid_2_invisible:
...
lemma states_equiv_valid_unit_cases':
  ...
  apply clarsimp
  apply (rule states_equiv_valid_invisible[where P="\<top>", simplified])
    apply (fastforce simp: modifies_at_most_def equiv_but_for_labels_def elim: states_equiv_for_guard_imp)
   apply simp
  apply simp
  done

lemmas states_equiv_valid_unit_cases = states_equiv_valid_unit_cases'[OF _ modifies_at_mostI]
...
lemma set_object_states_equiv_valid:
  "states_equiv_valid aag L \<top> (set_object ptr obj)"
...
lemma set_vcpu_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (set_vcpu vr vcpu)"
  ...
  apply (rule_tac P'="vcpu_at vr" in equiv_valid_guard_necessary)
   apply (rule wp_pre)
    ...
    apply wp
   apply (clarsimp simp: obj_at_def)
  apply (wpsimp wp: set_object_states_equiv_valid)
  done

lemma get_vcpu_states_equiv_valid[wp]:
  "states_equiv_valid aag L (K (L (pasObjectAbs aag vr))) (get_vcpu vr)"
...
lemma vcpu_update_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (vcpu_update vr f)"
  unfolding vcpu_update_def
  apply (rule wp_pre)
   apply (rule_tac ptr=vr in states_equiv_valid_unit_cases)
     apply wpsimp+
  done
...
lemma do_machine_op_states_equiv_valid':
  assumes equiv_dmo:
  ...
  assumes guard:
    "\<And>s. P s \<Longrightarrow> Q (numlistregs_for (L o pasObjectAbs aag) s) (cur_vcpu_for (L o pasObjectAbs aag) s)
                    ...
                    (machine_state s)"
  shows
    "states_equiv_valid aag L P (do_machine_op f)"
...
lemma do_machine_op_states_equiv_valid:
  ...
  shows
    "states_equiv_valid aag L \<top> (do_machine_op f)"
 apply (rule do_machine_op_states_equiv_valid'[where Q="\<lambda>_ _ _. \<top>"])
...
lemma readVCPUHardwareReg_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. cur_vcpu_for (L o pasObjectAbs aag) s = Some b \<and>
                                 (vcpuRegSavedWhenDisabled reg \<longrightarrow> b))
                      (do_machine_op (readVCPUHardwareReg reg))"
  unfolding readVCPUHardwareReg_def
  apply (wpsimp wp: do_machine_op_states_equiv_valid')
...
lemma dmo_mol_states_equiv_valid:
  "states_equiv_valid aag L \<top> (do_machine_op (machine_op_lift mop))"
  apply (rule do_machine_op_states_equiv_valid)
    apply (rule equiv_valid_guard_imp[OF machine_op_lift_ev])
    apply simp
   apply wp+
  done

lemma writeVCPUHardwareReg_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op (writeVCPUHardwareReg reg val))"
  unfolding writeVCPUHardwareReg_def dmo_distr
  apply (wpsimp wp: dmo_mol_states_equiv_valid)
    apply (wpsimp wp: do_machine_op_states_equiv_valid' modify_ev)
   apply wpsimp
...
lemma vcpu_save_reg_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. current_vcpu s = Some (vr,b) \<and> (vcpuRegSavedWhenDisabled reg \<longrightarrow> b))
                      (vcpu_save_reg vr reg)"
  (is "states_equiv_valid _ _ ?P _")
  unfolding vcpu_save_reg_def
  apply (rule wp_pre)
   apply (rule_tac P="?P" and ptr=vr in states_equiv_valid_unit_cases)
     apply wpsimp+
  done
...
lemma get_gic_vcpu_ctrl_hcr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. cur_vcpu_for (L o pasObjectAbs aag) s = Some True)
                      (do_machine_op (get_gic_vcpu_ctrl_hcr))"
...
lemma get_gic_vcpu_ctrl_vmcr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. cur_vcpu_for (L o pasObjectAbs aag) s = Some b)
                      (do_machine_op (get_gic_vcpu_ctrl_vmcr))"
...
lemma get_gic_vcpu_ctrl_apr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. cur_vcpu_for (L o pasObjectAbs aag) s = Some b)
                      (do_machine_op (get_gic_vcpu_ctrl_apr))"
...
lemma get_gic_vcpu_ctrl_lr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. (\<exists>b. cur_vcpu_for (L o pasObjectAbs aag) s = Some b) \<and>
...
lemma vgic_update_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (vgic_update vr f)"
  unfolding vgic_update_def by wp

lemma vgic_lr_update_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (vgic_update_lr vr reg irq)"
  unfolding vgic_update_lr_def by wp

lemma vcpu_save_hcr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,True))
                      (vcpu_save_hcr vr)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma vcpu_save_vmcr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. current_vcpu s = Some (vr,b))
                      (vcpu_save_vmcr vr)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma vcpu_save_apr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>b. current_vcpu s = Some (vr,b))
                      (vcpu_save_apr vr)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma vcpu_save_lr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. (\<exists>b. current_vcpu s = Some (vr,b)) \<and> valid_numlistregs s \<and> reg < numlistregs s)
                      (vcpu_save_lr vr reg)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma vcpu_save_lrs_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. (\<exists>b. current_vcpu s = Some (vr,b)) \<and> valid_numlistregs s)
                      (vcpu_save_lrs vr)"
  (is "states_equiv_valid _ _ ?P _")
  unfolding vcpu_save_lrs_def
  apply (rule wp_pre)
   apply (rule_tac P="?P" and ptr=vr in states_equiv_valid_unit_cases)
     apply (wpsimp wp: mapM_x_ev)
         apply (rule wp_pre)
          apply wpsimp
         apply (prop_tac "\<forall>x \<in> set rv. x < numlistregs s")
          apply (erule conjunct1)
         apply clarsimp
         apply assumption
        apply wpsimp+
     apply (fastforce simp: reads_equiv_def2 affects_equiv_def2 states_equiv_for_def equiv_hyp_def equiv_for_def)
    apply (wpsimp wp: mapM_x_wp_inv simp: vcpu_save_lr_def get_gic_vcpu_ctrl_lr_def dmo_distr)+
  done
...
lemma check_export_arch_timer_states_equiv_valid'':
  "states_equiv_valid aag L (\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None)
                      (do_machine_op check_export_arch_timer)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma check_export_arch_timer_states_equiv_valid':
  "states_equiv_valid aag L (\<lambda>s. \<exists>vr. cur_vcpu_for (L o pasObjectAbs aag) s = Some True)
                      (do_machine_op check_export_arch_timer)"
  (is "states_equiv_valid _ _ ?P _")
  unfolding check_export_arch_timer_def dmo_distr
  apply (rule_tac P'="?P" in bind_ev_pre)
...
lemma check_export_arch_timer_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<exists>vr. current_vcpu s = Some (vr,True))
                      (do_machine_op check_export_arch_timer)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma save_virt_timer_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,True))
                      (save_virt_timer vr)"
  (is "states_equiv_valid _ _ ?P _")
  unfolding save_virt_timer_def
  apply (wpsimp wp: vcpu_save_reg_states_equiv_valid)
  apply auto
  done

lemma dmo_dsb_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op dsb)"
  unfolding dsb_def
  by (wpsimp wp: dmo_mol_states_equiv_valid)

lemma dmo_isb_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op isb)"
  unfolding isb_def
  by (wpsimp wp: dmo_mol_states_equiv_valid)

lemma dmo_setHCR_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op (setHCR val))"
  unfolding setHCR_def
  by (wpsimp wp: dmo_mol_states_equiv_valid)

(* FIXME AARCH64 IF: move *)
lemma mapM_K_bind_discarded:
  "mapM f xs >>= K_bind g = mapM_x f xs >>= K_bind g"
  by (simp add: mapM_x_mapM)

lemma vcpu_save_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = cv \<and> valid_numlistregs s) (vcpu_save cv)"
  apply (simp only: vcpu_save_def fun_app_def mapM_K_bind_discarded bind_assoc[symmetric]
                    vcpu_save_vgic_defs[symmetric])
  apply (wpsimp wp: when_ev mapM_ev mapM_wp_inv)+
  apply auto
  done

lemma dmo_maskInterrupt_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op (maskInterrupt m irq))"
...
lemma enableFpuEL01_states_equiv_valid'':
  "states_equiv_valid aag L (\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None)
                      (do_machine_op enableFpuEL01)"
  (is "states_equiv_valid _ _ ?P _")
...
lemma enableFpuEL01_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<forall>vr b. current_vcpu s = Some (vr,b) \<longrightarrow> b)
                      (do_machine_op enableFpuEL01)"
  (is "states_equiv_valid _ _ ?P _")
  apply (rule_tac P="\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None" in equiv_valid_cases'')
    apply (prop_tac "equiv_hyp (L o pasObjectAbs aag) s t")
...
lemma setSCTLR_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<forall>vr b. current_vcpu s = Some (vr,b) \<longrightarrow> b)
                      (do_machine_op (setSCTLR val))"
  (is "states_equiv_valid _ _ ?P _")
  apply (rule_tac P="\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None" in equiv_valid_cases'')
    apply (prop_tac "equiv_hyp (L o pasObjectAbs aag) s t")
     apply (auto simp: states_equiv_for_def equiv_hyp_def equiv_for_def)[1]
    apply (auto simp: cur_vcpu_for_None' equiv_hyp_def equiv_for_def)[1]
   apply (wpsimp wp: setSCTLR_states_equiv_valid'')
  apply (wpsimp wp: setSCTLR_states_equiv_valid')
  done

lemma set_gic_vcpu_ctrl_hcr_states_equiv_valid'':
  "states_equiv_valid aag L (\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None)
                      (do_machine_op (set_gic_vcpu_ctrl_hcr val))"
  (is "states_equiv_valid _ _ ?P _")
...
lemma set_gic_vcpu_ctrl_hcr_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. \<forall>vr b. current_vcpu s = Some (vr,b) \<longrightarrow> b)
                      (do_machine_op (set_gic_vcpu_ctrl_hcr val))"
  (is "states_equiv_valid _ _ ?P _")
  apply (rule_tac P="\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None" in equiv_valid_cases'')
    apply (prop_tac "equiv_hyp (L o pasObjectAbs aag) s t")
     apply (auto simp: states_equiv_for_def equiv_hyp_def equiv_for_def)[1]
    apply (auto simp: cur_vcpu_for_None' equiv_hyp_def equiv_for_def)[1]
   apply (wpsimp wp: set_gic_vcpu_ctrl_hcr_states_equiv_valid'')
  apply (wpsimp wp: set_gic_vcpu_ctrl_hcr_states_equiv_valid')
  done

lemma vcpu_disable_Some_states_equiv_valid[wp]:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,True)) (vcpu_disable (Some vr))"
...
definition states_equiv_for_non_hyp ::
  "(obj_ref \<Rightarrow> bool) \<Rightarrow> (irq \<Rightarrow> bool) \<Rightarrow> (asid \<Rightarrow> bool) \<Rightarrow>
   (domain \<Rightarrow> bool) \<Rightarrow> det_state \<Rightarrow> det_state \<Rightarrow> bool"
  where
...
lemma vcpu_restore_states_equiv_for_non_hyp[wp]:
  ...
  by (wpsimp wp: states_equiv_for_non_hyp_lift equiv_for_lift
                 equiv_asids_lift equiv_fpu_lift equiv_machine_state_lift)
...
lemma vcpus_of_vcpu_proj:
  "vcpus_of s vr = Some vcpu
   \<Longrightarrow> vcpu_proj UNIV UNIV True True True vr s = Some (vcpu_mask (numlistregs s) vcpu)"
  by (clarsimp simp: vcpu_proj_def cong: if_cong)

lemma vcpu_mask_truncate_helper:
  "vcpu_mask (numlistregs s) (vcpu_state.truncate vcpu) =
   vcpu_state.truncate (vcpu_mask (numlistregs s) vcpu)"
  by (clarsimp simp: vcpu_state.defs)
...
lemma vcpu_restore_helper:
  ...
  apply (rule_tac P'="\<lambda>s. vcpus_of s vr = Some vcpu \<and> valid_arch_state s \<and>
   ...
   apply simp
  apply clarsimp
  apply (clarsimp simp: vcpu_mask_truncate_helper)
  apply (clarsimp simp: vcpu_proj_def vcpu_state.defs cong: if_cong)
  done
...
lemma vcpu_restore_2_helper:
  ...
  unfolding vcpu_restore_2_def
  apply (rule_tac Q'="\<lambda>_ s. vcpu_mask (numlistregs s) (vcpu_state.truncate vcpu)
                             = vcpu_mask (numlistregs s) (vcpu_state (machine_state s))
                          \<and> current_vcpu s = Some (vr,True)" in hoare_strengthen_post)
   apply wp
    apply (rule hoare_vcg_conj_lift)
     apply simp
     apply (wp vcpu_restore_helper[simplified])
    apply simp
    apply wpsimp+
  apply (clarsimp simp: cur_vcpu_of_def)
  done
...
abbreviation states_equiv_for_labels_non_hyp ::
  "'a PAS \<Rightarrow> ('a \<Rightarrow> bool) \<Rightarrow> det_state \<Rightarrow> det_state \<Rightarrow> bool"
  where
...
abbreviation states_equiv_valid_non_hyp where
  "states_equiv_valid_non_hyp aag L P f \<equiv> equiv_valid_inv \<top>\<top> (states_equiv_for_labels_non_hyp aag L) P f"
...
lemma states_equiv_valid_non_hyp_unobservable:
  assumes f:
  ...
  shows
    "states_equiv_valid_non_hyp aag l \<top> (f :: (det_state,unit) nondet_monad)"
...
lemma states_equiv_for_non_hyp_current_vcpu_update[simp]:
  "states_equiv_for_non_hyp P Q R S st (s\<lparr>arch_state := arch_state s\<lparr>arm_current_vcpu := cv\<rparr>\<rparr>) =
   states_equiv_for_non_hyp P Q R S st s"
  by (clarsimp simp: states_equiv_for_non_hyp_def equiv_asids_def equiv_asid_def
                     equiv_for_def cur_fpu_for_def equiv_fpu_def get_tcb_def)
...
lemma vcpu_restore_2_states_equiv_valid:
  "states_equiv_valid aag L (valid_arch_state) (vcpu_restore_2 vr)"
  apply (rule equiv_valid_conseq)
    apply (rule equiv_valid_split)
     apply (subst vcpu_restore_2_def)[1]
     apply (rule states_equiv_valid_non_hyp_unobservable[of _ L aag])
     apply (wpsimp)+
...
lemma vcpu_enable_2_states_equiv_valid:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,False) \<and> valid_arch_state s) (vcpu_enable_2 vr)"
  apply (rule equiv_valid_conseq)
      apply (rule equiv_valid_split)
...
lemma states_equiv_valid_modify_disable:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,True) \<and> L (pasObjectAbs aag vr))
                      (modify (\<lambda>s. s\<lparr>arch_state := arch_state s\<lparr>arm_current_vcpu := Some (vr, False)\<rparr>\<rparr>))"
  apply (wpsimp wp: modify_ev')
...
lemma vcpu_disable_2_states_equiv_valid:
  "states_equiv_valid aag L (\<lambda>s. current_vcpu s = Some (vr,True) \<and> valid_arch_state s) (vcpu_disable_2 vr)"
  unfolding vcpu_disable_2_def
  apply (rule equiv_valid_guard_imp)
   apply (rule states_equiv_valid_unit_cases)
     apply clarsimp
     apply (wp vcpu_disable_Some_states_equiv_valid states_equiv_valid_modify_disable)
     apply simp
    apply (wp modify_current_vcpu_equiv_but_for_labels)
    apply clarsimp
    apply assumption
...
lemma vcpu_switch_states_equiv_valid:
  notes ev2l_bind = equiv_valid_2_bind_pre[where P=\<top> and P'=\<top> and R'="\<top>\<top>", rotated, simplified]
  shows "states_equiv_valid aag L (\<lambda>s. valid_arch_state s) (vcpu_switch vopt)"
  ...
  apply (rule_tac P="\<lambda>s. cur_vcpu_for (L o pasObjectAbs aag) s = None" in equiv_valid_cases'')
   ...
   prefer 2
   apply (wpsimp wp: when_ev vcpu_disable_2_states_equiv_valid
                     vcpu_restore_2_states_equiv_valid vcpu_enable_2_states_equiv_valid gets_ev')
  ...
  apply (rule equiv_valid_2_bind_pre[OF _ gets_any_evrv])
      ...
      apply (rule ev2_case_option)
         apply (rule ev_evrv)
         apply (rule vcpu_restore_2_states_equiv_valid)
        ...
        apply (subst return_bind[where f="\<lambda>_. vcpu_restore_2 _", symmetric],
            ...
            apply (rule_tac P=\<top> and P'=\<top>
                         in states_equiv_valid_2_invisible[OF modifies_at_mostI modifies_at_mostI])
...
lemma vcpu_switch_reads_respects:
  ...
  by (wpsimp wp: reads_respects_from_labels[OF vcpu_switch_states_equiv_valid])
...
lemma perform_vcpu_invocation_reads_respects:
  ...
  by (cases iv; wpsimp wp: associate_vcpu_tcb_reads_respects
                           invoke_vcpu_inject_irq_reads_respects
                           invoke_vcpu_read_register_reads_respects
                           invoke_vcpu_write_register_reads_respects
                           invoke_vcpu_ack_vppi_reads_respects)
...
lemma perform_sgi_invocation_reads_respects:
  "reads_respects aag l \<top> (perform_sgi_invocation api)"
  unfolding perform_sgi_invocation_def sendSGI_def
