...
lemmas bit_minus_one_leq_less = bit0.minus_one_leq_less bit1.minus_one_leq_less
lemmas bit_zero_least = bit0.zero_least bit1.zero_least
...
lemma level_le_2_cases:
  ...
  apply (erule disjE_R)
   ...
   apply (drule meta_mp)
    apply (rule bit_minus_one_leq_less)
     apply (erule order.strict_implies_order)
    apply (erule bit_zero_least)
   apply clarsimp
  apply clarsimp
  done
...
lemma pt_walk_vref_for_levelD:
  ...
  apply (erule_tac x="pptr_from_pte (the (ptes (level_type top_level) (pt_slot_offset top_level pt vref)))"
                in meta_allE)
...
lemma do_user_op_reads_respects_g:
  ...
  apply (rule spec_equiv_valid_guard_imp)
   apply (wpsimp wp: dmo_user_memory_update_reads_respects_g dmo_device_state_update_reads_respects_g
                     dmo_device_state_update_reads_respects_g select_ev dmo_wp)
  apply clarsimp
  apply (rule conjI)
   apply clarsimp
   apply (drule spec)+
...
(* FIXME AARCH64 IF: this is currently used in do_user_op_if_reads_respects_g
   where aag contains a free type variable, so this can't be a locale assumption.
   Investigate whether we can tweak the interface, or otherwise add a comment. *)
