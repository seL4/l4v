...
lemma dmo_enableFpu_reads_respects[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op enableFpu)"
...
lemma dmo_disableFpu_states_equiv_valid[wp]:
  "states_equiv_valid aag L \<top> (do_machine_op disableFpu)"
...
lemma as_user_states_equiv_valid_roa:
  "states_equiv_valid aag L (K (det f \<and> (L (pasObjectAbs aag thread)))) (as_user thread f)"
  apply (simp add: as_user_def split_def)
  apply (rule gen_asm_ev)
  apply (wp set_object_states_equiv_valid select_f_ev gets_the_ev)
  apply (auto simp: equiv_for_def get_tcb_def arch_tcb_context_set_def states_equiv_for_def)
  done

lemma states_equiv_valid_unobservable_unit_return:
  assumes f:
    "\<And>P Q R S st. \<lbrace>states_equiv_for P Q R S st\<rbrace> f \<lbrace>\<lambda>_. states_equiv_for P Q R S st\<rbrace>"
  shows
    "states_equiv_valid aag L \<top> (f::(unit,det_ext) s_monad)"
...
lemma dmo_readFpuState_states_equiv_valid:
  "states_equiv_valid aag L (cur_fpu_for (L o pasObjectAbs aag)) (do_machine_op readFpuState)"
...
lemma save_fpu_state_states_equiv_valid:
  "states_equiv_valid aag L (\<lambda>s. current_fpu s = Some ptr) (save_fpu_state ptr)"
  (is "states_equiv_valid _ _ ?P _")
  unfolding save_fpu_state_def
  apply (rule equiv_valid_guard_imp)
   apply (rule_tac P="?P" and ptr=ptr in states_equiv_valid_unit_cases)
     apply (wpsimp wp: as_user_states_equiv_valid_roa dmo_readFpuState_states_equiv_valid)
...
lemma fpu_save_states_equiv_valid:
  "states_equiv_valid aag L valid_cur_fpu fpu_save"
  ...
  apply (rule_tac P="cur_fpu_for (L o pasObjectAbs aag)" in equiv_valid_cases'')
   ...
   apply (wpsimp wp: maybeM_ev gets_ev'' save_fpu_state_states_equiv_valid
  ...
  apply (rule wp_pre)
   ...
   apply (rule_tac R'="\<top>\<top>"
     ...
     apply (rule equiv_valid_2_guard_imp)
       apply (rule states_equiv_valid_2_invisible)
           apply (rule modifies_at_mostI)
           apply (wpsimp wp: hoare_vcg_imp_lift hoare_vcg_all_lift)
          apply (rule modifies_at_mostI)
          apply (wpsimp wp: hoare_vcg_imp_lift hoare_vcg_all_lift)
         apply (wp | simp)+
  apply (auto simp: cur_fpu_for_def is_arch_cur_fpu_def)
  done
...
lemma switch_local_fpu_owner_equiv_kheap:
  ...
  apply (rule equiv_valid_guard_imp)
   apply (rule_tac B="\<lambda>s t. equiv_for (L \<circ> pasObjectAbs aag) kheap s t \<and>
                            equiv_fpu (L \<circ> pasObjectAbs aag) s t"
               and P'=valid_cur_fpu in bind_ev_general)
  by (wpsimp wp: fpu_restore_equiv_kheap[where P="L o pasObjectAbs aag"]
                 fpu_save_states_equiv_valid[where aag=aag and L=L]
           simp: comp_def states_equiv_for_def equiv_for_disj equiv_fpu_def
      | rule equiv_valid_conseq[where P=valid_cur_fpu] | fastforce)+
...
abbreviation states_equiv_valid_non_fpu where
  "states_equiv_valid_non_fpu aag L P f \<equiv> equiv_valid_inv \<top>\<top> (states_equiv_for_labels_non_fpu aag L) P f"

lemma states_equiv_for_non_fpu_symmetric:
  "states_equiv_for_non_fpu P Q R S s t \<Longrightarrow> states_equiv_for_non_fpu P Q R S t s"
  by (auto simp: states_equiv_for_non_fpu_sym)

lemma states_equiv_valid_non_fpu_unobservable:
  assumes "\<And>P Q R S st. f \<lbrace>states_equiv_for_non_fpu P Q R S st\<rbrace>"
  ...
  assumes "\<And>P. f \<lbrace>\<lambda>s. P (work_units_completed s)\<rbrace>"
  assumes l: "\<And>P. f \<lbrace>\<lambda>s. P (irq_state (machine_state s))\<rbrace>"
  shows "states_equiv_valid_non_fpu aag l \<top> (f :: (det_state,unit) nondet_monad)"
  apply (clarsimp simp: equiv_valid_def2 equiv_valid_2_def)
...
lemma switch_local_fpu_owner_states_equiv_valid[wp]:
  "states_equiv_valid aag L (valid_cur_fpu) (switch_local_fpu_owner new_owner)"
  apply (rule equiv_valid_conseq)
    apply (rule equiv_valid_split)
     apply (rule states_equiv_valid_non_fpu_unobservable[of _ L aag])
...
lemma switch_local_fpu_owner_reads_respects[wp]:
  ...
  apply (rule equiv_valid_guard_imp)
   apply (rule reads_respects_from_labels)
        apply (rule switch_local_fpu_owner_states_equiv_valid)
       apply wpsimp+
  done
