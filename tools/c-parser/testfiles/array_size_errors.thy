...
fun expect_error file expected thy =
  case Exn.capture (IsarInstall.interactive_install file) thy of
    Exn.Res _ => error ("Expected C parser error in " ^ file)
  | Exn.Exn (ERROR msg) =>
...
ML \<open>
  val _ =
    ...
    |> expect_error "errors/unsized_local_array.c"
                    "Array a in function f must have a size or an initialiser"
    |> expect_error "errors/nonconst_array_init.c"
                    "Array a initialised with non-constant expression"
\<close>
