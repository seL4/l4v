...
lemma field_desc_empty [simp]:
  ...
  by (force simp: field_desc_def update_ti_t_def intro: ext)
...
lemma wf_desc_list_append [simp]:
  "wf_desc_list (xs@ys) = (wf_desc_list xs \<and> wf_desc_list ys \<and>
      dt_snd ` set xs \<inter> dt_snd ` set ys = {})"
  by (induct_tac xs, auto)
...
lemma field_lookup_list_empty [simp]:
  "\<And>n. field_lookup_list ts [] n = None"
  by (induct ts, auto)
...
lemma field_lookup_list_append [rule_format]:
  "\<And>n. field_lookup_list (xs@ys) f n = (
            case field_lookup_list xs f n of
...
lemma field_lookup_list_None:
  "\<And>f fs m. f \<notin> dt_snd ` set ts \<Longrightarrow> field_lookup_list ts (f#fs) m = None"
  by (induct ts, clarsimp)
     (case_tac a, auto)

lemma field_lookup_list_Some [rule_format]:
  "\<And>f fs m. f \<in> dt_snd ` set ts \<Longrightarrow> field_lookup_list ts [f] m \<noteq> None"
  by (induct ts, clarsimp)
     (case_tac a, auto)

lemma field_lookup_offset_le:
  shows "\<And>s m n f. field_lookup t f m = Some ((s::'a typ_desc),n) \<Longrightarrow>
            m \<le> n" and
...
proof (induct t and st and ts and x)
  ...
  proof
    ...
    next
      ...
      moreover hence "field_lookup_pair x f m' \<noteq> None"
...
lemma field_lookup_offset2:
  ...
  by (simp add: field_lookup_offset_le
                field_lookup_offset [where m=m, symmetric])

lemma field_lookup_list_offset:
  "(field_lookup_list ts f m = Some (t',m + n)) =
      (field_lookup_list ts f 0 = Some (t',n))"
  by (simp add: field_lookup_offset' [where m'=0])

lemma field_lookup_list_offset2:
  "field_lookup_list ts f m = Some (t',n) \<Longrightarrow>
      field_lookup_list ts f 0 = Some (t',n - m)"
  by (simp add: field_lookup_offset_le
                field_lookup_list_offset [where m=m, symmetric])

lemma field_lookup_list_offset3:
  "field_lookup_list ts f 0 = Some (t',n) \<Longrightarrow>
      field_lookup_list ts f m = Some (t',m + n)"
...
lemma field_lookup_map:
  shows "\<And>f m s. field_lookup t f m = s \<Longrightarrow>
      ...
      "\<And>f m s. field_lookup_struct st f m = s \<Longrightarrow>
          field_lookup_struct (map_td_struct fupd st) f m = map_td_flr fupd s"
          and
...
apply(simp add: field_offset_def)
done
...
lemma sub_tag_sub':
  ...
  by (induct t and ts and xs and x) (auto, force)
...
apply(insert td_set_offset_size' [of t undefined undefined undefined])
apply clarsimp
...
apply(insert td_set_offset_size' [of undefined st undefined undefined])
apply clarsimp
done
...
lemma td_set_export_uinfo':
  "\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set t m \<longrightarrow> (export_uinfo s,n) \<in> td_set (export_uinfo t) m"
  "\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_struct st m \<longrightarrow> (export_uinfo s,n) \<in> td_set_struct (map_td_struct field_norm st) m"
  "\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_list ts m \<longrightarrow> (export_uinfo s,n) \<in> td_set_list (map_td_list field_norm ts) m"
  "\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_pair x m \<longrightarrow> (export_uinfo s,n) \<in> td_set_pair (map_td_pair field_norm x) m"
apply(induct t and st and ts and x)
     apply(auto simp: )
...
lemma td_set_export_uinfo:
  "(\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set t m \<longrightarrow> (export_uinfo s,n) \<in> td_set (export_uinfo t) m) \<and>
    (\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_struct st m \<longrightarrow> (export_uinfo s,n) \<in> td_set_struct (map_td_struct field_norm st) m) \<and>
    (\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_list ts m \<longrightarrow> (export_uinfo s,n) \<in> td_set_list (map_td_list field_norm ts) m) \<and>
    (\<forall>f m n s. ((s::'a typ_info),n) \<in> td_set_pair x m \<longrightarrow> (export_uinfo s,n) \<in> td_set_pair (map_td_pair field_norm x) m)"
...
apply(insert td_set_export_uinfo [of t undefined undefined undefined])
apply clarsimp
done

lemma td_set_field_lookup'':
  "\<forall>s m n. wf_desc t \<longrightarrow> (((s::'a typ_desc),m + n) \<in> td_set t m \<longrightarrow> (\<exists>f. field_lookup t f m =
      Some (s,m+n)))"
  ...
  "\<forall>s m n. wf_desc_list ts \<longrightarrow> (((s::'a typ_desc),m + n) \<in> td_set_list ts m \<longrightarrow> (\<exists>f. field_lookup_list ts f m =
      Some (s,m+n)))"
  "\<forall>s m n. wf_desc_pair x \<longrightarrow> (((s::'a typ_desc),m + n) \<in> td_set_pair x m \<longrightarrow> (\<exists>f. field_lookup_pair x f m =
      Some (s,m+n)))"
apply(induct t and st and ts and x)
     apply clarsimp
 ...
 apply(rule, clarsimp)
  ...
  apply(rule_tac x="a#lista" in exI)
  apply(clarsimp)
 apply clarsimp
 apply(thin_tac "\<forall>s. ?P s")
 apply(drule_tac x=s in spec)
...
lemma td_set_field_lookup_rev'':
  "\<forall>s m n. (\<exists>f. field_lookup t f m =
      Some (s,m+n)) \<longrightarrow> (((s::'a typ_desc),m + n) \<in> td_set t m)"
  "\<forall>s m n. (\<exists>f. field_lookup_struct st f m = Some (s,m+n)) \<longrightarrow>(((s::'a typ_desc),m + n) \<in> td_set_struct st m)"
  "\<forall>s m n. (\<exists>f. field_lookup_list ts f m =
      Some (s,m+n)) \<longrightarrow>  (((s::'a typ_desc),m + n) \<in> td_set_list ts m)"
  "\<forall>s m n. (\<exists>f. field_lookup_pair x f m =
      Some (s,m+n)) \<longrightarrow>  (((s::'a typ_desc),m + n) \<in> td_set_pair x m)"
apply(induct t and st and ts and x)
     apply clarsimp
     apply(case_tac f, clarsimp)
     apply clarsimp
...
lemma td_set_field_lookup_rev':
  "(\<forall>s m n. (\<exists>f. field_lookup t f m =
      Some (s,m+n)) \<longrightarrow> (((s::'a typ_desc),m + n) \<in> td_set t m)) \<and>
    (\<forall>s m n. (\<exists>f. field_lookup_struct st f m = Some (s,m+n)) \<longrightarrow>(((s::'a typ_desc),m + n) \<in> td_set_struct st m
      )) \<and>
    (\<forall>s m n. (\<exists>f. field_lookup_list ts f m =
      Some (s,m+n)) \<longrightarrow>  (((s::'a typ_desc),m + n) \<in> td_set_list ts m)) \<and>
    (\<forall>s m n. (\<exists>f. field_lookup_pair x f m =
      Some (s,m+n)) \<longrightarrow>  (((s::'a typ_desc),m + n) \<in> td_set_pair x m))"
...
apply(insert td_set_field_lookup' [of t undefined undefined undefined])
apply(insert td_set_field_lookup_rev' [of t undefined undefined undefined])
apply clarsimp
...
apply(insert td_set_field_lookup_rev' [of undefined st undefined undefined])
apply clarsimp
apply(case_tac k, clarsimp)
apply(thin_tac "All ?P")
apply(drule_tac x=a in spec)
...
lemma field_lookup_struct_td_simp [simp]:
  "\<not> field_lookup_struct ts f m = Some (TypDesc ts nm, m)"
apply clarsimp
apply(drule td_set_struct_field_lookup_structD)
...
apply(insert td_set_field_lookup_rev' [of undefined undefined xs undefined])
apply clarsimp
apply(case_tac k, clarsimp)
apply(thin_tac "All ?P")
apply(thin_tac "All ?P")
...
apply(insert td_set_field_lookup_rev' [of undefined undefined undefined x])
apply clarsimp
apply(case_tac k, clarsimp)
apply(thin_tac "All ?P")
apply(thin_tac "All ?P")
...
apply(subgoal_tac "wf_size_desc a")
 apply(simp only: wf_size_desc_gt)
apply(subst td_set_wf_size_desc)
apply auto
done
...
apply(induct t and st and ts and x)
 ...
 apply(clarsimp split: option.splits)
   ...
   apply(thin_tac "\<forall>f g. ?P f g")+
   apply(case_tac dt_pair, simp )
...
apply(insert field_lookup_inject' [of t undefined undefined undefined])
apply(case_tac s)
apply clarsimp
apply(drule_tac x=f in spec)
...
lemma update_ti_t_update_ti_struct_t [simp]:
  ...
  by (auto simp: update_ti_t_def update_ti_struct_t_def intro!: ext)
...
lemma update_ti_struct_t_update_ti_list_t [simp]:
  ...
  by (auto simp: update_ti_struct_t_def update_ti_list_t_def intro!: ext)
...
apply(auto simp: fd_cons_double_update_def fu_commutes_def)
done
...
apply(auto simp: fd_cons_access_update_def)
...
apply(rule, clarsimp)
 apply(case_tac ba, auto)
       apply(drule field_lookup_list_offset2, fast)
      apply(clarsimp split: option.splits)
      apply(drule field_lookup_list_offset2, fast)
     apply(clarsimp split: option.splits)
     apply(drule field_lookup_list_offset2, fast)
...
apply(unfold wf_lf_def ti_ind_def field_desc_def fu_commutes_def)
apply clarsimp
apply rule
 apply clarsimp
...
lemma ti_ind_ld_td_struct:
  "ti_ind (lf_set_struct st fn) Y \<Longrightarrow> ti_ind {\<lparr> lf_fd = field_desc_struct st, lf_sz = size_td_struct st, lf_fn = [] \<rparr>} Y"
apply(subst (asm) ti_ind_fn)
  by (simp only: ti_ind_ld_td')

lemma ti_ind_ld_td_list:
  "ti_ind (lf_set_list ts fn) Y \<Longrightarrow> ti_ind {\<lparr> lf_fd = field_desc_list ts, lf_sz = size_td_list ts, lf_fn = [] \<rparr>} Y"
apply(subst (asm) ti_ind_fn)
  by (simp only: ti_ind_ld_td')

lemma ti_ind_ld_td_pair:
  "ti_ind (lf_set_pair x fn) Y \<Longrightarrow> ti_ind {\<lparr> lf_fd = field_desc_pair x, lf_sz = size_td_pair x, lf_fn = [] \<rparr>} Y"
apply(subst (asm) ti_ind_fn)
  by (simp only: ti_ind_ld_td')
...
apply(induct t and st and ts and x)
 ...
 apply rule
  apply(clarsimp simp: fd_cons_pair_def fd_cons_double_update_def field_desc_list_def field_desc_pair_def fd_cons_desc_def)
  apply(simp add: fu_commutes_def)
 apply rule
  apply(clarsimp simp: fd_cons_pair_def fd_cons_update_access_def field_desc_list_def field_desc_pair_def fd_cons_desc_def)
  apply(clarsimp simp: fd_cons_length_def)
 apply rule
  ...
  apply(drule_tac x=v' in spec)
  apply simp
 apply(clarsimp simp: fd_cons_length_def field_desc_list_def fd_cons_pair_def field_desc_pair_def fd_cons_desc_def)
apply clarsimp
apply(rotate_tac)
apply(subst (asm) wf_lf_fn, assumption)
apply(clarsimp simp: fd_cons_def fd_cons_pair_def field_desc_pair_def field_desc_def export_uinfo_def)
done

lemma wf_lf_fd_cons:
  "\<lbrakk> wf_lf (lf_set t fn); wf_desc t \<rbrakk> \<Longrightarrow> fd_cons t"
  apply(subst (asm) wf_lf_fn)
   apply simp
  by (simp only: wf_lf_fd_cons')
...
apply(induct t and st and ts and x)
     ...
     apply(rule, clarsimp)
      apply(erule (1) wf_lf_fd_cons_struct)
     apply(clarsimp)
...
apply(clarsimp simp: tf_set_def)
apply(erule (1) wf_fd_field_lookupD)
done

lemma tf_set_field_lookupD:
  "field_lookup t f m = Some (s,n) \<Longrightarrow> (s,f) \<in> tf_set t"
apply(clarsimp simp: tf_set_def)
apply(subst field_lookup_offset [where m=m,symmetric])
apply  simp
apply(drule field_lookup_offset_le)
apply arith
done
...
apply(induct t and st and ts and x)
 ...
 apply (rule conjI)
  ...
  apply assumption
 apply (rule conjI)
  apply(subgoal_tac "wf_fdp (tf_set_list xs)")
   apply simp
  apply(erule wf_fdp_mono)
  apply fast
 apply(clarsimp simp: wf_fdp_def)
 ...
 apply(rule conjI)
  ...
  apply(drule_tac x="dt_fst x" in spec, erule impE, rule_tac x="[dt_snd x]" in exI)
   ...
   apply(rule, clarsimp)
    apply(clarsimp simp: image_def)
    apply(drule_tac x="DTPair aa ba" in bspec, simp_all)[1]
   apply(clarsimp simp: image_def)
   apply(drule_tac x="DTPair aa b" in bspec, simp_all)[1]
  apply simp
...
apply(clarsimp simp: fd_consistent_def)
apply fast
done
...
apply(induct t and st and ts and x)
     apply(auto split: typ_struct_splits)
  apply(clarsimp simp: fd_cons_access_update_def field_desc_list_def fd_cons_struct_def fd_cons_desc_def)
 apply(clarsimp simp: fd_cons_access_update_def field_desc_list_def)
apply(clarsimp simp: fd_cons_access_update_def field_desc_list_def)
apply(simp add: fu_commutes_def)
apply(clarsimp simp: fa_fu_ind_def field_desc_pair_def field_desc_list_def)
...
apply(auto simp: wf_fdp_def)
apply(insert wf_fd_cons_access_update'(1) [of t])
apply clarsimp
done
...
apply(simp add: fu_commutes_def)
apply(simp add: fa_fu_ind_def field_desc_pair_def field_desc_list_def)
apply(drule wf_fd_cons_access_update_listD)
apply(clarsimp simp: fd_cons_access_update_def field_desc_list_def export_uinfo_def)
...
apply(insert wf_fd_norm_tu(1) [of t])
...
apply(insert wf_fd_norm_tu(2) [of t])
apply clarsimp
done
...
apply(insert wf_fd_norm_tu(3) [of t])
apply clarsimp
done
...
apply(insert wf_fd_norm_tu(4) [of t])
apply clarsimp
done
...
apply(unfold fd_cons_list_def)
apply(unfold fd_cons_desc_def)
apply rule
 apply(rule fd_cons_double_update_list_append)
   apply fast+
apply rule
 apply(rule fd_cons_update_access_list_append)
   apply fast+
apply rule
 apply(rule fd_cons_access_update_list_append)
   apply fast+
apply(rule fd_cons_length_list_append)
 apply fast+
done
...
apply(clarsimp simp: field_lvalue_def  field_ti_def field_offset_def
               field_offset_untyped_def split: option.splits)
...
apply(induct t and st and ts and x)
     ...
     apply(clarsimp simp: fd_cons_struct_def fd_cons_desc_def)
      apply(clarsimp simp:  fd_cons_access_update_def)
    apply clarsimp+
...
lemma fi_fa_consistentD:
  "\<lbrakk> field_lookup (typ_info_t TYPE('a::wf_type)) f 0 = Some (d,n);
      ...
      field_access (field_desc d) (from_bytes bs) (replicate (size_td d)0) = norm_tu (export_uinfo d) (take (size_td d) (drop n bs))
          "
...
apply(subst wf_fd_norm_tuD)
 ...
 apply simp
apply(frule field_access_update_take_dropD [where v=undefined and m=0 and bs'="replicate (size_td d) 0",simplified])
   apply fast
  apply simp
 apply simp
...
apply(clarsimp simp: super_update_bs_def)
done
...
apply(induct t and st and ts and x)
 ...
 apply(clarsimp simp: fd_cons_desc_def split: option.splits)
  ...
  apply(drule_tac x=v in spec)
  apply simp
  apply(subst drop_super_update_bs)
   apply simp+
  apply(subst take_super_update_bs)
   apply (simp+)[3]
 apply(simp add: fu_commutes_def)
 ...
 apply(subst take_super_update_bs2)
   apply(simp+)[2]
 apply(simp add: drop_super_update_bs2)
apply clarsimp
done
...
lemma fi_fu_consistentD:
  "\<lbrakk> field_lookup t f 0 = Some (s,n); wf_fd t; length bs = size_td t;
      ...
      field_update (field_desc t) (super_update_bs v bs n) w =
          field_update (field_desc s) v (field_update (field_desc t) bs w)"
apply(insert fi_fu_consistent(1) [of t])
apply clarsimp
...
apply(subgoal_tac "wf_fd (typ_info_t TYPE('a))")
 ...
 apply(subst wf_fd_norm_tuD)
   apply simp+
 apply(clarsimp simp: norm_desc_def access_ti\<^sub>0_def)
apply simp
done
...
apply(clarsimp simp: fs_path_def disj_fn_def)
apply auto
done
...
apply(clarsimp simp: wf_fdp_def)
apply(drule tf_set_field_lookupD)+
...
apply(clarsimp split: option.splits)
 ...
 apply(erule impE)
  ...
  apply clarsimp
  apply(drule_tac x="replicate (size_td_pair dt_pair) 0 @ bs'a" in spec)
  apply clarsimp
...
apply(erule impE)
 ...
 apply(drule_tac x=bsa in spec)
 apply clarsimp
 apply(drule_tac x="bs'a@replicate (size_td_list list) 0" in spec)
 apply clarsimp
 apply(drule wf_fd_cons_pairD)
 apply(clarsimp simp: fd_cons_pair_def fd_cons_length_def fd_cons_desc_def)
apply clarsimp
...
apply(insert field_lookup_fa_fu_lhs(3) [of ts])
apply clarsimp
done
...
apply(insert field_lookup_fa_fu_lhs(4) [of x])
apply clarsimp
done
...
apply(clarsimp split: option.splits)
 ...
 apply(erule impE)
  apply clarsimp
  apply(drule_tac x=v in spec)
  apply(drule_tac x="access_ti_pair dt_pair v (replicate (size_td_pair dt_pair) 0)@bsa" in spec)
  apply clarsimp
  apply(erule impE)
   apply(drule wf_fd_cons_pairD)
   apply(clarsimp simp: fd_cons_pair_def fd_cons_length_def fd_cons_desc_def)
  apply(drule_tac x="bs'a" in spec)
  apply clarsimp
  apply(drule wf_fd_cons_pairD)
...
apply(erule impE)
 apply clarsimp
 apply(drule_tac x=v in spec)
 apply(drule_tac x="bsa@access_ti_list list v (replicate (size_td_list list) 0)" in spec)
 apply(drule wf_fd_cons_pairD)
 apply(drule wf_fd_cons_listD)
 apply(clarsimp simp: fd_cons_pair_def fd_cons_list_def fd_cons_length_def fd_cons_update_access_def fd_cons_desc_def)
 apply force
done
...
apply(insert field_lookup_fa_fu_rhs(3) [of ts])
apply clarsimp
done
...
lemma field_lookup_fa_fu_rhs_pairD:
  "\<lbrakk> field_lookup_pair x f m = Some (s,n);
      ...
      fa_fu_ind d (field_desc (s::'a typ_info)) (size_td s) k"
apply(insert field_lookup_fa_fu_rhs(4) [of x])
apply clarsimp
done
...
apply(induct t and st and ts and x)
 ...
 apply(clarsimp simp: split: option.splits)
    ...
    apply(drule_tac x=d' in spec)
    apply clarsimp
   apply(drule (3) fa_fu_lookup_ind_list_pair)
    apply simp
   apply simp
  apply(drule (3) fa_fu_lookup_ind_pair_list)
   apply simp
  apply simp
 apply(drule_tac x=f in spec)
 apply(drule_tac x=m in spec)
...
apply(insert fa_fu_lookup_disj(1) [of t])
apply clarsimp
apply blast
done
...
apply(drule (3) fa_fu_lookup_disjD)
apply(clarsimp simp: fa_fu_ind_def)
done
...
lemma intvl_inter_le:
  assumes inter: "a + of_nat k = c + of_nat ka" and lt_d: "ka < d" and
      lt_ka: "k \<le> ka"
  shows "a \<in> {c..+d}"
proof -
  from lt_ka inter have "a = c + of_nat (ka - k)"
...
lemma fa_fu_lookup_disj_interD:
  "\<lbrakk> field_lookup t f m = Some (d,n); field_lookup t f' m = Some (d',n');
      ...
      {(of_nat n)::addr..+size_td (d::'a typ_info)} \<inter> {of_nat n'..+size_td d'} = {}"
apply(insert fa_fu_lookup_disj_inter(1) [of t])
apply clarsimp
done
...
apply(auto simp: from_bytes_def to_bytes_def)
apply(subgoal_tac "wf_fd (typ_info_t TYPE('a))")
 apply(drule wf_fd_consD)
...
apply(subgoal_tac "align_of (TYPE('a)) < addr_card")
 ...
 apply(subst power_add [symmetric])
  ...
  apply simp
 apply(rule_tac a="(2::nat)" in power_less_imp_less_exp)
  apply simp
 apply(subst (asm) addr_card)
 apply assumption
...
lemma simple_tag_not_aggregate2 [simp]:
  "\<not> typ_uinfo_t TYPE('a::simple_mem_type) = TypDesc (TypAggregate ts) tn"
apply clarsimp
apply(subgoal_tac "\<not> aggregate (typ_uinfo_t TYPE('a))")
 apply simp
apply(unfold typ_uinfo_t_def export_uinfo_def)
apply(subst aggregate_map)
apply(rule simple_tag)
done

lemma simple_tag_not_aggregate3 [simp]:
  "\<not> typ_info_t TYPE('a::simple_mem_type) = TypDesc (TypAggregate ts) tn"
apply clarsimp
apply(subgoal_tac "\<not> aggregate (typ_info_t TYPE('a))")
 apply simp
apply(rule simple_tag)
done

lemma field_of_t_mem:
  "field_of_t (p::'b::mem_type ptr) (q::'a::mem_type ptr) \<Longrightarrow> ptr_val p \<in> {ptr_val q..+size_of TYPE('a)}"
...
apply(subgoal_tac "size_td (typ_info_t TYPE('b)) > 0")
 apply simp
...
lemma fold_td_alt_def':
  "fold_td f t = (case t of TypDesc st nm \<Rightarrow> case st of
           TypScalar n algn d \<Rightarrow> d |
...
lemma fold_td_alt_def:
  "fold_td f t \<equiv> (case t of TypDesc st nm \<Rightarrow> case st of
           TypScalar n algn d \<Rightarrow> d |
           TypAggregate ts \<Rightarrow> f nm (map (\<lambda>x. (fold_td f (dt_fst x),dt_snd x)) ts))"
...
apply(subst map_td'_map')
apply(case_tac t, simp add: typ_struct_splits)
apply auto
apply(case_tac x, simp)
done
...
apply(induct t and st and ts and x)
     apply(auto simp: tnSum_def split: dt_pair.splits)
done

lemma size_td_fm:
  "size_td (t::'a typ_desc) \<equiv> fold_td tnSum (map_td (\<lambda>n algn d. n) t)"
apply(insert size_td_fm'(1) [of t])
apply auto
done
...
apply(induct t and st and ts and x)
     apply(auto simp: tnMax_def split: dt_pair.splits)
done

lemma align_td_fm:
  "align_td (t::'a typ_desc) \<equiv> fold_td tnMax (map_td (\<lambda>n algn d. algn) t)"
apply(insert align_td_fm'(1) [of t])
apply auto
done
...
apply(auto simp: image_def split: dt_pair.splits)
 ...
 apply assumption
apply(rule_tac x="f (dt_fst xa)" in exI)
apply(rule_tac x=xa in bexI)
 apply(case_tac xa, clarsimp)
apply assumption
done
